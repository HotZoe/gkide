...
///
/// definition of global variables for settable options
...
/// Default values for @b errorformat
///
/// The "%f|%l| %m" one is used for when the
/// contents of the quickfix window is written to a file.
#define DFLT_EFM                                                   \
    "%*[^\"]\"%f\"%*\\D%l: %m,"                                    \
    "\"%f\"%*\\D%l: %m,"                                           \
...
/// Possible values for @b encoding, check for BOM at start of file
#define ENC_UCSBOM         "ucs-bom"

/// default value for @b encoding
...
#define SHM_ALL ((uchar_kt[]) {                                            \
    ...
    SHM_OVER,          SHM_OVERALL,        SHM_SEARCH,    SHM_ATTENTION, \
    SHM_INTRO,         SHM_COMPLETIONMENU, SHM_RECORDING, SHM_FILEINFO,  \
    0, })
...
#define STL_ALL ((uchar_kt[]) {                                               \
    ...
    STL_BYTEVAL,     STL_BYTEVAL_X,       STL_ROFLAG,     STL_ROFLAG_ALT,   \
    ...
    STL_QUICKFIX,    STL_PERCENTAGE,      STL_ALTPERCENT, STL_ARGLISTSTAT,  \
    STL_PAGENUM,     STL_VIM_EXPR,        STL_SEPARATE,   STL_TRUNCMARK,    \
    STL_USER_HL,     STL_HIGHLIGHT,       STL_TABPAGENR,  STL_TABCLOSENR,   \
    STL_CLICK_FUNC, 0, })
...
#ifdef IN_OPTION_C
static char *(p_bo_values[]) = {
    "all",   "backspace",  "cursor",     "complete",
...
#ifdef IN_OPTION_C
static char *(p_ve_values[]) = {
    "block", "insert", "all", "onemore", NULL
};
#endif
...
/// @b indir values for buffer-local opions.
///
/// These need to be defined globally, so that
/// the BV_COUNT can be used with #b_p_scriptID.
enum
{
    BV_AI = 0,
    BV_AR,
...
/// @b indir values for window-local options.
/// These need to be defined globally, so that
/// the WV_COUNT can be used in the window structure.
enum
{
    WV_LIST = 0,
    WV_ARAB,
