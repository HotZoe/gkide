/// @file nvim/api/private/helpers.c
...
Object get_option_from(void *from, int type, String name, error_st *err)
{
    ...
    if(!flags)
    {
        api_set_error(err, kErrorTypeValidation,
                      "Invalid option name \"%s\"", name.data);
        return rv;
    }
    ...
    {
        api_set_error(err, kErrorTypeException,
                      "Unknown type for option \"%s\"", name.data);
    }
                   ...
                   error_st *err)
{
    ...
    if(value.type == kObjectTypeNil)
    {
        ...
        {
            api_set_error(err, kErrorTypeException,
                          "Unable to unset option \"%s\"", name.data);
            return;
        }
        else if(!(flags & SOPT_GLOBAL))
        {
            api_set_error(err, kErrorTypeException,
                          ...
                          "because it doesn't have a global value", name.data);
            return;
        }
        else
        {
...
filebuf_st *find_buffer_by_handle(Buffer buffer, error_st *err)
{
    if(buffer == 0)
    {
        return curbuf;
    ...
    filebuf_st *rv = handle_get_buffer(buffer);
                                 ...
                                 error_st *err)
{
    ...
    switch(opt_type)
    {
        ...
        case SREQ_BUF:
            switch_buffer(&save_curbuf, (filebuf_st *)from);
            set_option_value_err(key, numval, stringval, opt_flags, err);
...
ArrayOf(Dictionary) keymap_array(String mode, filebuf_st *buf)
{
