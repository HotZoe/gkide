...
// Flags used for a word.  Only the lowest byte can be used, the region byte comes above it.
...
// Values for SP_*ERROR are negative, positive values are used by read_cnt_string().
#define SP_TRUNCERROR   -1   ///< spell file truncated error
#define SP_FORMERROR    -2   ///< format error in spell file
#define SP_OTHERERROR   -3   ///< other error while reading spell file

/// Structure used to store words and other info for one language, loaded from a .spl file.
...
///      <len> <byte1> <byte2> ...
///
/// The "idxs" array stores the index of the child node corresponding to the byte in "byts".
...
#define SPELL_TOFOLD(c)      \
    (enc_utf8 && (c) >= 128  \
     ? utf_fold(c) : (c) < 256 ? (int)spelltab.st_fold[c] : (int)towlower(c))

#define SPELL_TOUPPER(c)    \
    (enc_utf8 && (c) >= 128 \
     ? mb_toupper(c) : (c) < 256 ? (int)spelltab.st_upper[c] : (int)towupper(c))
