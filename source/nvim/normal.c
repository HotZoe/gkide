...
/// The argument is a cmdarg_st.
typedef void (*nv_func_ft)(cmdarg_st *cap);
...
static const struct nv_cmd_s
{
    int cmd_char;         ///< (first) command character
    nv_func_ft cmd_func;  ///< function for this command
...
/// Compare functions for qsort() below, that checks the command
...
static int find_command(int cmdchar)
{
    ...
    // Special keys have a negative value, but are
    // sorted on their absolute value.
    if(cmdchar < 0)
    {
        cmdchar = -cmdchar;
    }

    // If the character is in the first part:
    // The character is the index into nv_cmd_idx[].
...
///   for example. Returns when re-entering ex mode(because ex mode recursion
///   is not allowed)
///
...
static void normal_prepare(normal_state_st *s)
{
    ...
    // After typing "3d" we return from normal_cmd() and come back
    // here, the "3" is remembered in "opcount".
    s->ca.opcount = opcount;

    // If there is an operator pending, then the command we take this
    // time will terminate it. Finish_op tells us to finish the operation
    // before returning this time (unless the operation was cancelled).
    int c = finish_op;
    ...
    // When not finishing an operator and
    // no register name typed, reset the count.
    ...
    // Restore counts from before receiving K_EVENT. This means after typing
    // "3", handling K_EVENT and then typing "2" we get "32", not "3 * 2".
...
static bool normal_need_redraw_mode_message(normal_state_st *s)
{
    return (// 'showmode' is set and messages can be printed
            ((p_smd && msg_silent == 0
            // must restart insert mode(ctrl+o or ctrl+l)
            // or we just entered visual mode
              ...
              && !msg_nowait
              // the command was the result of
              // direct user input and not a mapping
              && KeyTyped)
              // must restart insert mode, not in visual
              // mode and error message is being shown
        ...
        *cp = plain_vgetc();
        ...
        if(!lit)
        {
            ...
            if(*cp == Ctrl_K
               ...
               && ustrchr(p_cpo, CPO_DIGRAPH) == NULL)
            {
...
static void normal_finish_command(normal_state_st *s)
{
    ...
    // If we didn't start or finish an operator,
    // reset oap->regname, unless we need it later.
    if(!finish_op
       && !s->oa.op_type
...
normal_end:
    ...
    if(curwin->w_o_curbuf.wo_scb && s->toplevel)
    {
        validate_cursor(); // may need to update w_leftcol
        do_check_scrollbind(true);
    }

    if(curwin->w_o_curbuf.wo_crb && s->toplevel)
    {
        validate_cursor(); // may need to update w_leftcol
        do_check_cursorbind();
    }
...
static int normal_execute(nvim_state_st *state, int key)
{
           ...
           || s->c == K_KENTER))
    {
        ...
        // restart automatically. Insert the typed character in the typeahead
        // buffer, so that it can be mapped in Insert mode. Required for
        // ":lmap" to work.
    ...
    else if(s->ca.opcount != 0)
    {
        ...
        // into "d3w" which makes things fall into place pretty neatly. If you
        // give a count before AND after the operator, they are multiplied.
        if(s->ca.count0)
        {
    ...
    // above) when there is no pending operator. When called from main(),
    // save the count for use by the "count" built-in variable.
       ...
       && (nv_cmds[s->idx].cmd_flags & NV_RL))
    {
        ...
        // Only when typed by the user directly, not when
        // the result of a mapping or "x" translated to "dl".
    ...
    // Flush the showcmd characters onto the screen so
    // we can see them while the command is being executed.
    // Only do this when the shown command was actually displayed,
...
static void normal_check_interrupt(normal_state_st *s)
{
    ...
    // the ":g" command. For ":g/pat/vi" we reset "got_int" when used once.
...
static void normal_check_cursor_moved(normal_state_st *s)
{
    ...
    if(!finish_op
       && (has_event(EVENT_CURSORMOVED) || curwin->w_o_curbuf.wo_cole > 0)
...
static void normal_redraw(normal_state_st *s)
{
    ...
    if(keep_msg != NULL)
    {
        // msg_attr_keep() will set keep_msg to NULL,
        // must free the string here. Don't reset keep_msg,
        // msg_attr_keep() uses it to check for duplicates.
    ...
    if(s->conceal_update_lines
       && (s->conceal_old_cursor_line != s->conceal_new_cursor_line
           || conceal_cursor_line(curwin)
           || need_cursor_line_redraw))
    {
        if(s->conceal_old_cursor_line != s->conceal_new_cursor_line
           && s->conceal_old_cursor_line <= curbuf->b_ml.ml_line_count)
        {
...
static int normal_check(nvim_state_st *state)
{
    ...
    // If skip redraw is set (for ":" in wait_return()),
    // don't redraw now. If there is nothing in the stuff_buffer
    // or do_redraw is TRUE, update cursor and redraw.
    ...
    else if(do_redraw || stuff_empty())
    {
        ...
        // Scroll-binding for diff mode may have been postponed until here.
...
static void set_vcount_ca(cmdarg_st *cap, bool *set_prevcount)
{
...
void do_pending_operator(cmdarg_st *cap, int old_col, bool gui_yank)
{
    ...
    int lbr_saved = curwin->w_o_curbuf.wo_lbr;
    ...
    if((finish_op || VIsual_active) && oap->op_type != OP_NOP)
    {
        ...
        if((ustrchr(p_cpo, CPO_YANK) != NULL || oap->op_type != OP_YANK)
           ...
           && oap->op_type != OP_FOLDDELREC)
        {
            ...
            if(cap->cmdchar == '/' || cap->cmdchar == '?')
            {
                ...
                if(ustrchr(p_cpo, CPO_REDO) == NULL)
                {
            ...
            {
                // do_cmdline() has stored the first typed
                // line in "repeat_cmdline". When several lines
        ...
        if(redo_VIsual_busy)
        {
            // Redo of an operation on a Visual area. Use the
            // same size from redo_VIsual_line_count and redo_VIsual_vcol.
        ...
        else if(VIsual_active)
        {
               ...
               && cap->oap->op_type != OP_DELETE)
            {
                if(lt(VIsual, curwin->w_cursor))
                {
                    ...
                    curwin->w_cursor.col =
                        (columnum_kt)ustrlen(ml_get(curwin->w_cursor.lnum));
                }
                else
                {
                    curwin->w_cursor.col = 0;
                    VIsual.col = (columnum_kt)ustrlen(ml_get(VIsual.lnum));
                }
            ...
            {
                // If 'selection' is "exclusive", backup one
                // character for charwise selections.
        ...
        else
        {
            ...
            if(!VIsual_active && oap->motion_type == kMTLineWise)
            {
                ...
                {
                    oap->start.col = (columnum_kt)ustrlen(ml_get(oap->start.lnum));
                }
            }
        ...
        if(VIsual_active || redo_VIsual_busy)
        {
            ...
            {
                // Prepare for redoing.
                // Only use the nchar field for "r", otherwise it
                // might be the second char of the operator.
            ...
            else if(VIsual_mode == 'v')
            {
                   ...
                   && (include_line_break || !virtual_op))
                {
                    ...
                    // Try to include the newline, unless it's
                    // an operator that works on lines only.
                    if(*p_sel != 'o'
            ...
            if(!gui_yank)
            {
                   ...
                   && oap->motion_force == NUL)
                {
                    ...
                    curwin->w_o_curbuf.wo_lbr = lbr_saved;
                    redraw_curbuf_later(INVERTED);
                }
            }
        }
        ...
        empty_region_error = (oap->empty
                              && ustrchr(p_cpo, CPO_EMPTYREGION) != NULL);

        // Force a redraw when operating on an empty Visual region, when
        // 'modifiable is off or creating a fold.
        if(oap->is_VIsual
           && (oap->empty || !curbuf->b_p_ma || oap->op_type == OP_FOLD))
        {
            curwin->w_o_curbuf.wo_lbr = lbr_saved;
            redraw_curbuf_later(INVERTED);
        }
        ...
        // is kMTCharWise and oap->inclusive is false, we put op_end after
        // the last character in the previous line. If op_start is on or
        // before the first non-blank in the line, the operator becomes
        // linewise (strange, but that's the way vi does it).
        ...
        switch(oap->op_type)
        {
            ...
            case OP_CHANGE:
                ...
                else
                {
                    ...
                    // Restore linebreak, so that when
                    // the user edits it looks as before.
                    if(curwin->w_o_curbuf.wo_lbr != lbr_saved)
                    {
                        curwin->w_o_curbuf.wo_lbr = lbr_saved;
            ...
            case OP_FILTER:
                if(ustrchr(p_cpo, CPO_FILTER) != NULL)
                {
            ...
            case OP_APPEND:
                ...
                else
                {
                    ...
                    // TODO: when inserting in several
                    // lines, should format all the lines.
                    auto_format(false, true);
            ...
            case OP_REPLACE:
                ...
                {
                    // Restore linebreak, so that
                    // when the user edits it looks as before.
                    if(curwin->w_o_curbuf.wo_lbr != lbr_saved)
                    {
                        curwin->w_o_curbuf.wo_lbr = lbr_saved;
                        get_op_vcol(oap, redo_VIsual_mode, false);
        ...
        if(!gui_yank)
        {
            // if 'sol' not set, go back
            // to old column for some commands
            if(!p_sol
               && oap->motion_type == kMTLineWise
               && !oap->end_adjusted
...
static void op_function(oparg_st *oap)
{
    ...
    else
    {
        ...
        // Reset virtual_op so that 'virtualedit'
...
/// Move the current tab to tab in same column
/// as mouse or to end of the tabline if there is no tab there.
...
bool do_mouse(oparg_st *oap, int c, int dir, long count, bool fixindent)
{
    ...
    for(;;)
    {
        ...
        if(is_drag)
        {
            ...
            if(vpeekc() != NUL)
            {
                ...
                // Need to get the character,
                // peeking doesn't get the actual one.
                nc = safe_vgetc();
           ...
           && mouse_col < Columns)
        {
            ...
            switch(tab_page_click_defs[mouse_col].type)
            {
                ...
                case kStlClickTabSwitch:
                {
                    ...
                    {
                        // Go to specified tab page, or
                        // next one if not clicking on a label.
                        goto_tabpage(c1);
       ...
       && !(mod_mask & (MOD_MASK_SHIFT | MOD_MASK_CTRL)))
    {
        if(which_button == MOUSE_LEFT)
        {
            if(is_click)
            {
                // stop Visual mode for a left click in a
                // window, but not when on a status line
                if(VIsual_active)
                {
        ...
        else if(which_button == MOUSE_RIGHT)
        {
            ...
            {
                // Remember the start and end of
                // visual before moving the cursor.
    ...
    // If an operator is pending, ignore all
    // drags and releases until the next mouse click.
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(diff > 0)
                    {
                        VIsual = start_visual; // closest to end
                    }
                    else if(diff < 0)
                    {
                        VIsual = end_visual; // closest to start
                    }
                    else
                    {
                        // in the middle line
    ...
    if(which_button == MOUSE_MIDDLE)
    {
        ...
        // Remember where the paste started, so in edit()
        // Insstart can be set to this position
        if(restart_edit != 0)
        {
        ...
        do_put(regname, NULL, dir, count,
            ...
            && bt_quickfix(curbuf))
    {
        // Ctrl-Mouse click or double click in a quickfix
        // window jumps to the error under the mouse pointer.
            ...
            && mouse_has(MOUSE_VISUAL))
    {
        ...
        if((mod_mask & MOD_MASK_MULTI_CLICK) == MOD_MASK_2CLICK)
        {
            ...
            {
                // When not found a match or when
                // dragging: extend to include a word.
...
static int get_mouse_class(uchar_kt *p)
{
    ...
    if(c != NUL && ustrchr((uchar_kt *)"-+*/%<>&|^!=", c) != NULL)
    {
        return 1;
    }
...
/// Check if highlighting for visual mode is possible,
/// give a warning message if not.
...
/// End Visual mode.
///
/// This function should ALWAYS be called to end
/// Visual mode, except from do_pending_operator().
...
static void prep_redo_cmd(cmdarg_st *cap)
{
...
static void unshift_special(cmdarg_st *cap)
{
    switch(cap->cmdchar)
    {
        case K_S_RIGHT:
...
/// If the mode is currently displayed clear the
/// command line or update the command displayed.
...
bool add_to_showcmd(int c)
{
    ...
    size_t old_len = ustrlen(showcmd_buf);
    size_t extra_len = ustrlen(p);
...
static void del_from_showcmd(int len)
{
    ...
    old_len = (int)ustrlen(showcmd_buf);
...
/// push_showcmd() and pop_showcmd() are used when waiting for
/// the user to type something and there is a partial mapping.
void push_showcmd(void)
{
    if(p_sc)
    {
        ustrcpy(old_showcmd_buf, showcmd_buf);
    }
}
...
void pop_showcmd(void)
{
    ...
    ustrcpy(showcmd_buf, old_showcmd_buf);
    display_showcmd();
}
...
void do_check_scrollbind(bool check)
{
    ...
    static filebuf_st *old_buf = NULL;
    static columnum_kt old_leftcol = 0;

    if(check && curwin->w_o_curbuf.wo_scb)
    {
        // If a ":syncbind" command was just used,
        // don't scroll, only reset the values.
        if(did_syncbind)
        {
            did_syncbind = false;
        }
        else if(curwin == old_curwin)
        {
            ...
            // when 'diff' is set.
            if((curwin->w_buffer == old_buf || curwin->w_o_curbuf.wo_diff)
        ...
        else if(ustrchr(p_sbo, 'j'))
        {
            // jump flag set in 'scrollopt'
            //
...
void check_scrollbind(linenum_kt topline_diff, long leftcol_diff)
{
    ...
    want_ver = (ustrchr(p_sbo, 'v') && topline_diff != 0);
    want_ver |= old_curwin->w_o_curbuf.wo_diff;
    want_hor = (ustrchr(p_sbo, 'h') && (leftcol_diff || topline_diff != 0));
    ...
    FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
    {
        ...
        if(curwin == old_curwin || !curwin->w_o_curbuf.wo_scb)
        {
            continue;
        }
        ...
        if(want_ver)
        {
            if(old_curwin->w_o_curbuf.wo_diff && curwin->w_o_curbuf.wo_diff)
            {
...
///
/// Used for CTRL-Q and CTRL-S to avoid
/// problems with terminals that use xon/xoff.
static void nv_ignore(cmdarg_st *cap)
{
    cap->retval |= CA_COMMAND_BUSY; // don't call edit() now
}

/// Command character that doesn't do anything,
...
static void nv_error(cmdarg_st *cap)
{
    clearopbeep(cap->oap);
}

/// <Help> and <F1> commands.
static void nv_help(cmdarg_st *cap)
{
...
/// CTRL-A and CTRL-X: Add or subtract
/// from letter or number under cursor.
static void nv_addsub(cmdarg_st *cap)
{
...
static void nv_page(cmdarg_st *cap)
{
               ...
               int flags_arg)
{
    ...
    // Put "\V" before the pattern to avoid that
    // the special meaning of "." and "~" causes trouble.
    assert(len <= INT_MAX);

    sprintf((char *)pat,
            is_kwc_ptr(ptr) ? "\\V\\<%.*s\\>" : "\\V%.*s",
            (int)len, ptr);
...
static bool nv_screengo(oparg_st *oap, int dir, long dist)
{
    ...
    if(curwin->w_cursor.col > 0 && curwin->w_o_curbuf.wo_wrap)
    {
...
static void nv_mousescroll(cmdarg_st *cap)
{
    win_st *old_curwin = curwin;
...
static void nv_mouse(cmdarg_st *cap)
{
...
static void nv_scroll_line(cmdarg_st *cap)
{
    if(!checkclearop(cap->oap))
    {
...
void scroll_redraw(int up, long count)
{
    ...
    if(p_so)
    {
              ...
              && curwin->w_topfill == prev_topfill)
        {
            ...
            // Mark w_topline as valid, otherwise the
            // screen jumps back at the end of the file.
            check_cursor_moved(curwin);
            curwin->w_valid |= kWVF_TopLine;
        }
    }
...
static void nv_zet(cmdarg_st *cap)
{
    int n;
    columnum_kt col;
    int nchar = cap->nchar;
    long old_fdl = curwin->w_o_curbuf.wo_fdl;
    int old_fen = curwin->w_o_curbuf.wo_fen;
    bool undo = false;
...
dozet:
    ...
    if(cap->nchar != 'f'
       && cap->nchar != 'F'
       && !(VIsual_active && ustrchr((uchar_kt *)"dcCoO", cap->nchar))
    ...
    switch(nchar)
    {
        ...
        case '^': // "z^", "z-" and "zb": put cursor at bottom of screen
            // Strange Vi behavior: <count>z^ finds line at top of window
        ...
        case 'F': // "zF": create fold command
        case 'f': // "zf": create fold operator
            if(foldManualAllowed(true))
            {
                cap->nchar = 'f';
                nv_operator(cap);
                curwin->w_o_curbuf.wo_fen = true;
        ...
        case 'd': // "zd": delete fold at cursor
        case 'D': // "zD": delete fold at cursor recursively
        ...
        case 'n': // "zn": fold none: reset 'foldenable'
            curwin->w_o_curbuf.wo_fen = false;
            break;

        case 'N': // "zN": fold Normal: set 'foldenable'
            curwin->w_o_curbuf.wo_fen = true;
            break;

        case 'i': // "zi": invert folding: toggle 'foldenable'
            curwin->w_o_curbuf.wo_fen = !curwin->w_o_curbuf.wo_fen;
            break;

        case 'a': // "za": open closed fold or close open fold at cursor
            if(hasFolding(curwin->w_cursor.lnum, NULL, NULL))
            {
        ...
        case 'A': // "zA": open fold at cursor recursively
            if(hasFolding(curwin->w_cursor.lnum, NULL, NULL))
            {
        ...
        case 'o': // "zo": open fold at cursor or Visual area
            if(VIsual_active)
            {
                nv_operator(cap);
            }
        ...
        case 'O': // "zO": open fold recursively
            if(VIsual_active)
            {
                nv_operator(cap);
            }
        ...
        case 'c': // "zc": close fold at cursor or Visual area
            if(VIsual_active)
            {
                nv_operator(cap);
            }
        ...
        case 'C': // "zC": close fold recursively
            if(VIsual_active)
            {
                nv_operator(cap);
            }
        ...
        case 'v': // "zv": open folds at the cursor
            foldOpenCursor();
            break;

        case 'x': // "zx": re-apply 'foldlevel' and open folds at the cursor
            curwin->w_o_curbuf.wo_fen = true;
        ...
        case 'X': // "zX": undo manual opens/closes, re-apply 'foldlevel'
            curwin->w_o_curbuf.wo_fen = true;
            curwin->w_foldinvalid = true; // recompute folds
        ...
        case 'm': // "zm": fold more
            ...
            {
                curwin->w_o_curbuf.wo_fdl -= cap->count1;
        ...
        case 'r': // "zr": reduce folding
            curwin->w_o_curbuf.wo_fdl += cap->count1;
            {
                int d = getDeepestNesting();

                if(curwin->w_o_curbuf.wo_fdl >= d)
                {
                    curwin->w_o_curbuf.wo_fdl = d;
                }
            }
            break;

        case 'R': // "zR": open all folds
            curwin->w_o_curbuf.wo_fdl = getDeepestNesting();
            old_fdl = -1; // force an update
            break;
        ...
        case 'u': // "zug" and "zuw": undo "zg" and "zw"
            ...
            if(ustrchr((uchar_kt *)"gGwW", nchar) == NULL)
            {
                clearopbeep(cap->oap);
                break;
            }
    ...
    if(old_fen != curwin->w_o_curbuf.wo_fen)
    {
        if(foldmethodIsDiff(curwin) && curwin->w_o_curbuf.wo_scb)
        {
            ...
            FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
            {
                if(wp != curwin
                   && foldmethodIsDiff(wp)
                   && wp->w_o_curbuf.wo_scb)
                {
                    wp->w_o_curbuf.wo_fen = curwin->w_o_curbuf.wo_fen;
    ...
    if(old_fdl != curwin->w_o_curbuf.wo_fdl)
    {
        newFoldLevel();
    }
}
...
/// "Q" command.
static void nv_exmode(cmdarg_st *cap)
{
...
/// Handle a ":" command.
static void nv_colon(cmdarg_st *cap)
{
    int old_p_im;
    bool cmd_result;
        ...
        if(cmd_result == false)
        {
            ...
            clearop(cap->oap);
                ...
                && (cap->oap->start.lnum > curbuf->b_ml.ml_line_count
                    || cap->oap->start.col
                       > (columnum_kt)ustrlen(ml_get(cap->oap->start.lnum))
                    || did_emsg))
        {
            // The start of the operator has
            // become invalid by the Ex command.
            clearopbeep(cap->oap);
        }
    }
}
...
static void nv_ctrlg(cmdarg_st *cap)
{
    if(VIsual_active)
    {
...
static void nv_ctrlh(cmdarg_st *cap)
{
    if(VIsual_active && VIsual_select)
    {
...
static void nv_clear(cmdarg_st *cap)
{
    if(!checkclearop(cap->oap))
    {
...
static void nv_ctrlo(cmdarg_st *cap)
{
    if(VIsual_active && VIsual_select)
    {
        VIsual_select = false;
...
static void nv_hat(cmdarg_st *cap)
{
    if(!checkclearopq(cap->oap))
    {
...
static void nv_Zet(cmdarg_st *cap)
{
    if(!checkclearopq(cap->oap))
    {
        switch(cap->nchar)
        {
            case 'Z': // "ZZ": equivalent to ":x".
                do_cmdline_cmd("x");
                break;

            case 'Q': // "ZQ": equivalent to ":q!" (Elvis compatible).
                do_cmdline_cmd("q!");
                break;
...
static void nv_ident(cmdarg_st *cap)
{
    uchar_kt *ptr = NULL;
    uchar_kt *p;
    ...
    bool kp_help = (ustrcmp(kp, ":he") == 0 || ustrcmp(kp, ":help") == 0);
    size_t buf_size = n * 2 + 30 + ustrlen(kp);
    ...
    switch(cmdchar)
    {
        ...
        case 'K':
            ...
            else
            {
                ...
                // really what we want?
                bool isman = (ustrcmp(kp, "man") == 0);
                bool isman_s = (ustrcmp(kp, "man -s") == 0);

                if(cap->count0 != 0 && !(isman || isman_s))
                {
                    snprintf(buf, buf_size, ".,.+%" PRId64,
                ...
                if(cap->count0 != 0 && (isman || isman_s))
                {
                    snprintf(buf + ustrlen(buf), buf_size - ustrlen(buf),
    ...
    if(cmdchar == 'K' && !kp_help)
    {
        ...
        else
        {
            ...
            p = ustrdup_escape_shell(ptr, true, true);
        }

        xfree(ptr);
        char *newbuf = xrealloc(buf, ustrlen(buf) + ustrlen(p) + 1);
        buf = newbuf;
        ustrcat(buf, p);
        xfree(p);
    }
    else
    {
        ...
        p = (uchar_kt *)buf + ustrlen(buf);

        while(n-- > 0)
        {
            // put a backslash before \ and some others
            if(ustrchr(aux_ptr, *ptr) != NULL)
            {
                *p++ = '\\';
            }
...
bool get_visual_text(cmdarg_st *cap, uchar_kt **pp, size_t *lenp)
{
    if(VIsual_mode != 'V')
    {
        unadjust_for_sel();
...
static void nv_tagpop(cmdarg_st *cap)
{
    if(!checkclearopq(cap->oap))
    {
...
static void nv_scroll(cmdarg_st *cap)
{
    long n;
    int used = 0;
    linenum_kt lnum;
...
static void nv_right(cmdarg_st *cap)
{
    long n;
    int PAST_LINE;
    ...
    // In virtual mode, there's no such thing as "PAST_LINE",
    ...
    for(n = cap->count1; n > 0; --n)
    {
           ...
           || (PAST_LINE && *get_cursor_pos_ptr() == NUL))
        {
            ...
            if(((cap->cmdchar == ' ' && ustrchr(p_ww, 's') != NULL)
                || (cap->cmdchar == 'l' && ustrchr(p_ww, 'l') != NULL)
                || (cap->cmdchar == K_RIGHT && ustrchr(p_ww, '>') != NULL))
               && curwin->w_cursor.lnum < curbuf->b_ml.ml_line_count)
            {
...
static void nv_left(cmdarg_st *cap)
{
    long n;
    ...
    for(n = cap->count1; n > 0; --n)
    {
        if(oneleft() == false)
        {
            ...
            if((((cap->cmdchar == K_BS || cap->cmdchar == Ctrl_H)
                 && ustrchr(p_ww, 'b') != NULL)
                || (cap->cmdchar == 'h' && ustrchr(p_ww, 'h') != NULL)
                || (cap->cmdchar == K_LEFT && ustrchr(p_ww, '<') != NULL))
               && curwin->w_cursor.lnum > 1)
            {
                --(curwin->w_cursor.lnum);
...
static void nv_up(cmdarg_st *cap)
{
...
static void nv_down(cmdarg_st *cap)
{
    if(mod_mask & MOD_MASK_SHIFT)
    {
        // <S-Down> is page down
...
static void nv_gotofile(cmdarg_st *cap)
{
    uchar_kt *ptr;
    linenum_kt lnum = -1;
...
static void nv_end(cmdarg_st *cap)
{
...
/// Handle the "$" command.
static void nv_dollar(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTCharWise;
    cap->oap->inclusive = true;
...
static void nv_next(cmdarg_st *cap)
{
    ...
    if(i == 1 && equalpos(old, curwin->w_cursor))
    {
        ...
        // which can happen when an offset is given and the cursor
        // is on the last char in the buffer: Repeat with count + 1.
        cap->count1 += 1;
...
static int normal_search(cmdarg_st *cap, int dir, uchar_kt *pat, int opt)
{
    int i;
    cap->oap->motion_type = kMTCharWise;
    cap->oap->inclusive = false;
    ...
    // "/$" will put the cursor after the  end of
    // the line, may need to correct that here
    check_cursor();
    return i;
}
...
static void nv_csearch(cmdarg_st *cap)
{
    bool t_cmd;
...
static void nv_brackets(cmdarg_st *cap)
{
    ...
    else
    {
        ...
        // define      "]d"  "[d"   "]D"  "[D"   "]^D"  "[^D"
        if(ustrchr((uchar_kt *) "iI\011dD\004", cap->nchar) != NULL)
        {
            uchar_kt *ptr;
            size_t len;
            ...
            {
                find_pattern_in_path(ptr,
                                     0,
                                     len,
                                     true,
                                     cap->count0 == 0
                                     ? !isupper(cap->nchar) : false,
                                     (((cap->nchar & 0xf) == ('d' & 0xf))
        ...
        else
        {
            ...
            if((cap->cmdchar == '['
                && ustrchr((uchar_kt *)"{(*/#mM", cap->nchar) != NULL)
               || (cap->cmdchar == ']'
                   && ustrchr((uchar_kt *)"})*/#mM", cap->nchar) != NULL))
            {
                if(cap->nchar == '*')
                ...
                // Handle "[m", "]m", "[M" and "[M". The findmatchlimit()
                // only brought us to the match for "[m" and "]M" when inside
                // a method. Try finding the '{' or '}' we want to be at.
                    ...
                    while(n > 0)
                    {
                        for(;;)
                        {
                            ...
                            if(c == '{' || c == '}')
                            {
                                ...
                                // if no match found at all, we started outside
                                // of the class and we're inside now. Just go on.
            ...
            else if(cap->nchar == '[' || cap->nchar == ']')
            {
                ...
                // Imitate strange Vi behaviour: When using
                // "]]" with an operator we also stop at '}'.
                if(!findpar(&cap->oap->inclusive,
                            cap->arg,
                            cap->count1,
                            flag,
                            (cap->oap->op_type != OP_NOP
...
static void nv_percent(cmdarg_st *cap)
{
    apos_st *pos;
...
/// Handle "(" and ")" commands.
/// cap->arg is BACKWARD for "(" and FORWARD for ")".
static void nv_brace(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTCharWise;
    cap->oap->use_reg_one = true;

    // The motion used to be inclusive for "(",
    // but that is not what Vi does.
    cap->oap->inclusive = false;
    curwin->w_set_curswant = true;
...
static void nv_mark(cmdarg_st *cap)
{
    if(!checkclearop(cap->oap))
    {
...
static void nv_findpar(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTCharWise;
    cap->oap->inclusive = false;
    cap->oap->use_reg_one = true;
...
static void nv_undo(cmdarg_st *cap)
{
...
/// <Undo> command.
static void nv_kundo(cmdarg_st *cap)
{
    if(!checkclearopq(cap->oap))
    {
...
static void nv_replace(cmdarg_st *cap)
{
    uchar_kt *ptr;
    int had_ctrl_v;
    long n;
...
static void v_swap_corners(int cmdchar)
{
    ...
    if(cmdchar == 'O' && VIsual_mode == Ctrl_V)
    {
        ...
        // 'selection "exclusive" and cursor at right-bottom
        // corner: move it right one column
...
/// "gr".
static void nv_vreplace(cmdarg_st *cap)
{
    if(VIsual_active)
    {
        cap->cmdchar = 'r';
...
static void n_swapchar(cmdarg_st *cap)
{
    long n;
    apos_st startpos;
    int did_change = 0;
    ...
    if(did_change)
    {
        changed_lines(startpos.lnum, startpos.col,
...
/// Move cursor to mark.
static void nv_cursormark(cmdarg_st *cap, int flag, apos_st *pos)
{
...
static void v_visop(cmdarg_st *cap)
{
    ...
    cap->cmdchar = *(ustrchr(trans, cap->cmdchar) + 1);
    nv_operator(cap);
}

/// "s" and "S" commands.
static void nv_subst(cmdarg_st *cap)
{
    if(VIsual_active)
    {
...
static void nv_abbrev(cmdarg_st *cap)
{
...
static void nv_optrans(cmdarg_st *cap)
{
...
static void nv_gomark(cmdarg_st *cap)
{
    int c;
    apos_st *pos;
...
static void nv_pcmark(cmdarg_st *cap)
{
    apos_st *pos;
    linenum_kt lnum = curwin->w_cursor.lnum;
    int old_KeyTyped = KeyTyped; // getting file may reset it
...
/// Handle '"' command.
static void nv_regname(cmdarg_st *cap)
{
    if(checkclearop(cap->oap))
    {
        return;
...
/// Always start Select mode, cap->arg is true.
/// Handle CTRL-Q just like CTRL-V.
static void nv_visual(cmdarg_st *cap)
{
    ...
    // 'v', 'V' and CTRL-V can be used while an operator is
    // pending to make it characterwise, linewise, or blockwise.
    if(cap->oap->op_type != OP_NOP)
    {
    ...
    if(VIsual_active)
    {
        ...
        {
            // toggle char/block modeor char/line mode
        ...
        if(cap->count0 > 0 && resel_VIsual_mode != NUL)
        {
            ...
            if(VIsual_mode == 'v')
            {
                if(resel_VIsual_line_count <= 1)
                {
                    ...
                    curwin->w_curswant =
                        (curwin->w_virtcol
                         + resel_VIsual_vcol * (int)cap->count0
                         - 1);
                }
                else
                {
...
{
    VIsual_select = (stuff_empty()
                     && typebuf_typed()
                     && (ustrchr(p_slm, c) != NULL));
}
...
static void n_start_visual_mode(int c)
{
    ...
    // Corner case:
    // the 0 position in a tab may change when going into virtualedit.
    // Recalculate curwin->w_cursor to avoid bad hilighting.
    if(c == Ctrl_V
       && (ve_flags & VE_BLOCK)
       && gchar_cursor() == TAB)
    {
...
static void nv_window(cmdarg_st *cap)
{
    if(!checkclearop(cap->oap))
    {
...
/// CTRL-Z: Suspend
static void nv_suspend(cmdarg_st *cap)
{
    clearop(cap->oap);
    ...
    switch(cap->nchar)
    {
        ...
        case 'v':
            ...
            else
            {
                ...
                // the Visual area, tpos to the end
                if(VIsual_active)
                {
                    i = VIsual_mode;
                ...
                // Set Visual to the start and w_cursor to the end of the
                // Visual area. Make sure they are on an existing character.
                check_cursor();
                VIsual = curwin->w_cursor;
                ...
                // When called from normal "g" command: start Select
                // mode when 'selectmode' contains "cmd". When called
                // for K_SELECT, always start Select mode.
                if(cap->arg)
                {
                    VIsual_select = true;
                }
        ...
        // "gV": Don't reselect the previous Visual
        // area after a Select mode mapping of menu.
        case 'V':
            VIsual_reselect = false;
            break;
        ...
        case K_DOWN:
            ...
            // with 'nowrap' it works just like the normal
            // "j" command; also when in a closed fold
            if(!curwin->w_o_curbuf.wo_wrap
        ...
        case K_UP:
            ...
            // with 'nowrap' it works just like the normal
            // "k" command; also when in a closed fold
            if(!curwin->w_o_curbuf.wo_wrap
               || hasFolding(curwin->w_cursor.lnum, NULL, NULL))
            {
                oap->motion_type = kMTLineWise;
        ...
        case K_KHOME:
            ...
            if(curwin->w_o_curbuf.wo_wrap && curwin->w_width != 0)
            {
            ...
            // 'relativenumber' is on and lines are wrapping the middle
            // can be more to the left.
            if(cap->nchar == 'm')
            {
                i += (curwin->w_width
                      - curwin_col_off()
                      + ((curwin->w_o_curbuf.wo_wrap && i > 0)
                         ? curwin_col_off2() : 0)) / 2;
            }
        ...
        case '_':
            // "g_": to the last non-blank character in
            // the line or <count> lines downward.
            cap->oap->motion_type = kMTCharWise;
            cap->oap->inclusive = true;
            curwin->w_curswant = MAXCOL;
        ...
        case 'i':
            if(curbuf->b_last_insert.mark.lnum != 0)
            {
                ...
                check_cursor_lnum();
                i = (int)ustrlen(get_cursor_line_ptr());
        ...
        // "gd": Find first occurrence of pattern under the cursor
        // in the current function "gD": idem, but in the current file.
        case 'd':
        case 'D':
        ...
        // "gP" and "gp": same as "P" and "p"
        // but leave cursor just after new text
        case 'p':
        case 'P':
            nv_put(cap);
            break;
...
static void n_opencmd(cmdarg_st *cap)
{
    ...
    if(!checkclearopq(cap->oap))
    {
        ...
        linenum_kt tl = curwin->w_cursor.lnum - (cap->cmdchar == 'O' ? 1 : 0);
        linenum_kt bl = curwin->w_cursor.lnum + (cap->cmdchar == 'o' ? 1 : 0);
        if(u_save(tl, bl)
           && open_line(cap->cmdchar == 'O'
                        ? BACKWARD : FORWARD,
...
static void nv_dot(cmdarg_st *cap)
{
    if(!checkclearopq(cap->oap))
    {
...
/// CTRL-R: undo undo
static void nv_redo(cmdarg_st *cap)
{
    if(!checkclearopq(cap->oap))
    {
...
/// '~' command: If tilde is not an operator and Visual
/// is off: swap case of a single character.
static void nv_tilde(cmdarg_st *cap)
{
...
static void nv_operator(cmdarg_st *cap)
{
    int op_type;
...
static void nv_lineop(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTLineWise;
...
/// <Home> command.
static void nv_home(cmdarg_st *cap)
{
    ...
    // Don't move cursor past eol
    // (only necessary in a one-character line).
    ins_at_eol = false;
}

/// "|" command.
static void nv_pipe(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTCharWise;
    cap->oap->inclusive = false;
    beginline(0);
...
/// cap->arg is 1 for "B"
static void nv_bck_word(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTCharWise;
    cap->oap->inclusive = false;
    curwin->w_set_curswant = true;
...
static void nv_wordcmd(cmdarg_st *cap)
{
    int n;
    bool word_end;
    bool flag = false;
    ...
    if(!word_end && cap->oap->op_type == OP_CHANGE)
    {
        ...
        if(n != NUL && !ascii_iswhite(n))
        {
            ...
            // effectively map "cw" to "ce", and "cW" to "cE", provided that
            // we are not on a space or a TAB. This seems impolite at first,
            // but it's really more what we mean when we say "cw".
            //
            // Another strangeness: When standing on the end of a word "ce"
            // will change until the end of the next word, but "cw" will change
            // only one character! This is done by setting "flag".
            if(ustrchr(p_cpo, CPO_CHANGEW) != NULL)
            {
                cap->oap->inclusive = true;
                word_end = true;
            }
...
static void nv_beginline(cmdarg_st *cap)
{
    cap->oap->motion_type = kMTCharWise;
    cap->oap->inclusive = false;
    beginline(cap->arg);
...
static void adjust_for_sel(cmdarg_st *cap)
{
    if(VIsual_active
       && cap->oap->inclusive
       && *p_sel == 'e'
...
/// Exclude last character at end of Visual area
/// for 'selection' == "exclusive". Should check VIsual_mode
...
static bool unadjust_for_sel(void)
{
    ...
    if(*p_sel == 'e' && !equalpos(VIsual, curwin->w_cursor))
    {
        ...
        {
            --pp->lnum;
            pp->col = (columnum_kt)ustrlen(ml_get(pp->lnum));
            return true;
        }
    }
...
static void nv_select(cmdarg_st *cap)
{
    if(VIsual_active)
    {
        VIsual_select = true;
...
static void nv_goto(cmdarg_st *cap)
{
    linenum_kt lnum;
...
/// CTRL-\ in Normal mode.
static void nv_normal(cmdarg_st *cap)
{
...
static void nv_esc(cmdarg_st *cap)
{
    int no_reason;
    ...
    if(cap->arg)
    {
        ...
        // it won't be set again below when halfway through a mapping.
        if(!p_im)
        {
            restart_edit = 0;
        }
...
static void nv_edit(cmdarg_st *cap)
{
    ...
    else if(!checkclearopq(cap->oap))
    {
        switch(cap->cmdchar)
        {
            case 'A': // "A"ppend after the line
                ...
                {
                    curwin->w_cursor.col +=
                        (columnum_kt)ustrlen(get_cursor_pos_ptr());
                }
...
/// @param cmd
/// @param startln
static void invoke_edit(cmdarg_st *cap, int repl, int cmd, int startln)
{
...
/// "a" or "i" while an operator is
/// pending or in Visual mode: object motion.
static void nv_object(cmdarg_st *cap)
{
    bool flag;
    bool include;
    uchar_kt *mps_save;
...
static void nv_record(cmdarg_st *cap)
{
    ...
    else if(!checkclearop(cap->oap))
    {
        ...
        {
            // (stop) recording into a named
            // register, unless executing a register.
...
/// Handle the "@r" command.
static void nv_at(cmdarg_st *cap)
{
    if(checkclearop(cap->oap))
    {
        return;
...
static void nv_halfpage(cmdarg_st *cap)
{
...
static void nv_join(cmdarg_st *cap)
{
    ...
    else if(!checkclearop(cap->oap))
    {
        ...
        prep_redo(cap->oap->regname, cap->count0, NUL,
                  cap->cmdchar, NUL, NUL, cap->nchar);
    ...
    else
    {
        ...
        if(VIsual_active)
        {
            ...
            if(regname == 0
               ...
               || regname == '-')
            {
                // The delete might overwrite the
                // register we want to put, save it first
                savereg = copy_register(regname);
            }

            // To place the cursor correctly after a blockwise put,
            // and to leave the text in the correct position when putting
            // over a selection with 'virtualedit' and past the end of the
            // line, we use the 'c' operator in do_put(), which requires the
            // visual selection to still be active.
                   ...
                   && curwin->w_cursor.lnum < curbuf->b_op_start.lnum))
            {
                ...
                // line or end of file, put forward.
                dir = FORWARD;
            }
        ...
        // When all lines were selected and deleted do_put()
        // leaves an empty line that needs to be deleted now.
        if(empty && *ml_get(curbuf->b_ml.ml_line_count) == NUL)
        {
            ml_delete(curbuf->b_ml.ml_line_count, true);

            // If the cursor was in that line,
            // move it to the end of the last line.
            if(curwin->w_cursor.lnum > curbuf->b_ml.ml_line_count)
            {
                curwin->w_cursor.lnum = curbuf->b_ml.ml_line_count;
                coladvance((columnum_kt)MAXCOL);
...
/// "o" and "O" commands.
static void nv_open(cmdarg_st *cap)
{
    // "do" is ":diffget"
...
static void nv_event(cmdarg_st *cap)
{
    // Garbage collection should have been executed before blocking for
    // events in the `os_inchar` in `state_enter`, but we also disable it
    // here in case the `os_inchar` branch was not executed
    // (!multiqueue_empty(loop.events), which could have `may_garbage_collect`
    // set to true in `normal_check`).
    //
    // That is because here we may run code that calls `os_inchar`
    // later(`f_confirm` or `get_keystroke` for example), but in these cases
    // it is not safe to perform garbage collection because there could be
    // unreferenced lists or dicts being used.
