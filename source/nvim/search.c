/// @file nvim/search.c
///
/// code for normal mode searching commands
///
/// This file contains various searching-related routines.
...
static uchar_kt lastc[2] = {NUL, NUL};
static uchar_kt lastc_bytes[MB_MAXBYTES + 1];
...
/// translate search pattern for regexp_compile()
///
/// @param regmatch
...
int search_regcomp(uchar_kt *pat,
                   int pat_save,
                   int pat_use,
                   int options,
                   regmmatch_st *regmatch)
{
    ...
    regmatch->regprog = regexp_compile(pat, magic ? RE_MAGIC : 0);
...
void save_re_pat(int idx, uchar_kt *pat, int magic)
{
    if(spats[idx].pat != pat)
    {
        free_spat(&spats[idx]);
        spats[idx].pat = ustrdup(pat);
...
void save_search_patterns(void)
{
    if(save_level++ == 0)
    {
        ...
        if(spats[0].pat != NULL)
        {
            saved_spats[0].pat = ustrdup(spats[0].pat);
        }
        ...
        if(spats[1].pat != NULL)
        {
            saved_spats[1].pat = ustrdup(spats[1].pat);
        }
...
int ignorecase(uchar_kt *pat)
{
    return ignorecase_opt(pat, p_ic, p_scs);
}

/// As ignorecase() put pass the "ic" and "scs" flags.
int ignorecase_opt(uchar_kt *pat, int ic_in, int scs)
{
    int ic = ic_in;
...
/// Return TRUE if pattern @b pat has an uppercase character.
int pat_has_uppercase(uchar_kt *pat)
{
    uchar_kt *p = pat;
...
void set_last_csearch(int c, uchar_kt *s, int len)
{
    *lastc = c;
    lastc_bytelen = len;
...
uchar_kt *last_search_pat(void)
{
    return spats[last_idx].pat;
}

/// Reset search direction to forward.
/// For "gd" and "gD" commands.
...
void set_last_search_pat(const uchar_kt *s, int idx, int magic, int setlast)
{
    free_spat(&spats[idx]);
    ...
    if(save_level)
    {
        ...
        {
            saved_spats[idx].pat = ustrdup(spats[idx].pat);
        }
...
void last_pat_prog(regmmatch_st *regmatch)
{
    ...
    (void)search_regcomp((uchar_kt *)"", 0, last_idx, SEARCH_KEEP, regmatch);
    --emsg_off;
}
...
///
/// @param win
/// window to search in, can be NULL for a buffer without a window!
///
/// @param buf
/// @param pos
/// @param dir
             ...
             proftime_kt *tm)
{
    ...
    do
    {
        // When not accepting a match at the start position set
        // "extra_col" to a non-zero value. Don't do that when
        // starting at MAXCOL, since MAXCOL + 1 is zero.
        if(pos->col == MAXCOL)
        {
            start_char_len = 0;
        }
                ...
                && pos->col < MAXCOL - 2)
        {
            // Watch out for the "col" being MAXCOL - 2,
            // used in a closed fold.
        ...
        {
            for(/* nothing */; lnum > 0 && lnum <= buf->b_ml.ml_line_count;
                ...
                if(nmatched > 0)
                {
                    ...
                    if(dir == FORWARD && at_first_line)
                    {
                                     ...
                                     < (int)start_pos.col + extra_col)))
                        {
                            ...
                            // forward.
                            if(ustrchr(p_cpo, CPO_SEARCH) != NULL)
                            {
                                if(nmatched > 1)
                                {
                                ...
                                matchcol = endpos.col;
                                // for empty match (matchcol == matchpos.col):
                                // advance one char
                            }
                            else
                            {
                            ...
                            if(ptr[matchcol] == NUL
                               || (nmatched = vim_regexec_multi(&regmatch, win,
                                                                buf, lnum,
                                                                matchcol, tm)) == 0)
                            {
                                match_ok = false;
                            ...
                            // For bigger values the next line pointer ptr might
                            // not be a buffer line.
                            if(matchpos.lnum != 0)
                            {
                                break;
                            }
                    ...
                    if(dir == BACKWARD)
                    {
                        ...
                        // If there is only a match after the
                        // cursor, skip this match.
                        if(!match_ok)
                        {
                            continue;
                        }
                            ...
                            && matchpos.col == endpos.col))
                    {
                        ...
                        if(endpos.col == 0)
                        {
                            if(pos->lnum > 1) // just in case
                            {
                                ...
                                pos->col =
                                    (columnum_kt)ustrlen(ml_get_buf(buf,
                                                               pos->lnum,
                                                               FALSE));
                            }
                        }
            ...
            {
                give_warning((uchar_kt *)_(dir == BACKWARD
                                         ? top_bot_msg : bot_top_msg),
                             true);
            }
        }
    ...
    {
        pos->lnum = buf->b_ml.ml_line_count;
        pos->col = (int)ustrlen(ml_get_buf(buf, pos->lnum, FALSE));
...
/// Search for the 'count'th occurrence of pattern 'pat'
/// in direction 'dirc' If 'dirc' is 0: use previous dir.
///
...
///
/// @note
/// If spats[0].off.line == TRUE and spats[0].off.off == 0 this
              ...
              proftime_kt *tm)
{
    ...
    long c;
    uchar_kt *dircp;
    uchar_kt *strcopy = NULL;
    uchar_kt *ps;

    // A line offset is not remembered, this is vi compatible.
    if(spats[0].off.line && ustrchr(p_cpo, CPO_LINEOFF) != NULL)
    {
    ...
    // If the cursor is in a closed fold,
    // don't find another match in the same fold.
    if(dirc == '/')
    {
    ...
    for(;;)
    {
        ...
        if(pat != NULL && *pat != NUL) // look for (new) offset
        {
            ...
            if(strcopy != ps)
            {
                ...
                searchcmdlen += (int)(ustrlen(pat) - ustrlen(strcopy));
                pat = strcopy;
                searchstr = strcopy;
            }
...
/// white-space), starting from pos and going in direction dir. @b pos will
/// contain the position of the match found. Blank lines match only if
/// ADDING is set. if p_ic is set then the pattern must be in lowercase.
///
/// @return
/// OK for success, or FAIL if no line found.
int search_for_exact_line(filebuf_st *buf, apos_st *pos, int dir, uchar_kt *pat)
{
    linenum_kt start = 0;
    uchar_kt *ptr;
    uchar_kt *p;
    ...
    for(;;)
    {
        ...
        else if(*p != NUL) // Ignore empty lines.
        {
            ...
            if((p_ic
                ? mb_strnicmp(p, pat, (size_t)compl_length)
                : ustrncmp(p, pat, compl_length)) == 0)
            {
                return OK;
            }
        }
...
int searchc(cmdarg_st *cap, int t_cmd)
{
    ...
    else // repeat previous search
    {
        ...
        // at.
        if(ustrchr(p_cpo, CPO_SCOLON) == NULL && count == 1 && t_cmd)
        {
            stop = FALSE;
        }
    }
...
/// Return FALSE if "col" is zero. Update "*prevcol" to the column
/// of the previous character, unless "prevcol" is NULL.
/// Handles multibyte string correctly.
static int check_prevcol(uchar_kt *linep, int col, int ch, int *prevcol)
{
    --col;
...
static int find_rawstring_end(uchar_kt *linep, apos_st *startpos, apos_st *endpos)
{
    ...
    delim_copy = ustrndup(linep + startpos->col + 1, delim_len);
    ...
    for(lnum = startpos->lnum; lnum <= endpos->lnum; ++lnum)
    {
                        ...
                        ? startpos->col + 1 : 0); *p; ++p)
        {
            ...
            if(*p == ')'
               && p[delim_len + 1] == '"'
               && ustrncmp(delim_copy, p + 1, delim_len) == 0)
            {
                found = true;
                break;
            }
...
/// find the matching paren or brace, if it exists within
/// maxtravel lines of the cursor. A maxtravel of 0 means search until falling
/// off the edge of the file.
///
/// @param oap
/// is only used to set oap->motion_type for a linewise motion,
/// it can be NULL
///
/// @param initc
/// is the character to find a match for. NUL means to find the
...
///
/// @param flags
/// - FM_BACKWARD   search backwards (when initc is '/', '*' or '#')
...
apos_st *findmatchlimit(oparg_st *oap, int initc, int flags, int64_t maxtravel)
{
    ...
    uchar_kt *linep; // pointer to current line
    uchar_kt *ptr;
    ...
    cpo_match = (ustrchr(p_cpo, CPO_MATCH) != NULL);
    cpo_bsl = (ustrchr(p_cpo, CPO_MATCHBSL) != NULL);
    ...
    {
        // Either initc is '#', or no initc was given
        // and we need to look under the cursor.
        if(initc == '#')
        {
            hash_dir = dir;
        }
        else
        {
            ...
            if(!cpo_match)
            {
                ...
                if(*ptr == '#' && pos.col <= (columnum_kt)(ptr - linep))
                {
                    ...
                    if(ustrncmp(ptr, "if", 2) == 0
                       || ustrncmp(ptr, "endif", 5) == 0
                       || ustrncmp(ptr, "el", 2) == 0)
                    {
                        hash_dir = 1;
                    }
        ...
        if(hash_dir)
        {
            ...
            if(initc != '#')
            {
                ...
                if(ustrncmp(ptr, "if", 2) == 0 || ustrncmp(ptr, "el", 2) == 0)
                {
                    hash_dir = 1;
                }
                else if(ustrncmp(ptr, "endif", 5) == 0)
                {
                    hash_dir = -1;
                }
                else
            ...
            while(!got_int)
            {
                ...
                else
                {
                    ...
                    else if(initc == '#' && ustrncmp(ptr, "el", 2) == 0)
                    {
                        if(count == 0)
                        {
                            return &pos;
    ...
    while(!got_int)
    {
        ...
        if(backwards)
        {
            ...
            if(pos.col == 0) // at start of line, go to prev. one
            {
                ...
                linep = ml_get(pos.lnum);
                pos.col = (columnum_kt)ustrlen(linep); // pos.col on trailing NUL
                do_quotes = -1;
                line_breakcheck();
        ...
        else if(do_quotes == -1)
        {
            ...
            // result is 1 with even number of quotes
            do_quotes &= 1;

            // If we find an uneven count, check current line
            // and previous one for a '\' at the end.
            if(!do_quotes)
            {
                inquote = FALSE;
                ...
                if(pos.lnum > 1)
                {
                    ...
                    // ml_get() only keeps one line,
                    // need to get linep again
                    linep = ml_get(pos.lnum);
                }
            }
        }
        ...
        switch(c)
        {
            ...
            case '"':
                // a quote that is preceded with an odd number of
            ...
            // Skip things in single quotes: 'x' or '\x'.  Be careful for
            // single single quotes, eg jon's. Things like '\233' or '\x3f'
            // are not skipped, there is never a brace in them.
            ...
            default:
                   ...
                   && (c == initc || c == findc))
                {
                    ...
                    {
                        for(col = pos.col;
                            check_prevcol(linep, col, '\\', &col); /* none */)
                        {
                            bslcnt++;
                        }
                    }
...
/// @todo
/// skip strings.
static int check_linecomment(uchar_kt *line)
{
    uchar_kt  *p;
    p = line;
    ...
    if(curbuf->b_p_lisp)
    {
        if(ustrchr(p, ';') != NULL) // there may be comments
        {
            int in_str = FALSE; // inside of string
            p = line; // scan from start

            while((p = xstrpbrk(p, (uchar_kt *)"\";")) != NULL)
            {
                if(*p == '"')
                {
                    if(in_str)
...
void showmatch(int c)
{
            ...
            && lpos->lnum < curwin->w_botline)
    {
               ...
               && vcol < curwin->w_leftcol + curwin->w_width))
        {
            ...
            // brief pause, unless 'm' is present in 'cpo' and
            // a character is available.
            if(ustrchr(p_cpo, CPO_SHOWMATCH) != NULL)
            {
...
/// Find the start of the next sentence in direction
...
int findsent(int dir, long count)
{
    ...
    while(count--)
    {
        ...
        if(gchar_pos(&pos) == NUL)
        {
            ...
            // if on the start of a paragraph or a section
            // and searching forward, go to the next line
        }
        else if(dir == FORWARD
                && pos.col == 0
        ...
        startlnum = pos.lnum;
        cpo_J = ustrchr(p_cpo, CPO_ENDOFSENT) != NULL;
        ...
        for(;;) // find end of sentence
        {
            ...
            if(c == '.' || c == '!' || c == '?')
            {
                ...
                while(ustrchr((uchar_kt *)")]\"'",
                                 c = gchar_pos(&tpos)) != NULL);
...
/// @param count
/// @param what
/// - If @b what is NUL we go to the next paragraph.
/// - If @b what is '{' or '}' we go to the next section.
///
/// @param both
/// If @b both is TRUE also stop at '}'.
///
/// @return
             ...
             int both)
{
    ...
    {
        if((curwin->w_cursor.col = (columnum_kt)ustrlen(ml_get(curr))) != 0)
        {
            --curwin->w_cursor.col;
            *pincl = true;
        }
...
static int inmacro(uchar_kt *opt, uchar_kt *s)
{
    uchar_kt *macro;
...
///
/// @return
/// FAIL if the cursor was already at the end of the file.
...
int fwd_word(long count, int bigword, int eol)
{
    ...
    while(--count >= 0)
    {
        // When inside a range of folded lines,
...
///
/// @return
/// FAIL if top of the file was reached.
int bck_word(long count, int bigword, int stop)
{
    ...
    {
        // When inside a range of folded lines,
        // move to the first char of the first line.
...
int end_word(long count, int bigword, int stop, int empty)
{
    ...
    while(--count >= 0)
    {
        ...
        // If we're in the middle of a word, we just
        // have to move to the end of it.
...
/// @param eol      TRUE: stop at end of line.
///
/// @return
...
///
/// @return
/// TRUE when end-of-file reached, FALSE otherwise.
...
///
/// @param count
/// @param at_start_send   cursor is at start of sentence
...
extend:
    ...
    // If the cursor started on a blank, check if it
    // is just before the start of the next sentence.
                  ...
                  int other)
{
    ...
    // user wants.
    save_cpo = p_cpo;
    p_cpo = (uchar_kt *)(ustrchr(p_cpo, CPO_MATCHBSL) != NULL ? "%M" : "%");
...
again:
    ...
    // Search for matching "</aaa>".
    // First isolate the "aaa".
    inc_cursor();
    ...
    {
        uchar_kt *c = get_cursor_pos_ptr();

        // Exclude the '<' of the end tag.
        // If the closing tag is on new line, do not decrement cursor,
        // but make operation exclusive, so that the linefeed will be selected
    ...
    if(!do_include)
    {
        ...
        // If we now have the same text as before reset
    ...
    if(VIsual_active)
    {
        // If the end is before the start there is no
        // text between tags, select the char under the cursor.
    ...
    else
    {
        ...
        if(lt(end_pos, start_pos))
        {
            ...
            // there is no text between tags; operate on an empty area.
            curwin->w_cursor = start_pos;
            oap->inclusive = false;
        }
        else
...
extend:
    ...
    // If there are no empty lines at the end, try to find some
    // empty lines at the start (unless that has been done already).
...
/// Quote character escaped by one of the characters in
/// "escape" is not counted as a quote.
///
/// @param escape   escape characters, can be NULL
///
/// @return
/// column number of "quotechar" or -1 when not found.
static int find_next_quote(uchar_kt *line,
                           int col,
                           int quotechar,
                           uchar_kt *escape)
{
    ...
    for(;;)
    {
        ...
        else if(escape != NULL && ustrchr(escape, c))
        {
            ++col;
        }
        else if(c == quotechar)
...
/// Search backwards in @b line from column @b col_start to find @b quotechar.
/// Quote character escaped by one of the characters in @b escape is not
/// counted as a quote.
///
/// @param line
/// @param col_start
...
///
/// @return
/// the found column or zero.
static int find_prev_quote(uchar_kt *line,
                           int col_start,
                           int quotechar,
                           uchar_kt *escape)
{
    ...
    while(col_start > 0)
    {
        ...
        {
            while(col_start - n > 0
                  && ustrchr(escape, line[col_start - n - 1]) != NULL)
            {
                ++n;
            }
        }
...
int current_quote(oparg_st *oap, long count, int include, int quotechar)
{
    ...
    if(!vis_empty)
    {
        // Check if the existing selection exactly
        // spans the text inside quotes.
        if(vis_bef_curs)
        {
    ...
    if(!vis_empty && line[col_start] == quotechar)
    {
        ...
        if(vis_bef_curs)
        {
            // Assume we are on a closing quote:
            // move to after the next opening quote.
            ...
            col_end = find_next_quote(line, col_start + 1,
                                      quotechar, curbuf->b_p_qe);
        ...
        else
        {
            ...
            col_start = find_prev_quote(line, col_end,
                                        quotechar, curbuf->b_p_qe);
    ...
    else
    {
        ...
        col_start = find_prev_quote(line, col_start,
                                    quotechar, curbuf->b_p_qe);
    ...
    // When "include" is TRUE, include spaces after
    // closing quote or before the starting quote.
    if(include)
    {
...
int current_search(long count, int forward)
{
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        else if(!i && !result)
        {
            ...
            else // try again from end of buffer
            {
                ...
                pos.col =
                    (columnum_kt)ustrlen(ml_get(curwin->w_buffer->b_ml.ml_line_count));
            }
        }
    ...
    // move to match, except for zero-width matches, in which case,
    // we are already on the next match
    if(!one_char)
    {
        searchit(curwin, curbuf, &pos,
                 (forward ? FORWARD : BACKWARD),
                 spats[last_idx].pat, 0L,
                 flags | SEARCH_KEEP, RE_SEARCH, 0, NULL);
    }
...
static int is_one_char(uchar_kt *pattern, bool move)
{
                ...
                SEARCH_KEEP + flag, RE_SEARCH, 0, NULL) != FAIL)
    {
        ...
        do
        {
            ...
            nmatched = vim_regexec_multi(&regmatch, curwin,
                                         curbuf, pos.lnum,
                                         regmatch.startpos[0].col, NULL);
...
///
void find_pattern_in_path(uchar_kt *ptr,
                          int dir,
                          size_t len,
                          int whole,
                          int skip_comments,
                          ...
                          linenum_kt end_lnum)
{
    ...
    // is faster than quote_meta/regcomp/regexec "ptr"
    if(type != CHECK_PATH
       && type != FIND_DEFINE
       && !(compl_cont_status & CONT_SOL))
    {
    ...
    if(*inc_opt != NUL)
    {
        incl_regmatch.regprog =
            regexp_compile(inc_opt, p_magic ? RE_MAGIC : 0);
    ...
    if(type == FIND_DEFINE
       && (*curbuf->b_p_def != NUL || *p_def != NUL))
    {
        def_regmatch.regprog = regexp_compile(*curbuf->b_p_def == NUL
    ...
    for(;;)
    {
           ...
           && vim_regexec(&incl_regmatch, line, (columnum_kt)0))
        {
            ...
            if(inc_opt != NULL && strstr((char *)inc_opt, "\\zs") != NULL)
            {
                ...
                new_fname =
                    find_file_name_in_path(incl_regmatch.startp[0],
            ...
            else
            {
                ...
                new_fname =
                    file_name_in_line(incl_regmatch.endp[0], 0,
                                      FNAME_EXP | FNAME_INCL | FNAME_REL,
                                      1L, p_fname, NULL);
            }
            ...
            if(new_fname != NULL)
            {
                ...
                for(i = 0;; i++)
                {
                    ...
                    if(kEqualFiles
                       & path_full_compare(new_fname, files[i].name, true))
                    {
                        if(type != CHECK_PATH
                           && action == ACTION_SHOW_ALL && files[i].matched)
                        {
                            ...
                            // don't display if 'q' typed at "--more--"
                            if(!got_int)
                            {
                                // message
                   ...
                   || (new_fname == NULL && !already_searched)))
            {
                ...
                if(!got_int)
                {
                    // don't display if 'q' typed for "--more--" message
                    for(i = 0; i <= depth_displayed; i++)
                    {
                        MSG_PUTS("  ");
                    }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            for(p = incl_regmatch.endp[0];
                                *p && !is_file_name_char(*p); p++)
                            { /* nothing */ }

                            for(i = 0; is_file_name_char(p[i]); i++)
                            { /* nothing */ }
                        }
            ...
            if(new_fname != NULL)
            {
                ...
                else
                {
                    if(++depth == old_files)
                    {
                        // Something wrong. We will forget one of our
                        // already visited files now.
...
search_line:
            ...
            if(def_regmatch.regprog == NULL || define_matched)
            {
                   ...
                   || (compl_cont_status & CONT_SOL))
                {
                    ...
                    {
                        matched = !ustrncmp(startp, ptr, len);
                    }
                        ...
                        && vim_regexec(&regmatch, line, (columnum_kt)(p - line)))
                {
                    ...
                    {
                        if((*line != '#'
                            || ustrncmp(skipwhite(line + 1), "define", 6) != 0)
        ...
        if(matched)
        {
            if(action == ACTION_EXPAND)
            {
                ...
                {
                    // IOSIZE > compl_length, so the ustrncpy works
                    ustrncpy(IObuff, aux, i);
                    ...
                    // we read a line, set "already" to check this "line"
                    // later if depth >= 0 we'll increase files[depth].lnum
                    // far bellow
            ...
            else if(action == ACTION_SHOW_ALL)
            {
                ...
                if(curr_fname != prev_fname)
                {
                    ...
                    // don't display if 'q' typed at "--more--" message
                    if(!got_int)
                    {
                ...
                // Set matched flag for this file and
                // all the ones that include it
                for(i = 0; i <= depth; ++i)
                {
            ...
            else if(--count <= 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // autocommands may have changed the lnum,
...
static void show_pat_in_path(uchar_kt *line,
                             int type,
                             int did_show,
                             int action,
                             FILE *fp,
...
/// If true set substitute pattern as last used.
/// Otherwise sets search pattern.
...
/// @return
/// true if search pattern was the last used one
