...
FUNC_ATTR_NONNULL_ALL
{
    ...
    if(host_end && addr != host_end)
    {
        ...
        *host_end = '\0';
        ...
        intmax_t iport;
        int ret = getdigits_safe(&(uchar_kt *) { (uchar_kt *)port }, &iport);
        ...
        int retval = uv_getaddrinfo(&loop->uv,
                                    &request,
                                    NULL,
                                    addr,
                                    port,
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    if(result < 0)
    {
        if(result == UV_EACCES)
        {
            ...
            *path_tail((uchar_kt *)watcher->addr) = NUL;

            if(!os_path_exists((uchar_kt *)watcher->addr))
            {
                result = UV_ENOENT;
            }
        }
