...
FUNC_ATTR_NONNULL_ALL
{
    ...
    else
    {
        ...
        if(last_container.special_val == NULL && (obj.is_special_string         ||
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    if(really_hasnul)
    {
        ...
        list->lv_refcount++;
        typval_T tv;
        create_special_dict(&tv, binary ? kMPBinary : kMPString,
                            ((typval_T)
        {
            .v_type = VAR_LIST,
            .v_lock = VAR_UNLOCKED,
        ...
        if(elw_ret == -1)
        {
            ...
            return (typval_T)
            {
                .v_type = VAR_UNKNOWN, .v_lock = VAR_UNLOCKED
            };
        }
...
FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_ALWAYS_INLINE
{
    ...
    if(len == 0)
    {
        POP(((typval_T)
        {
            .v_type = VAR_STRING, .vval = { .v_string = NULL },
        }), false);
        goto parse_json_string_ret;
    }
        ...
        if(*t == '\\')
        {
            ...
            switch(*t)
            {
                case 'u':
                {
                    ...
                    t += 4;
                    unsigned long ch;
                    vim_str2nr((char_u *) ubuf, NULL, NULL, STR2NR_HEX | STR2NR_FORCE, NULL, &ch, 4);
...
parse_json_number_check:
    ...
    typval_T tv = {.v_type = VAR_NUMBER, .v_lock = VAR_UNLOCKED, };
