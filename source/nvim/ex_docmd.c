/// @file nvim/ex_docmd.c
///
/// functions for executing an Ex command line.
...
static void save_dbg_stuff(excmd_dbg_st *dsp)
{
    ...
    // Necessary for debugging an inactive
...
///
/// @param improved  TRUE for "improved Ex" mode
...
int do_cmdline(uchar_kt *cmdline,
               ...
               int flags)
{
    uchar_kt *next_cmdline; // next cmd to execute
    uchar_kt *cmdline_copy = NULL; // copy of cmd line
    ...
    do
    {
        ...
        if(next_cmdline == NULL)
        {
            ...
            if(flags & DOCMD_KEEPLINE)
            {
                ...
                if(count == 0)
                {
                    repeat_cmdline = ustrdup(next_cmdline);
                }
                else
                {
                    repeat_cmdline = NULL;
        ...
        {
            next_cmdline = ustrdup(next_cmdline);
        }
        ...
        recursive++;
        next_cmdline = do_one_cmd(&cmdline_copy,
                                  flags,
                                  &cstack,
                                  cmd_getline,
                                  cmd_cookie);
        ...
        else
        {
            ...
            xstrmove(cmdline_copy, next_cmdline);
    ...
    while(!((got_int || (did_emsg && force_abort) || did_throw) && cstack.cs_trylevel == 0)
          && !(did_emsg
               ...
               && (getline_equal(fgetline, cookie, getexmodeline)
                   || getline_equal(fgetline, cookie, getexline)))
          && (next_cmdline != NULL || cstack.cs_idx >= 0 || (flags & DOCMD_REPEAT)));
    // empty-while-body
    ...
    do_errthrow(&cstack,
                getline_equal(fgetline, cookie, get_func_line)
                ? (uchar_kt *)"endfunction" : (uchar_kt *)NULL);
    ...
    if(trylevel == 0)
    {
        ...
        if(did_throw)
        {
            ...
            switch(current_exception->type)
            {
                ...
                default:
                    p = ustrdup((uchar_kt *)_(e_internal));
            }
    ...
    else
    {
           ...
           && ex_nesting_level + 1 <= debug_break_level)
        {
            do_debug(getline_equal(fgetline, cookie, getsourceline)
                     ? (uchar_kt *)_("End of sourced file")
                     : (uchar_kt *)_("End of function"));
        }
    }

    // Restore the exception environment
    // (done after returning from the debugger).
    if(flags & DOCMD_EXCRESET)
    {
...
/// If @b fgetline is get_loop_line(), return TRUE if the
/// getline it uses equals @b func. Otherwise return TRUE
/// when @b fgetline equals @b func.
///
/// @param fgetline
/// @param cookie    argument for fgetline()
/// @param func
...
/// Helper function to apply an offset for buffer commands,
/// i.e. ":bdelete", ":bwipeout", etc.
///
                          ...
                          void *cookie)
{
    ...
    // Reset browse, confirm, etc..
    // They are restored when returning, for recursive calls.
    save_cmdmod = cmdmod;
        ...
        switch(*p)
        {
            ...
            case 'f':
            {
                ...
                cmdmod.filter_regmatch.regprog =
                    regexp_compile(reg_pat, RE_MAGIC);
            ...
            case 't':
            {
                if(checkforcmd(&p, "tab", 3))
                {
                    ...
                    else
                    {
                        ...
                        {
                            errormsg = (uchar_kt *)_(e_invrange);
                            goto doend;
                        }
    ...
    uchar_kt *after_modifier = ea.cmd;
    ...
    // NUM
    //
    // The ea.cmd pointer is updated to point to the first character
    // following the range spec. If an initial address is found, but
    // no second, the upper bound is equal to the lower.
       ...
       && has_event(EVENT_CMDUNDEFINED))
    {
        ...
        p = ustrndup(ea.cmd, p - ea.cmd);
        int ret = apply_autocmds(EVENT_CMDUNDEFINED, p, p, TRUE, NULL);

        xfree(p);

        // If the autocommands did something and didn't cause an error,
        // try finding the command again.
    ...
    if(p == NULL)
    {
        if(!ea.skip)
        {
            errormsg = (uchar_kt *)_("E464: Ambiguous use of user-defined command");
        }
    ...
    if(ea.cmdidx == CMD_SIZE)
    {
        if(!ea.skip)
        {
            ustrcpy(IObuff, _("E492: Not an editor command"));
    ...
    if(!ea.skip)
    {
           ...
           && (!curbuf->terminal || ea.cmdidx != CMD_put))
        {
            ...
            errormsg = (uchar_kt *)_(e_modifiable);
            goto doend;
        }
    ...
    if(!ea.skip && !ni)
    {
        ...
        if(!global_busy && ea.line1 > ea.line2)
        {
            if(msg_silent == 0)
            {
                ...
                {
                    errormsg = (uchar_kt *)_("E493: Backwards range given");
                    goto doend;
                }
    ...
    if(((ea.argt & WHOLEFOLD)  || ea.addr_count >= 2)
       && !global_busy
       && ea.addr_type == ADDR_LINES)
    {
        // Put the first line at the start of a closed fold,
        // put the last line at the end of a closed fold.
        (void)hasFolding(ea.line1, &ea.line1, NULL);
        (void)hasFolding(ea.line2, NULL, &ea.line2);
    }

    // For the ":make" and ":grep" commands we insert the
    // 'makeprg'/'grepprg' option here, so things like % get expanded.
    ...
    if(ea.cmdidx == CMD_write || ea.cmdidx == CMD_update)
    {
        if(*ea.arg == '>') // append
        {
            ...
            {
                errormsg = (uchar_kt *)_("E494: Use w or w>>");
                goto doend;
            }
            ...
            || ea.usefilter)
    {
        ...
        // Also do this for ":read !cmd", ":write !cmd"
        // and ":global". Any others?
        for(p = ea.arg; *p; ++p)
        {
    ...
    {
        if(valid_yank_reg(*ea.arg,
                          (ea.cmdidx != CMD_put && !IS_USER_CMDIDX(ea.cmdidx))))
        {
            ea.regname = *ea.arg++;
            ...
            {
                set_expr_line(ustrdup(ea.arg));
                ea.arg += ustrlen(ea.arg);
            }
    ...
    if((ea.argt & COUNT)
       && ascii_isdigit(*ea.arg)
       && (!(ea.argt & BUFNAME)
           || *(p = skipdigits(ea.arg)) == NUL
           || ascii_iswhite(*p)))
    {
        ...
        {
            errormsg = (uchar_kt *)_(e_zerocount);
            goto doend;
        }
    ...
    if(!ni
       ...
       && *ea.arg != NUL
       && *ea.arg != '"'
       && (*ea.arg != '|' || (ea.argt & TRLBAR) == 0))
    {
        errormsg = (uchar_kt *)_(e_trailing);
        goto doend;
    }

    if(!ni && (ea.argt & NEEDARG) && *ea.arg == NUL)
    {
        errormsg = (uchar_kt *)_(e_argreq);
        goto doend;
    }

    // Skip the command when it's not going to be executed.
    // The commands like :if, :endif, etc. always need to be
    // executed. Also make an exception for commands that handle
    // a trailing command themselves.
    if(ea.skip)
    {
        switch(ea.cmdidx)
        {
    ...
    // Accept buffer name.  Cannot be used at the same time
    // with a buffer number. Don't do this for a user command.
    if((ea.argt & BUFNAME)
       && *ea.arg != NUL
       && ea.addr_count == 0
       && !IS_USER_CMDIDX(ea.cmdidx))
    {
        ...
        if(ea.cmdidx == CMD_bdelete
           || ea.cmdidx == CMD_bwipeout
        ...
        ea.line2 = buflist_findpat(ea.arg,
                                   p,
                                   (ea.argt & BUFUNL) != 0,
                                   FALSE,
                                   FALSE);
...
///
int checkforcmd(uchar_kt **pp, char *cmd, int len)
{
    int i;

    for(i = 0; cmd[i] != NUL; ++i)
    {
        if(((uchar_kt *)cmd)[i] != (*pp)[i])
        {
            break;
        }
    }
...
/// invisible otherwise.
static void append_command(uchar_kt *cmd)
{
    uchar_kt *d;
    uchar_kt *s = cmd;
    ustrcat(IObuff, ": ");
    d = IObuff + ustrlen(IObuff);
...
static uchar_kt *find_command(exargs_st *eap, int *full)
{
    ...
    else
    {
        ...
        if(p == eap->cmd && ustrchr((uchar_kt *)"@!=><&~#", *p) != NULL)
        {
            ++p;
        }
        ...
        if(*eap->cmd == 'd' && (p[-1] == 'l' || p[-1] == 'p'))
        {
            ...
            for(i = 0; i < len; ++i)
            {
                if(eap->cmd[i] != ((uchar_kt *)"delete")[i])
                {
                    break;
                }
            }
        ...
        {
            if(ustrncmp(excmd_info[(int)eap->cmdidx].cmd_name,
...
/// @param compl    completion flags or NULL
///
/// @return
...
const char *set_one_cmd_context(expand_st *xp, const char *buff)
{
    ...
    for(cmd = buff; ustrchr((const uchar_kt *)" \t:|", *cmd) != NULL; cmd++)
    { /* empty body */ }

    xp->xp_pattern = (uchar_kt *)cmd;
    ...
    cmd = (const char *)skip_range((const uchar_kt *)cmd, &xp->xp_context);
    ...
    else
    {
        ...
        // check for non-alpha command
        if(p == cmd && ustrchr((const uchar_kt *)"@*!=><&~#", *p) != NULL)
        {
            p++;
        }
        ...
        {
            if(ustrncmp(excmd_info[(int)ea.cmdidx].cmd_name, cmd, len) == 0)
            {
                break;
            }
        }
    ...
    if(ea.cmdidx == CMD_SIZE)
    {
        if(*cmd == 's' && ustrchr((const uchar_kt *)"cgriI", cmd[1]) != NULL)
        {
            ea.cmdidx = CMD_substitute;
            p = cmd + 1;
        }
        ...
        {
            ea.cmd = (uchar_kt *)cmd;
            p = (const char *)find_ucmd(&ea, (uchar_kt *)p, NULL, xp, &context);
    ...
    if((ea.argt & EDITCMD) && !usefilter && *arg == '+')
    {
        ...
        p = arg + 1;
        arg = (const char *)skip_cmd_arg((uchar_kt *)arg, false);
    ...
    if(ea.argt & XFILE)
    {
        ...
        // Allow spaces within back-quotes to count as
        // part of the argument being expanded.
        xp->xp_pattern = skipwhite((const uchar_kt *)arg);
        ...
        // If we are still inside the quotes, and we
        // passed a space, just expand from there.
        if(bow != NULL && in_quote)
        {
            xp->xp_pattern = (uchar_kt *)bow;
        }
        ...
        if(usefilter || ea.cmdidx == CMD_bang || ea.cmdidx == CMD_terminal)
        {
            ...
            if(xp->xp_pattern == skipwhite((const uchar_kt *)arg))
            {
    ...
    switch(ea.cmdidx)
    {
        ...
        case CMD_filter:
            if(*arg != NUL)
            {
                arg = (const char *)skip_vimgrep_pat((uchar_kt *)arg, NULL, NULL);
            }
            ...
            return (const char *)skipwhite((const uchar_kt *)arg);
        ...
        case CMD_match:
            if(*arg == NUL || !ends_excmd(*arg))
            {
                ...
                arg = (const char *)skipwhite(skiptowhite((const uchar_kt *)arg));

                if(*arg != NUL)
                {
                    xp->xp_context = EXPAND_NOTHING;
                    arg = (const char *)skip_regexp((uchar_kt *)arg + 1,
                                                    (uint8_t)(*arg),
                                                    p_magic,
                                                    NULL);
                }
            ...
            return (const char *)find_nextcmd((uchar_kt *)arg);
        ...
        case CMD_command:
            ...
            while(*arg == '-')
            {
                ...
                if(*p == NUL)
                {
                    ...
                    if(ustrnicmp(arg, "complete", p - arg) == 0)
                    {
                        xp->xp_context = EXPAND_USER_COMPLETE;
                        xp->xp_pattern = (uchar_kt *)p + 1;
                        return NULL;
                    }
                    else if(ustrnicmp(arg, "nargs", p - arg) == 0)
                    {
                        xp->xp_context = EXPAND_USER_NARGS;
                        xp->xp_pattern = (uchar_kt *)p + 1;
                        return NULL;
                    }
                    else if(ustrnicmp(arg, "addr", p - arg) == 0)
                    {
                ...
                arg = (const char *)skipwhite((uchar_kt *)p);
            }
            ...
            return (const char *)skipwhite((const uchar_kt *)p);
        ...
        case CMD_substitute:
        {
            ...
            {
                arg++; // Skip "from" part.
                arg = (const char *)skip_regexp((uchar_kt *)arg,
                                                delim,
                                                p_magic,
                                                NULL);
            }
        ...
        case CMD_dsplit:
            // Skip count.
            arg = (const char *)skipwhite(skipdigits((const uchar_kt *)arg));
            ...
            if(*arg == '/')
            {
                ...
                if(*arg)
                {
                    arg = (const char *)skipwhite((const uchar_kt *)arg + 1);
        ...
        case CMD_autocmd:
            return (const char *)set_context_in_autocmd(xp, (uchar_kt *)arg, false);

        case CMD_doautocmd:
        case CMD_doautoall:
            return (const char *)set_context_in_autocmd(xp, (uchar_kt *)arg, true);

        case CMD_set:
            set_context_in_set_cmd(xp, (uchar_kt *)arg, 0);
            break;
        ...
        case CMD_call:
        case CMD_return:
            set_context_for_expression(xp, (uchar_kt *)arg, ea.cmdidx);
            break;

        case CMD_unlet:
            while((xp->xp_pattern = (uchar_kt *)strchr(arg, ' ')) != NULL)
            {
        ...
        case CMD_sign:
            set_context_in_sign_cmd(xp, (uchar_kt *)arg);
            break;
        ...
        case CMD_USER_BUF:
            if(context != EXPAND_NOTHING)
            {
                ...
                if(!(ea.argt & XFILE))
                {
                    ...
                    {
                        return (const char *)set_context_in_menu_cmd(xp,
                                                                     (uchar_kt *)cmd,
                                                                     (uchar_kt *)arg,
                                                                     forceit);
                    }
                    ...
                    {
                        return (const char *)set_context_in_map_cmd(xp,
                                                                    (uchar_kt *)"map",
                                                                    (uchar_kt *)arg,
                                                                    forceit,
                                                                    false,
...
uchar_kt *skip_range(const uchar_kt *cmd, int *ctx)
{
    unsigned delim;

    while(ustrchr((uchar_kt *)" \t0123456789.$%'/?-+,;", *cmd) != NULL)
    {
        if(*cmd == '\'')
        {
            if(*++cmd == NUL && ctx != NULL)
...
/// @param eap
/// @param ptr
/// @param addr_type      flag: one of ADDR_LINES, ...
/// @param skip           only skip the address, don't use it
/// @param to_other_file  lag: may jump to other file
                            ...
                            int address_count)
{
    ...
    do
    {
        ...
        for(;;)
        {
            ...
            {
                lnum = compute_buffer_local_count(addr_type,
                                                  lnum,
                                                  (i == '-') ? -1 * n : n);
            }
            else
...
{
    while(ustrchr((uchar_kt *)"lp#", *eap->arg) != NULL)
    {
        if(*eap->arg == 'l')
        {
            eap->flags |= EXFLAG_LIST;
...
void ex_ni(exargs_st *eap)
{
    if(!eap->skip)
    {
        eap->errmsg = (uchar_kt *)N_("E319: The command is not available in this version");
    }
}
...
static uchar_kt *replace_makeprg(exargs_st *eap, uchar_kt *p, uchar_kt **cmdlinep)
{
    ...
    if((eap->cmdidx == CMD_make
        ...
        || eap->cmdidx == CMD_lgrepadd) && !grep_internal(eap->cmdidx))
    {
        ...
        if((pos = (uchar_kt *)strstr((char *)program, "$*")) != NULL)
        {
            // replace $* by given arguments
            i = 1;

            while((pos = (uchar_kt *)strstr((char *)pos + 2, "$*")) != NULL)
            {
                ++i;
            }

            len = (int)ustrlen(p);
            new_cmdline = xmalloc(ustrlen(program) + i * (len - 2) + 1);
            ptr = new_cmdline;

            while((pos = (uchar_kt *)strstr((char *)program, "$*")) != NULL)
            {
                i = (int)(pos - program);
        ...
        {
            new_cmdline = xmalloc(ustrlen(program) + ustrlen(p) + 2);
            ustrcpy(new_cmdline, program);
            ustrcat(new_cmdline, " ");
            ustrcat(new_cmdline, p);
        }
...
int expand_filename(exargs_st *eap, uchar_kt **cmdlinep, uchar_kt **errormsgp)
{
    ...
    while(*p != NUL)
    {
        ...
        if(ustrchr(repl, '$') != NULL || ustrchr(repl, '~') != NULL)
        {
            uchar_kt *l = repl;
           ...
           && !(eap->argt & NOSPC))
        {
            ...
            // Don't escape a backslash here, because
            // rem_backslash() doesn't remove it later.
            static uchar_kt *nobslash = (uchar_kt *)" \t\"|";
            ...
            for(l = repl; *l; ++l)
            {
                if(ustrchr(ESCAPE_CHARS, *l) != NULL)
                {
                    l = ustrdup_escape(repl, ESCAPE_CHARS);
                    xfree(repl);
                    repl = l;
                    break;
                }
        ...
        if((eap->usefilter || eap->cmdidx == CMD_bang)
           && xstrpbrk(repl, (uchar_kt *)"!") != NULL)
        {
            uchar_kt *l;
            l = ustrdup_escape(repl, (uchar_kt *)"!");
            xfree(repl);
            repl = l;
        }
    ...
    if((eap->argt & NOSPC) && !eap->usefilter)
    {
        ...
        if(has_wildcards)
        {
            // May expand environment variables. This can be done
            // much faster with expand_env() than with something
            // else (e.g., calling a shell). After expanding environment
            // variables, check again if there are still wildcards present.
            if(ustrchr(eap->arg, '$') != NULL
               || ustrchr(eap->arg, '~') != NULL)
            {
            ...
            if(p != NULL)
            {
                (void)repl_cmdline(eap, eap->arg, ustrlen(eap->arg), p, cmdlinep);
            }
        }
...
/// Replace part of the command line, keeping eap->cmd,
/// eap->arg and eap->nextcmd correct.
///
/// @param eap
/// @param src
/// points to the part that is to be replaced, of length @b srclen.
///
/// @param srclen
/// @param repl
                            ...
                            uchar_kt **cmdlinep)
{
    ...
    size_t len = ustrlen(repl);
    size_t i = (size_t)(src - *cmdlinep) + ustrlen(src + srclen) + len + 3;

    if(eap->nextcmd != NULL)
    {
        i += ustrlen(eap->nextcmd); // add space for next command
    }

    uchar_kt *new_cmdline = xmalloc(i);
    ...
    ustrcpy(new_cmdline + i, src + srclen);
    src = new_cmdline + i; // remember where to continue

    if(eap->nextcmd != NULL)
    {
        // append next command
        i = ustrlen(new_cmdline) + 1;
        ustrcpy(new_cmdline + i, eap->nextcmd);
...
void separate_nextcmd(exargs_st *eap)
{
    ...
    for(; *p; mb_ptr_adv(p))
    {
        ...
        // :redir @" doesn't either.
                 ...
                 && (eap->cmdidx != CMD_redir || p != eap->arg + 1 || p[-1] != '@'))
                || *p == '|' || *p == '\n')
        {
            // We remove the '\' before the '|', unless USECTRLV is used
            // AND 'b' is present in 'cpoptions'.
            if((ustrchr(p_cpo, CPO_BAR) == NULL
                || !(eap->argt & USECTRLV)) && *(p - 1) == '\\')
            {
                xstrmove(p - 1, p); // remove the '\'
                --p;
            }
            else
            {
    ...
    if(!(eap->argt & NOTRLCOM))
    {
        ...
        ustr_del_trailing_spaces(eap->arg);
    }
}

/// get + command from ex argument
static uchar_kt *getargcmd(uchar_kt **argp)
{
    uchar_kt *arg = *argp;
...
/// Find end of "+command" argument.
/// Skip over "\ " and "\\".
///
/// @param p
/// @param rembs  TRUE to halve the number of backslashes
static uchar_kt *skip_cmd_arg(uchar_kt *p, int rembs)
{
...
static int getargopt(exargs_st *eap)
{
    ...
    int *pp = NULL;
    int bad_char_idx;
    uchar_kt *arg = eap->arg + 2;

    // ":edit ++[no]bin[ary] file"
    if(ustrncmp(arg, "bin", 3) == 0 || ustrncmp(arg, "nobin", 5) == 0)
    {
        if(*arg == 'n')
        {
            arg += 2;
    ...
    if(ustrncmp(arg, "ff", 2) == 0)
    {
        arg += 2;
        pp = &eap->force_ff;
    }
    else if(ustrncmp(arg, "fileformat", 10) == 0)
    {
        arg += 10;
        pp = &eap->force_ff;
    }
    else if(ustrncmp(arg, "enc", 3) == 0)
    {
        if(ustrncmp(arg, "encoding", 8) == 0)
        {
            arg += 8;
        }
        else
    ...
    *arg = NUL;
    ...
    {
        // Check ++bad= argument.
        // Must be a single-byte character, "keep" or "drop".
        p = eap->cmd + bad_char_idx;

        if(ustricmp(p, "keep") == 0)
        {
            eap->bad_char = BAD_KEEP;
        }
        else if(ustricmp(p, "drop") == 0)
        {
...
static int get_tabpage_arg(exargs_st *eap)
{
    ...
    if(eap->arg && *eap->arg != NUL)
    {
        ...
        // argument +N/-N means: go to N places to the
...
uchar_kt *find_nextcmd(const uchar_kt *p)
{
    while(*p != '|' && *p != '\n')
    {
        if(*p == NUL)
...
uchar_kt *check_nextcmd(uchar_kt *p)
{
    p = skipwhite(p);
...
/// if there are more files to edit, and this is the last window,
/// and forceit not used, and not repeated twice on a row
                          ...
                          int force)
{
    ...
    for(i = 0; i < gap->ga_len; ++i)
    {
        ...
        len = ustrlen(cmd->uc_name);
        cmp = ustrncmp(name, cmd->uc_name, name_len);
...
static void uc_list(uchar_kt *name, size_t name_len)
{
    int i, j;
    int found = FALSE;
    usrcmd_st *cmd;
    ...
    for(;;)
    {
        for(i = 0; i < gap->ga_len; ++i)
        {
            ...
            // Skip commands which don't match the
            // requested prefix and commands filtered out.
            if(ustrncmp(name, cmd->uc_name, name_len) != 0
            ...
            for(j = 0; addr_type_complete[j].expand != -1; j++)
            {
                ...
                {
                    ustrcpy(IObuff + len, addr_type_complete[j].name);
                    len += (int)ustrlen(IObuff + len);
                    break;
                }
            }
            ...
            for(j = 0; command_complete[j].expand != 0; ++j)
            {
                ...
                {
                    ustrcpy(IObuff + len, command_complete[j].name);
                    len += (int)ustrlen(IObuff + len);
                    break;
                }
            }
...
static int uc_scan_attr(uchar_kt *attr,
                        size_t len,
                        uint32_t *argt,
                        long *def,
                        int *flags,
                        ...
                        int *addr_type_arg)
{
    ...
    if(ustrnicmp(attr, "bang", len) == 0)
    {
        *argt |= BANG;
    }
    else if(ustrnicmp(attr, "buffer", len) == 0)
    {
        *flags |= UC_BUFFER;
    }
    else if(ustrnicmp(attr, "register", len) == 0)
    {
        *argt |= REGSTR;
    }
    else if(ustrnicmp(attr, "bar", len) == 0)
    {
        *argt |= TRLBAR;
    }
    else
    {
        ...
        if(ustrnicmp(attr, "nargs", attrlen) == 0)
        {
            if(vallen == 1)
            {
                if(*val == '0')
...
wrong_nargs:
        ...
        else if(ustrnicmp(attr, "range", attrlen) == 0)
        {
            *argt |= RANGE;
...
invalid_count:
        ...
        else if(ustrnicmp(attr, "count", attrlen) == 0)
        {
        ...
        else if(ustrnicmp(attr, "complete", attrlen) == 0)
        {
            if(val == NULL)
            {
        ...
        else if(ustrnicmp(attr, "addr", attrlen) == 0)
        {
            *argt |= RANGE;
...
static void ex_command(exargs_st *eap)
{
    ...
    // If there is nothing after the name, and no
    // attributes were specified, we are listing commands
    p = skipwhite(end);
    ...
    else if(!ASCII_ISUPPER(*name))
    {
        ...
        return;
            ...
            || (name_len <= 4
                && ustrncmp(name, "Next", name_len > 4 ? 4 : name_len) == 0))
    {
...
static void ex_delcommand(exargs_st *eap)
{
    ...
    for(;;)
    {
        ...
        {
            cmd = USER_CMD_GA(gap, i);
            cmp = ustrcmp(eap->arg, cmd->uc_name);
...
static uchar_kt *uc_split_args(uchar_kt *arg, size_t *lenp)
{
    int len;
    uchar_kt *p;
    uchar_kt *q;
...
static size_t add_cmd_modifier(uchar_kt *buf, char *mod_str, bool *multi_mods)
{
    size_t result = ustrlen(mod_str);
...
///
/// @param code
/// points to the '<'. @b len the length of the <> (inclusive).
///
/// @param len
/// @param buf
/// is where the result is to be added.
///
/// @param cmd
...
///
/// @param split_buf
/// points to a buffer used for splitting, caller should free it.
///
/// @param split_len
/// is the length of what @b split_buf contains.
///
/// @return
/// - the length of the replacement, which has been added to @b buf.
/// - -1 if there was no match, and only the "<" has been copied.
                            ...
                            size_t *split_len)
{
    ...
    if((ustrchr((uchar_kt *)"qQfF", *p) != NULL) && p[1] == '-')
    {
        quote = (*p == 'q' || *p == 'Q') ? 1 : 2;
        p += 2;
        l -= 2;
    ...
    else if(ustrnicmp(p, "args>", l) == 0)
    {
        type = ct_ARGS;
    }
    else if(ustrnicmp(p, "bang>", l) == 0)
    {
        type = ct_BANG;
    }
    else if(ustrnicmp(p, "count>", l) == 0)
    {
        type = ct_COUNT;
    }
    else if(ustrnicmp(p, "line1>", l) == 0)
    {
        type = ct_LINE1;
    }
    else if(ustrnicmp(p, "line2>", l) == 0)
    {
        type = ct_LINE2;
    }
    else if(ustrnicmp(p, "lt>", l) == 0)
    {
        type = ct_LT;
    }
    else if(ustrnicmp(p, "reg>", l) == 0 || ustrnicmp(p, "register>", l) == 0)
    {
        type = ct_REGISTER;
    }
    else if(ustrnicmp(p, "mods>", l) == 0)
    {
        type = ct_MODS;
    }
    ...
    switch(type)
    {
        ...
        case ct_COUNT:
        {
            ...
            long num = (type == ct_LINE1)
                       ? eap->line1 : (type == ct_LINE2)
                                      ? eap->line2 : (eap->addr_count > 0)
                                                     ? eap->line2 : cmd->uc_def;
            size_t num_len;
            sprintf(num_buf, "%" PRId64, (int64_t)num);
            num_len = ustrlen(num_buf);
            result = num_len;
...
static void do_ucmd(exargs_st *eap)
{
    ...
    uchar_kt *end = NULL;
    size_t split_len = 0;
    uchar_kt *split_buf = NULL;
    script_id_kt save_current_SID = current_SID;
    ...
    for(;;)
    {
        ...
        for(;;)
        {
            ...
            ++end; // Include the '>'
            len = start - p; // Take everything up to the '<'
        ...
        totlen += ustrlen(p); // Add on the trailing characters
...
static uchar_kt *get_user_command_name(int idx)
{
...
uchar_kt *get_user_cmd_flags(expand_st *FUNC_ARGS_UNUSED_REALY(xp), int idx)
{
    ...
    return (uchar_kt *)user_cmd_flags[idx];
}
...
uchar_kt *get_user_cmd_nargs(expand_st *FUNC_ARGS_UNUSED_REALY(xp), int idx)
{
    ...
    return (uchar_kt *)user_cmd_nargs[idx];
}
...
int parse_addr_type_arg(uchar_kt *value,
                        int vallen,
                        uint32_t *argt,
                        int *addr_type_arg)
{
    ...
    {
        a = (int)ustrlen(addr_type_complete[i].name) == vallen;
        b = ustrncmp(value, addr_type_complete[i].name, vallen) == 0;
...
int parse_compl_arg(const uchar_kt *value,
                    int vallen,
                    int *complp,
                    uint32_t *argt,
                    uchar_kt **compl_arg)
{
    ...
    {
        if((int)ustrlen(command_complete[i].name) == valend
           && ustrncmp(value, command_complete[i].name, valend) == 0)
        {
    ...
    if(*complp != EXPAND_USER_DEFINED
       && *complp != EXPAND_USER_LIST
       && arg != NULL)
    {
    ...
    if(arg != NULL)
    {
        *compl_arg = ustrndup(arg, (int)arglen);
    }

    return OK;
}

int cmdcomplete_str_to_type(uchar_kt *complete_str)
{
    for(int i = 0; command_complete[i].expand != 0; i++)
    {
        if(ustrcmp(complete_str, command_complete[i].name) == 0)
        {
...
static void ex_colorscheme(exargs_st *eap)
{
    if(*eap->arg == NUL)
    {
        uchar_kt *expr = ustrdup((uchar_kt *)"g:colors_name");
        uchar_kt *p = NULL;
        ++emsg_off;
...
/// Close window "win" and take care of handling closing the last
/// window for a modified buffer.
///
/// @param forceit
/// @param win
    ...
    else
    {
        for(; !got_int; os_breakcheck())
        {
            print_line(eap->line1,
                       (eap->cmdidx == CMD_number
                        || eap->cmdidx == CMD_pound
                        || (eap->flags & EXFLAG_NR)),
                       eap->cmdidx == CMD_list
                       || (eap->flags & EXFLAG_LIST));
...
void alist_expand(int *fnum_list, int fnum_len)
{
    int i;
    int old_arg_count;
    uchar_kt **old_arg_files;
    uchar_kt **new_arg_files;
    int new_arg_file_count;
    uchar_kt *save_p_su = p_su;
    ...
    {
        old_arg_files[i] = ustrdup(garg_list[i].ae_fname);
    }
...
void alist_add(arglist_st *al, uchar_kt *fname, int set_fnum)
{
    if(fname == NULL)
    {
        return; // don't add NULL file names
    }
...
void ex_splitview(exargs_st *eap)
{
    ...
    // But it's OK when doing ":tab split".
    ...
    {
        fname = find_file_in_path(eap->arg,
                                  ustrlen(eap->arg),
                                  FNAME_MESS,
                                  TRUE,
                                  curbuf->b_ffname);
    ...
    {
        if(win_new_tabpage(cmdmod.tab != 0
                           ? cmdmod.tab : eap->addr_count == 0
...
static void ex_tabs(exargs_st *FUNC_ARGS_UNUSED_REALY(eap))
{
    ...
    FOR_ALL_TABS(tp)
    {
        ...
        msg_putchar('\n');
        xsnprintf((char *)IObuff, IOSIZE, _("Tab page %d"), tabcount++);
        ...
        FOR_ALL_WINDOWS_IN_TAB(wp, tp)
        {
            ...
            {
                ustrlcpy(IObuff, buf_spname(wp->w_buffer), IOSIZE);
            }
            else
            {
...
static void ex_find(exargs_st *eap)
{
    ...
    if(eap->addr_count > 0)
    {
        ...
        // This matters when it appears several times in the path.
        count = eap->line2;
...
void do_exedit(exargs_st *eap, win_st *old_curwin)
{
            ...
            || *eap->arg != NUL)
    {
        ...
        // 'readonly' doesn't make sense in an empty buffer
        setpcmark();

        if(do_ecmd(0,
                   (eap->cmdidx == CMD_enew ? NULL : eap->arg),
                   NULL,
                   eap,
                   eap->do_ecmd_lnum,
                   ...
                   old_curwin == NULL ? curwin : NULL) == FAIL)
        {
            ...
            // If the window was split, close it.
            if(old_curwin != NULL)
            {
    ...
    // if ":split file" worked, set alternate
    // file name in old window to new file
    if(old_curwin != NULL
       && *eap->arg != NUL
       && curwin != old_curwin
       && win_valid(old_curwin)
...
static void ex_read(exargs_st *eap)
{
    ...
    else
    {
        ...
        {
            if(ustrchr(p_cpo, CPO_ALTREAD) != NULL)
            {
...
void post_chdir(CdScope scope)
{
    ...
    {
        // If still in global directory,
        // set CWD as the global directory.
        if(globaldir == NULL && prev_dir != NULL)
        {
            globaldir = ustrdup(prev_dir);
        }
    }
    ...
    {
        case kCdScopeGlobal:
            // We are now in the global directory,
            // no need to remember its name.
            xfree(globaldir);
            globaldir = NULL;
            break;

        case kCdScopeTab:
            curtab->tp_localdir = (uchar_kt *)xstrdup(cwd);
            break;

        case kCdScopeWindow:
            curwin->w_localdir = (uchar_kt *)xstrdup(cwd);
            break;
...
void ex_cd(exargs_st *eap)
{
    ...
    #endif
    {
        ...
        if(*new_dir == NUL)
        {
            ...
            expand_env((uchar_kt *)"$HOME", NameBuff, MAXPATHL);
            new_dir = NameBuff;
        }
        #endif
        ...
        else
        {
            ...
            // Echo the new current directory
...
/// Sleep for "msec" milliseconds, but keep
/// checking for a CTRL-C every second.
...
static void ex_wincmd(exargs_st *eap)
{
    ...
    {
        // CTRL-W g and CTRL-W CTRL-G
        // have an extra command character
        if(eap->arg[1] == NUL)
        {
            EMSG(_(e_invarg));
            return;
...
static void ex_operators(exargs_st *eap)
{
    ...
    {
        setpcmark(); // position cursor for undo
...
static void ex_at(exargs_st *eap)
{
    ...
    if(do_execreg(c, TRUE, ustrchr(p_cpo, CPO_EXECBUF) != NULL, TRUE) == FAIL)
    {
        beep_flush();
    }
    else
    {
        ...
        // Continue until the stuff buffer is empty and
        // all added characters have been consumed.
        while(!stuff_empty() || typebuf.tb_len > prev_len)
        {
            (void)do_cmdline(NULL,
                             getexline,
                             NULL,
...
static void ex_redir(exargs_st *eap)
{
        ...
        else if(*arg == '@')
        {
            ...
            if(valid_yank_reg(*arg, true) && *arg != '_')
            {
                ...
                else
                {
                    ...
                    // Make register empty when not using
                    // @A-@Z and the command is valid.
                    if(*arg == NUL && !isupper(redir_reg))
                    {
                        write_reg_contents(redir_reg, (uchar_kt *)"", 0, false);
                    }
                }
            }
    ...
    // Can happen for cmdline completion that
    // indirectly invokes a command to catch its output.
...
static void ex_redraw(exargs_st *eap)
{
    ...
    update_screen(eap->forceit
                  ? CLEAR : VIsual_active ? INVERTED : 0);
    ...
    // Reset msg_didout, so that a
    // message that's there is overwritten.
    msg_didout = FALSE;
    msg_col = 0;
...
static void ex_mkrc(exargs_st *eap)
{
    ...
    // We also don't use the short file name when 'acd'
    // is set, that is checked later.
    did_lcd = FALSE;
    char *fname;
    ...
    fd = open_exfile((uchar_kt *) fname, eap->forceit, WRITEBIN);
    ...
    if(fd != NULL)
    {
        ...
        if(!failed && view_session)
        {
            ...
            if(eap->cmdidx == CMD_mksession)
            {
                uchar_kt *dirnow; // current directory
                ...
                {
                    if(change_to_filedir((uchar_kt *) fname) == OK)
                    {
                        shorten_fnames(true);
                    }
                }
...
FILE *open_exfile(uchar_kt *fname, int forceit, char *mode)
{
    FILE  *fd;
...
static void ex_mark(exargs_st *eap)
{
    ...
    if(*eap->arg == NUL)
    {
        EMSG(_(e_argreq)); // No argument
    }
    else if(eap->arg[1] != NUL)
    {
        EMSG(_(e_trailing)); // more than one character
    }
    else
    {
        pos = curwin->w_cursor; // save curwin->w_cursor
...
static void ex_normal(exargs_st *eap)
{
    ...
    if(tabuf.typebuf_valid)
    {
        ...
        // When no range given, execute it just once, without
        // positioning the cursor first.
        do
        {
            if(eap->addr_count != 0)
            {
    ...
    {
        // Some function (terminal_enter()) was aware
        // of ex_normal and decided to override the
        // value of restart_edit anyway.
...
void exec_normal_cmd(uchar_kt *cmd, int remap, bool silent)
{
...
static void ex_tag_cmd(exargs_st *eap, uchar_kt *name)
{
    ...
    do_tag(eap->arg,
           cmd,
           eap->addr_count > 0 ? (int)eap->line2 : 1,
           eap->forceit,
           TRUE);
}

/// Check "str" for starting with a special cmdline variable.
/// If found return one of the SPEC_ values and set "*usedlen"
/// to the length of the variable. Otherwise return -1 and
/// "*usedlen" is unchanged.
ssize_t find_cmdline_var(const uchar_kt *src, size_t *usedlen)
FUNC_ATTR_NONNULL_ALL
{
    size_t len;
...
# define SPEC_AMATCH 9
    ...
    {
        len = ustrlen(spec_str[i]);

        if(ustrncmp(src, spec_str[i], len) == 0)
        {
            *usedlen = len;
            assert(i <= SSIZE_MAX);
            return (ssize_t)i;
...
/// @param errorms    pointer to error message
                  ...
                  int *escaped)
{
    ...
    *errormsg = NULL;
    ...
    {
        *usedlen = 0;
        xstrmove(src - 1, src); // remove backslash
        return NULL;
    }
    ...
    else
    {
        ...
        switch(spec_idx)
        {
            ...
            case SPEC_HASH: // '#' or "#99": alternate file
                ...
                *usedlen = (size_t)(s - src); // length of what we expand
                ...
                if(src[1] == '<')
                {
                    ...
                    result = (uchar_kt *)tv_list_find_str(get_vim_var_list(VV_OLDFILES), i - 1);
                ...
                else
                {
                    ...
                    {
                        *errormsg =
                            (uchar_kt *)_("E194: No alternate file name to substitute for '#'");
                        return NULL;
                    }
            ...
            case SPEC_AFILE: // file name for autocommand
                ...
                if(result != NULL && !autocmd_fname_full)
                {
                    ...
                    result = (uchar_kt *)FullName_save((char *)autocmd_fname, FALSE);
                    xfree(autocmd_fname);
                ...
                {
                    *errormsg =
                        (uchar_kt *)_("E495: no autocommand file name to "
            ...
            case SPEC_ABUF: // buffer number for autocommand
                ...
                {
                    *errormsg =
                        (uchar_kt *)_("E496: no autocommand buffer number "
                                    "to substitute for \"<abuf>\"");
                    return NULL;
                }
            ...
            case SPEC_AMATCH: // match name for autocommand
                ...
                {
                    *errormsg =
                        (uchar_kt *)_("E497: no autocommand match name "
                                    "to substitute for \"<amatch>\"");
                    return NULL;
                }
            ...
            case SPEC_SFILE: // file name for ":so" command
                ...
                {
                    *errormsg =
                        (uchar_kt *)_("E498: no :source file name to "
                                    "substitute for \"<sfile>\"");
                    return NULL;
                }
            ...
            case SPEC_SLNUM: // line in file for ":so" command
                ...
                {
                    *errormsg = (uchar_kt *)_("E842: no line number "
                                            "to use for \"<slnum>\"");
                    return NULL;
                }
        ...
        resultlen = ustrlen(result); // length of new string
        ...
        if(src[*usedlen] == '<')
        {
            ...
            if((s = ustrrchr(result, '.')) != NULL && s >= path_tail(result))
            {
    ...
    if(resultlen == 0 || valid != VALID_HEAD + VALID_PATH)
    {
        if(valid != VALID_HEAD + VALID_PATH)
        {
            ...
            *errormsg =
                (uchar_kt *)_("E499: Empty file name for '%' or '#', "
                            "only works with \":p:h\"");
        }
        else
        {
            *errormsg = (uchar_kt *)_("E500: Evaluates to an empty string");
        }
    ...
    {
        result = ustrndup(result, resultlen);
    }
...
/// and return it in one allocated string.
/// Spaces and backslashes in the file names are escaped with a backslash.
static uchar_kt *arg_all(void)
{
    int len;
    int idx;
...
/// Expand the <sfile> string in @b arg.
///
/// Returns an allocated string, or NULL for any error.
uchar_kt *expand_sfile(uchar_kt *arg)
{
    uchar_kt *errormsg;
    size_t len;
    uchar_kt *result;
    ...
    for(p = result; *p;)
    {
        ...
        else
        {
            ...
            {
                p += srclen;
                continue; // no match (cannot happen)
            }

            len = ustrlen(result) - srclen + ustrlen(repl) + 1;
            newres = xmalloc(len);
            memmove(newres, result, (size_t)(p - result));
            ustrcpy(newres + (p - result), repl);
            len = ustrlen(newres);
            ustrcat(newres, p + srclen);
            xfree(repl);
            xfree(result);
            result = newres;
...
static int makeopens(FILE *fd, uchar_kt *dirnow)
{
    ...
    // and then other sessionable variables.
    ...
    if(ssop_flags & SSOP_RESIZE)
    {
        ...
        if(fprintf(fd,
                   "set lines=%" PRId64 " columns=%" PRId64,
                   (int64_t)Rows,
                   (int64_t)Columns) < 0
           || put_eol(fd) == FAIL)
        {
    ...
    // May repeat putting Windows for each tab, when "tabpages" is
    // in 'sessionoptions'. Don't use goto_tabpage(), it may change
    // directory and trigger autocommands.
    ...
    for(tabnr = 1;; ++tabnr)
    {
        ...
        if(put_line(fd, "set winminheight=1 winminwidth=1 winheight=1 winwidth=1") == FAIL)
        {
            return FAIL;
        }
        ...
        // each window. For further tab pages it's the window where we do "tabedit".
    ...
    if(put_line(fd,
                "if exists('s:wipebuf') "
                "&& getbufvar(s:wipebuf, '&buftype') isnot# 'terminal'") == FAIL)
    {
        return FAIL;
    }
...
static int ses_win_rec(FILE *fd, frame_st *fr)
{
    ...
    if(fr->fr_layout != FR_LEAF)
    {
        ...
        if(frc != NULL)
            ...
            {
                // Make window as big as possible so that we
                // have lots of room to split.
                if(put_line(fd, "wincmd _ | wincmd |") == FAIL
                   || put_line(fd, fr->fr_layout == FR_COL ? "split" : "vsplit") == FAIL)
                {
                    return FAIL;
                }
        ...
        if(count > 0
           && (fprintf(fd, fr->fr_layout == FR_COL ? "%dwincmd k" : "%dwincmd h", count) < 0
               || put_eol(fd) == FAIL))
        {
            return FAIL;
        }
                    ...
                    int current_arg_idx)
{
    ...
    // For ":mkview" only when 'viewoptions' contains "cursor".
...
static int ses_fname(FILE *fd, filebuf_st *buf, unsigned *flagp)
{
    ...
    // the session file will be sourced. Don't do this for ":mkview", we don't
    // know the current directory. Don't do this after ":lcd", we don't keep
    // track of what the current directory is.
...
static int ses_put_fname(FILE *fd, uchar_kt *name, unsigned *flagp)
{
    ...
    p = (uchar_kt *)vim_strsave_fnameescape((const char *)sname, false);
    xfree(sname);
...
static void ex_loadview(exargs_st *eap)
{
    ...
    if(fname != NULL)
    {
        if(do_source((uchar_kt *)fname, FALSE, kLoadSftAuto) == FAIL)
        {
...
static char *get_view_file(int c)
{
    ...
    char *retval = xmalloc(strlen(sname) + len + ustrlen(p_vdir) + 9);
    ustrcpy(retval, p_vdir);
    add_pathsep(retval);
...
void dialog_msg(uchar_kt *buff, char *format, uchar_kt *fname)
{
    if(fname == NULL)
    {
        fname = (uchar_kt *)_("Untitled");
    }

    xsnprintf((char *)buff, DIALOG_MSG_SIZE, format, fname);
}

/// ":behave {mswin,xterm}"
static void ex_behave(exargs_st *eap)
{
    if(ustrcmp(eap->arg, "mswin") == 0)
    {
    ...
    else if(ustrcmp(eap->arg, "xterm") == 0)
    {
...
uchar_kt *get_behave_arg(expand_st *FUNC_ARGS_UNUSED_REALY(xp), int idx)
{
    if(idx == 0)
    {
        return (uchar_kt *)"mswin";
    }

    if(idx == 1)
    {
        return (uchar_kt *)"xterm";
    }
...
static void ex_filetype(exargs_st *eap)
{
    ...
    if(*eap->arg == NUL)
    {
        ...
        smsg("filetype detection:%s  plugin:%s  indent:%s",
             ...
             filetype_plugin == kTrue ? (filetype_detect == kTrue ? "ON" : "(on)") :
             "OFF", // NOLINT(whitespace/line_length)
             filetype_indent == kTrue ? (filetype_detect == kTrue ? "ON" : "(on)") :
             "OFF");// NOLINT(whitespace/line_length)
        return;
    }
    ...
    if(ustrcmp(arg, "on") == 0 || ustrcmp(arg, "detect") == 0)
    {
        if(*arg == 'o' || !filetype_detect)
        {
            source_runtime((uchar_kt *)FILETYPE_FILE, DIP_ALL);
            filetype_detect = kTrue;

            if(plugin)
            {
                source_runtime((uchar_kt *)FTPLUGIN_FILE, DIP_ALL);
                filetype_plugin = kTrue;
            }

            if(indent)
            {
                source_runtime((uchar_kt *)INDENT_FILE, DIP_ALL);
        ...
        if(*arg == 'd')
        {
            (void)do_doautocmd((uchar_kt *)"filetypedetect BufRead", true, NULL);
            do_modelines(0);
        }
    }
    else if(ustrcmp(arg, "off") == 0)
    {
        if(plugin || indent)
        {
            if(plugin)
            {
                source_runtime((uchar_kt *)FTPLUGOF_FILE, DIP_ALL);
                filetype_plugin = kFalse;
            }

            if(indent)
            {
                source_runtime((uchar_kt *)INDOFF_FILE, DIP_ALL);
                filetype_indent = kFalse;
            }
        }
        else
        {
            source_runtime((uchar_kt *)FTOFF_FILE, DIP_ALL);
...
void filetype_maybe_enable(void)
{
    ...
    {
        source_runtime((uchar_kt *)FILETYPE_FILE, true);
        filetype_detect = kTrue;
        source_runtime((uchar_kt *)FTPLUGIN_FILE, true);
        filetype_plugin = kTrue;
        source_runtime((uchar_kt *)INDENT_FILE, true);
        filetype_indent = kTrue;
    }
}
...
static void ex_match(exargs_st *eap)
{
    ...
    else if((ustrnicmp(eap->arg, "none", 4) == 0
    ...
    else
    {
        ...
        if(!eap->skip)
        {
            g = ustrndup(eap->arg, (int)(p - eap->arg));
        }
...
static void ex_terminal(exargs_st *eap)
{
    ...
    {
        // Run {cmd} in 'shell'.
        char *name = (char *)ustrdup_escape(eap->arg, (uchar_kt *)"\"\\");
    ...
    else
    {
        ...
        while(*p != NULL)
        {
            ...
            xstrncat(shell_argv, tempstring, sizeof(shell_argv));
            p++;
        }
...
/// Checks if @b cmd is "previewable" (i.e. supported by 'inccommand').
///
/// @param[in] cmd Commandline to check. May start with a range.
///
/// @return true if `cmd` is previewable
bool cmd_can_preview(uchar_kt *cmd)
{
    if(cmd == NULL)
    {
        return false;
    ...
    uchar_kt *end = find_command(&ea, NULL);
