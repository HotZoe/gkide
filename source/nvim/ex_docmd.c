    ...
    if(trylevel == 0)
    {
        ...
        if(did_throw)
        {
            ...
            switch(current_exception->type)
            {
                ...
                default:
                    p = vim_strsave((uchar_kt *)_(e_internal));
            }
...
static uchar_kt *find_command(exargs_st *eap, int *full)
{
    ...
    else
    {
        ...
        if(p == eap->cmd && vim_strchr((uchar_kt *)"@!=><&~#", *p) != NULL)
        {
            ++p;
        }
...
const char *set_one_cmd_context(expand_st *xp, const char *buff)
{
    ...
    for(cmd = buff; vim_strchr((const uchar_kt *)" \t:|", *cmd) != NULL; cmd++)
    { /* empty body */ }
    ...
    else
    {
        ...
        // check for non-alpha command
        if(p == cmd && vim_strchr((const uchar_kt *)"@*!=><&~#", *p) != NULL)
        {
            p++;
        }
    ...
    if(ea.cmdidx == CMD_SIZE)
    {
        if(*cmd == 's' && vim_strchr((const uchar_kt *)"cgriI", cmd[1]) != NULL)
        {
            ea.cmdidx = CMD_substitute;
            p = cmd + 1;
        }
...
uchar_kt *skip_range(const uchar_kt *cmd, int *ctx)
{
    ...
    while(vim_strchr((uchar_kt *)" \t0123456789.$%'/?-+,;", *cmd) != NULL)
    {
        if(*cmd == '\'')
        {
            if(*++cmd == NUL && ctx != NULL)
...
int expand_filename(exargs_st *eap, uchar_kt **cmdlinep, uchar_kt **errormsgp)
{
    ...
    while(*p != NUL)
    {
        ...
        if((eap->usefilter || eap->cmdidx == CMD_bang)
           && vim_strpbrk(repl, (uchar_kt *)"!") != NULL)
        {
            uchar_kt *l;
            l = vim_strsave_escaped(repl, (uchar_kt *)"!");
            xfree(repl);
            repl = l;
        }
       ...
       && n >= 0 && quitmore == 0)
    {
        if(message)
        {
            if((p_confirm || cmdmod.confirm) && curbuf->b_fname != NULL)
            {
                ...
                {
                    vim_snprintf((char *)buff,
                                 DIALOG_MSG_SIZE,
                                 _("%d more files to edit. Quit anyway?"),
                                 n);
                }
                            ...
                            size_t *split_len)
{
    ...
    if((vim_strchr((uchar_kt *)"qQfF", *p) != NULL) && p[1] == '-')
    {
        quote = (*p == 'q' || *p == 'Q') ? 1 : 2;
        p += 2;
        l -= 2;
...
static void ex_colorscheme(exargs_st *eap)
{
    if(*eap->arg == NUL)
    {
        uchar_kt *expr = vim_strsave((uchar_kt *)"g:colors_name");
        uchar_kt *p = NULL;
        ++emsg_off;
    ...
    if(fd != NULL)
    {
        ...
        if(!failed && view_session)
        {
            ...
            if(eap->cmdidx == CMD_mksession)
            {
                ...
                {
                    if(vim_chdirfile((uchar_kt *) fname) == OK)
                    {
                        shorten_fnames(true);
                    }
                }
...
static void ex_terminal(exargs_st *eap)
{
    ...
    {
        // Run {cmd} in 'shell'.
        char *name = (char *)vim_strsave_escaped(eap->arg, (uchar_kt *)"\"\\");
