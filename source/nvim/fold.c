...
typedef struct
{
    linenr_T fd_top;       ///< first line of fold; for nested fold relative to parent
...
/// @param recurse  TRUE when closing/opening recursive
/// @param donep
static linenr_T setManualFoldWin(win_T *wp,
                                 linenr_T lnum,
                                 int opening,
                                 int recurse,
                                 int *donep)
...
static void deleteFoldEntry(garray_T *gap, int idx, int recursive)
{
    ...
    {
        // Move nested folds one level up, to overwrite the fold that is deleted.
...
///
/// @return
static int check_closed(win_T *win,
                        fold_T *fp,
                        int *use_levelp,
                        int level,
                        int *maybe_smallp,
    ...
    for(int i = 0; i < gap->ga_len; i++)
    {
        if(fp->fd_flags != FD_LEVEL)
        {
            if(!GA_EMPTY(&fp->fd_nested))
            {
                ...
                if(put_foldopen_recurse(fd, wp, &fp->fd_nested, off + fp->fd_top) == FAIL)
                {
                    return FAIL;
                }
