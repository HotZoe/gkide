...
#define PV_WIN       0x2000  ///< window-local option
...
static uchar_kt  *p_com;
static uchar_kt  *p_cms;
...
static char *(p_bufhidden_values[]) = {
...
static char *(p_fdm_values[]) = {
    "manual", "expr", "marker", "indent", "syntax", "diff", NULL
};
static char *(p_cot_values[]) = {
    "menu", "menuone", "longest", "preview", "noinsert", "noselect",  NULL
};
...
///
/// @note
/// @b dest and @b src must not overlap
///
/// @return
/// a c-string pointer, which point to the c-string ending of @b dest
static char *strcpy_comma_escaped(char *dest,
                                  const char *src,
                                  const size_t len)
...
/// @param[in]     path      directory to append.
/// @param[in]     path_len  the length of @b path, including NUL.
///
/// @return @b dest + appended_characters_length
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    {
        // do not copy the c-string ending NUL, following make sure it
        dest = strcpy_comma_escaped(dest, path, path_len - 1);

        *dest++ = ','; // add a comma
        *dest = NUL; // make sure c-string
    }
...
static void init_plugin_runtimepath(void)
{
    assert(NULL != gkide_sys_home);
    assert(NULL != gkide_usr_home);

    char *const std_sys_path = xstrdup(gkide_sys_home);
    char *const std_usr_path = xstrdup(gkide_usr_home);
    char *const plg_sys_path = concat_fnames_realloc(std_sys_path, "plg", true);
    char *const plg_usr_path = concat_fnames_realloc(std_usr_path, "plugin", true);
    char *const plg_env_path = vim_getenv(ENV_GKIDE_NVIM_PLUGIN);

    // c-string length + NUL + escape-char-number
    size_t plg_sys_len = 0;
    size_t plg_usr_len = 0;
    size_t plg_env_len = 0;

    plg_sys_len = strlen(plg_sys_path) + 1;

    if(os_path_exists((uchar_kt *)plg_usr_path))
    {
        plg_usr_len = strlen(plg_usr_path) + 1;
    }

    if(NULL != plg_env_path)
    {
    ...
    char *const plugin_path = xmalloc(plugin_len);
    char *ptr = plugin_path;

    ptr = plugin_path_append(ptr, plg_sys_path, plg_sys_len);
    ptr = plugin_path_append(ptr, plg_usr_path, plg_usr_len);
    ptr = plugin_path_append(ptr, plg_env_path, plg_env_len);
    ptr[-1] = NUL; // strip trailing comma

    assert((size_t)(ptr - plugin_path) == plugin_len);
    DEV_TRACE_MSG("pluginpath: %s", plugin_path);

    set_string_default("runtimepath", plugin_path, true);
    set_string_default("packpath", plugin_path, false);
    //set_string_default("pluginpath", plugin_path, true); // todo
...
// init the directory options to its default value
static void init_dir_options(void)
{
    char *opt_val = NULL;

    // List of directories for the backup file, separated with commas.
    // $GKIDE_USR_HOME/auto/backup, nvim option name: backupdir, bdir
    opt_val = stdpath_user_auto_subpath("backup", false, false);
    // if not exist and can't create, just set to NULL
    set_string_default("backupdir", opt_val, true);

    // Name of the directory where to store files for |:mkview|
    // $GKIDE_USR_HOME/auto/view, nvim option name: viewdir, vdir
    opt_val = stdpath_user_auto_subpath("view", false, false);
    // if not exist and can't create, just set to NULL
    set_string_default("viewdir", opt_val, true);

    // List of directory names for undo files, separated with commas.
    // $GKIDE_USR_HOME/auto/undo, nvim option name: undodir, udir
    opt_val = stdpath_user_auto_subpath("undo", false, false);
    // if not exist and can't create, just set to NULL
    set_string_default("undodir", opt_val, true);

    // List of directory names for swap files, separated with commas.
    // $GKIDE_USR_HOME/auto/swap, nvim option name: swapdir, sdir
    opt_val = stdpath_user_auto_subpath("swap", false, false);
    // if not exist and can't create, just set to NULL
    set_string_default("swapdir", opt_val, true);
}
...
void init_options_part_1(void)
{
    ...
    // Set default value for &runtimepath.
    ...
    // This is not in didset_options() because it only
    // depends on 'encoding'.
    init_spell_chartab();
    ...
    // enc_locale() will try to find the encoding
    // of the current locale. This will be used when
    // 'default' is used as encoding specifier in 'fileencodings'
    uchar_kt *p = enc_locale();

    if(p == NULL)
    {
        // use utf-8 as 'default' if locale encoding can't be detected.
        p = (uchar_kt *)xmemdupz("utf-8", sizeof("utf-8")-1);
    }
...
///
/// @param allocated
/// If true, do not copy default as it was already allocated.
static void set_string_default(const char *name,
                               char *val,
                               bool allocated)
FUNC_ATTR_NONNULL_ALL
{
...
void set_number_default(char *name, long val)
{
    ...
    if(opt_idx >= 0)
    {
        options[opt_idx].def_val[VI_DEFAULT] = (uchar_kt *)val;
    }
}
...
void init_options_part_3(void)
{
    ...
    // initializations, where 'shell' might have been
...

    {
                ...
                || fnamecmp(p, "fish") == 0)
        {
            if(do_sp)
            {
                p_sp = (uchar_kt *)"2>&1| tee";
                options[idx_sp].def_val[VI_DEFAULT] = p_sp;
            }

            if(do_srr)
            {
                p_srr = (uchar_kt *)">%s 2>&1";
                options[idx_srr].def_val[VI_DEFAULT] = p_srr;
            }
        }
...
/// 'title' and 'icon' only default to true if they have not
/// been set or reset in .vimrc and we can read the old value.
/// When 'title' and 'icon' have been reset in .vimrc, we won't
/// even check if they can be reset. This reduces startup time
/// when using X on a remote machine.
...
/// Parse @b arg for option settings.
///
/// @b arg may be IObuff, but only when no errors can be present
...
int do_set(uchar_kt *arg,  int opt_flags)
{
    ...
    uchar_kt nextchar; // next non-white char after option name
    ...
    uchar_kt *varp = NULL; // pointer to variable for current option
    ...
    while(*arg != NUL)
    {
        ...
        else
        {
            ...
            if(*arg == '<')
            {
                ...
                if(arg[1] == 't'
                   && arg[2] == '_') // could be term code
                {
                    opt_idx = findoption_len((const char *)arg + 1,
                                             (size_t)(len - 1));
                }
            ...
            {
                errmsg = (uchar_kt *)N_("E518: Unknown option");
                goto skip;
            }
                   ...
                   && !(flags & P_BOOL)))
            {
                ...
                {
                    errmsg = (uchar_kt *)N_("E846: Key code not set");
                    goto skip;
                }
            ...
            else
            {
                if(flags & P_BOOL) // boolean
                {
                    ...
                    errmsg = (uchar_kt *)set_bool_option(opt_idx,
                                                       varp,
                                                       (int)value,
                                                       opt_flags);
                }
                else // Numeric or string.
                {
                    ...
                    if(flags & P_NUM) // numeric
                    {
                        ...
                        {
                            errmsg = (uchar_kt *)N_("E521: Number required after =");
                            goto skip;
                        }
                        ...
                        errmsg = (uchar_kt *)set_num_option(opt_idx,
                                                          varp,
                                                          value,
                                                          errbuf,
                                                          sizeof(errbuf),
                    ...
                    {
                        uchar_kt *save_arg = NULL;
                        uchar_kt *s = NULL;
                        uchar_kt *oldval = NULL; // previous value if *varp
                        uchar_kt *newval;
                        uchar_kt *origval = NULL;
                        ...
                        // The old value is kept until we are
                        // sure that the new value is valid.
                        oldval = *(uchar_kt **)varp;
                        ...
                        else
                        {
                            ...
                            else if(varp == (uchar_kt *) & p_bs
                                    && ascii_isdigit(**(uchar_kt **)varp))
                            {
                                i = getdigits_int((uchar_kt **)varp);

                                switch(i)
                                {
                                case 0:
                                    *(uchar_kt **)varp = empty_option;
                                    break;
                            ...
                            // Remove '>' before 'sdir' and 'bdir', for
                            // backwards compatibility with version 3.0
                            /// @todo: no compatibility
                            else if(*arg == '>'
                                    && (varp == (uchar_kt *)&p_sdir
                                        || varp == (uchar_kt *)&p_bdir))
                            {
                                ++arg;
                            }
                        ...
                        *(uchar_kt **)(varp) = newval;
                        ...
                        // value for ":set" on local options.
                        errmsg = did_set_string_option(opt_idx,
                                                       (uchar_kt **)varp,
                                                       new_value_alloced,
                                                       oldval,
                                                       errbuf,
                        ...
                        if(saved_origval != NULL)
                        {
                            ...
                            apply_autocmds(EVENT_OPTIONSET,
                                           (uchar_kt *)options[opt_idx].fullname,
                                           NULL, false, NULL);
                    ...
                    {
                        // key code option(FIXME(tarruda):
                        // Show a warning or something similar)
                    }
                }
...
static uchar_kt *illegal_char(uchar_kt *errbuf, int c)
{
    if(errbuf == NULL)
    {
        return (uchar_kt *)"";
...
static int string_to_key(uchar_kt *arg)
{
    if(*arg == '<')
    {
...
static uchar_kt *check_cedit(void)
{
    int n;
...
/// (eg ', :, ", or /), and return its associated value in
/// the 'shada' string. Only works for number parameters,
/// not for 'r' or 'n'.
///
/// If the parameter is not specified in the string or there
/// is no following number, return -1.
...
/// Find the parameter represented by the given character (eg ''', ':', '"',
/// or '/') in the 'shada' option and return a pointer to the string after it.
///
/// @return NULL if the parameter is not specified in the string.
uchar_kt *find_shada_parameter(int type)
{
    uchar_kt  *p;
...
static uchar_kt *option_expand(int opt_idx, uchar_kt *val)
{
    ...
    if(val == NULL)
    {
        val = *(uchar_kt **)options[opt_idx].var;
    }
    ...
    expand_env_esc(val, NameBuff, MAXPATHL,
                   (uchar_kt **)options[opt_idx].var == &p_tags,
                   FALSE,
...
void clear_string_option(uchar_kt **pp)
{
    if(*pp != empty_option)
    {
        xfree(*pp);
...
static void check_string_option(uchar_kt **pp)
{
    if(*pp == NULL)
    {
        *pp = empty_option;
...
int was_set_insecurely(uchar_kt *opt, int opt_flags)
{
...
///
/// if @b opt_idx == -1), @b name is used, otherwise @b opt_idx is used.
///
/// When @b set_sid is zero set the scriptID to current_SID.
///
/// When @b set_sid is SID_NONE don't set the scriptID.
/// Otherwise set the scriptID to "set_sid".
///
/// @param name
/// @param opt_idx
/// @param val
                              ...
                              int set_sid)
{
    ...
    s = vim_strsave(val);
    {
        ...
        *varp = s;
        ...
        // For buffer/window local option
        // may also set the global value.
        if(both)
        {
...
static void set_string_option_global(int opt_idx, uchar_kt **varp)
{
    uchar_kt **p, *s;
    ...
    {
        p = (uchar_kt **)GLOBAL_WO(varp);
    }
    else
    {
        p = (uchar_kt **)options[opt_idx].var;
    }
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    *varp = s;
    ...
    char *const r =
        (char *)did_set_string_option(opt_idx,
                                      (uchar_kt **)varp,
                                      (int)true,
                                      (uchar_kt *)oldval,
                                      NULL,
...
static bool valid_filetype(uchar_kt *val)
{
...
static uchar_kt *did_set_string_option(int opt_idx,
                                     uchar_kt **varp,
                                     int new_value_alloced,
                                     uchar_kt *oldval,
                                     uchar_kt *errbuf,
                                     int opt_flags)
{
            ...
            || varp == &p_isf)
    {
        ...
        // refill g_chartab[] If the new option is invalid, use
        // old value.  'lisp' option: refill g_chartab[] for '-' char
    ...
    else if(varp == &p_ambw || (int *)varp == &p_emoji)
    {
        ...
        {
            errmsg = (uchar_kt *)_("E834: Conflicts with value of 'listchars'");
        }
        else if(set_chars_option(&p_fcs) != NULL)
        {
            errmsg = (uchar_kt *)_("E835: Conflicts with value of 'fillchars'");
        }
    }
    // 'background'
    else if(varp == &p_bg)
    {
        if(check_opt_strings(p_bg, p_bg_values, FALSE) == OK)
        {
            ...
            if(dark != (*p_bg == 'd')
               && get_var_value("g:colors_name") != NULL)
            {
    ...
    else if(gvarp == &p_com)
    {
        for(s = *varp; *s;)
        {
            ...
            if(*s++ == NUL)
            {
                errmsg = (uchar_kt *)N_("E524: Missing colon");
            }
            else if(*s == ',' || *s == NUL)
            {
                errmsg = (uchar_kt *)N_("E525: Zero length string");
            }
    ...
    else if(varp == &p_shada)
    {
        ...
        for(s = p_shada; *s;)
        {
            ...
            else if(*s)
            {
                if(errbuf != NULL)
                {
                    errmsg = (uchar_kt *)N_("E527: Missing comma");
                }
                else
                {
                    errmsg = (uchar_kt *)"";
        ...
        {
            errmsg = (uchar_kt *)N_("E528: Must specify a ' value");
        }
    }
    // 'showbreak'
    else if(varp == &p_sbr)
    {
        for(s = p_sbr; *s;)
        {
            if(ptr2cells(s) != 1)
            {
                errmsg = (uchar_kt *)N_("E595: contains unprintable "
                                      "or wide character");
            }
    ...
    {
        // When 'spelllang' or 'spellfile' is set and there is a window
        // for this buffer in which 'spell' is set load the wordlists.
    ...
    else if(gvarp == &p_stl || varp == &p_ruf)
    {
        ...
        if(varp == &p_ruf && *s == '%')
        {
            ...
            if(wid
               && *s == '('
               && (errmsg = check_stl_option(p_ruf)) == NULL)
            {
                ru_wid = wid;
            }
            else
    ...
    // check if it is a valid value for 'complete'
    else if(gvarp == &p_cpt)
    {
        for(s = *varp; *s;)
        {
    ...
    else if(gvarp == &curwin->w_o_allbuf.wo_fmr)
    {
        ...
        if(p == NULL)
        {
            errmsg = (uchar_kt *)N_("E536: comma required");
        }
    ...
    else if(gvarp == &p_cms)
    {
        ...
        {
            errmsg = (uchar_kt *)N_("E537: 'commentstring' must be "
                                  "empty or contain %s");
        }
    }
    ...
    else
    {
        ...
        // Use "free_oldval", because recursiveness may change
        // the flags under our fingers (esp. init_highlight()).
        if(free_oldval)
        {
        ...
        if(varp == &(curwin->w_s->b_p_spl))
        {
            ...
            // They could set 'spellcapcheck' depending on
            // the language. Use the first name in 'spelllang'
...
skip:
        ...
        if(*++s == NUL)
        {
            // illegal trailing comma as in "set cc=80,"
            return e_invarg;
        }
    }
    ...
    else
    {
        ...
        // sort the columns for faster usage
        // on screen redraw inside win_line()
...
/// Returns error message, NULL if it's OK.
static uchar_kt *set_chars_option(uchar_kt **varp)
{
    uchar_kt *p, *s;
    int c1, c2 = 0;
    ...
    // first round: check for valid value,
    // second round: assign values
    for(round = 0; round <= 1; ++round)
    {
        if(round > 0)
        {
            // After checking that the value is valid:
            // set defaults: space for 'fillchars', NUL for 'listchars'
            for(i = 0; i < entries; ++i)
            {
                if(tab[i].cp != NULL)
                {
        ...
        while(*p)
        {
            for(i = 0; i < entries; ++i)
            {
                ...
                {
                    s = p + len + 1;
                    c1 = mb_ptr2char_adv((const uchar_kt **)&s);
                    ...
                    if(tab[i].cp == &lcs_tab2)
                    {
                        ...
                        c2 = mb_ptr2char_adv((const uchar_kt **)&s);
...
uchar_kt *check_stl_option(uchar_kt *s)
{
    int itemcnt = 0;
    int groupdepth = 0;
    static uchar_kt errbuf[80];
    ...
    while(*s && itemcnt < STL_MAX_ITEM)
    {
        ...
        if(*s == '{')
        {
            ...
            if(*s != '}')
            {
                return (uchar_kt *)N_("E540: Unclosed expression sequence");
            }
        }
    }
    ...
    {
        return (uchar_kt *)N_("E541: too many items");
    }

    if(groupdepth != 0)
    {
        return (uchar_kt *)N_("E542: unbalanced groups");
    }

    return NULL;
}

static uchar_kt *did_set_spell_option(bool is_spellfile)
{
    uchar_kt  *errmsg = NULL;
...
static bool parse_winhl_opt(win_st *wp)
{
    ...
    while(*p)
    {
        ...
        int hl_id = syn_check_group((uchar_kt *)hi, (int)(commap-hi));
...
/// Set the scriptID for an option, taking
                             ...
                             const int opt_flags)
{
    ...
    *(int *)varp = value; // set the new value
    ...
    else if((int *)varp == &curbuf->b_p_udf || (int *)varp == &p_udf)
    {
        ...
        if(curbuf->b_p_udf || p_udf)
        {
            ...
            FOR_ALL_BUFFERS(bp)
            {
                ...
                // When 'undofile' is set globally: for every buffer,
                // otherwise only for the current buffer: Try to read
    ...
    {
        // when 'fixeol' is changed,
        // redraw the window title
        redraw_titles();
    }
    // when 'bomb' is changed, redraw the
    // window title and tab page text
    ...
    else if((int *)varp == &curbuf->b_p_bl
            && old_value != curbuf->b_p_bl)
    {
    ...
    else if((int *)varp == (int *)&curbuf->b_p_swf)
    {
        ...
        {
            // no need to reset curbuf->b_may_swap,
            // ml_open_file() will check buf->b_p_swf
    ...
    else if(varp == (uchar_kt *)&(curbuf->b_p_lisp))
    {
    ...
    {
        // when 'title' changed, may need to
        // change the title; same for 'icon'
        did_set_title(false);
    }
    ...
    {
        // Change directories when
        // the 'acd' option is set now.
        do_autochdir();
    }
    // 'diff'
    ...
    // In case some second language keymapping options have
    // changed, check and correct the setting in a consistent way.
    ...
    if((int *)varp == &curwin->w_o_curbuf.wo_arab)
    {
        if(curwin->w_o_curbuf.wo_arab)
        {
            ...
            if(!p_tbidi)
            {
                ...
                // (major part of what Arabic requires)
                if(!p_arshape)
                {
                    p_arshape = TRUE;
                    redraw_later_clear();
        ...
        else
        {
            ...
            if(!p_tbidi)
            {
                ...
                // 'arabicshape' isn't reset, it is a global
                // option and another window may still need it "on".
            }

            // 'delcombine' isn't reset, it is a global option
            // and another window may still want it "on".
                            ...
                            int opt_flags)
{
    ...
    *pp = value;
    ...
    {
        // recompute window positions and heights
        shell_new_rows();
    }
    // 'foldlevel'
    ...
    // when 'updatecount' changes from
    // zero to non-zero, open swap files
    else if(pp == &p_uc)
    {
        if(p_uc < 0)
        {
    ...
    {
        // use the old value, otherwise
        // u_sync() may not work properly
        p_ul = old_value;
        u_sync(TRUE);
        p_ul = value;
    }
    ...
    // If the screen (shell) height has been changed,
    // assume it is the physical screenheight.
    if(old_Rows != Rows || old_Columns != Columns)
    {
        // Changing the screen size is not
        // allowed while updating the screen.
        if(updating_screen)
        {
            *pp = old_value;
        }
        ...
        {
            // Postpone the resizing; check the size
            // and cmdline position for messages.
            check_shellsize();
    ...
    {
        // Normal buffer: reset local
        // 'scrollback' after updating the global value.
        curbuf->b_p_scbk = -1;
    }
...
/// Called after an option changed:
...
/// - unknown option: -3.
int get_option_value(uchar_kt *name,
                     long *numval,
                     uchar_kt **stringval,
                     int opt_flags)
{
    ...
    {
        // Special case: 'modified' is b_changed, but we also
        // want to consider it set when 'ff' or 'fenc' changed.
...
/// Returns the option attributes and its value.
/// Unlike the above function it will return either global
/// value or local value of the option depending on what was requested,
/// but it will never return global value if it was requested to return
/// local one and vice versa. Neither it will return buffer-local value
/// if it was requested to return window-local one.
///
/// Pretends that option is absent if it is not present in the requested
/// scope (i.e. has no global, window-local or buffer-local value depending
/// on opt_type).
///
/// @return
/// - flags
                            ...
                            void *from)
{
    ...
    if(p->indir == PV_NONE)
    {
        ...
        {
            // Did not request global-only option
            return 0;
        }
    }
    else
    {
        ...
        if(p->indir & PV_WIN)
        {
            ...
            {
                // Requested buffer-local,
                // not window-local option
                return 0;
            }
            else
            {
    ...
    else
    {
        if(opt_type == SREQ_BUF)
        {
            ...
            // but we also want to consider it set when
            // 'ff' or 'fenc' changed.
            if(p->indir == PV_MOD)
            {
        ...
        else
        {
            ...
            if(varp != NULL) // hidden option is not changed
            {
                if(number == 0 && string != NULL)
                {
                    ...
                    // Either we are given a string or we
                    // are setting option to zero.
                ...
                {
                    return set_bool_option(opt_idx, varp,
                                           (int)number, opt_flags);
                }
            }
        }
    }
...
int find_key_option_len(const uchar_kt *arg, size_t len)
{
    int key;
    int modifiers;

    // Don't use get_special_key_code() for t_xx,
    // we don't want it to call add_termcap_entry().
    ...
    else
    {
        ...
        key = find_special_key(&arg, len + 1,
...
/// @param all
/// - if @b all == 0: show changed options
...
///   autocommand may have set them when doing ":edit file" and the
///   user has set them back at the default or fresh value. When
///   "local_only" is TRUE, don't write fresh values, only local
///   values (for ":mkview"). (fresh value = value used for a new
///   buffer or window for a local option).
///
               ...
               && ((pri == 1) == ((p->flags & P_PRI_MKRC) != 0)))
            {
                ...
                for(; round <= 2; varp = varp_local, ++round)
                {
                    ...
                    else // P_STRING
                    {
                        ...
                        // Don't set 'syntax' and 'filetype' again if the value
                        // is already right, avoids reloading the syntax file.
                        if(p->indir == PV_SYN || p->indir == PV_FT)
                        {
                            if(fprintf(fd, "if &%s != '%s'",
                                       p->fullname,
                                       *(uchar_kt **)(varp)) < 0
                               || put_eol(fd) < 0)
                            {
...
/// Generate set commands for the local fold options only.
///
/// Used when 'sessionoptions' or 'viewoptions' contains
                      ...
                      int expand)
{
    ...
    if(*valuep != NULL)
    {
        ...
        else if(expand)
        {
            ...
            usr_home_replace(NULL, *valuep, buf, MAXPATHL);
...
static int put_setnum(FILE *fd, char *cmd, char *name, long *valuep)
{
    ...
    if(wc_use_keyname((uchar_kt *)valuep, &wc))
    {
...
{
    // global/local option using global value
    if(value < 0)
    {
        return OK;
    }
...
/// columns needed by standard ruler
#define COL_RULER    17
...
void unset_global_local_option(char *name, void *from)
{
    ...
    switch((int)p->indir)
    {
        // global option with local value:
        // use local value if it's been set
        case PV_EP:
...
static uchar_kt *get_varp_scope(vimoption_st *p, int opt_flags)
{
    if((opt_flags & OPT_GLOBAL) && p->indir != PV_NONE)
    {
        if(p->var == VAR_WIN)
        {
            return (uchar_kt *)GLOBAL_WO(get_varp(p));
        }
    ...
    if((opt_flags & OPT_LOCAL) && ((int)p->indir & PV_BOTH))
    {
        ...
        {
            case PV_FP:
                return (uchar_kt *)&(curbuf->b_p_fp);

            case PV_EFM:
                return (uchar_kt *)&(curbuf->b_p_efm);

            case PV_GP:
                return (uchar_kt *)&(curbuf->b_p_gp);

            case PV_MP:
                return (uchar_kt *)&(curbuf->b_p_mp);

            case PV_EP:
                return (uchar_kt *)&(curbuf->b_p_ep);

            case PV_KP:
                return (uchar_kt *)&(curbuf->b_p_kp);

            case PV_PATH:
                return (uchar_kt *)&(curbuf->b_p_path);

            case PV_AR:
                return (uchar_kt *)&(curbuf->b_p_ar);

            case PV_TAGS:
                return (uchar_kt *)&(curbuf->b_p_tags);

            case PV_TC:
                return (uchar_kt *)&(curbuf->b_p_tc);

            case PV_DEF:
                return (uchar_kt *)&(curbuf->b_p_def);

            case PV_INC:
                return (uchar_kt *)&(curbuf->b_p_inc);

            case PV_DICT:
                return (uchar_kt *)&(curbuf->b_p_dict);
            ...
            case PV_LW:
                return (uchar_kt *)&(curbuf->b_p_lw);

            case PV_BKC:
                return (uchar_kt *)&(curbuf->b_p_bkc);
        }
    ...
    switch((int)p->indir)
    {
        ...
        case PV_EP:
            return *curbuf->b_p_ep != NUL
                   ? (uchar_kt *)&curbuf->b_p_ep : p->var;

        case PV_KP:
            return *curbuf->b_p_kp != NUL
                   ? (uchar_kt *)&curbuf->b_p_kp : p->var;

        case PV_PATH:
            return *curbuf->b_p_path != NUL
                   ? (uchar_kt *)&(curbuf->b_p_path) : p->var;

        case PV_AR:
            return curbuf->b_p_ar >= 0
                   ? (uchar_kt *)&(curbuf->b_p_ar) : p->var;

        case PV_TAGS:
            return *curbuf->b_p_tags != NUL
                   ? (uchar_kt *)&(curbuf->b_p_tags) : p->var;

        case PV_TC:
            return *curbuf->b_p_tc != NUL
                   ? (uchar_kt *)&(curbuf->b_p_tc) : p->var;

        case PV_BKC:
            return *curbuf->b_p_bkc != NUL
                   ? (uchar_kt *)&(curbuf->b_p_bkc) : p->var;

        case PV_DEF:
            return *curbuf->b_p_def != NUL
                   ? (uchar_kt *)&(curbuf->b_p_def) : p->var;

        case PV_INC:
            return *curbuf->b_p_inc != NUL
                   ? (uchar_kt *)&(curbuf->b_p_inc) : p->var;

        case PV_DICT:
            return *curbuf->b_p_dict != NUL
                   ? (uchar_kt *)&(curbuf->b_p_dict) : p->var;

        case PV_TSR:
            return *curbuf->b_p_tsr != NUL
                   ? (uchar_kt *)&(curbuf->b_p_tsr) : p->var;

        case PV_FP:
            return *curbuf->b_p_fp != NUL
                   ? (uchar_kt *)&(curbuf->b_p_fp) : p->var;

        case PV_EFM:
            return *curbuf->b_p_efm != NUL
                   ? (uchar_kt *)&(curbuf->b_p_efm) : p->var;

        case PV_GP:
            return *curbuf->b_p_gp != NUL
                   ? (uchar_kt *)&(curbuf->b_p_gp) : p->var;
        ...
        case PV_BIN:
            return (uchar_kt *)&(curbuf->b_p_bin);

        case PV_BOMB:
            return (uchar_kt *)&(curbuf->b_p_bomb);

        case PV_BH:
            return (uchar_kt *)&(curbuf->b_p_bh);

        case PV_BT:
            return (uchar_kt *)&(curbuf->b_p_bt);

        case PV_BL:
            return (uchar_kt *)&(curbuf->b_p_bl);

        case PV_CI:
            return (uchar_kt *)&(curbuf->b_p_ci);

        case PV_CIN:
            return (uchar_kt *)&(curbuf->b_p_cin);

        case PV_CINK:
            return (uchar_kt *)&(curbuf->b_p_cink);

        case PV_CINO:
            return (uchar_kt *)&(curbuf->b_p_cino);

        case PV_CINW:
            return (uchar_kt *)&(curbuf->b_p_cinw);

        case PV_COM:
            return (uchar_kt *)&(curbuf->b_p_com);

        case PV_CMS:
            return (uchar_kt *)&(curbuf->b_p_cms);

        case PV_CPT:
            return (uchar_kt *)&(curbuf->b_p_cpt);

        case PV_CFU:
            return (uchar_kt *)&(curbuf->b_p_cfu);

        case PV_OFU:
            return (uchar_kt *)&(curbuf->b_p_ofu);

        case PV_EOL:
            return (uchar_kt *)&(curbuf->b_p_eol);

        case PV_FIXEOL:
            return (uchar_kt *)&(curbuf->b_p_fixeol);

        case PV_ET:
            return (uchar_kt *)&(curbuf->b_p_et);

        case PV_FENC:
            return (uchar_kt *)&(curbuf->b_p_fenc);

        case PV_FF:
            return (uchar_kt *)&(curbuf->b_p_ff);

        case PV_FT:
            return (uchar_kt *)&(curbuf->b_p_ft);

        case PV_FO:
            return (uchar_kt *)&(curbuf->b_p_fo);

        case PV_FLP:
            return (uchar_kt *)&(curbuf->b_p_flp);

        case PV_IMI:
            return (uchar_kt *)&(curbuf->b_p_iminsert);

        case PV_IMS:
            return (uchar_kt *)&(curbuf->b_p_imsearch);

        case PV_INF:
            return (uchar_kt *)&(curbuf->b_p_inf);

        case PV_ISK:
            return (uchar_kt *)&(curbuf->b_p_isk);

        case PV_INEX:
            return (uchar_kt *)&(curbuf->b_p_inex);

        case PV_INDE:
            return (uchar_kt *)&(curbuf->b_p_inde);

        case PV_INDK:
            return (uchar_kt *)&(curbuf->b_p_indk);

        case PV_FEX:
            return (uchar_kt *)&(curbuf->b_p_fex);

        case PV_LISP:
            return (uchar_kt *)&(curbuf->b_p_lisp);

        case PV_ML:
            return (uchar_kt *)&(curbuf->b_p_ml);

        case PV_MPS:
            return (uchar_kt *)&(curbuf->b_p_mps);

        case PV_MA:
            return (uchar_kt *)&(curbuf->b_p_ma);

        case PV_MOD:
            return (uchar_kt *)&(curbuf->b_changed);

        case PV_NF:
            return (uchar_kt *)&(curbuf->b_p_nf);

        case PV_PI:
            return (uchar_kt *)&(curbuf->b_p_pi);

        case PV_QE:
            return (uchar_kt *)&(curbuf->b_p_qe);

        case PV_RO:
            return (uchar_kt *)&(curbuf->b_p_ro);

        case PV_SCBK:
            return (uchar_kt *)&(curbuf->b_p_scbk);

        case PV_SI:
            return (uchar_kt *)&(curbuf->b_p_si);

        case PV_STS:
            return (uchar_kt *)&(curbuf->b_p_sts);

        case PV_SUA:
            return (uchar_kt *)&(curbuf->b_p_sua);

        case PV_SWF:
            return (uchar_kt *)&(curbuf->b_p_swf);

        case PV_SMC:
            return (uchar_kt *)&(curbuf->b_p_smc);

        case PV_SYN:
            return (uchar_kt *)&(curbuf->b_p_syn);

        case PV_SPC:
            return (uchar_kt *)&(curwin->w_s->b_p_spc);

        case PV_SPF:
            return (uchar_kt *)&(curwin->w_s->b_p_spf);

        case PV_SPL:
            return (uchar_kt *)&(curwin->w_s->b_p_spl);

        case PV_SW:
            return (uchar_kt *)&(curbuf->b_p_sw);

        case PV_TS:
            return (uchar_kt *)&(curbuf->b_p_ts);

        case PV_TW:
            return (uchar_kt *)&(curbuf->b_p_tw);

        case PV_UDF:
            return (uchar_kt *)&(curbuf->b_p_udf);

        case PV_WM:
            return (uchar_kt *)&(curbuf->b_p_wm);
...
/// Get the value of 'equalprg', either the
/// buffer-local one or the global one.
uchar_kt *get_equalprg(void)
{
...
void buf_copy_options(filebuf_st *buf, int flags)
{
    ...
    if(p_cpo != NULL)
    {
        ...
        if(should_copy || (flags & BCO_ALWAYS))
        {
            ...
            // If not already initialized, set 'readonly'
            // and copy 'fileformat'.
            ...
            // Don't copy the options set by ex_help(), use the saved
            // values, when going from a help buffer to a non-help buffer.
            // Don't touch these at all when BCO_NOHELP is used and going
            // from or to a help buffer.
            if(dont_do_help)
            {
        ...
        // When the options should be copied (ignoring BCO_ALWAYS),
        // set the flag that indicates that the options have been initialized.
        if(should_copy)
        {
...
static uchar_kt expand_option_name[5] = {'t', '_', NUL, NUL, NUL};
...
void set_context_in_set_cmd(expand_st *xp, uchar_kt *arg, int opt_flags)
{
    ...
    if(*arg == '<')
    {
        ...
        nextchar = *++p;
        is_term_option = TRUE;
        expand_option_name[2] = (uchar_kt)KEY2TERMCAP0(key);
    ...
    if(flags & P_EXPAND)
    {
           ...
           || p == (uchar_kt *)&p_vdir)
        {
            ...
            if(p == (uchar_kt *)&p_path || p == (uchar_kt *)&p_cdpath)
            {
                   ...
                   uchar_kt ***file)
{
    ...
    for(loop = 0; loop <= 1; ++loop)
    {
        ...
        for(size_t opt_idx = 0;
            (str = (uchar_kt *)options[opt_idx].fullname) != NULL;
            opt_idx++)
        {
            if(options[opt_idx].var == NULL)
            {
        ...
        if(loop == 0)
        {
            ...
            *file = (uchar_kt **)xmalloc((size_t)(*num_file) * sizeof(uchar_kt *));
        }
    }
...
void ExpandOldSetting(int *num_file, uchar_kt ***file)
{
    uchar_kt *var = NULL;
    *num_file = 0;
    *file = (uchar_kt **)xmalloc(sizeof(uchar_kt *));
    ...
    // at the start and before a file name character.
...
/// Get the value for the numeric or string option *opp in a nice
...
/// Return TRUE if "varp" points to 'wildchar' or 'wildcharm'
/// and it can be printed as a keyname.
///
/// "*wcp" is set to the value of the option if it's 'wildchar'
/// or 'wildcharm'.
static int wc_use_keyname(uchar_kt *varp, long *wcp)
{
...
// Any character has an equivalent 'langmap' character.
// This is used for keyboards that have a special language
// mode that sends characters above 128 (although other characters
// can be translated too). The "to" field is a Vim command character.
// This avoids having to switch the keyboard back to ASCII mode when
// leaving Insert mode.
//
...
/// the language map can be changed at any time!
static void langmap_set(void)
{
    ...
    for(p = p_langmap; p[0] != NUL;)
    {
        ...
        while(p[0])
        {
            ...
            if(to == NUL)
            {
                EMSG2(_("E357: 'langmap': Matching "
                        "character missing for %s"), transchar(from));
            ...
            {
                assert(to <= UCHAR_MAX);
                langmap_mapchar[from & 255] = (uchar_kt)to;
            }
            ...
            if(p2 != NULL)
            {
                ...
                if(*p == ';')
                {
                    ...
                    if(p[0] != NUL)
                    {
                        if(p[0] != ',')
                        {
                            EMSG2(_("E358: 'langmap': Extra "
                                    "characters after semicolon: %s"), p);
...
static void paste_option_changed(void)
{
    ...
    if(p_paste)
    {
        ...
        // Always set the option values, also
        // when 'paste' is set when it is already on.
...
/// Check and set user current used configuration file
/// environment variable. When set it is convenient for
/// user to edit it, the form is: <b>:edit $USRNVIMRC</b>
void check_and_set_usrnvimrc(uchar_kt *fname)
{
    if(fname == NULL)
    {
        return;
    ...
    uchar_kt *env_val = (uchar_kt *)vim_getenv("USRNVIMRC");
    ...
    env_val = (uchar_kt *)FullName_save((char *)fname, FALSE);
...
/// Check and set the current dynamic configuration file
/// environment variable. This can be used to set project
/// related configuration. When set it is convenient for
/// user to edit it, the form is: <b>:edit $DYNNVIMRC</b>
void check_and_set_dynnvimrc(uchar_kt *fname)
{
    if(fname == NULL)
    {
        return;
    ...
    uchar_kt *env_val = (uchar_kt *)vim_getenv("DYNNVIMRC");
...
///
/// @return
/// - OK for correct value, FAIL otherwise.
/// - Empty is always OK.
static int check_opt_strings(uchar_kt *val, char **values, int list)
{
...
/// - OK for correct value, FAIL otherwise.
/// - Empty is always OK.
static int opt_strings_flags(uchar_kt *val,
                             char **values,
                             unsigned *flagp,
                             bool list)
{
...
/// whatBS_INDENT, BS_EOL or BS_START
...
/// from when editing started (save_file_ff() called). Also when 'endofline'
...
///
/// If there is a match set "*initc" to the matching character and
/// "*findc" to the opposite character. Set "*backwards" to the direction.
...
/// Like get_fileformat(), but override 'fileformat'
...
uchar_kt *skip_to_option_part(const uchar_kt *p)
{
    if(*p == ',')
    {
        p++;
...
/// Isolate one part of a string option separated by @b sep_chars.
///
...
size_t copy_option_part(uchar_kt **option,
                        uchar_kt *buf,
                        size_t maxlen,
                        char *sep_chars)
{
