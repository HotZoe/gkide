...
/// - Move the cursor directly, the text is scrolled
///   to keep the cursor in the window.
/// - Scroll the text, the cursor is moved into the text
...
static void comp_botline(win_st *wp)
{
    ...
    for(; lnum <= wp->w_buffer->b_ml.ml_line_count; ++lnum)
    {
        ...
        if(lnum <= wp->w_cursor.lnum && last >= wp->w_cursor.lnum)
        {
            ...
            wp->w_valid |= (kWVF_CLRow | kWVF_CLHeight);
        }
    ...
    wp->w_botline = lnum;
    wp->w_valid |= kWVF_BotLine | kWVF_BotLineAp;
...
{
    if((wp->w_o_curbuf.wo_rnu || wp->w_o_curbuf.wo_cul)
       && (wp->w_valid & kWVF_CLRow) == 0
       && !pum_visible())
    {
...
void update_topline(void)
{
    ...
    if(curwin->w_height == 0)
    {
        ...
        curwin->w_valid |= kWVF_BotLine | kWVF_BotLineAp;
    ...
    if(check_botline)
    {
        if(!(curwin->w_valid & kWVF_BotLineAp))
        {
            validate_botline();
        }
        ...
        if(curwin->w_botline <= curbuf->b_ml.ml_line_count)
        {
            ...
            if(check_botline)
            {
                ...
                if(hasAnyFolding(curwin))
                {
                    ...
                    line_count = curwin->w_cursor.lnum
                                 - curwin->w_botline
                                 + 1
                                 + p_so;
...
/// Return true when there are not 'scrolloff' lines
/// above the cursor for the current window.
...
void changed_window_setting_win(win_st *wp)
{
    ...
    wp->w_valid &= ~(kWVF_BotLine | kWVF_BotLineAp | kWVF_TopLine);
...
void set_topline(win_st *wp, linenum_kt lnum)
{
    ...
    // Don't set kWVF_TopLine here, 'scrolloff' needs to be checked.
    redraw_later(VALID);
}
...
/// Call this function when the length of a line
/// (in screen characters) above the cursor have changed.
/// Need to take care of w_botline separately!
...
{
    if(!(curwin->w_valid & kWVF_BotLine))
    {
        comp_botline(curwin);
    }
}
...
int cursor_valid(void)
{
    ...
    return (curwin->w_valid & (kWVF_WinRow | kWVF_WinCol))
            == (kWVF_WinRow | kWVF_WinCol);
}
...
void validate_cursor(void)
{
    ...
    int col_row = (kWVF_WinCol | kWVF_WinRow);
    if((curwin->w_valid & col_row) != col_row)
    {
        curs_columns(true);
    }
}
...
static void curs_rows(win_st *wp)
{
    ...
    for(linenum_kt lnum = wp->w_topline; lnum < wp->w_cursor.lnum; ++i)
    {
        ...
        if(valid && (lnum != wp->w_topline || !wp->w_o_curbuf.wo_diff))
        {
    ...
    if(!(wp->w_valid & kWVF_CLHeight))
    {
                   ...
                   || wp->w_lines[i].wl_lnum != wp->w_cursor.lnum)))
        {
            ...
            {
                wp->w_cline_height =
                    plines_win_nofill(wp, wp->w_cursor.lnum, true)
                    + wp->w_topfill;
            }
            else
            {
...
void validate_virtcol_win(win_st *wp)
{
    ...
    if(!(wp->w_valid & kWVF_FileCol))
    {
        getvvcol(wp, &wp->w_cursor, NULL, &(wp->w_virtcol), NULL);
        wp->w_valid |= kWVF_FileCol;

        if(wp->w_o_curbuf.wo_cuc && !pum_visible())
        {
            redraw_win_later(wp, SOME_VALID);
        }
    }
...
static void validate_cheight(void)
{
    ...
    if(!(curwin->w_valid & kWVF_CLHeight))
    {
        if(curwin->w_cursor.lnum == curwin->w_topline)
        {
...
{
    int ret_val = (wp->w_o_curbuf.wo_nu || wp->w_o_curbuf.wo_rnu)
                  ? number_width(wp) + 1 : 0;

    ret_val += (cmdwin_type == 0 || wp != curwin ? 0 : 1);
    ret_val += (int)wp->w_o_curbuf.wo_fdc;
    ret_val += (signcolumn_on(wp) ? 2 : 0);
...
void curs_columns(int may_scroll)
{
    ...
    else if(curwin->w_o_curbuf.wo_wrap && curwin->w_width != 0)
    {
        ...
        if(curwin->w_wcol >= curwin->w_width)
        {
            ...
            // When cursor wraps to first char of next
            // line in Insert mode, the 'showbreak' string
    ...
    // No line wrapping:
    // compute curwin->w_leftcol if scrolling is on and line is not folded.
    // If scrolling is off, curwin->w_leftcol is assumed to be 0
    else if(may_scroll && !curwin->w_cline_folded)
    {
        // If Cursor is left of the screen, scroll rightwards.
        // If Cursor is right of the screen, scroll leftwards
        // If we get closer to the edge than 'sidescrolloff',
        // scroll a little extra
        ...
        if(off_left < 0 || off_right > 0)
        {
            ...
            // When far off or not enough room on either side,
    ...
    if((curwin->w_wrow >= curwin->w_height
        || ((prev_skipcol > 0 || curwin->w_wrow + p_so >= curwin->w_height)
            && (p_lines = plines_win_nofill(curwin,
                                            curwin->w_cursor.lnum,
                                            false)) - 1 >= curwin->w_height))
       && curwin->w_height != 0
       ...
       && curwin->w_width != 0)
    {
        ...
        // Compute last display line of the buffer line
        // that we want at the bottom of the window.
        if(p_lines == 0)
        {
    ...
    if(curwin->w_o_curbuf.wo_cuc
       && (curwin->w_valid & kWVF_FileCol) == 0
       && !pum_visible())
    {
        redraw_later(SOME_VALID);
    }

    curwin->w_valid |= kWVF_WinCol | kWVF_WinRow | kWVF_FileCol;
}

/// Scroll the current window down by "line_count" logical lines. "CTRL-Y"
///
/// @param line_count
/// @param byfold      true: count a closed fold as one line
...
{
    if((byfold && hasAnyFolding(curwin)) || curwin->w_o_curbuf.wo_diff)
    {
    ...
    curwin->w_valid &= ~(kWVF_WinRow | kWVF_CLRow | kWVF_BotLine);
...
/// Use as many filler lines as possible for w_topline.
/// Make sure w_topline is still visible.
...
/// Scroll the screen one line down, but don't do it if
/// it would move the cursor off the screen.
...
/// Scroll the screen one line up, but don't do it if
/// it would move the cursor off the screen.
void scrollup_clamp(void)
{
    ...
    int start_row = curwin->w_wrow
                    - plines_nofill(curwin->w_topline)
                    - curwin->w_topfill;
...
/// Add one line below "lp->lnum". This can be a filler line,
...
void scroll_cursor_top(int min_scroll, int always)
{
    ...
    // "used" already contains the number of filler lines above,
    // don't add it again. Hide filler lines above cursor line by
    // adding them to "extra".
    ...
    {
        // If "always" is false, only adjust topline to a lower
        // value, higher value may happen with wrapping lines
...
/// Scroll at least "min_scroll" lines. If "set_topbot" is true, set
/// topline and botline first (for "zb"). This is messy stuff!!!
void scroll_cursor_bot(int min_scroll, int set_topbot)
{
    ...
    if(set_topbot)
    {
        ...
        if(curwin->w_topline != old_topline
           || curwin->w_topfill != old_topfill)
        {
            curwin->w_valid &= ~(kWVF_WinRow | kWVF_CLRow);
        }
    }
    else
    {
    ...
    // If the cursor is below botline, we will at least scroll
    // by the height of the cursor line. Correct for empty lines,
    ...
    // If topline didn't change we need to restore w_botline
    // and w_empty_rows (we changed them). If topline did change,
    // update_screen() will set botline.
...
/// Correct the cursor position so that it is in a part of
/// the screen at least 'so' lines from the top and bottom, if possible.
...
int onepage(int dir, long count)
{
    ...
    for(; count > 0; --count)
    {
        ...
        if(dir == FORWARD)
        {
            ...
            {
                // For the overlap, start with the line just below
                // the window and go upwards.
                loff.lnum = curwin->w_botline;
        ...
        else // dir == BACKWARDS
        {
            ...
            // Find the line just above the new topline to get the
            // right line at the bottom of the window.
            n = 0;
            ...
            else
            {
                ...
                // Avoid getting stuck on very long lines.
                if(loff.lnum >= curwin->w_topline
                   && (loff.lnum > curwin->w_topline || loff.fill >= curwin->w_topfill))
                {
                    // First try using the maximum number of filler lines.
                    // If that's not enough, backup one line.
...
void halfpage(bool flag, linenum_kt Prenum)
{
    ...
    if(Prenum)
    {
        curwin->w_o_curbuf.wo_scr = (Prenum > curwin->w_height)
                          ? curwin->w_height
                          : Prenum;
    }

    assert(curwin->w_o_curbuf.wo_scr <= INT_MAX);

    int n = curwin->w_o_curbuf.wo_scr <= curwin->w_height
            ? (int)curwin->w_o_curbuf.wo_scr
            : curwin->w_height;
    ...
    if(flag)
    {
        ...
        while(n > 0 && curwin->w_botline <= curbuf->b_ml.ml_line_count)
        {
            ...
            curwin->w_valid &= ~(kWVF_CLRow | kWVF_WinRow);
            scrolled += i;
...
void do_check_cursorbind(void)
{
    ...
    FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
    {
        ...
        if(curwin != old_curwin && curwin->w_o_curbuf.wo_crb)
        {
            if(curwin->w_o_curbuf.wo_diff)
            {
                curwin->w_cursor.lnum =
