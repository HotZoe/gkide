...
local build_convert_table = function(ut_fp, props, cond_func, nl_index, table_name)
    ...
    for _, p in ipairs(props) do
        if cond_func(p) then
            ...
            if start >= 0 
               and add == (nl - n) 
               and (step == 0 or n - end_ == step) then
...
local build_case_table = function(ut_fp, dataprops, table_name, index)
    ...
    return build_convert_table(ut_fp, 
                               dataprops, 
                               cond_func, 
                               index, 
                               'to' .. table_name)
...
local doublewidth = build_width_table(ut_fp, 
                                      dataprops, 
                                      widthprops,
                                      {W=true, F=true}, 
                                      'doublewidth')
local ambiwidth = build_width_table(ut_fp, 
                                    dataprops, 
                                    widthprops,
                                    {A=true}, 
                                    'ambiguous')
