...
/// Negative characters are special key codes.
/// Only characters below -0x200 are used to so that the absolute value can't
/// be mistaken for a single-byte character.
...
#define K_SELECT_STRING         (uchar_kt *)"\200\365X"
...
#define TO_SPECIAL(a, b)    ((a) == KS_SPECIAL             \
                             ? K_SPECIAL : (a) ==  KS_ZERO \
                             ? K_ZERO : TERMCAP2KEY(a, b))
...
#define CPO_TO_CPO_FLAGS   \
    (((ustrchr(p_cpo, CPO_BSLASH) == NULL) ? 0 : FLAG_CPO_BSLASH) \
     |(ustrchr(p_cpo, CPO_SPECI) == NULL ? 0 : FLAG_CPO_SPECI))
