...
/// Find the start of a comment, not knowing if we are in a
/// comment right now. Search starts at w_cursor.lnum and
/// goes backwards.
///
...
static uchar_kt *skip_string(uchar_kt *p)
{
    int i;
    ...
    for(;; ++p)
    {
        ...
        else if(p[0] == 'R' && p[1] == '"')
        {
            ...
            if(paren != NULL)
            {
                ...
                for(p += 3; *p; ++p)
                {
                    if(p[0] == ')' && ustrncmp(p + 1, delim, delim_len) == 0
                       && p[delim_len + 1] == '"')
                    {
                        p += delim_len + 1;
                        break;
...
bool cin_is_cinword(uchar_kt *line)
{
    bool retval = false;
    size_t cinw_len = ustrlen(curbuf->b_p_cinw) + 1;
    uchar_kt *cinw_buf = xmalloc(cinw_len);
    line = skipwhite(line);

    for(uchar_kt *cinw = curbuf->b_p_cinw; *cinw;)
    {
        size_t len = copy_option_part(&cinw, cinw_buf, cinw_len, ",");

        if(ustrncmp(line, cinw_buf, len) == 0
           && (!is_kwc(line[len]) || !is_kwc(line[len - 1])))
        {
            retval = true;
            break;
        }
...
static uchar_kt *cin_skipcomment(uchar_kt *s)
{
    while(*s)
    {
        uchar_kt *prev_s = s;
        ...
        // Perl/shell # comment comment continues until eol.
...
/// Return TRUE if there is no code at *s.
/// White space and comments are not considered code.
static int cin_nocode(uchar_kt *s)
{
...
static bool cin_has_js_key(uchar_kt *text)
{
    uchar_kt *s = skipwhite(text);
    uchar_kt quote = 0;
    ...
    if(!is_id_char(*s)) // need at least one ID character
    {
        return FALSE;
    }
...
static int cin_islabel_skip(uchar_kt **s)
{
    if(!is_id_char(**s)) // need at least one ID character
    {
        return FALSE;
...
{
    uchar_kt *s = cin_skipcomment(get_cursor_line_ptr());
...
static int cin_isinit(void)
{
    ...
    if(cin_ends_in(s, (uchar_kt *)"=", (uchar_kt *)"{"))
    {
        return TRUE;
    }
...
int cin_iscase(uchar_kt *s, int strict)
{
    s = cin_skipcomment(s);
...
static int cin_isdefault(uchar_kt *s)
{
    return ustrncmp(s, "default", 7) == 0
           && *(s = cin_skipcomment(s + 7)) == ':'
           && s[1] != ':';
}

/// Recognize a "public/private/protected" scope declaration label.
int cin_isscopedecl(uchar_kt *s)
{
    int i;
    s = cin_skipcomment(s);

    if(ustrncmp(s, "public", 6) == 0)
    {
        i = 6;
    }
    else if(ustrncmp(s, "protected", 9) == 0)
    {
        i = 9;
    }
    else if(ustrncmp(s, "private", 7) == 0)
    {
        i = 7;
    }
    else
...
static int cin_is_cpp_namespace(uchar_kt *s)
{
    uchar_kt *p;
    int has_name = FALSE;
    s = cin_skipcomment(s);

    if(ustrncmp(s, "namespace", 9) == 0 && (s[9] == NUL || !is_kwc(s[9])))
    {
        ...
        while(*p != NUL)
        {
            ...
            else if(is_kwc(*p))
            {
                if(has_name)
                {
                    // word character after skipping past name
                    return FALSE;
                }
...
///    case 234:    a = b;
///             ^
static uchar_kt *after_label(uchar_kt *l)
{
    for(; *l; ++l)
    {
        if(*l == ':')
...
///
/// @return Returns zero when it doesn't look like a declaration.
static int cin_first_id_amount(void)
{
    ...
    if(len == 6 && ustrncmp(p, "static", 6) == 0)
    {
        p = skipwhite(p + 6);
        len = (int)(skiptowhite(p) - p);
    }

    if(len == 6 && ustrncmp(p, "struct", 6) == 0)
    {
        p = skipwhite(p + 6);
    }
    else if(len == 4 && ustrncmp(p, "enum", 4) == 0)
    {
        p = skipwhite(p + 4);
    }
    else if((len == 8 && ustrncmp(p, "unsigned", 8) == 0)
            || (len == 6 && ustrncmp(p, "signed", 6) == 0))
    {
        s = skipwhite(p + len);

        if((ustrncmp(s, "int", 3) == 0 && ascii_iswhite(s[3]))
           || (ustrncmp(s, "long", 4) == 0 && ascii_iswhite(s[4]))
           || (ustrncmp(s, "short", 5) == 0 && ascii_iswhite(s[5]))
           || (ustrncmp(s, "char", 4) == 0 && ascii_iswhite(s[4])))
        {
            p = s;
        }
    }

    for(len = 0; is_id_char(p[len]); ++len)
        ;
...
static int cin_get_equal_amount(linenum_kt lnum)
{
    ...
    while(*s != NUL && ustrchr((uchar_kt *)"=;{}\"'", *s) == NULL)
    {
...
/// Recognize a preprocessor statement:
/// Any line that starts with '#'.
static int cin_ispreproc(uchar_kt *s)
{
    if(*skipwhite(s) == '#')
    {
        return TRUE;
...
static int cin_iscomment(uchar_kt *p)
{
    return p[0] == '/' && (p[1] == '*' || p[1] == '/');
}

/// Recognize the start of a "//" comment.
static int cin_islinecomment(uchar_kt *p)
{
    return p[0] == '/' && p[1] == '/';
}
...
///
/// If a line begins with an "else", only consider it terminated
/// if no unmatched opening braces follow (handle "else { foo();" correctly).
///
/// @param s
...
///
/// @return
/// the character terminating the line (ending char's have precedence if
/// both apply in order to determine initializations).
static uchar_kt cin_isterminated(uchar_kt *s, int incl_open, int incl_comma)
{
    uchar_kt found_start = 0;
    unsigned n_open = 0;
    int is_else = FALSE;
    s = cin_skipcomment(s);
...
static int cin_isfuncdecl(uchar_kt **sp, linenum_kt first_lnum, linenum_kt min_lnum)
{
    ...
    while(*s && *s != ';' && *s != '\'' && *s != '"')
    {
        ...
        if((*s == ',' && cin_nocode(s + 1)) || s[1] == NUL || cin_nocode(s))
        {
            ...
            // Require a comma at end of the line or a comma
            // or ')' at the start of next line.
            s = skipwhite(s);
...
static int cin_isif(uchar_kt *p)
{
    return ustrncmp(p, "if", 2) == 0 && !is_id_char(p[2]);
}

static int cin_iselse(uchar_kt *p)
{
    if(*p == '}') // accept "} else"
    {
    ...
    return ustrncmp(p, "else", 4) == 0 && !is_id_char(p[4]);
}

static int cin_isdo(uchar_kt *p)
{
    return ustrncmp(p, "do", 2) == 0 && !is_id_char(p[2]);
}

/// Check if this is a "while" that should have a matching "do".
/// We only accept a "while (condition) ;", with only white space
/// between the ')' and ';'. The condition may be spread over several lines.
///
/// @param p
/// @param lnum
...
/// string was found.
static int cin_is_if_for_while_before_offset(uchar_kt *line, int *poffset)
{
    int offset = *poffset;
    ...
    if(offset >= 1)
    {
        ...
        if(offset >= 2)
        {
            ...
            if(!ustrncmp(line + offset, "while", 5))
            {
                goto probablyFound;
            }
        }
...
probablyFound:
    ...
    if(!offset || !is_id_char(line[offset - 1]))
    {
        *poffset = offset;
        return 1;
    }
...
static int cin_isbreak(uchar_kt *p)
{
    return ustrncmp(p, "break", 5) == 0 && !is_id_char(p[5]);
}
...
static int cin_is_cpp_baseclass(cpp_baseclass_cache_st *cached)
{
    ...
    for(;;)
    {
        ...
        else if((ustrncmp(s, "class", 5) == 0 && !is_id_char(s[5]))
                || (ustrncmp(s, "struct", 6) == 0 && !is_id_char(s[6])))
        {
        ...
        else
        {
            ...
            {
                // Constructor-initialization is assumed
                // if we come across something like "):"
...
static int get_baseclass_amount(int col)
{
    ...
    if(col == 0)
    {
        ...
        if(!cin_ends_in(get_cursor_line_ptr(), (uchar_kt *)",", NULL))
        {
...
/// Return TRUE if string "s" ends with the string "find", possibly
/// followed by white space and comments. Skip strings and comments.
/// Ignore "ignore" after "find" if it's not NULL.
static int cin_ends_in(uchar_kt *s, uchar_kt *find, uchar_kt *ignore)
{
    uchar_kt *p = s;
    uchar_kt  *r;
    int len = (int)ustrlen(find);

    while(*p != NUL)
    {
        p = cin_skipcomment(p);

        if(ustrncmp(p, find, len) == 0)
        {
            r = skipwhite(p + len);

            if(ignore != NULL && ustrncmp(r, ignore, ustrlen(ignore)) == 0)
            {
                r = skipwhite(r + ustrlen(ignore));
            }
...
static int cin_starts_with(uchar_kt *s, char *word)
{
    int l = (int)ustrlen(word);
    return ustrncmp(s, word, l) == 0 && !is_id_char(s[l]);
}
...
/// Ignore a '{' that is in a comment, makes indenting
...
retry:
    ...
    if((trypos = findmatchlimit(NULL, c, 0, ind_maxp_wk)) != NULL)
    {
        ...
        else
        {
            ...
            if((trypos_wk = ind_find_start_CORS()) != NULL)
            {
                ind_maxp_wk = ind_maxparen
                              - (int)(cursor_save.lnum - trypos_wk->lnum);
...
/// Return ind_maxparen corrected for the difference in line number between
...
/// Set w_cursor.col to the column number of the last unmatched ')'
/// or '{' in line "l". "l" must point to the start of the line.
static int find_last_paren(uchar_kt *l, int start, int end)
{
    int i;
    int retval = FALSE;
    int open_count = 0;
...
void parse_cino(filebuf_st *buf)
{
    uchar_kt *p;
    uchar_kt *l;
    int divider;
    ...
    //
    // Spaces from a block's opening brace the
    // prevailing indent for that block should be.
    buf->b_ind_level = sw;

    // Spaces from the edge of the line an open brace
    // that's at the end of a line is imagined to be.
    buf->b_ind_open_imag = 0;

    // Spaces from the prevailing indent for a line
    // that is not preceded by an opening brace.
    buf->b_ind_no_brace = 0;

    // Column where the first { of a function should be located }.
    buf->b_ind_first_open = 0;

    // Spaces from the prevailing indent a leftmost open
    // brace should be located.
    buf->b_ind_open_extra = 0;

    // Spaces from the matching open brace (real location for one at
    // the left edge; imaginary location from one that ends a line)
    // the matching close brace should be located.
    buf->b_ind_close_extra = 0;

    // Spaces from the edge of the line an open brace sitting in the
    // leftmost column is imagined to be.
    buf->b_ind_open_left_imag = 0;

    // Spaces jump labels should be shifted to the left if N is
    // non-negative, otherwise the jump label will be put to column 1.
    ...
    // Handle continuation lines containing conditions
    // of if(), for() and while().
    ...
    for(p = buf->b_p_cino; *p;)
    {
        ...
        uchar_kt *digits_start = p; // remember where the digits start
        ...
        // When adding an entry here, also update the default
        // 'cinoptions' in doc/indent.txt, and add explanation for it!
        switch(*l)
        {
            case '>':
                buf->b_ind_level = n;
...
#define LOOKFOR_COMMA           12
    ...
    // ml_get is valid!
    linecopy = ustrdup(ml_get(cur_curpos.lnum));
    ...
    if((curmod & kInsertMode)
       && curwin->w_cursor.col < (columnum_kt)ustrlen(linecopy)
    ...
    if(!cin_iscomment(theline) && comment_pos != NULL)
    {
        ...
        uchar_kt lead_start[COM_MAX_LEN]; // start-comment string
        ...
        *lead_middle = NUL;
        ...
        while(*p != NUL)
        {
            ...
            if(what == COM_START)
            {
                ustrcpy(lead_start, lead_end);
                lead_start_len = (int)ustrlen(lead_start);
                start_off = off;
                start_align = align;
            }
            else if(what == COM_MIDDLE)
            {
                ustrcpy(lead_middle, lead_end);
                lead_middle_len = (int)ustrlen(lead_middle);
            }
            else if(what == COM_END)
            {
                // If our line starts with the middle comment string, line it
                // up with the comment opener per the 'comments' option.
                if(ustrncmp(theline, lead_middle, lead_middle_len) == 0
                   && ustrncmp(theline, lead_end, ustrlen(lead_end)) != 0)
                {
                    done = TRUE;
                    ...
                    if(curwin->w_cursor.lnum > 1)
                    {
                        ...
                        // line, use the indent of that line.
                        look = skipwhite(ml_get(curwin->w_cursor.lnum - 1));

                        if(ustrncmp(look, lead_start, lead_start_len) == 0)
                        {
                            amount = get_indent_lnum(curwin->w_cursor.lnum - 1);
                        }
                        else if(ustrncmp(look, lead_middle, lead_middle_len) == 0)
                        {
                            amount = get_indent_lnum(curwin->w_cursor.lnum - 1);
                            break;
                        }
                        else if(ustrncmp(ml_get(comment_pos->lnum) + comment_pos->col,
                                        lead_start, lead_start_len) != 0)
                        {
                            // If the start comment string doesn't match with the
                            // start of the comment, skip this entry.
                            continue;
                        }
                    }
                    ...
                    {
                        amount += ustr_scrsize(lead_start)
                                  - ustr_scrsize(lead_middle);
                    }
       ...
       || trypos != NULL)
    {
        ...
        if(trypos != NULL)
        {
            ...
            // indent of a previous non-empty line that matches the same paren.
            ...
            if(amount == -1)
            {
                       ...
                       && ignore_paren_col == 0))
                {
                    ...
                    // When b_ind_unclosed_wrapped is set and the matching paren
                    // is the last nonwhite character of the line, use either
                    // the indent of the current line or the indentation of the
                    // next outer paren and add b_ind_unclosed_wrapped
                    // (for very long lines).
                    if(theline[0] != ')')
                    {
                        cur_amount = MAXCOL;
                    ...
                    // character after it if we did the above "if".
                ...
                else
                {
                    ...
                    // Use b_ind_unclosed once, when the first '('
                    // is not inside braces
                    if(col == MAXCOL)
                    {
                    ...
                    // positions, to avoid giving it more indent than the
                    // previous lines:
                    //  func_long_name(         if (x
                    //  arg                 && yy
                    //  )     ^ not here           )    ^ not here
        ...
        {
            // We are inside braces, there is a { before this line at
            // the position stored in tryposBrace.
            ...
            bool js_cur_has_key = curbuf->b_ind_js
                                  ? cin_has_js_key(theline) : false;
            ...
            {
                // If we're looking at an "else",
                // try to find an "if" to match it with.
                // If we're looking at a "while",
                // try to find a "do" to match it with.
                ...
                // Search backwards. If we find something we recognize,
                // line up with that.
                //
                ...
                for(;;)
                {
                    ...
                    // If we went all the way back to the start
                    // of our scope, line up with it.
                    if(curwin->w_cursor.lnum <= ourscope)
                    {
                        ...
                        // don't add ind_continuation, otherwise it is a
                        ...
                        if(lookfor == LOOKFOR_ENUM_OR_INIT)
                        {
                            ...
                            {
                                // nothing found (abuse curbuf->b_ind_maxparen
                                // as limit) assume terminated line
                                // (i.e. a variable initialization)
                                if(cont_amount > 0)
                                {
                                    amount = cont_amount;
                                }
                            ...
                            // If we're in a comment or raw string now,
                            // skip to the start of it.
                               ...
                               || !cin_isfuncdecl(&l, curwin->w_cursor.lnum, 0))
                            {
                                ...
                                // if it is an enum declaration or an
                                // assignment, we are done.
                    ...
                    if(iscase || cin_isscopedecl(l))
                    {
                        ...
                        // When looking for a "do" we
                        // are not interested in labels.
                        if(whilelevel > 0)
                        {
                            continue;
                        }
                        ...
                        {
                            // Check that this case label is
                            // not for another switch()
                    ...
                    // Are we at the start of a cpp base class
                    // declaration or constructor initialization?
                    n = FALSE;
                    ...
                    if(lookfor == LOOKFOR_COMMA)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // line above is start of the scope,
                                // thus current line is the one that stars a
                                // (possibly broken) line ending in a comma.
                                break;
                            }
                        }
                    ...
                    {
                        if(lookfor != LOOKFOR_ENUM_OR_INIT
                           && (*skipwhite(l) == '[' || l[ustrlen(l) - 1] == '['))
                        {
                            amount += ind_continuation;
                        }

                        // If we're in the middle of a paren thing,
                        // Go back to the line that starts it so we
                        // can get the right prevailing indent
                        //     if ( foo &&
                        //          bar )
                        // Position the cursor over the rightmost paren,
                        // so that matching it will take us back to the
                        // start of the line.
                        ...
                        // we also look for matching braces.
                        ...
                        // Skip over continuation lines to find the one
                        // to get the indent from, line ending with backslash
                        if(terminated == ',')
                        {
                            while(curwin->w_cursor.lnum > 1)
                            {
                        ...
                        // and it starts with a '{', line it up with this line.
                        //      while (not)
                        // ->   {
                        //      }
                        ...
                        if(cin_is_cinword(l) || cin_iselse(skipwhite(l)))
                        {
                            ...
                            // Special trick: when expecting the while ()
                            // after a do, line up with the while()
                            //     do
                            //      x = 1;
                            // ->  here
                            l = skipwhite(get_cursor_line_ptr());
                            ...
                            {
                                // If we're looking at "} else", let's make
                                // sure we find the opening brace of the
                                // enclosing scope, not the one from "if () {".
                                if(*l == '}')
                                {
                                    curwin->w_cursor.col =
                        ...
                        {
                            // Found two unterminated lines on a row,
                            // line up with the last one.
                            //   c = 99 +
                            //      100 +
                            // ->       here;
                            ...
                            if(lookfor == LOOKFOR_ENUM_OR_INIT)
                            {
                                // Found two lines ending in ',', lineup with
                                // the lowest one, but check for cpp base class
                                ...
                                // Ignore unterminated lines in between,
                                // but reduce indent.
                            ...
                            else
                            {
                                ...
                                // If previous line ends in ',', check whether
                                // we are in an initialization or enum
                                // struct xxx =
                                // {
                                //      sizeof a,
                                //      124 };
                                // or a normal possible continuation line.
                                // but only, of no other statement has been
                                // found yet.
                                ...
                                else
                                {
                                    ...
                                    {
                                        cont_amount =
                                            cin_get_equal_amount(curwin->w_cursor.lnum);
                                    }
                    ...
                    else
                    {
                        ...
                        // Found a terminated line above a terminated line or
                        // "if" etc. line. Use the amount of the line below us.
                        //   x = 1;             x = 1;
                        //   if (asdf)          y = 2;
                        //       while (asdf)     ->here;
                        //      here;
                        ...
                        // First line above the one we're indenting is
                        // terminated.
...
term_again:
                            ...
                            // When aligning with the case statement,
                            // don't align with a statement after it.
                            ...
                            // When a terminated line starts with "else" skip
                            // to the matching "if":
                            //       else 3;
                            //       indent this;
                            // Need to use the scope of this "else".  XXX
                            ...
                            // If we're at the end of a block, skip to
    ...
    if(cur_curpos.lnum < curbuf->b_ml.ml_line_count
       && !cin_nocode(theline)
       && ustrchr(theline, '{') == NULL
       && ustrchr(theline, '}') == NULL
       && !cin_ends_in(theline, (uchar_kt *)":", NULL)
       && !cin_ends_in(theline, (uchar_kt *)",", NULL)
    ...
    while(curwin->w_cursor.lnum > 1)
    {
        ...
        // If we're in a comment or raw string
        // now, skip to the start of it.
        if((trypos = ind_find_start_CORS()) != NULL)
        {
            curwin->w_cursor.lnum = trypos->lnum + 1;
            curwin->w_cursor.col = 0;
        ...
        // If the previous line ends in ',', use one level of indentation:
        // int foo,
        //     bar;
        ...
        if(cin_ends_in(l, (uchar_kt *)",", NULL)
           || (*l != NUL && (n = l[ustrlen(l) - 1]) == '\\'))
        {
        ...
        // Find a line only has a semicolon that belongs to a
        // previous line ending in '}', e.g. before an #endif.
        ...
        if(*(look = skipwhite(l)) == ';' && cin_nocode(look + 1))
        {
            ...
            while(curwin->w_cursor.lnum > 1)
            {
                ...
                if(!(cin_nocode(look)
                     || cin_ispreproc_cont(&look, &curwin->w_cursor.lnum)))
                {
                    break;
                }
            }
            ...
            if(curwin->w_cursor.lnum > 0
               && cin_ends_in(look, (uchar_kt *)"}", NULL))
            {
                break;
            }
        ...
        // If the PREVIOUS line is a function declaration,
        // the current line (and the ones that follow) needs
...
static int find_match(int lookfor, linenum_kt ourscope)
{
    ...
    while(curwin->w_cursor.lnum > ourscope + 1)
    {
        ...
        if(cin_isif(look))
        {
            ...
            // When looking for an "if" ignore
            // "while"s that get in the way.
