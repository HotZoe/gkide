...
#define CTRL_X_MSG(i) \
    ctrl_x_msgs[(i) & ~CTRL_X_WANT_IDENT]
...
struct compl_S
{
    compl_T *cp_next;
    compl_T *cp_prev;
    uchar_kt  *cp_str;  ///< matched text
    char cp_icase;    ///< TRUE or FALSE: ignore case

    /// text for the menu
    uchar_kt *(cp_text[CPT_COUNT]);

    /// file containing the match, allocated when cp_flags has FREE_FNAME
    uchar_kt *cp_fname;
...
/// points to the currently selected entry.
static compl_T *compl_curr_match = NULL;

/// different from compl_curr_match during ins_compl_get_exp()
static compl_T *compl_shown_match = NULL;

/// After using a cursor key <Enter> selects a match
/// in the popup menu, otherwise it inserts a line break.
static int compl_enter_selects = FALSE;

/// When "compl_leader" is not NULL only matches
/// that start with this string are used.
static uchar_kt *compl_leader = NULL;

/// put longest common string in compl_leader
static int compl_get_longest = FALSE;

/// FALSE: select & insertTRUE: noinsert
static int compl_no_insert = FALSE;

/// FALSE: select & insert TRUE: noselect
static int compl_no_select = FALSE;

/// Selected one of the matches. When FALSE the
/// match was edited or using the longest common string.
...
/// When the first completion is done "compl_started" is set.
/// When it's FALSE the word to be completed must be located.
static int compl_started = FALSE;

static int compl_matches = 0;
static uchar_kt *compl_pattern = NULL;
...
/// call u_save() before inserting a char. Set when edit() is called.
/// after that arrow_used is used.
static int ins_need_undo;

/// auto_format() added an extra space under the cursor
...
static void insert_enter(InsertState *s)
{
    ...
    s->old_topfill = -1;
    s->replaceState = kReplaceMode;

    // Remember whether editing was restarted after CTRL-O
    did_restart_edit = restart_edit;

    // sleep before redrawing, needed for
    // "CTRL-O :" that results in an error message
    ...
    // Trigger InsertEnter autocommands.
    ...
    {
        curmod = kVReplaceMode;
        s->replaceState = kVReplaceMode;
    ...
    // it might move when the cursor is
    ...
    // unless 'insertmode' is set or restarting.
    ...
    {
        new_insert_skip = (int)ustrlen(s->ptr);
        xfree(s->ptr);
    }
    ...
    do
    {
        ...
        // If s->count != 0, `ins_esc` will prepare the redo buffer for
        // reprocessing and return false, causing `state_enter` to be
        // called again.
...
static int insert_check(nvim_state_st *state)
{
    ...
    // If there is no typeahead may check
    // for timestamps (e.g., for when a
       ...
       && curwin->w_topfill == s->old_topfill)
    {
        ...
        if(curwin->w_wcol < s->mincol - curbuf->b_p_ts
           && curwin->w_wrow
              == curwin->w_winrow + curwin->w_height - 1 - p_so
...
static int insert_execute(nvim_state_st *state, int key)
{
    ...
    // Don't want K_EVENT with cursorhold for the second key,
    // e.g., after CTRL-V.
    did_cursorhold = true;
        ...
        if(!compl_used_match)
        {
            ...
            if(s->c == Ctrl_L
               && (!CTRL_X_MODE_LINE_OR_EVAL(ctrl_x_mode)
                   || (int)ustrlen(compl_shown_match->cp_str)
                      > curwin->w_cursor.col - compl_col))
            {
            ...
            if(ins_compl_accept_char(s->c))
            {
                ...
                uchar_kt *str = do_insert_char_pre(s->c);
                uchar_kt *p;
    ...
    if(cindent_on() && ctrl_x_mode == 0)
    {
        ...
        if(can_cindent
           && in_cinkeys(s->c, '*', s->line_is_white)
           && stop_arrow() == OK)
        {
            do_c_expr_indent();
        }
...
static int insert_handle_key(InsertState *s)
{
    ...
    switch(s->c)
    {
        ...
        case Ctrl_C: // End input mode
            ...
            // through a mapping, don't leave Insert mode
            if(goto_im())
            {
                if(got_int)
                {
        ...
        case Ctrl_U: // delete all inserted text in current line
            ...
            {
                s->did_backspace =
                    ins_bs(s->c, BACKSPACE_LINE, &s->inserted_space);
...
normalchar:
            ...
            if(!p_paste)
            {
                ...
                // If the new value is already inserted or
                // an empty string then don't insert any character.
                if(s->c == NUL)
                {
                    break;
                }
            ...
            auto_format(false, true);
            // When inserting a character the cursor
            // line must never be in a closed fold.
            foldOpenCursor();
            break;
        } // switch-default-end
    }
...
static void ins_redraw(int ready)
{
       ...
       && !pum_visible())
    {
        ...
        {
            // Make sure curswant is correct,
            // an autocommand may call getcurpos()
            update_curswant();
...
static void ins_ctrl_v(void)
{
    ...
    // may need to redraw when
    // no more chars available now
    ins_redraw(FALSE);
    ...
    // when the line fits in 'columns' the '^' is at the
    // start of the next line and will not removed by the redraw
    if(did_putchar)
    {
        edit_unputchar();
    }
...
static uchar_kt pc_bytes[MB_MAXBYTES + 1]; ///< saved bytes
static int pc_attr;
static int pc_row;
static int pc_col;
...
/// Call this function before moving the cursor
/// from the normal insert position in insert mode.
...
/// - if @b round is TRUE, round the indent to 'shiftwidth'
///   (only with _INC and _Dec).
///
/// @param type
/// @param amount
/// @param round
                   ...
                   int call_changed_bytes)
{
    ...
    // Set the new indent.
    // The cursor will be put on the first non-blank.
    if(type == INDENT_SET)
    {
    ...
    else
    {
        ...
        // When changing the indent while the cursor
        // is in it, reset Insstart_col to 0.
        insstart_less = MAXCOL;
    }
    ...
    // For 'kReplaceMode' mode, may have to fix the replace stack, if it's possible.
    ...
    // For 'kVReplaceMode' mode, we also have to fix the replace stack. In
    // this case it is always possible because we backspace over the whole
    // line and then put it back again the way we wanted it.
    if(curmod & kModFlgVReplace)
    {
        // Save new line
        new_line = ustrdup(get_cursor_line_ptr());
...
/// Truncate the space at the end of a line.
/// This is to be used only in an insert mode.
/// It handles fixing the replace stack for @b kReplaceMode and @b kVReplaceMode modes.
void truncate_spaces(uchar_kt *line)
{
    int i;

    // find start of trailing white space
    for(i = (int)ustrlen(line) - 1; i >= 0 && ascii_iswhite(line[i]); i--)
    {
...
/// Handles @b kReplaceMode and @b kVReplaceMode modes correctly.
/// May also be used when not in insert mode at all.
/// Will attempt not to go before "col" even when there
/// is a composing character.
...
{
    // CTRL-X after CTRL-X CTRL-V doesn't do anything,
    // so that CTRL-X CTRL-V works like CTRL-N
    if(ctrl_x_mode != CTRL_X_CMDLINE)
    {
        // if the next ^X<> won't ADD nothing,
        // then reset compl_cont_status
        ...
        edit_submode = (uchar_kt *)_(CTRL_X_MSG(ctrl_x_mode));
...
/// Check that the character "c" a valid key to go to or
/// keep us in CTRL-X mode? This depends on the current mode.
///
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    switch(ctrl_x_mode)
    {
        case CTRL_X_FILES:
            ...
            return is_file_name_char(c) && !vim_ispathsep(c);

        case CTRL_X_CMDLINE:
        case CTRL_X_OMNI:
            // Command line and Omni completion can work with just about any
            // printable character, but do stop at white space.
            return is_print_char(c) && !ascii_iswhite(c);

        case CTRL_X_WHOLE_LINE:
            // For while line completion a space can be part of the line.
            return is_print_char(c);
    }
...
/// the rest of the word to be in
int ins_compl_add_infercase(uchar_kt *str,
                            int len,
                            int icase,
                            uchar_kt *fname,
                            int dir,
                            int flags)
{
    ...
    if(p_ic && curbuf->b_p_inf && len > 0)
    {
        ...
        // Rule 2:
        // No lower case, 2nd consecutive letter converted to upper case.
        if(!has_lower)
        {
            const uchar_kt *p = compl_orig_text;
        ...
        xfree(wca);
        return ins_compl_add(IObuff, len, icase, fname,
                             NULL, false, dir, flags, false);
    }

    return ins_compl_add(str, len, icase, fname,
                         NULL, false, dir, flags, false);
}
...
/// @param[in]  len    Match length, -1 to use #ustrlen.
...
///
/// @note
/// Will free strings in case of error. cptext itself will not be freed.
///
...
static int ins_compl_add(uchar_kt *const str,
                         int len,
                         const bool icase,
                         uchar_kt *const fname,
                         uchar_kt *const *const cptext,
...
FUNC_ATTR_NONNULL_ARG(1)
{
    ...
    if(compl_first_match != NULL && !adup)
    {
        ...
        do
        {
            if(!(match->cp_flags & ORIGINAL_TEXT)
                 && ustrncmp(match->cp_str, str, len) == 0
    ...
    if(fname != NULL
       ...
       && ustrcmp(fname, compl_curr_match->cp_fname) == 0)
    {
        match->cp_fname = compl_curr_match->cp_fname;
    }
    else if(fname != NULL)
    {
        match->cp_fname = ustrdup(fname);
        flags |= FREE_FNAME;
    }
    else
    {
    ...
    {
        // if there's nothing before, it is the first match
...
/// Check that "str[len]" matches with "match->cp_str",
/// considering "match->cp_icase".
///
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    if(match->cp_icase)
    {
        return ustrnicmp(match->cp_str, str, len) == 0;
    }

    return ustrncmp(match->cp_str, str, len) == 0;
}
...
static void ins_compl_longest_match(compl_T *match)
{
    ...
    if(compl_leader == NULL)
    {
        ...
        compl_leader = ustrdup(match->cp_str);
        had_match = (curwin->w_cursor.col > compl_col);
        ins_compl_delete();
        ins_bytes(compl_leader + ins_compl_len());
        ins_redraw(FALSE);

        // When the match isn't there (to avoid matching itself)
        // remove it again after redrawing.
        if(!had_match)
        {
            ins_compl_delete();
        }
                                  ...
                                  int icase)
{
    ...
    {
        if((add_r = ins_compl_add(matches[i],
                                  -1,
                                  icase,
                                  NULL,
                                  NULL,
...
void set_completion(columnum_kt startcol, list_st *list)
{
    ...
    compl_orig_text =
        ustrndup(get_cursor_line_ptr() + compl_col, compl_length);

    if(ins_compl_add(compl_orig_text,
                     -1,
                     p_ic,
                     NULL,
                     NULL,
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    {
        if(comp == NULL
           || ((comp->cp_flags & ORIGINAL_TEXT) == 0 && ++i == 2))
        {
            break;
        }
...
void ins_compl_show_pum(void)
{
    ...
    if(compl_match_array == NULL)
    {
        ...
        if(compl_leader != NULL)
        {
            lead_len = (int)ustrlen(compl_leader);
        }
...
static void ins_compl_dictionaries(uchar_kt *dict_start,
                                   uchar_kt *pat,
                                   int flags,
                                   int thesaurus)
{
    ...
    {
        uchar_kt *pat_esc = ustrdup_escape(pat, (uchar_kt *)"\\");
        size_t len = ustrlen(pat_esc) + 10;

        ptr = xmalloc(len);
        xsnprintf((char *)ptr, len, "^\\s*\\zs\\V%s", pat_esc);
        regmatch.regprog = regexp_compile(ptr, RE_MAGIC);
        xfree(pat_esc);
        xfree(ptr);
    }
    else
    {
        regmatch.regprog = regexp_compile(pat, p_magic ? RE_MAGIC : 0);
    ...
    while(*dict != NUL && !got_int && !compl_interrupted)
    {
        ...
        else
        {
            ...
            if(!thesaurus && ustrcmp(buf, "spell") == 0)
            {
                count = -1;
            }
            else if(ustrchr(buf, '`') != NULL
        ...
        {
            // avoid warning for using "files" uninit
            ins_compl_files(count, files, thesaurus,
                            flags, &regmatch, buf, &dir);
    ...
    for(i = 0; i < count && !got_int && !compl_interrupted; i++)
    {
        ...
        while(!got_int && !compl_interrupted && !vim_fgets(buf, LSIZE, fp))
        {
            ...
            while(vim_regexec(regmatch, buf, (columnum_kt)(ptr - buf)))
            {
                ...
                // avoid expensive call to vim_regexec() when at end of line
                if(*ptr == '\n' || got_int)
                {
                    break;
                }
...
static uchar_kt *find_line_end(uchar_kt *ptr)
{
    uchar_kt *s;
    s = ptr + ustrlen(ptr);
...
static void ins_compl_free(void)
{
    ...
    do
    {
        ...
        // several entries may use the
        // same fname, free it just once.
...
static int ins_compl_bs(void)
{
    ...
    xfree(compl_leader);
    compl_leader = ustrndup(line + compl_col, (int)(p - line) - compl_col);
...
/// Check that we need to find matches again,
/// ins_compl_restart() is to be called.
...
/// Append one character to the match leader.
/// May reduce the number of matches.
static void ins_compl_addleader(int c)
{
    ...
    compl_leader = ustrndup(get_cursor_line_ptr() + compl_col,
...
static void ins_compl_set_original_text(uchar_kt *str)
{
    ...
    if(compl_first_match->cp_flags & ORIGINAL_TEXT)
    {
        ...
        compl_first_match->cp_str = ustrdup(str);
    }
}
...
static void ins_compl_addfrommatch(void)
{
    ...
    if((int)ustrlen(p) <= len)
    {
        // When still at the original match use the first
        // entry that matches the leader.
        if(compl_shown_match->cp_flags & ORIGINAL_TEXT)
        {
            p = NULL;

            for(cp = compl_shown_match->cp_next;
                cp != NULL && cp != compl_first_match;
                cp = cp->cp_next)
            {
                if(compl_leader == NULL
                   || ins_compl_equal(cp,
                                      compl_leader,
                                      (int)ustrlen(compl_leader)))
                {
                    p = cp->cp_str;
                    break;
                }
...
static bool ins_compl_prep(int c)
{
    ...
    if(ctrl_x_mode == CTRL_X_NOT_DEFINED_YET)
    {
        // We have just typed CTRL-X and aren't quite sure
        // which CTRL-X mode it will be yet. Now we decide.
        switch(c)
        {
            case Ctrl_E:
            case Ctrl_Y:
                ...
                if(!(curmod & kModFlgReplace))
                {
                    edit_submode = (uchar_kt *)_(" (insert) Scroll (^E/^Y)");
                }
                else
                {
                    edit_submode = (uchar_kt *)_(" (replace) Scroll (^E/^Y)");
                }
            ...
            case Ctrl_N:
                ...
                // doesn't change when going to ADDING mode
            ...
            default:
                ...
                // compl_cont_status = 0 (eg, as if we had just started ^X mode).
                ...
                // ^P expansions respectively. In mode 0 an extra ^X is needed
                // since ^X^P goes to ADDING mode
                if(c == Ctrl_X)
                {
    ...
    if(compl_started || ctrl_x_mode == CTRL_X_FINISHED)
    {
        ...
        if((ctrl_x_mode == 0
            ...
            && c != Ctrl_P
            && c != Ctrl_R
            && !ins_compl_pum_key(c)) || ctrl_x_mode == CTRL_X_FINISHED)
        {
            ...
            // Avoid the popup menu remains displayed
            // when leaving the command line window.
    ...
    // reset continue_* if we left expansion-mode,
    // if we stay they'll be (re)set properly in ins_complete()
    if(!vim_is_ctrl_x_key(c))
    {
        compl_cont_status = 0;
        compl_cont_mode = 0;
...
static void ins_compl_fixRedoBufForLeader(uchar_kt *ptr_arg)
{
    int len;
    uchar_kt *p;
    uchar_kt *ptr = ptr_arg;
...
/// or 'omnifunc', and get matches in "matches".
///
...
static void expand_by_function(int type, uchar_kt *base)
{
    ...
    const uchar_kt *const args[2] = { (uchar_kt *)"0", base };
    pos = curwin->w_cursor;
    curwin_save = curwin;
    curbuf_save = curbuf;
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    return ins_compl_add((uchar_kt *)word,
                         -1,
                         icase,
                         NULL,
                         (uchar_kt **)cptext,
...
static int ins_compl_get_exp(apos_st *ini)
{
    ...
    // For ^N/^P loop over all the
    // flags/windows/buffers in 'complete'
    for(;;)
    {
           ...
           && (!compl_started || found_all))
        {
            ...
            if(*e_cpt == '.' && !curbuf->b_scanned)
            {
                ...
                // Remember the first match so that the loop
                // stops when we wrap and come back there a second time.
                set_match_pos = TRUE;
            }
            else if(ustrchr((uchar_kt *)"buwU", *e_cpt) != NULL
                     && (ins_buf = ins_compl_next_buf(ins_buf, *e_cpt)) != curbuf)
            {
                ...
                xsnprintf((char *)IObuff,
                          ...
                          ? ins_buf->b_fname : ins_buf->b_sfname);
            ...
            else
            {
                ...
                {
                    type = CTRL_X_TAGS;
                    xsnprintf((char *)IObuff, IOSIZE, _("Scanning tags."));
                    (void)msg_trunc_attr(IObuff, TRUE, hl_attr(HLF_R));
                }
                else
                {
        ...
        switch(type)
        {
            ...
            case CTRL_X_PATH_PATTERNS:
            case CTRL_X_PATH_DEFINES:
                find_pattern_in_path(compl_pattern,
                                     compl_direction,
                                     ustrlen(compl_pattern),
                                     FALSE,
                                     FALSE,
                                     ((type == CTRL_X_PATH_DEFINES
            ...
            case CTRL_X_TAGS:
                ...
                // Find up to TAG_MANY matches.
                // Avoids that an enormous number
                // of matches is found when compl_pattern is empty
                if(find_tags(compl_pattern,
                             &num_matches,
                             &matches,
                             TAG_REGEXP | TAG_NAMES | TAG_NOIC
                             | TAG_INS_COMP | (l_ctrl_x_mode ? TAG_VERBOSE : 0),
                             TAG_MANY,
                             curbuf->b_ffname) == OK && num_matches > 0)
                {
            ...
            case CTRL_X_CMDLINE:
                if(expand_cmdline(&compl_xp,
                                  compl_pattern,
                                  (int)ustrlen(compl_pattern),
                                  &num_matches,
                                  &matches) == EXPAND_OK)
                {
            ...
            default: // normal ^P/^N and ^X^L
                ...
                // Buffers other than curbuf are scanned from the beginning or
                // the end but never from the middle, thus setting nowrapscan
                // in this buffers is a good idea, on the other hand, we always
                // set wrapscan for curbuf to avoid missing matches
                save_p_ws = p_ws;
                ...
                for(;;)
                {
                    ...
                    // CTRL_X_MODE_LINE_OR_EVAL(l_ctrl_x_mode) || word-wise
                    // search that has added a word that was at the beginning
                    // of the line.
                    if(CTRL_X_MODE_LINE_OR_EVAL(l_ctrl_x_mode)
                       || (compl_cont_status & CONT_SOL))
                    {
                    ...
                    else
                    {
                           ...
                           && len == compl_length)
                        {
                            if(pos->lnum < ins_buf->b_ml.ml_line_count)
                            {
                                ...
                                // IOSIZE is always greater than compl_length,
                                // so the next ustrncpy always works
                                ustrncpy(IObuff, ptr, len);
                                ptr = ml_get_buf(ins_buf, pos->lnum + 1, false);
                                ...
                                if(tmp_ptr > ptr)
                                {
                                    ...
                                    ustrncpy(IObuff + len, ptr, tmp_ptr - ptr);
                                    len += (int)(tmp_ptr - ptr);
                                    flags |= CONT_S_IPOS;
                                }
                    ...
                    if(ins_compl_add_infercase(ptr,
                                               len,
                                               p_ic,
                                               ins_buf == curbuf
                                                          ? NULL
        ...
        // check if compl_curr_match has changed,
        // (e.g. other type of expansion added something)
...
static void ins_compl_delete(void)
{
    ...
    // TODO(vim): is this sufficient for redrawing?
    // Redrawing everything causes flicker, thus we can't do that.
    changed_cline_bef_curs();
...
/// Return the total number of matches, or -1 if still unknown
///
...
///
/// @param allow_get_expansion
/// @param count
/// Repeat completion this many times; should be at least 1
///
/// @param insert_match
                          ...
                          int in_compl_func)
{
       ...
       && (compl_shown_match->cp_flags & ORIGINAL_TEXT) == 0)
    {
        ...
        while(!ins_compl_equal(compl_shown_match,
                               compl_leader, (int)ustrlen(compl_leader))
        ...
        // If we didn't find it searching forward, and
        // compl_shows_dir is backward, find the last match.
    ...
    // Repeat this for when <PageUp> or <PageDown> is typed.
    //  But don't wrap around.
    while(--todo >= 0)
    {
        ...
        else
        {
            ...
            while(compl_pending != 0
                  && compl_direction == compl_shows_dir && advance)
            {
    ...
    if(!allow_get_expansion)
    {
        ...
        // Delete old text to be replaced, since we're
        // still searching and don't want to match ourselves!
        ins_compl_delete();
    }

    // Enter will select a match when the match
    // wasn't inserted and the popup menu is visible.
    if(compl_no_insert && !started)
    {
    ...
    {
        ustrcpy(IObuff, "match in file ");
        i = (ustr_scrsize(compl_shown_match->cp_fname) + 16) - sc_col;
        ...
        ustrcat(IObuff, compl_shown_match->cp_fname + i);
        msg(IObuff);
...
void ins_compl_check_keys(int frequency, int in_compl_func)
{
    ...
    // Don't check when reading keys from a script.
    // That would break the test scripts
    if(using_script())
    {
        return;
    }
    ...
    // Check for a typed key.
    // Do use mappings, otherwise vim_is_ctrl_x_key()
    ...
    if(c != NUL)
    {
        ...
        {
            // Need to get the character to have KeyTyped set.
            // We'll put it back with vungetc() below. But skip K_IGNORE.
            c = safe_vgetc();

            if(c != K_IGNORE)
            {
                // Don't interrupt completion when the character
                // wasn't typed, e.g., when doing @q to replay keys.
...
/// Check that "c" is a valid completion
/// key only while the popup menu is shown
///
/// @param  c  character to check
...
static int ins_complete(int c, bool enable_pum)
{
    ...
    if(!compl_started)
    {
        ...
        // the line has been inserted, we'll look for that
...
// IObuff is used to add a "word from the next
...
#define MIN_SPACE 75
        ...
        // Work out completion pattern and original text
        if(ctrl_x_mode == 0 || (ctrl_x_mode & CTRL_X_WANT_IDENT))
        {
               ...
               || ctrl_x_mode == CTRL_X_PATH_DEFINES)
            {
                if(!(compl_cont_status & CONT_ADDING))
                {
                    while(--startcol >= 0 && is_id_char(line[startcol]))
                    { /*empty body */ }
                ...
                {
                    compl_pattern =
                        ustr_foldcase(line + compl_col, compl_length, NULL, 0);
                }
                else
                {
                    compl_pattern = ustrndup(line + compl_col, compl_length);
                }
            }
            else if(compl_cont_status & CONT_ADDING)
            {
                uchar_kt *prefix = (uchar_kt *)"\\<";

                // we need up to 2 extra chars for the prefix
                compl_pattern =
                    xmalloc(quote_meta(NULL, line + compl_col, compl_length) + 2);

                if(!is_kwc_ptr(line + compl_col)
                   || (compl_col > 0 &&
                       (is_kwc_ptr(mb_prevptr(line, line + compl_col)))))
                {
                    prefix = (uchar_kt *)"";
                }

                ustrcpy((char *)compl_pattern, prefix);

                (void)quote_meta(compl_pattern + ustrlen(prefix),
                                 line + compl_col, compl_length);
            }
            else if(--startcol < 0
                    || !is_kwc_ptr(mb_prevptr(line, line + startcol + 1)))
            {
                // Match any word of at least two chars
                compl_pattern = ustrdup((uchar_kt *)"\\<\\k\\k");
                compl_col += curs_col;
                compl_length = 0;
            }
            else
            {
                ...
                if(compl_length == 1)
                {
                    // Only match word with at least two chars
                    // there's no need to call quote_meta,
                    // xmalloc(7) is enough
                    compl_pattern = xmalloc(7);
                    ustrcpy((char *)compl_pattern, "\\<");

                    (void)quote_meta(compl_pattern + 2, line + compl_col, 1);
                    ustrcat((char *)compl_pattern, "\\k");
                }
                else
                {
                    compl_pattern =
                        xmalloc(quote_meta(NULL, line + compl_col, compl_length)
                                + 2);

                    ustrcpy((char *)compl_pattern, "\\<");

                    (void)quote_meta(compl_pattern + 2,
                                     line + compl_col, compl_length);
                }
            }
        }
        ...
        else if(ctrl_x_mode == CTRL_X_FILES)
        {
            ...
            compl_pattern =
                addstar(line + compl_col, compl_length, EXPAND_FILES);
        }
        else if(ctrl_x_mode == CTRL_X_CMDLINE)
        {
            compl_pattern = ustrndup(line, curs_col);

            set_cmd_context(&compl_xp,
                            compl_pattern,
                            (int)ustrlen(compl_pattern),
                            curs_col,
                            false);
            ...
            {
                // No completion possible, use an empty pattern
                // to get a "pattern not found" message.
                compl_col = curs_col;
            }
            else
            {
        ...
        else if(ctrl_x_mode == CTRL_X_FUNCTION || ctrl_x_mode == CTRL_X_OMNI)
        {
            ...
            const uchar_kt *const args[2] = { (uchar_kt *)"1", NULL };
            pos = curwin->w_cursor;
            curwin_save = curwin;
            curbuf_save = curbuf;
            ...
            // Reset extended parameters of completion,
            // when start new completion.
            compl_opt_refresh_always = FALSE;
        ...
        if(compl_cont_status & CONT_ADDING)
        {
            edit_submode_pre = (uchar_kt *)_(" Adding");

            if(CTRL_X_MODE_LINE_OR_EVAL(ctrl_x_mode))
            {
                // Insert a new line, keep indentation but ignore 'comments'
                uchar_kt *old = curbuf->b_p_com;
                curbuf->b_p_com = (uchar_kt *)"";
                compl_startpos.lnum = curwin->w_cursor.lnum;
                compl_startpos.col = compl_col;
                ins_eol('\r');
                curbuf->b_p_com = old;
        ...
        {
            edit_submode = (uchar_kt *)_(ctrl_x_msgs[CTRL_X_LOCAL_MSG]);
        }
        else
        {
            edit_submode = (uchar_kt *)_(CTRL_X_MSG(ctrl_x_mode));
        ...
        // we need to fix the redo buffer.
        ins_compl_fixRedoBufForLeader(NULL);

        // Always add completion for the original text.
        xfree(compl_orig_text);
        compl_orig_text = ustrndup(line + compl_col, compl_length);
        ...
        // be called before line = ml_get(), or when this address is
        // no longer needed.
        edit_submode_extra = (uchar_kt *)_("-- Searching...");
    ...
    // Eat the ESC that vgetc() returns after a
    // CTRL-C to avoid leaving Insert mode.
    ...
    if(compl_first_match == compl_first_match->cp_next)
    {
        ...
        // (such as M in M'exico) if not tried already.
        if(compl_length > 1
    ...
    if(edit_submode_extra == NULL)
    {
        ...
        {
            edit_submode_extra = (uchar_kt *)_("Back at original");
            edit_submode_highl = HLF_W;
        }
        else if(compl_cont_status & CONT_S_IPOS)
        {
            edit_submode_extra = (uchar_kt *)_("Word from other line");
            edit_submode_highl = HLF_COUNT;
        }
        else if(compl_curr_match->cp_next == compl_curr_match->cp_prev)
        {
            edit_submode_extra = (uchar_kt *)_("The only match");
            edit_submode_highl = HLF_COUNT;
        }
        else
        {
            ...
            if(compl_curr_match->cp_number == -1)
            {
                ...
                else // BACKWARD
                {
                    ...
                    if(match != NULL)
                    {
                        // go down and assign all numbers
                        // which are not assigned yet
            ...
            if(compl_curr_match->cp_number != -1)
            {
                ...
                static uchar_kt match_ref[81];

                if(compl_matches > 0)
                {
                    xsnprintf((char *)match_ref,
                              sizeof(match_ref),
                              _("match %d of %d"),
...
static unsigned quote_meta(uchar_kt *dest, uchar_kt *src, int len)
{
...
///
/// @note
/// passes the flags value straight through to internal_format() which,
...
void insertchar(int c,  int flags, int second_indent)
{
                           ...
                           || Insstart_blank_vcol <= (columnum_kt)textwidth))))))
    {
           ...
           && (force_format || virtcol > (columnum_kt)textwidth))
        {
            ...
            // It may be required to save for undo again,
            // e.g. when setline() was called.
            ins_need_undo = TRUE;
        }
    ...
    if(did_ai && c == end_comment_pending)
    {
        ...
        if(i > 0 && ustrchr(p, COM_MIDDLE) != NULL) // Just checking
        {
            // Skip middle-comment string, find end of middle flags
            while(*p && p[-1] != ':')
            {
                ++p;
            }
                            ...
                            int c)
{
    ...
    if(!curbuf->b_p_ai && !(curmod & kModFlgVReplace))
    {
        cc = gchar_cursor();
    ...
    while(!got_int)
    {
        ...
        {
            leader_len =
                get_leader_len(get_cursor_line_ptr(), NULL, FALSE, TRUE);
        }
        else
        {
            leader_len = 0;
        ...
        // stack functions. 'kVReplaceMode' does not use this, and backspaces
        ...
        if(curmod & kModFlgVReplace)
        {
            // In 'kVReplaceMode' mode, we will backspace over the text to be
            // wrapped, so save a copy now to put on the next line.
            saved_text = ustrdup(get_cursor_pos_ptr());
            curwin->w_cursor.col = orig_col;
        ...
        if(curmod & kModFlgVReplace)
        {
            // In 'kVReplaceMode' mode we have backspaced over the text to be
...
void auto_format(int trailblank, int prev_line)
{
    ...
    wasatend = (pos.col == (columnum_kt)ustrlen(old));
    ...
    // With the 'c' flag in 'formatoptions'
    // and 't' missing: only format comments.
    ...
    if(!wasatend && has_format_option(FO_WHITE_PAR))
    {
        ...
        len = (columnum_kt)ustrlen(new);

        if(curwin->w_cursor.col == len)
        {
            pnew = ustrndup(new, len + 2);
            pnew[len] = ' ';
            pnew[len + 1] = NUL;
...
/// When an extra space was added to continue a paragraph
/// for auto-formatting, delete it now. The space must be
/// under the cursor, just after the insert position.
///
...
static void check_auto_format(int end_insert)
{
    ...
    if(did_add_space)
    {
        ...
        else
        {
            ...
            if(c != NUL)
            {
                // The space is no longer at the
                // end of the line, delete it.
                del_char(FALSE);
                did_add_space = FALSE;
            }
        }
...
int comp_textwidth(int ff)
{
    ...
    {
        // The width is the window width minus 'wrapmargin'
        // minus all the things that add to the margin.
...
static void redo_literal(int c)
{
    ...
    // Only digits need special treatment.
    // Translate them into a string of three digits.
    if(ascii_isdigit(c))
    {
        xsnprintf(buf, sizeof(buf), "%03d", c);
...
/// Called when starting CTRL_X_SPELL mode:
...
///
/// @param end_insert_pos  called by ins_esc()
/// @param esc
...
static void stop_insert(apos_st *end_insert_pos, int esc, int nomove)
{
    ...
    if(did_restart_edit == 0
       || (ptr != NULL && (int)ustrlen(ptr) > new_insert_skip))
    {
        xfree(last_insert);
        last_insert = ptr;
    ...
    if(!arrow_used && end_insert_pos != NULL)
    {
        ...
        if(!nomove
           && did_ai
           && (esc || (ustrchr(p_cpo, CPO_INDENT) == NULL
           ...
           && end_insert_pos->lnum <= curbuf->b_ml.ml_line_count)
        {
            ...
            // <C-S-Right> may have started Visual mode,
            // adjust the position for deleted characters.
            if(VIsual_active && VIsual.lnum == curwin->w_cursor.lnum)
            {
                int len = (int)ustrlen(get_cursor_line_ptr());
...
uchar_kt *add_char2buf(int c, uchar_kt *s)
{
    int i;
    int len;
    uchar_kt temp[MB_MAXBYTES + 1];
    ...
    for(i = 0; i < len; ++i)
    {
        ...
        // Need to escape K_SPECIAL and CSI
        // like in the typeahead buffer.
        if(c == K_SPECIAL)
        {
            *s++ = K_SPECIAL;
            *s++ = KS_SPECIAL;
...
/// - if flags & BL_SOL, move to first non-white.
///   if startofline is set, otherwise keep "curswant" column
    ...
    else
    {
        ...
        if(flags & (BL_WHITE | BL_SOL))
        {
            ...
            for(ptr = get_cursor_line_ptr();
                ascii_iswhite(*ptr) && !((flags & BL_FIX) && ptr[1] == NUL);
                ++ptr)
            {
                ++curwin->w_cursor.col;
            }
...
int oneright(void)
{
    ...
    if(virtual_active())
    {
        ...
        coladvance(getviscol()
                   + ((*ptr != TAB && is_print_char((*mb_ptr2char)(ptr)))
                      ? ptr2cells(ptr) : 1));
...
int stuff_inserted(int c, long count, int no_esc)
{
    ...
    if((esc_ptr = (uchar_kt *)ustrrchr(ptr, ESC)) != NULL)
    {
    ...
    // starts with ^D.
    last_ptr = (esc_ptr ? esc_ptr : ptr + ustrlen(ptr)) - 1;
...
uchar_kt *get_last_insert_save(void)
{
    int len;
    uchar_kt *s;
    ...
    s = ustrdup(last_insert + last_insert_skip);
    len = (int)ustrlen(s);
...
static uchar_kt *replace_stack = NULL;
...
void replace_push(int c)
{
    ...
    uchar_kt *p = replace_stack + replace_stack_nr - replace_offset;
...
int replace_push_mb(uchar_kt *p)
{
    int j;
    int l = (*mb_ptr2len)(p);
...
{
    return (replace_stack_nr == 0)
            ? -1 : (int)replace_stack[--replace_stack_nr];
}

/// Join the top two items on the replace stack.
/// This removes to "off"'th NUL encountered.
///
...
/// before the cursor. Can only be used in @b kReplaceMode or @b kVReplaceMode mode.
static void replace_pop_ins(void)
{
    int cc;
    int oldState = curmod;
    curmod = kNormalMode; // don't want 'kReplaceMode' here
...
static void replace_do_bs(int limit_col)
{
    ...
    if(cc > 0)
    {
        ...
        {
            // Get the number of screen cells used by
            // the character we are going to delete.
        ...
        if(l_State & kModFlgVReplace)
        {
            ...
            ins_len = (int)ustrlen(p) - orig_len;
            vcol = start_vcol;
            ...
            // Delete spaces that were inserted after
            // the cursor to keep the text aligned.
...
/// surrounding lines. Fixing the cursor position seems really easy.
/// I'm very confused what all the part that handles Control-T is doing that
/// I'm not. "get_the_indent" should be get_c_indent,
/// get_expr_indent or get_lisp_indent.
...
bool in_cinkeys(int keytyped, int when, bool line_is_empty)
{
    ...
    while(*look)
    {
        ...
        else if(*look == 'O')
        {
            ...
            look++;
            // 'e' means to check for "else" at start of line
            // and just before the cursor.
        }
        else if(*look == 'e')
        {
            if(try_match && keytyped == 'e' && curwin->w_cursor.col >= 4)
            {
                ...
                if(skipwhite(p) == p + curwin->w_cursor.col - 4
                   && ustrncmp(p + curwin->w_cursor.col - 4, "else", 4) == 0)
                {
                    return true;
                }
            }
        ...
        else if(*look == '<')
        {
            if(try_match)
            {
                ...
                if(ustrchr((uchar_kt *)"<>!*oOe0:", look[1]) != NULL
                   && keytyped == look[1])
                {
                    return true;
                }
        ...
        else if(*look == '=' && look[1] != ',' && look[1] != NUL)
        {
               ...
               && curwin->w_cursor.col >= (columnum_kt)(p - look))
            {
                ...
                if(keytyped == KEY_COMPLETE)
                {
                    ...
                    if(s + (p - look) <= line + curwin->w_cursor.col
                       && (icase
                           ...
                           : ustrncmp(s, look, p - look)) == 0)
                    {
                        match = TRUE;
                    }
                }
                else // TODO: multi-byte
                {
                    if(keytyped == (int)p[-1]
                       || (icase
                           && keytyped < 256
                           && TOLOWER_LOC(keytyped) == TOLOWER_LOC((int)p[-1])))
                    {
                        ...
                        if((curwin->w_cursor.col == (columnum_kt)(p - look)
                            || !is_kwc(line[-(p - look) - 1]))
                           && (icase
                               ? mb_strnicmp(line - (p - look),
                                             look, (size_t)(p - look))
                               : ustrncmp(line - (p - look),
                                         look, p - look)) == 0)
                        {
                            match = TRUE;
                        }
                ...
                {
                    // "0=word": Check if there are only
...
int hkmap(int c)
{
    ...
    if(p_hkmapp)
    {
        ...
        static uchar_kt map[26] =
        {
            (uchar_kt)hALEF, /*a*/ (uchar_kt)BET,   /*b*/ (uchar_kt)hKAF,     /*c*/
            (uchar_kt)DALET, /*d*/ (uchar_kt)-1,    /*e*/ (uchar_kt)PEIsofit, /*f*/
            (uchar_kt)GIMEL, /*g*/ (uchar_kt)HEI,   /*h*/ (uchar_kt)IUD,      /*i*/
            (uchar_kt)HET,   /*j*/ (uchar_kt)KOF,   /*k*/ (uchar_kt)LAMED,    /*l*/
            (uchar_kt)MEM,   /*m*/ (uchar_kt)NUN,   /*n*/ (uchar_kt)SAMEH,    /*o*/
            (uchar_kt)PEI,   /*p*/ (uchar_kt)-1,    /*q*/ (uchar_kt)RESH,     /*r*/
            (uchar_kt)ZAIN,  /*s*/ (uchar_kt)TAV,   /*t*/ (uchar_kt)TET,      /*u*/
            (uchar_kt)VAV,   /*v*/ (uchar_kt)hSHIN, /*w*/ (uchar_kt)-1,       /*x*/
            (uchar_kt)AIN,   /*y*/ (uchar_kt)ZADI,  /*z*/
        };
        ...
        {
            // NOTE: islower() does not do the right thing for us on Linux
            // so we do this the same was as 5.7 and previous, so it works
            // correctly on all systems. Specifically, the e.g. Delete and
            // Arrow keys are munged and won't work if e.g. searching for
            // Hebrew text.
...
static void ins_reg(void)
{
    ...
    // Don't call u_sync() while typing the expression
    // or giving an error message for it. Only call it explicitly.
    ++no_u_sync;
...
FUNC_ATTR_NONNULL_ARG(1)
{
    ...
    if(!arrow_used)
    {
        ...
        // Repeating insert may take a long time.
        ...
        if(--*count > 0)
        {
            ...
            if(ustrchr(p_cpo, CPO_REPLCNT) != NULL)
            {
                curmod &= ~kModFlgReplace;
            }
...
FUNC_ATTR_NONNULL_ARG(3)
{
    ...
    if(curwin->w_cursor.col == 0)
    {
        ...
        else
        {
            ...
            // In 'kReplaceMode' mode we have to put back the text that was replaced
            ...
            do
            {
                ...
                if(mode == BACKSPACE_WORD && !ascii_isspace(cc))
                {
                    ...
                    temp = is_kwc(cc);
                        ...
                        && ((ascii_isspace(cc) || is_kwc(cc) != temp)
    ...
    if(ustrchr(p_cpo, CPO_BACKSPACE) != NULL && dollar_vcol == -1)
    {
        dollar_vcol = curwin->w_virtcol;
    }
...
static void ins_left(bool end_change)
{
    ...
    else if(ustrchr(p_ww, '[') != NULL && curwin->w_cursor.lnum > 1)
    {
...
static void ins_right(bool end_change)
{
    ...
    else if(ustrchr(p_ww, ']') != NULL
            && curwin->w_cursor.lnum < curbuf->b_ml.ml_line_count)
    {
        // if 'whichwrap' set for cursor in insert mode,
        // may move the cursor to the next line
        start_arrow(&curwin->w_cursor);
        curwin->w_set_curswant = TRUE;
        ++curwin->w_cursor.lnum;
        curwin->w_cursor.col = 0;
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    // chars. For 'kVReplaceMode' mode, we use ins_char() for all characters.
    ins_char(' ');
    ...
    if(!curbuf->b_p_et && (get_sts_value() || (p_sta && ind)))
    {
        ...
        if(curmod & kModFlgVReplace)
        {
            pos = curwin->w_cursor;
            cursor = &pos;
            saved_line = ustrdup(get_cursor_line_ptr());
        ...
        if(ustrchr(p_cpo, CPO_LISTWM) == NULL)
        {
            curwin->w_o_curbuf.wo_list = FALSE;
        }
        ...
        // Beware of 'breakindent', 'showbreak' and
        // 'linebreak' adding extra virtual columns.
        while(ascii_iswhite(*ptr))
        {
            i = lbr_chartabsize(NULL, (uchar_kt *)"\t", vcol);
        ...
        if(change_col >= 0)
        {
            ...
            // In 'kVReplaceMode' mode, we haven't changed anything yet.
...
static bool ins_eol(int c)
{
    ...
    if((curmod & kModFlgReplace) && !(curmod & kModFlgVReplace))
    {
        replace_push(NUL);
    }
    ...
    // Put cursor on NUL if on the last char
    // and coladd is 1 (happens after CTRL-O).
    ...
    if(revins_on)
    {
        curwin->w_cursor.col += (columnum_kt)ustrlen(get_cursor_pos_ptr());
    }
    ...
    // When inserting a line the cursor
    // line  must never be in a closed fold.
    foldOpenCursor();
...
/// or NUL when nothing remaining to be done.
...
static uchar_kt *do_insert_char_pre(int c)
{
    ...
    if(apply_autocmds(EVENT_INSERTCHARPRE, NULL, NULL, FALSE, curbuf))
    {
        ...
        if(ustrcmp(buf, get_vim_var_str(VV_CHAR)) != 0)
        {
            res = ustrdup(get_vim_var_str(VV_CHAR));
        }
    }
...
static void show_pum(int prev_w_wrow, int prev_w_leftcol)
{
    ...
    // If the cursor moved or the display
    // scrolled we need to remove the pum first.
    setcursor();
