...
FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    if(ret.has_type_key)
    {
        ...
        else if(ret.type == kObjectTypeArray)
        {
            ...
            // should be ignored.
            if(ret.maxidx != 0 && ret.maxidx != (tsize
                                                 - ret.has_type_key
                                                 - other_keys_num
                                                 - has_val_key
                                                 - ret.string_keys_num))
        ...
        if(cur.container)
        {
            if(cur.obj->type == kObjectTypeDictionary)
            {
                ...
                if(next_key_found)
                {
                    ...
                    cur.obj->data.dictionary.items[idx].key = (String) { .data = xmemdupz(s, len),
                                                                         .size = len, };
                    kv_push(stack, cur);
        ...
        switch(lua_type(lstate, -1))
        {
            ...
            case LUA_TTABLE:
            {
                ...
                switch(table_props.type)
                {
                    case kObjectTypeArray:
                    {
                        *cur.obj = ARRAY_OBJ(((Array) { .items = NULL,
                                                        .size = 0,
                                                        .capacity = 0, }));
                    ...
                    case kObjectTypeDictionary:
                    {
                        *cur.obj = DICTIONARY_OBJ(((Dictionary) { .items = NULL,
                                                                  .size = 0,
                                                                  .capacity = 0, }));
