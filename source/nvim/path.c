...
///
/// @param s1
/// First file name. Environment variables in this name will be expanded.
///
/// @param s2
/// Second file name.
///
/// @param checkname
/// When both files don't exist, only compare their names.
///
/// @return
/// Enum of type FileComparison. @see FileComparison.
FileComparison path_full_compare(uchar_kt *s1, uchar_kt *s2, int checkname)
{
    assert(s1 && s2);

    uchar_kt exp1[MAXPATHL];
...
FUNC_ATTR_NONNULL_RET
{
    ...
    const uchar_kt *tail = get_past_head(fname);
    const uchar_kt *p = tail;
    ...
    return (uchar_kt *)tail;
}

/// Get pointer to tail of @b fname, including path separators.
///
/// Takes care of "c:/" and "//".
/// That means <b>path_tail_with_sep("dir///file.txt")</b> will
/// return a pointer to <b>"///file.txt"</b>.
///
...
/// - Never NULL.
uchar_kt *path_tail_with_sep(uchar_kt *fname)
{
    assert(fname != NULL);

    // Don't remove the '/' from "c:/file".
    uchar_kt *past_head = get_past_head(fname);
    uchar_kt *tail = path_tail(fname);
...
///
/// @param[in]
/// invocation A program invocation in the form: "path/to/exe [args]".
///
/// @param[out]
/// len Stores the length of the executable name.
///
/// @post
/// if @b len is not null, stores the length of the executable name.
///
/// @return
...
FUNC_ATTR_NONNULL_ARG(1)
{
    ...
    while(*p != NUL && *p != ' ')
    {
        ...
        if(was_sep)
        {
            // Now tail points one past the separator.
            tail = p;
        }
    }
...
///
/// @return
/// Pointer to first found path separator + 1.
...
uchar_kt *get_past_head(const uchar_kt *path)
{
    const uchar_kt *retval = path;
...
/// Like vim_ispathsep(), but exclude the colon for MS-Windows.
...
/// return TRUE if @b c is a path list separator.
...
/// Shorten the path of a file from
/// @b ~/foo/../.bar/fname to @b ~/f/../.b/fname
/// It's done in-place.
uchar_kt *shorten_dir(uchar_kt *str)
{
    uchar_kt *tail = path_tail(str);
    uchar_kt *d = str;
    bool skip = false;

    for(uchar_kt *s = str;; ++s)
    {
        // copy the whole tail
        if(s >= tail)
        {
...
/// Return TRUE if the directory of @b fname exists, FALSE otherwise.
/// Also returns TRUE if there is no directory name.
/// @b fname must be writable!.
bool dir_of_file_exists(uchar_kt *fname)
{
    uchar_kt *p = path_tail_with_sep(fname);
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    if(p_fic)
    {
        return mb_strnicmp((const uchar_kt *)fname1,
                           (const uchar_kt *)fname2, len);
    }
...
///
/// @param[in]  fname1
/// First fname to append to.
///
/// @param[in]  len1
/// Length of fname1.
///
/// @param[in]  fname2
/// Secord part of the file name.
///
/// @param[in]  len2
/// Length of fname2.
///
/// @param[in]  sep
/// If true and fname1 does not end with a path separator,
/// add a path separator before fname2.
///
/// @return fname1
static inline char *do_concat_fnames(char *fname1,
                                     const size_t len1,
                                     const char *fname2,
                                     const size_t len2,
                                     const bool sep)
...
///
/// @param fname1
/// is the first part of the path or filename
///
/// @param fname2
/// is the second half of the path or filename
///
/// @param sep
/// is a flag to indicate a path separator should be added if necessary
///
/// @return
...
///
/// Like concat_fnames(), but in place of allocating new memory
/// it reallocates @b fname1. For this reason @b fname1 must be
/// allocated with xmalloc, and can no longer be used after running
...
FUNC_ATTR_NONNULL_RET
{
    ...
    return do_concat_fnames(xrealloc(fname1, len1 + len2 + 3),
                            len1, fname2, len2, sep);
}

/// Adds a path separator to a filename, unless it already ends in one.
///
/// @return
/// true if the path separator was added or already existed.
...
///
/// @param fname
/// is the filename to save
///
/// @param force
/// is a flag to expand @b fname even if it looks absolute
///
/// @return
...
uchar_kt *save_absolute_path(const uchar_kt *name)
FUNC_ATTR_MALLOC
FUNC_ATTR_NONNULL_ALL
{
    if(!path_is_absolute_path(name))
    {
        return (uchar_kt *)FullName_save((char *)name, true);
    }

    return ustrdup((uchar_kt *) name);
}

/// Checks if a path has a wildcard character including '~',
/// unless at the end.
///
/// @param p
/// The path to expand.
...
bool path_has_wildcard(const uchar_kt *p)
FUNC_ATTR_NONNULL_ALL
{
    for(; *p; mb_ptr_adv(p))
    {
        ...
        if(ustrchr((uchar_kt *)wildcards, *p) != NULL
           || (p[0] == '~' && p[1] != NUL))
        {
            return true;
        }
...
/// @returns Unix: True if it contains one of *?[{.
/// @returns Windows: True if it contains one of *?[.
bool path_has_exp_wildcard(const uchar_kt *p)
FUNC_ATTR_NONNULL_ALL
{
...
/// @remark "**" in @b path requests recursive expansion.
///
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    *e = NUL;
    ...
    // Now we have one wildcard component between "s" and "e". Remove
    // backslashes between "wildoff" and the start of the wildcard component.
    ...
    uchar_kt *pat = file_pat_to_reg_pat(s, e, NULL, false);
    ...
    regmatch.regprog = regexp_compile(pat, RE_MAGIC);
    ...
    // If "**" is by itself, this is the first time we encounter it
    // and more is following then find matches without any directory.
    if(!didstar
       && stardepth < 100
       && starstar
       && e - s == 2
    ...
    *s = NUL;
    ...
    if(os_file_is_readable(dirpath) && os_scandir(&dir, dirpath))
    {
        ...
        while((name = (uchar_kt *)scandir_next_with_dots(&dir))
              && name != NULL)
        {
            if((name[0] != '.'
                || starts_with_dot
                       ...
                       && fnamencmp(path + (s - buf), name, e - s) == 0)))
            {
                ...
                {
                    // For "**" in the pattern first go
                    // deeper in the tree to find matches.
                    ustrcpy(buf + len, "/**");
                    ustrcpy(buf + len + 3, path_end);
                    ++stardepth;
                    (void)do_path_expand(gap, buf, len + 1, flags, true);
                    --stardepth;
                }

                ustrcpy(buf + len, path_end);
    ...
    if(matches > 0)
    {
        qsort(((uchar_kt **)gap->ga_data) + start_len, matches,
              sizeof(uchar_kt *), pstrcmp);
    }

    return matches;
}

/// Moves @b *psep back to the previous path separator in @b path.
/// Returns FAIL is @b *psep ends up at the beginning of @b path.
static int find_previous_pathsep(uchar_kt *path, uchar_kt **psep)
{
...
static bool is_unique(uchar_kt *maybe_unique, garray_st *gap, int i)
{
    ...
    for(int j = 0; j < gap->ga_len; j++)
    {
        ...
        size_t candidate_len = ustrlen(maybe_unique);
        size_t other_path_len = ustrlen(other_paths[j]);
        ...
        uchar_kt *rival = other_paths[j] + other_path_len - candidate_len;
...
static void expand_path_option(uchar_kt *curdir, garray_st *gap)
{
    ...
    while(*path_option != NUL)
    {
        ...
        if(buf[0] == '.' && (buf[1] == NUL || vim_ispathsep(buf[1])))
        {
            ...
            uchar_kt *p = path_tail(curbuf->b_ffname);
            size_t len = (size_t)(p - curbuf->b_ffname);

            if(len + ustrlen(buf) >= MAXPATHL)
            {
                continue;
            }
        ...
        {
            ustrcpy(buf, curdir);  // relative to current directory
        }
        ...
        else if(!path_is_absolute_path(buf))
        {
            ...
            size_t len = ustrlen(curdir);

            if(len + ustrlen(buf) + 3 > MAXPATHL)
            {
                continue;
            }

            xstrmove(buf + len + 1, buf);
            ustrcpy(buf, curdir);
            buf[len] = (uchar_kt)OS_PATH_SEP_CHAR;
            simplify_filename(buf);
        }

        GA_APPEND(uchar_kt *, gap, ustrdup(buf));
    }
...
static void uniquefy_paths(garray_st *gap, uchar_kt *pattern)
{
    ...
    ga_init(&path_ga, (int)sizeof(uchar_kt *), 1);
    // We need to prepend a '*' at the beginning of file_pattern so that the
    // regex matches anywhere in the path. FIXME: is this valid for all
    // possible patterns?
    size_t len = ustrlen(pattern);
    uchar_kt *file_pattern = xmalloc(len + 2);
    file_pattern[0] = '*';
    file_pattern[1] = NUL;
    ustrcat(file_pattern, pattern);
    uchar_kt *pat = file_pat_to_reg_pat(file_pattern, NULL, NULL, true);
    xfree(file_pattern);
    ...
    uchar_kt *curdir = xmalloc(MAXPATHL);
    os_dirname(curdir, MAXPATHL);
    expand_path_option(curdir, &path_ga);
    in_curdir = xcalloc((size_t)gap->ga_len, sizeof(uchar_kt *));

    for(int i = 0; i < gap->ga_len && !got_int; i++)
    {
        uchar_kt *path = fnames[i];
        int is_in_curdir;
        uchar_kt *dir_end = (uchar_kt *)gettail_dir((const char *)path);
        uchar_kt      *pathsep_p;
        uchar_kt      *path_cutoff;
        len = ustrlen(path);
        ...
        {
            sort_again = true;
            memmove(path, path_cutoff, ustrlen(path_cutoff) + 1);
        }
        else
        {
            ...
            while(find_previous_pathsep(path, &pathsep_p))
            {
                ...
                {
                    sort_again = true;
                    memmove(path, pathsep_p + 1, ustrlen(pathsep_p));
                    break;
                }
            }
        }
        ...
        if(path_is_absolute_path(path))
        {
              ...
              )
            {
                ...
                xstrmove(path + ustrlen(path), short_name);
            }
        }
    ...
    for(int i = 0; i < gap->ga_len && !got_int; i++)
    {
        ...
        {
            ustrcpy(fnames[i], short_name);
            continue;
        }

        rel_path = xmalloc(ustrlen(short_name) + ustrlen(OS_PATH_SEP_STR) + 2);
        ustrcpy(rel_path, ".");
        add_pathsep((char *)rel_path);
        ustrcat(rel_path, short_name);
        xfree(fnames[i]);
        fnames[i] = rel_path;
        sort_again = true;
        os_breakcheck();
 ...
 */
static bool has_env_var(uchar_kt *p)
{
    for(; *p; mb_ptr_adv(p))
    {
        if(*p == '\\' && p[1] != NUL)
 ...
 * Allowing for escaping.
 */
static bool has_special_wildchar(uchar_kt *p)
{
    for(; *p; mb_ptr_adv(p))
    {
        if(*p == '\\' && p[1] != NUL)
        ...
        else if(ustrchr((uchar_kt *)SPECIAL_WILDCHAR, *p) != NULL)
        {
            return true;
        }
    }
                         ...
                         uchar_kt ***file, int flags)
{
     ...
     */
    ga_init(&ga, (int)sizeof(uchar_kt *), 30);
    ...
    for(int i = 0; i < num_pat; ++i)
    {
        ...
        if(vim_backtick(p))
        {
            ...
            {
                recursive = false;
                FreeWild(ga.ga_len, (uchar_kt **)ga.ga_data);
                *num_file = 0;
                *file = NULL;
                return FAIL;
            }
        ...
        {
            uchar_kt      *t = backslash_halve_save(p);
    ...
    *num_file = ga.ga_len;
    *file = (ga.ga_data != NULL) ? (uchar_kt **)ga.ga_data : (uchar_kt **)"";
    recursive = false;
 ...
 */
static int vim_backtick(uchar_kt *p)
{
    return *p == '`' && *(p + 1) != NUL && *(p + ustrlen(p) - 1) == '`';
}
...
)
{
    ...
    uchar_kt *cmd = ustrndup(pat + 1, ustrlen(pat) - 2);
...
)
{
    ...
    if(xstrpbrk(f, (uchar_kt *)FNAME_ILLEGAL) != NULL)
    {
        return;
    }
    ...
    uchar_kt *p = xmalloc(ustrlen(f) + 1 + isdir);
    ustrcpy(p, f);
#ifdef BACKSLASH_IN_FILENAME
    slash_adjust(p);
#endif
 ...
 */
void simplify_filename(uchar_kt *filename)
{
    int components = 0;
    uchar_kt      *p, *tail, *start;
    ...
    do
    {
        ...
        if(vim_ispathsep(*p))
        {
            xstrmove(p, p + 1);    /* remove duplicate "/" */
        }
                ...
                && (vim_ispathsep(p[2]) || p[2] == NUL))
        {
            ...
            if(components > 0)                /* strip one preceding component */
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        xstrmove(p, tail);                   /* strip previous component */
                    }
            ...
            {
                xstrmove(p, tail);    /* strip ".." or "../" */
            }
            else
            {
                if(p == start + 2 && p[-2] == '.')              /* leading "./../" */
                {
                    xstrmove(p - 2, p);                            /* strip leading "./" */
                    tail -= 2;
                }
        ...
        else
        {
            ...
            p = (uchar_kt *)path_next_component((const char *)p);
        }
    } while(*p != NUL);
}
...
find_file_name_in_path(
    ...
    int options,
    long count,
    uchar_kt *rel_fname         /* file we are searching relative to */
)
{
    uchar_kt *file_name;
    ...
    {
        tofree = (uchar_kt *) eval_includeexpr((char *) ptr, len);
 ...
 */
bool vim_isAbsName(uchar_kt *name)
{
...
FUNC_ATTR_NONNULL_ARG(2)
{
    *buf = NUL;
    ...
    {
        xstrncpy(buf, fname, len); // truncate
        return FAIL;
    }

    if(path_with_url(fname))
    {
        xstrncpy(buf, fname, len);
        return OK;
    }

    int rv = path_get_absolute_path((uchar_kt *)fname, (uchar_kt *)buf, len, force);

    if(rv == FAIL)
    {
        xstrncpy(buf, fname, len); // something failed; use the filename
    }
...
char *fix_fname(const char *fname)
{
    ...
    if(!vim_isAbsName((uchar_kt *)fname)
       || strstr(fname, "..") != NULL
       || strstr(fname, "//") != NULL
#ifdef BACKSLASH_IN_FILENAME
       || strstr(fname, "\\\\") != NULL
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    while((entry = (uchar_kt *) os_scandir_next(&dir)))
    {
        // Only accept names that differ in case and are the same byte
        // length. TODO: accept different length name.
        if(ustricmp(tail, entry) == 0 && ustrlen(tail) == ustrlen(entry))
        {
            uchar_kt newname[MAXPATHL + 1];
            // Verify the inode is equal.
            ustrlcpy(newname, name, MAXPATHL + 1);
            ustrlcpy(newname + (tail - name), entry,
...
uchar_kt *path_shorten_fname_if_possible(uchar_kt *full_path)
{
    uchar_kt *dirname = xmalloc(MAXPATHL);
    uchar_kt *p = full_path;
...
uchar_kt *path_shorten_fname(uchar_kt *full_path, uchar_kt *dir_name)
{
    if(full_path == NULL)
    {
        return NULL;
...
///                        unchanged or *num_file is set to 0 and *file is set
///                        to NULL or points to "".
int expand_wildcards_eval(uchar_kt **pat, int *num_file, uchar_kt ***file,
                          int flags)
{
    int ret = FAIL;
    uchar_kt      *eval_pat = NULL;
    uchar_kt      *exp_pat = *pat;
    uchar_kt      *ignored_msg;
    size_t usedlen;
    ...
    if(*exp_pat == '%' || *exp_pat == '#' || *exp_pat == '<')
    {
        ...
        if(eval_pat != NULL)
        {
            exp_pat = ustrdup_concat(eval_pat, exp_pat + usedlen);
        }
    }
...
///                      NULL or points to "".
int expand_wildcards(int num_pat, uchar_kt **pat, int *num_files, uchar_kt ***files,
                     int flags)
{
    int retval;
    int i, j;
    ...
    if(*p_wig)
    {
        ...
        {
            ffname = (uchar_kt *)FullName_save((char *)(*files)[i], false);
 ...
 */
int match_suffix(uchar_kt *fname)
{
#define MAXSUFLEN 30  // maximum length of a file suffix
    uchar_kt suf_buf[MAXSUFLEN];
    size_t fnamelen = ustrlen(fname);
    size_t setsuflen = 0;

    for(uchar_kt *setsuf = p_su; *setsuf;)
    {
...
int path_full_dir_name(char *directory, char *buffer, size_t len)
{
    ...
    if(ustrlen(directory) == 0)
    {
        return os_dirname((uchar_kt *) buffer, len);
    }

    char old_dir[MAXPATHL];

    // Get current directory name.
    if(os_dirname((uchar_kt *) old_dir, MAXPATHL) == FAIL)
    {
        return FAIL;
    }
    ...
    if(retval == FAIL || os_dirname((uchar_kt *) buffer, len) == FAIL)
    {
...
int append_path(char *path, const char *to_append, size_t max_len)
{
    ...
    if(current_length > 0 && !vim_ispathsep_nocolon(path[current_length-1]))
    {
        ...
        xstrncat(path, OS_PATH_SEP_STR, max_len);
    }
    ...
    xstrncat(path, to_append, max_len);
    return OK;
}
...
///
/// @return FAIL for failure, OK for success.
static int path_get_absolute_path(const uchar_kt *fname, uchar_kt *buf,
...
///
/// @return true if @b fname is absolute.
int path_is_absolute_path(const uchar_kt *fname)
{
#ifdef HOST_OS_WINDOWS
...
void path_guess_exepath(const char *argv0, char *buf, size_t bufsize) FUNC_ATTR_NONNULL_ALL
{
    ...
    if(path == NULL || path_is_absolute_path((uchar_kt *)argv0))
    {
        xstrncpy(buf, argv0, bufsize);
    }
    else if(argv0[0] == '.' || strchr(argv0, OS_PATH_SEP_CHAR))
    {
        ...
        xstrncat(buf, OS_PATH_SEP_STR, bufsize);
        xstrncat(buf, argv0, bufsize);
    }
    else
    {
        ...
        do
        {
            ...
            xstrncpy((char *)NameBuff, dir, dir_len + 1);
            xstrncat((char *)NameBuff, OS_PATH_SEP_STR, sizeof(NameBuff));
            xstrncat((char *)NameBuff, argv0, sizeof(NameBuff));

            if(os_can_exe(NameBuff, NULL, false))
            {
                xstrncpy(buf, (char *)NameBuff, bufsize);
                return;
            }
        } while(iter != NULL);
