/// @file nvim/ex_cmds.c
///
/// some functions for command line commands
...
void ex_align(exarg_T *eap)
{
        ...
        curwin->w_cursor.lnum <= eap->line2; ++curwin->w_cursor.lnum)
    {
        ...
        else
        {
            ...
            len = linelen(eap->cmdidx == CMD_right ? &has_tab : NULL)
                  - get_indent();
            ...
            else
            {
                ...
                // Make sure that embedded TABs don't make
                // the text go too far to the right.
                if(has_tab)
                {
                    while(new_indent > 0)
                    {
...
static uchar_kt *sortbuf1;
static uchar_kt *sortbuf2;
...
static int sort_compare(const void *s1, const void *s2)
{
    ...
    // When sorting numbers "start_col_nr" is
    // the number, not the column number.
    if(sort_nr)
    {
...
void ex_sort(exarg_T *eap)
{
    ...
    // From here on "sort_nr" is used as
    // a flag for any integer number sorting.
    sort_nr += sort_what;
...
void ex_retab(exarg_T *eap)
{
    ...
    long old_len;
    uchar_kt *ptr;
    uchar_kt *new_line = (uchar_kt *)1; // init to non-NULL
...
int do_move(linenr_T line1, linenr_T line2, linenr_T dest)
{
    ...
    mark_adjust_nofold(last_line - num_lines + 1,
                       last_line,
...
void do_bang(int addr_count, exarg_T *eap, int forceit, int do_in, int do_out)
{
    ...
    // Disallow shell commands from .exrc and .vimrc in
    ...
    {
        // Careful:
        // This may recursively call do_bang() again! (because of autocommands)
                      ...
                      int do_out)
{
    ...
    {
        // Use a pipe to fetch stdout of
        // the command, do not use a temp file.
        shell_flags |= kShellOptRead;
        curwin->w_cursor.lnum = line2;
    }
    else if(do_in && !do_out && !p_stmp)
    {
        // Use a pipe to write stdin of
        // the command, do not use a temp file.
    ...
    {
        // Use a pipe to write stdin and fetch
        // stdout of the command, do not use a temp file.
    ...
    if(itmp != NULL
       && buf_write(curbuf, itmp, NULL, line1, line2,
                    eap, false, false, false, true) == FAIL)
    {
...
///
void do_shell(uchar_kt *cmd, int flags)
{
    int save_nwr;
    ...
    // resulted in a "delete line 1" command to the terminal.
    ui_cursor_goto(msg_row, msg_col);
...
uchar_kt *make_filter_cmd(uchar_kt *cmd, uchar_kt *itmp, uchar_kt *otmp)
{
#if defined(UNIX)
    bool is_fish_shell =
...
///
/// @param[in]  opt
/// Separator or format string to append: will append
/// `printf(' ' . opt, fname)` if `%s` is found in `opt` or
/// a space, opt, a space and then fname if `%s` is not found there.
///
/// @param[in] fname
/// File name to append.
...
int rename_buffer(uchar_kt *new_fname)
{
    uchar_kt *fname;
    uchar_kt *sfname;
    uchar_kt *xfname;
...
int do_write(exarg_T *eap)
{
    ...
    {
        fname = ffname;
        free_fname = (uchar_kt *)fix_fname((char *)ffname);
    ...
    if(other)
    {
        ...
        {
            // Overwriting a file that is loaded
            // in another buffer is not a good idea.
            EMSG(_(e_bufloaded));
            goto theend;
        }
    }
    ...
    if(check_overwrite(eap, curbuf, fname, ffname, other) == OK)
    {
        if(eap->cmdidx == CMD_saveas && alt_buf != NULL)
        {
            ...
            // Autocommands may have changed buffer
            // names, esp. when 'autochdir' is set.
            fname = curbuf->b_sfname;
        }

        name_was_missing = curbuf->b_ffname == NULL;

        retval = buf_write(curbuf,
                           ffname,
                           fname,
                           eap->line1,
                           eap->line2,
        ...
        // Change directories when the 'acd' option is
        // set and the file name got changed or set.
                    ...
                    int other)
{
       ...
       && os_path_exists(ffname))
    {
        ...
        // For ":w! filename" check that
        // no swap file exists for "filename".
...
///
/// @param fnum
/// file number; if zero use ffname/sfname
///
/// @param ffname
/// the file name
...
///
/// @param sfname
/// the short file name (or NULL)
///
/// @param eap
/// contains the command to be executed after loading the file
...
///
/// @param oldwin
/// Should be "curwin" when editing a new buffer in the current
            ...
            win_T *oldwin)
{
    ...
    // TRUE if autocommands brought us into the buffer unexpectedly
    int auto_buf = FALSE;
    ...
    if(other_file)
    {
        ...
        if(buf != curbuf)
        {
            ...
            else
            {
                ...
                // This will set oldwin->w_buffer to NULL.
                u_sync(false);
    ...
    // otherwise autocommands may cause ml_get errors.
    RedrawingDisabled++;
    ...
    {
        // Don't make a buffer listed if it's a help buffer.
        // Useful when using CTRL-O to go back to a help file.
        set_buflisted(TRUE);
    }

    // If autocommands change buffers under our fingers,
    // forget about editing the file.
    if(buf != curbuf)
    {
        goto theend;
    }
    ...
    if(!other_file && !oldbuf) // re-use the buffer
    {
        ...
        // If autocommands deleted the buffer we were
        // going to re-edit, give up and jump to the end.
        if(!bufref_valid(&bufref))
        {
            delbuf_msg(new_name); // Frees new_name.
            goto theend;
    ...
    if(!auto_buf)
    {
        ...
        // Careful: open_buffer() and apply_autocmds()
        // may change the current buffer and window.
        ...
        // Even when cursor didn't move
        // we need to recompute topline.
        changed_line_abv_curs();
        maketitle();
    }
    ...
    // Check if cursors in other windows
    // on the same buffer are still valid
    check_lnums(FALSE);
...
static void delbuf_msg(uchar_kt *name)
{
...
void ex_append(exarg_T *eap)
{
    ...
    for(;;)
    {
        ...
        // Using ^ CTRL-D in getexmodeline()
        // makes us repeat the indent.
        if(ex_keep_indent)
        {
...
int check_secure(void)
{
    ...
    // In the sandbox more things are not allowed,
    // including the things disallowed in secure mode.
    if(sandbox != 0)
    {
        EMSG(_(e_sandbox));
        return TRUE;
...
/// command string
///
/// @param[in,out] subflags
/// current flags defined for the :substitute command
///
/// @param[in,out] which_pat
/// pattern type from which to get default search
///
/// @returns
...
static fbuf_st *do_sub(exarg_T *eap, proftime_T timeout)
{
    ...
    uchar_kt *pat = NULL, *sub = NULL; // init for GCC
    int delimiter;
       ...
       && vim_strchr((uchar_kt *)"0123456789cegriIp|\"", *cmd) == NULL)
    {
        ...
        if(*cmd == '\\')
        {
            ...
            pat = (uchar_kt *)""; // empty search pattern
    ...
    else if(!eap->skip) // use previous pattern and substitution
    {
        ...
        sub = (uchar_kt *) old_sub.sub;

        // Vi compatibility quirk: repeating with
        // ":s" keeps the cursor in the last column after using "$".
    ...
    if(search_regcomp(pat, RE_SUBST, which_pat,
                      (preview ? 0 : SEARCH_HIS), &regmatch) == FAIL)
    {
    ...
    for(linenr_T lnum = eap->line1;
        ...
        lnum++)
    {
        long nmatch = vim_regexec_multi(&regmatch,
                                        curwin,
                                        curbuf,
                                        lnum,
                                        (colnr_T)0,
        ...
        if(nmatch)
        {
            ...
            // new_start        The new text, all that has been produced sofar.
            ...
            for(;;)
            {
                ...
                if(subflags.do_ask && !preview)
                {
                    ...
                    while(subflags.do_ask)
                    {
                        ...
                        else
                        {
                            ...
                            // write message same
                            // highlighting as for wait_return
                ...
                // so that we can use "\=col(".").
                curwin->w_cursor.col = regmatch.startpos[0].col;

                // 3. Substitute the string.
                // During 'inccommand' preview only do this if
...
skip:
                   ...
                   || regmatch.startpos[0].lnum > 0)
                {
                    if(new_start != NULL)
                    {
                        ...
                        // When asking, undo is saved each time,
                        // must also set changed flag each time.
                        if(subflags.do_ask)
                        {
    ...
    if(preview && !aborting())
    {
        if(got_quit) // Substitution is too slow, disable 'inccommand'.
        {
            set_string_option_direct((uchar_kt *)"icm",
                ...
                && matched_lines.size != 0
                && pat != NULL)
        {
            // preserve 'modified' during preview
            curbuf->b_changed = save_b_changed;

            preview_buf = show_sub(eap,
                                   old_cursor,
                                   pat,
                                   sub,
                                   &matched_lines);
...
void ex_global(exarg_T *eap)
{
    ...
    if(global_busy)
    {
        // will increment global_busy
        EMSG(_("E147: Cannot do :global recursive"));
        return;
    }
...
void global_exe(uchar_kt *cmd)
{
    ...
    while(!got_int && (lnum = ml_firstmarked()) != 0 && global_busy == 1)
    {
        ...
        if(*cmd == NUL || *cmd == '\n')
        {
            do_cmdline((uchar_kt *)"p", NULL, NULL, DOCMD_NOWAIT);
        }
        else
        {
...
/// match some string for which help is requested.
///
/// @param matched_string
/// @param offset           offset for match
/// @param wrong_case       no matching case
int help_heuristic(uchar_kt *matched_string, int offset, int wrong_case)
{
    uchar_kt *p;
    int num_letters;
    num_letters = 0;
    ...
    // Features are less interesting than the subjects
    // themselves, but "+" alone is not a feature.
...
int find_help_tags(uchar_kt *arg,
                   int *num_matches,
                   uchar_kt ***matches,
                   int keep_lang)
{
    ...
    uchar_kt *s;
    int flags;
    uchar_kt *d = IObuff; // assume IObuff is long enough!
    ...
    if(i < 0) // no match in table
    {
        ...
        else
        {
            ...
            for(s = arg; *s; s++)
            {
                // Replace "|" with "bar" and '"' with "quote" to
                // match the name of the tags for these commands.
                // Replace "*" with ".*" and "?" with "." to match
                // command line completion.
                // Insert a backslash before '~', '$' and '.' to avoid
                ...
                *d++ = *s;
                ...
                // Fixes CTRL-] on 'option'. (would include the trailing '.').
                if(*s == '\'' && s > arg && *arg == '\'')
                {
                    break;
                }
      ...
      )
    {
        for(lnum = 1; lnum < curbuf->b_ml.ml_line_count; ++lnum)
        {
            ...
            while(*p != NUL)
            {
                ...
                rt = (uchar_kt *)vim_getenv("VIMRUNTIME");
                ...
                if(path_full_compare(rt, NameBuff, FALSE) != kEqualFiles)
                {
                    ...
                    // Note:
                    // We cannot just do `&NameBuff` because it is
                    // a statically sized array so `NameBuff == &NameBuff`
                       ...
                       && fcount > 0)
                    {
                        ...
                        // If foo.abx is found use it instead
                        // of foo.txt in the same directory.
...
///
/// @param ext
/// Suffix of the help files (".txt", ".itx", ".frx", etc.)
///
/// @param tagname
/// Name of the tags file ("tags" for English, "tags-fr" for French)
///
/// @param add_help_tags
/// Whether to add the "help-tags" tag
static void helptags_one(uchar_kt *dir,
                         uchar_kt *ext,
                         uchar_kt *tagfname,
                         bool add_help_tags)
{
    ...
    // Note:
    // We cannot just do `&NameBuff` because it is a statically sized array
    ...
    if(gen_expand_wildcards(1,
                            ...
                            &filecount,
                            &files,
                            EW_FILE|EW_SILENT) == FAIL || filecount == 0)
    {
        if(!got_int)
        {
    ...
    // If using the "++t" argument or generating tags
    // for "$VIMRUNTIME/doc" add the "help-tags" tag.
    ga_init(&ga, (int)sizeof(uchar_kt *), 100);
    ...
    for(fi = 0; fi < filecount && !got_int; ++fi)
    {
        ...
        while(!vim_fgets(IObuff, IOSIZE, fd) && !got_int)
        {
            ...
            while(p1 != NULL)
            {
                p2 = (uchar_kt *)strchr((const char *)p1 + 1, '*'); // Find second '*'.
    ...
    if(!got_int)
    {
        ...
        {
            p1 = ((uchar_kt **)ga.ga_data)[i - 1];
            p2 = ((uchar_kt **)ga.ga_data)[i];
...
static void do_helptags(uchar_kt *dirname, bool add_help_tags)
{
    ...
    // Go over all files in the directory to
    // find out what languages are present.
    int j;
    ga_init(&ga, 1, 10);
    ...
    for(int i = 0; i < filecount; i++)
    {
        ...
        if(j == ga.ga_len)
        {
            // New language, add it.
            ga_grow(&ga, 2);
            ((uchar_kt *)ga.ga_data)[ga.ga_len++] = lang[0];
            ((uchar_kt *)ga.ga_data)[ga.ga_len++] = lang[1];
        }
    }
...
static void helptags_cb(uchar_kt *fname, void *cookie)
{
...
static int sign_cmd_idx(uchar_kt *begin_cmd, uchar_kt *end_cmd)
{
    int  idx;
    char save = *end_cmd;
    *end_cmd = NUL;
...
uchar_kt *get_sign_name(expand_T *FUNC_ARGS_UNUSED_REALY(x), int idx)
{
    ...
    {
        case EXP_SUBCMD:
            return (uchar_kt *)cmds[idx];

        case EXP_DEFINE:
        {
            char *define_arg[] = { "icon=", "linehl=", "text=", "texthl=", NULL };
            return (uchar_kt *)define_arg[idx];
        }

        case EXP_PLACE:
        {
            char *place_arg[] = { "line=", "name=", "file=", "buffer=", NULL };
            return (uchar_kt *)place_arg[idx];
        }

        case EXP_UNPLACE:
        {
            char *unplace_arg[] = { "file=", "buffer=", NULL };
            return (uchar_kt *)unplace_arg[idx];
        }
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    if(split && win_split((int)p_cwh, WSP_BOT) != FAIL)
    {
        ...
        int hl_id = syn_check_group((uchar_kt *)"Substitute", 13);
        ...
        for(size_t line = 0; line < matched_lines->size; line++)
        {
            ...
            // Put "|lnum| line" into `str` and
            // append it to the preview buffer.
            snprintf(str,
                     line_size,
                     "|%*ld| %s",
                     col_width - 3,
...
void ex_substitute(exarg_T *eap)
{
    ...
    if(save_changedtick != curbuf->b_changedtick)
    {
        ...
        // Restore newhead. It is meaningless when curhead
        // is valid, but we must restore it so that undotree()
        // is identical before/after the preview.
...
uchar_kt *skip_vimgrep_pat(uchar_kt *p, uchar_kt **s, int *flags)
{
    int c;
...
void ex_oldfiles(exarg_T *eap)
{
    ...
    else
    {
        ...
        for(li = l->lv_first; li != NULL && !got_int; li = li->li_next)
        {
            ...
            if(!message_filtered((uchar_kt *)fname))
            {
                msg_outnum(nr);
                MSG_PUTS(": ");
                msg_outtrans((uchar_kt *)tv_get_string(&li->li_tv));
                msg_clr_eos();
                msg_putchar('\n');
        ...
        if(cmdmod.browse)
        {
            ...
            if(nr > 0 && nr <= l->lv_len)
            {
                ...
                char *const s = (char *)expand_env_save((uchar_kt *)p);
                eap->arg = (uchar_kt *)s;
                eap->cmdidx = CMD_edit;
                cmdmod.browse = false;
                do_exedit(eap, NULL);
