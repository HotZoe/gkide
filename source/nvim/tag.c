...
// is used to find duplicates, ga_match[] to keep them in sequence.
// At the end, the matches from ga_match[] are concatenated, to make
// a list sorted on priority.
...
static uchar_kt *nofile_fname = NULL;

#ifdef INCLUDE_GENERATED_DECLARATIONS
    #include "tag.c.generated.h"
#endif

static uchar_kt *bottommsg = (uchar_kt *)N_("E555: at bottom of tag stack");
static uchar_kt *topmsg = (uchar_kt *)N_("E556: at top of tag stack");
static uchar_kt *tagmatchname = NULL; ///< name of last used tag
...
int do_tag(uchar_kt *tag, int type, int count, int forceit,  int verbose)
{
    ...
    // name to use for priority computation
    uchar_kt *buf_ffname = curbuf->b_ffname;

    // remember the matches for the last used tag
    static int num_matches = 0;

    // limit used for match search
    static int max_num_matches = 0;

    static uchar_kt **matches = NULL;
    static int flags;
    ...
    if(!p_tgst && *tag != NUL)
    {
        ...
        if(g_do_tagpreview != 0)
        {
            ...
            ptag_entry.tagname = ustrdup(tag);
        }
    }
    else
    {
               ...
               || type == DT_CSCOPE))
        {
            if(g_do_tagpreview != 0)
            {
                if(ptag_entry.tagname != NULL
                   && ustrcmp(ptag_entry.tagname, tag) == 0)
                {
            ...
            else
            {
                ...
                // delete all tag stack entries above it.
                ...
                tagstack[tagstackidx].tagname = ustrdup(tag);
        ...
        {
            if(g_do_tagpreview != 0
               ? ptag_entry.tagname == NULL : tagstacklen == 0)
            {
                EMSG(_(e_tagstack)); // empty stack
                goto end_do_tag;
            }
            ...
            if(type == DT_POP) // go to older position
            {
                ...
                if((tagstackidx -= count) < 0)
                {
                    ...
                    // We weren't at the bottom of the stack,
                    // so jump all the way to the bottom now.
                    tagstackidx = 0;
                }
                ...
                // Make a copy of the fmark, autocommands may
                ...
                if(saved_fmark.fnum != curbuf->b_id)
                {
                    ...
                    // A BufReadPost autocommand may jump to
                    // the '" mark, but we don't what that here.
        ...
        {
            // For ":tag [arg]" or ":tselect"
            // remember position before the jump.
            saved_fmark = tagstack[tagstackidx].fmark;
    ...
    for(;;)
    {
        ...
        other_name = (tagmatchname == NULL || ustrcmp(tagmatchname, name) != 0);
           ...
           || other_name)
        {
            ...
            {
                // Find the position of each old match in the new list.
                // Need to use parse_match() to find the tag line.
                idx = 0;
                ...
                for(j = 0; j < num_matches; ++j)
                {
                    ...
                    for(i = idx; i < new_num_matches; ++i)
                    {
                        ...
                        if(ustrcmp(tagp.tagname, tagp2.tagname) == 0)
                        {
                            p = new_matches[i];
        ...
        else
        {
            ...
            else if(type == DT_SELECT || (type == DT_JUMP && num_matches > 1))
            {
                ...
                for(i = 0; i < num_matches && !got_int; i++)
                {
                    ...
                    while(p != command_end)
                    {
                        ...
                        // a command char or a backslash.
                        if(*p == '\\'
                           && (*(p + 1) == *tagp.command || *(p + 1) == '\\'))
                        {
                            ++p;
                        }
            ...
            else if(type == DT_LTAG)
            {
                ...
                for(i = 0; i < num_matches; ++i)
                {
                    ...
                    ustrlcpy(tag_name, tagp.tagname, len + 1);
                    p = tag_full_fname(&tagp); // Save the tag file name

                    ustrlcpy(fname, p, MAXPATHL + 1);
                    xfree(p);

                    // Get the line number or the
                    // search pattern used to locate the tag.
                    lnum = 0;
                    ...
                    else
                    {
                        ...
                        // Now, cmd_end points to the character after
                        // the command. Adjust it to point to the last
                        ...
                        // If "^" is present in the tag
                        // search pattern, then copy it first.
                        if(*cmd_start == '^')
                        {
                            ustrcpy(cmd, "^");
                            cmd_start++;
                        ...
                        // Precede the tag pattern
                        // with \V to make it very nomagic.
                        ustrcat(cmd, "\\V");
                        len += 2;
                        ...
                        ustrncat(cmd, cmd_start, cmd_len);
                        len += cmd_len;

                        if(cmd[len - 1] == '$')
                        {
                            // Replace '$' at the end
                            // of the search pattern with '\$'
                            cmd[len - 1] = '\\';
                            cmd[len] = '$';
                            len++;
                        }
                ...
                xsnprintf((char *)IObuff, IOSIZE, "ltag %s", tag);
               ...
               && !skip_msg)
            {
                ...
                if(ic)
                {
                    ustrcat(IObuff, _("  Using tag with different case!"));
                }
            ...
            xsnprintf((char *)IObuff, IOSIZE, ":ta %s\r", name);
...
/// Print the tag stack
void do_tags(exargs_st *FUNC_ARGS_UNUSED_MATCH(eap))
{
    int i;
    uchar_kt *name;
    taggy_st *tagstack = curwin->w_tagstack;
...
static int tag_strnicmp(uchar_kt *s1, uchar_kt *s2, size_t len)
{
    int i;
...
static void prepare_pats(pattern_st *pats, int has_re)
{
    ...
    if(has_re)
    {
        // When the pattern starts with '^' or "\\<",
        // binary searching can be used (much faster).
        if(pats->pat[0] == '^')
        {
    ...
    if(has_re)
    {
        pats->regmatch.regprog =
            regexp_compile(pats->pat, p_magic ? RE_MAGIC : 0);
    }
    else
    {
...
///
/// @param buf_ffname
/// name of buffer for priority
///
/// Return FAIL if search completely failed (*num_matches will
/// be 0, *matchesp will be NULL), OK otherwise.
///
...
///  2. A global tag with a full matching tag for another file.
///  3. A static tag with a full matching tag for another file.
///  4. A static or global tag with an ignore-case matching tag for the
///     current file.
///  5. A global tag with an ignore-case matching tag for another file.
///  6. A static tag with an ignore-case matching tag for another file.
///
              ...
              uchar_kt *buf_ffname)
{
    ...
    hashtable_st ht_match[MT_COUNT]; // stores matches by key
    hash_kt hash = 0;
    ...
    int help_save;
    int help_pri = 0;
    uchar_kt *help_lang_find = NULL; // lang to be found
    ...
    // Change the value of 'ignorecase' according to
    // 'tagcase' for the duration of this function.
    ...
    {
        ga_init(&ga_match[mtt], sizeof(uchar_kt *), 100);
        hash_init(&ht_match[mtt]);
    }

    ustrcpy(tag_fname, "from cscope"); // for error messages
    ...
    orgpat.len = (int)ustrlen(pat);

    if(curbuf->b_help)
    {
        // When "@ab" is specified use only the "ab" language,
        ...
        {
            saved_pat = ustrndup(pat, orgpat.len - 3);
    ...
    // This is only to avoid a compiler
    // warning for using search_info uninitialised.
    ...
    if((flags & TAG_KEEP_LANG)
       && help_lang_find == NULL
       && curbuf->b_fname != NULL
       && (i = (int)ustrlen(curbuf->b_fname)) > 4
       && ustricmp(curbuf->b_fname + i - 4, ".txt") == 0)
    {
        is_txt = true;
    }
    ...
    for(round = 1; round <= 2; ++round)
    {
            ...
            first_file = false)
        {
            ...
            else
            {
                if(curbuf->b_help)
                {
                    if(is_txt) // Keep en if the file extension is .txt
                    {
                        ustrcpy(help_lang, "en");
                    }
                    else
                    {
                        // Prefer help tags according to 'helplang'. Put the
                        // two-letter language name in help_lang[].
                        i = (int)ustrlen(tag_fname);

                        if(i > 3 && tag_fname[i - 3] == '-')
                        {
                            ustrcpy(help_lang, tag_fname + i - 2);
                        }
                        else
                        {
                            ustrcpy(help_lang, "en");
                    ...
                    // When searching for a specific language
                    // skip tags files for other languages.
                    if(help_lang_find != NULL
                       && ustricmp(help_lang, help_lang_find) != 0)
                    {
                        continue;
                    }

                    // For CTRL-] in a help file prefer
                    // a match with the same language.
                    if((flags & TAG_KEEP_LANG)
                       && help_lang_find == NULL
                       && curbuf->b_fname != NULL
                       && (i = (int)ustrlen(curbuf->b_fname)) > 4
                       ...
                       && curbuf->b_fname[i - 4] == '.'
                       && ustrnicmp(curbuf->b_fname + i - 3, help_lang, 2) == 0)
                    {
                        help_pri = 0;
                    }
                    else
                    {
                        ...
                        if(s == NULL || *s == NUL)
                        {
                            // Language not in 'helplang':
                            // use last, prefer English, unless found already.
                            ++help_pri;

                            if(ustricmp(help_lang, "en") != 0)
                            {
                                ++help_pri;
                            }
                        }
            ...
            for(;;)
            {
                ...
                // When mincount is TAG_MANY, stop when enough
                // matches have been found (for completion).
                ...
                // For binary search:
                // compute the next offset to use.
                if(state == TS_BINARY)
                {
                    offset = search_info.low_offset
                             + ((search_info.high_offset - search_info.low_offset) / 2);
                ...
                // When jumping around in the file, first read
                // a line to find the start of the next line.
                if(state == TS_BINARY || state == TS_SKIP_BACK)
                {
                ...
                else // Not jumping around in the file: Read the next line.
                {
                    do // skip empty and blank lines
                    {
                        if(use_cscope)
                        {
...
line_read_in:
                ...
                if(vimconv.vc_type != CONV_NONE)
                {
                    ...
                    // Convert every line.
                    // Converting the pattern from 'enc' to the
                    // tags file encoding doesn't work, because
                    ...
                    if(conv_line != NULL)
                    {
                        ...
                        {
                            ustrcpy(lbuf, conv_line);
                            xfree(conv_line);
                        }
                    }
                }

                // When still at the start of the file, check
                // for Emacs tags file format, and for "not sorted" flag.
                if(state == TS_START)
                {
                    // The header ends when the line sorts below "!_TAG_".
                    // When case is folded lower case letters sort before "_".
                    if(ustrncmp(lbuf, "!_TAG_", 6) <= 0
                       || (lbuf[0] == '!' && ASCII_ISLOWER(lbuf[1])))
                    {
                        if(ustrncmp(lbuf, "!_TAG_", 6) != 0)
                        {
                            // Non-header item before
                            // the header, e.g. "!" itself.
                            goto parse_line;
                        }

                        // Read header line.
                        if(ustrncmp(lbuf, "!_TAG_FILE_SORTED\t", 18) == 0)
                        {
                            tag_file_sorted = lbuf[18];
                        }

                        if(ustrncmp(lbuf, "!_TAG_FILE_ENCODING\t", 20) == 0)
                        {
                            // Prepare to convert every line from
                            // the specified encoding to 'encoding'.
                    ...
                    // When there is no tag head, or ignoring case, need to
                    // do a linear search. When no "!_TAG_" is found, default
                    // to binary search. If the tag file isn't sorted, the
                    // second loop will find it. When "!_TAG_FILE_SORTED"
                    // found: start binary search if flag set.
                    ...
                    {
                        // Binary search won't work for
                        // ignoring case, use linear search.
                        linear = TRUE;
                        state = TS_LINEAR;
                    }

                    // When starting a binary search, get the
                    // size of the file and compute the first offset.
                    if(state == TS_BINARY)
                    {
                        ...
                        else
                        {
                            ...
                            // Start the search in the middle of the file.
...
parse_line:
                ...
                {
                    tagp.tagname = lbuf;
                    tagp.tagname_end = ustrchr(lbuf, TAB);

                    if(tagp.tagname_end == NULL)
                    {
                        if(ustrchr(lbuf, NL) == NULL)
                        {
                            // Truncated line, ignore it. Has been reported
                            // for Mozilla JS with extremely long names.
                            if(p_verbose >= 5)
                            {
                                verbose_enter();
                        ...
                        line_error = TRUE; // Corrupted tag line.
                        break;
                    }
                    ...
                    // and there is no regexp, or the tag is too short.
                    ...
                    if(state == TS_BINARY)
                    {
                        ...
                        if(sortic)
                        {
                            tagcmp = tag_strnicmp(tagp.tagname,
                                                  orgpat.head,
                                                  (size_t)cmplen);
                        }
                        else
                        {
                            tagcmp = ustrncmp(tagp.tagname, orgpat.head, cmplen);
                        }
                        ...
                        if(tagcmp < 0)
                        {
                            ...
                            if(search_info.curr_offset < search_info.high_offset)
                            {
                                search_info.low_offset =
                                    search_info.curr_offset;
                    ...
                    // Can be a matching tag, isolate
                    // the file name and command.
                    if(tagp.fname == NULL)
                    {
                        tagp.fname = tagp.tagname_end + 1;
                    }

                    tagp.fname_end = ustrchr(tagp.fname, TAB);
                ...
                // First try matching with the pattern
                // literally (also when it is a regexp).
                cmplen = (int)(tagp.tagname_end - tagp.tagname);
                ...
                else
                {
                    ...
                    {
                        assert(cmplen >= 0);
                        match = mb_strnicmp(tagp.tagname,
                                            orgpat.pat,
                                            (size_t)cmplen) == 0;

                        if(match)
                        {
                            match_no_ic = (ustrncmp(tagp.tagname,
                                                   orgpat.pat,
                                                   cmplen) == 0);
                        }
                    }
                ...
                // Has a regexp:
                // Also find tags matching regexp.
                match_re = FALSE;
                ...
                if(!match && orgpat.regmatch.regprog != NULL)
                {
                    ...
                    *tagp.tagname_end = NUL;
                    ...
                    {
                        matchoff =
                            (int)(orgpat.regmatch.startp[0] - tagp.tagname);
                ...
                if(match)
                {
                    ...
                    else
                    {
                        ...
                        is_current = test_for_current(tagp.fname,
                                                      tagp.fname_end,
                                                      tag_fname,
                                                      buf_ffname);
                        {
                        ...
                        // sixteen tables to store this match
                        if(is_static)
                        {
                            if(is_current)
                            {
                    ...
                    // Store the info we need later, which depends on the kind
                    // of tags we are dealing with.
                    if(help_only)
                    {
                        #define ML_EXTRA  3

                        // Append the help-heuristic number after the tagname,
                        // for sorting it later. The heuristic is ignored for
                        ...
                        *tagp.tagname_end = NUL;
                        ...
                        mfp = xmalloc(sizeof(uchar_kt)
                                      ...
                                      + ML_EXTRA + 1);
                        p = mfp;
                        ustrcpy(p, tagp.tagname);
                        p[len] = '@';
                        ustrcpy(p + len + 1, help_lang);

                        snprintf((char *)p + len + 1 + ML_EXTRA,
                                 10,
                                 "%06d",
                                 help_heuristic(tagp.tagname,
                                                match_re ? matchoff : 0,
                                                !match_no_ic) + help_pri);
                    ...
                    else if(name_only)
                    {
                        if(get_it_again)
                        {
                            uchar_kt *temp_end = tagp.command;
                            ...
                            if(tagp.command + 2 < temp_end)
                            {
                                ...
                                mfp = xmalloc(len + 2);
                                ustrlcpy(mfp, tagp.command + 2, len + 1);
                            }
                            else
                            {
                                mfp = NULL;
                        ...
                        {
                            len = (int)(tagp.tagname_end - tagp.tagname);
                            mfp = xmalloc(sizeof(uchar_kt) + len + 1);
                            ustrlcpy(mfp, tagp.tagname, len + 1);
                    ...
                    {
                        #define TAG_SEP  0x02
                        size_t tag_fname_len = ustrlen(tag_fname);

                        // Save the tag in a buffer.
                        // Use 0x02 to separate fields (Can't use NUL,
                        // because the hash key is terminated by NUL).
                        // Emacs tag:
                        // <mtt><tag_fname><0x02><ebuf><0x02><lbuf><NUL>
                        //
                        // other tag:
                        // <mtt><tag_fname><0x02><0x02><lbuf><NUL>
                        //
                        // without Emacs tags:
                        // <mtt><tag_fname><0x02><lbuf><NUL>
                        //
                        // Here <mtt> is the "mtt" value plus 1 to avoid NUL.
                        len = (int)tag_fname_len + (int)ustrlen(lbuf) + 3;
                        mfp = xmalloc(sizeof(uchar_kt) + len + 1);
                        p = mfp;
                        p[0] = mtt + 1;
                        ustrcpy(p + 1, tag_fname);
                    ...
                    if(mfp != NULL)
                    {
                        ...
                        // "mfp" is used as a hash key, there is a NUL byte
                        // to end the part matters for comparing, more bytes
                        // may follow after it. E.g. help tags store the
                        // priority after the NUL.
                        if(use_cscope)
                        {
                            hash++;
                        }
...
findtag_end:
    ...
    if(match_count > 0)
    {
        matches = xmalloc(match_count * sizeof(uchar_kt *));
    }
    else
    {
        matches = NULL;
    ...
    for(mtt = 0; mtt < MT_COUNT; mtt++)
    {
        ...
        {
            mfp = ((uchar_kt **)(ga_match[mtt].ga_data))[i];
            ...
            else
            {
                ...
                matches[match_count++] = (uchar_kt *)mfp;
            }
        }
...
static void found_tagfile_cb(uchar_kt *fname,
                             void *FUNC_ARGS_UNUSED_MATCH(cookie))
{
    GA_APPEND(uchar_kt *, &tag_fnames, ustrdup(fname));
}
...
int get_tagfname(tagname_st *tnp, int first, uchar_kt *buf)
{
    ...
    if(curbuf->b_help)
    {
        ...
        {
            ga_clear_strings(&tag_fnames);
            ga_init(&tag_fnames, (int)sizeof(uchar_kt *), 10);

            do_in_runtimepath((uchar_kt *)"doc/tags doc/tags-??",
                              DIP_ALL,
                              found_tagfile_cb,
                              NULL);
        }
        ...
        {
            // Not found in 'runtimepath', use 'helpfile', if it exists
            // and wasn't used yet, replacing "help.txt" with "tags".
            ...
            ++tnp->tn_hf_idx;
            ustrcpy(buf, p_hf);
            ustrcpy(path_tail(buf), "tags");
        }
        else
        {
            ustrlcpy(buf,
                     ((uchar_kt **)(tag_fnames.ga_data))[tnp->tn_hf_idx++],
                     MAXPATHL);
        }
    ...
    if(first)
    {
        // Init. We make a copy of 'tags', because autocommands
        // may change the value without notifying us.
        tnp->tn_tags = ustrdup((*curbuf->b_p_tags != NUL)
                                   ? curbuf->b_p_tags
                                   : p_tags);
    ...
    for(;;)
    {
        ...
        else
        {
            ...
            buf[0] = NUL; // Copy next file name into buf.
            (void)copy_option_part(&tnp->tn_np, buf, MAXPATHL - 1, " ,");
            r_ptr = vim_findfile_stopdir(buf);

            // move the filename one char forward and
            // truncate the filepath with a NUL
            filename = path_tail(buf);
            xstrmove(filename + 1, filename);
            *filename++ = NUL;
...
/// Parse one line from the tags file. Find start/end of tag name,
/// start/end of file name and start of search pattern.
///
...
/// This is only recognized when both occurrence of 'file' are the same,
/// to avoid recognizing "string::string" or ":exit".
///
...
/// Find out the actual file name of a tag.
...
static uchar_kt *tag_full_fname(tag_ptr_st *tagp)
{
    ...
    *tagp->fname_end = NUL;
    uchar_kt *fullname = expand_tag_fname(tagp->fname, tagp->tag_fname, FALSE);
    *tagp->fname_end = c;
    return fullname;
}
...
///
/// @param lbuf        line from the tags file for this tag
/// @param forceit     :ta with !
...
static int jumpto_tag(uchar_kt *lbuf, int forceit, int keep_help)
{
    ...
    *pbuf_end = NUL;
    ...
    // For ":tab tag" open a new tab page.
    ...
    if(keep_help)
    {
        ...
        // For ":ptag" we need to use the flag from the window
        // where we came from.
        if(l_g_do_tagpreview != 0)
        {
    ...
    if(getfile_result <= 0) // got to the right file
    {
        ...
        // If 'cpoptions' contains 't', store the search pattern
        // for the "n" command. If 'cpoptions' does not contain 't',
        // the search pattern is not stored.
        if(ustrchr(p_cpo, CPO_TAGPAT) != NULL)
        {
            search_options = 0;
        }
        else
        ...
        // If the command is a search, try here. Reset 'smartcase'
        // for the search, since the search pattern was not typed
        // by the user. Only use do_search() when there is a full
        // search command, without anything following.
        str = pbuf;
        ...
        // search command with nothing following
        if(str > pbuf_end - 1)
        {
            save_p_ws = p_ws;
            save_p_ic = p_ic;
            ...
            else
            {
                int found = 1;
                int cc;
                p_ic = TRUE; // try again, ignore case now
            ...
            // A search command may have positioned the cursor
            // beyond the end of the line. May need to correct that here.
            check_cursor();
        }
        else
        {
        ...
        // When the command has done something that is not
        // allowed make sure the error message can be seen.
        if(secure == 2)
        {
            wait_return(TRUE);
        }
        ...
        if(retval == OK)
        {
            // For a help buffer: Put the cursor line at the
            // top of the window, the help subject will be below it.
            if(curbuf->b_help)
            {
                                  ...
                                  int expand)
{
    ...
    if(expand && path_has_wildcard(fname))
    {
        ...
        expanded_fname = ExpandOne(&xpc,
                                   fname,
                                   NULL,
                                   WILD_LIST_NOTFOUND | WILD_SILENT,
                                   WILD_EXPAND_FREE);
       ...
       && (p = path_tail(tag_fname)) != tag_fname)
    {
        ...
        ustrcpy(retval, tag_fname);

        ustrlcpy(retval + (p - tag_fname), fname, MAXPATHL - (p - tag_fname));
...
/// This is a bit slow, because of the full path compare in
/// path_full_compare().
///
/// @return
/// TRUE if tag for file "fname"
/// if tag file "tag_fname" is for current file.
static int test_for_current(uchar_kt *fname,
                            uchar_kt *fname_end,
                            uchar_kt *tag_fname,
                            uchar_kt *buf_ffname)
{
...
static int find_extra(uchar_kt **pp)
{
    uchar_kt *str = *pp;
...
int expand_tags(int tagnames, uchar_kt *pat, int *num_file, uchar_kt ***file)
{
    int i;
    int c;
    int tagnmflag;
                         ...
                         uchar_kt *end)
{
    ...
    if(start != NULL)
    {
        ...
        ustrlcpy(buf, start, len + 1);
    }

    buf[len] = NUL;

    retval = tv_dict_add_str(dict,
                             field_name,
                             ustrlen(field_name),
                             (const char *)buf);
...
int get_tags(list_st *list, uchar_kt *pat, uchar_kt *buf_fname)
{
    ...
    if(ret == OK && num_matches > 0)
    {
        for(i = 0; i < num_matches; ++i)
        {
            ...
            if(ustrncmp(tp.tagname, "!_TAG_", 6) == 0)
            {
                continue;
            }
            ...
            if(tp.command_end != NULL)
            {
                    ...
                    *p != NUL && *p != '\n' && *p != '\r'; ++p)
                {
                    ...
                    else if(ustrncmp(p, "file:", 5) == 0)
                    {
                        p += 4; // skip "file:" (static tag)
                    }
