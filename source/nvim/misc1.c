/// @file nvim/misc1.c
///
/// functions that didn't seem to fit elsewhere
...
int open_line(int dir, int flags, int second_line_indent)
{
    ...
    if(curbuf->b_p_ai || do_si)
    {
           ...
           && (p_extra == NULL || first_char != '{'))
        {
            ...
            if(dir == FORWARD)
            {
                // Skip preprocessor directives, unless
                // they are recognised as comments.
                if(lead_len == 0 && ptr[0] == '#')
                {
    ...
    if(lead_len > 0)
    {
        ...
        for(p = lead_flags; *p && *p != ':'; ++p)
        {
            ...
            if(*p == COM_END)
            {
                ...
                for(lead_repl = p;
                    lead_repl > curbuf->b_p_com
                    && lead_repl[-1] != ':'; --lead_repl)
                { /* empty body */ }

                lead_repl_len = (int)(p - lead_repl);

                // We can probably always add an extra space
                // when doing "O" on the comment-end
                extra_space = TRUE;
        ...
        if(lead_len > 0)
        {
            ...
            leader = xmalloc((size_t)(lead_len
                                      + lead_repl_len
                                      + extra_space
                                      + extra_len
            ...
            // if a new indent will be set below, remove
            // the indent that is in the comment leader
            if(newindent || did_si)
            {
                   ...
                   int include_space)
{
    ...
    while(line[i] != NUL)
    {
        ...
        for(list = curbuf->b_p_com; *list;)
        {
            ...
            *string++ = NUL; // isolate flags from string
            ...
            if(middle_match_len != 0 && j > middle_match_len)
            {
                ...
                // since it's a longer thus better match.
                middle_match_len = 0;
            }
        ...
        if(middle_match_len != 0)
        {
            // Use the previously found middle match after
            // failing to find a match with an end.
            if(!got_com && flags != NULL)
            {
                *flags = saved_flags;
            }
...
int get_last_leader_offset(uchar_kt *line, uchar_kt **flags)
{
    ...
    while(--i >= lower_check_bound)
    {
        ...
        for(list = curbuf->b_p_com; *list;)
        {
            ...
            if(string == NULL)
            {
                // If everything is fine, this cannot actually happen.
                continue;
            }
...
void ins_char(int c)
{
    ...
    // to insert a NUL byte. Happens for CTRL-Vu9900.
    if(buf[0] == 0)
    {
        buf[0] = '\n';
    }
    ...
    if(curmod & kModFlgReplace)
    {
        if(curmod & kModFlgVReplace)
        {
            ...
            while(oldp[col + oldlen] != NUL && vcol < new_vcol)
            {
                ...
                // Don't need to remove a TAB that
                // takes us to the right position.
...
/// Delete @b count bytes under the cursor.
/// If @b fixpos_arg is true, don't leave the cursor on the NUL after the line.
/// Caller must have prepared for undo.
///
...
/// @param undo    if TRUE, prepare for undo
...
///
/// @param lnum   the first line that needs displaying, @b lnume the
///               first line below the changed lines (BEFORE the change).
/// @param col    column in first line with change
/// @param lnume  line below last changed line
/// @param extra  number of extra lines (negative when deleting)
///
/// When only inserting lines, @b lnum and @b lnume are equal.
                           ...
                           long xtra)
{
    ...
    if(!cmdmod.keepjumps)
    {
        ...
        if(curbuf->b_new_change || curbuf->b_changelistlen == 0)
        {
            ...
            if(add)
            {
                ...
                if(curbuf->b_changelistlen == JUMPLISTSIZE)
                {
                    ...
                    {
                        // Correct position in changelist for other
        ...
        // The current window is always after the last change,
        // so that "g," takes you back to it.
...
///
/// @param col
/// column for message; non-zero when in insert mode and 'showmode' is on
...
int get_number(int colon, int *mouse_used)
{
    ...
    // When not printing messages, the user won't know
    // what to type, return a zero (as if CR was hit).
    if(msg_silent != 0)
    {
        return 0;
    }
...
/// When "mouse_used" is not NULL allow using the mouse and
/// in that case return the line number.
...
uchar_kt *get_users(expand_st *FUNC_ARGS_UNUSED_REALY(xp), int idx)
{
    init_users();
