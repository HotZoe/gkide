...
/// When converting, a read() or write() may leave some bytes to be
/// converted for the next call. The value is guessed ...
...
///
/// Relying on this value requires one to reset
/// it prior calling apply_autocmds_group.
static bool au_did_filetype INIT(= false);

void filemess(filebuf_st *buf, uchar_kt *name, uchar_kt *s, int attr)
{
    int msg_scroll_save;
    ...
    xstrncat((char *)IObuff, (const char *)s, IOSIZE);
...
///
/// 1. We allocate blocks with xmalloc(), as big as possible.
             ...
             int flags)
{
    ...
    int try_mac = (ustrchr(p_ffs, 'm') != NULL);
    int try_dos = (ustrchr(p_ffs, 'd') != NULL);
    int try_unix = (ustrchr(p_ffs, 'x') != NULL);
    ...
    // TRUE when iconv() failed and trying 'charconvert' next
    int did_iconv = FALSE;
#endif
    ...
    using_b_fname = (fname == curbuf->b_fname)
                    || (sfname == curbuf->b_fname);

    // After reading a file the cursor line changes but
    // we don't want to display the line.
    ex_no_reprint = TRUE;
    ...
    if(!filtering && !read_stdin && !read_buffer)
    {
        ...
        else if(apply_autocmds_exarg(EVENT_FILEREADCMD,
                                     sfname,
                                     sfname,
                                     FALSE,
                                     NULL,
    ...
    if(!read_buffer && !read_stdin)
    {
        ...
        // so we have to check for it before os_open().
    ...
    if(!read_buffer && !read_stdin)
    {
        if(!newfile
           || readonlymode
           || !(perm & 0222) || !os_file_is_writable((char *)fname))
        {
            file_readonly = true;
        }
    ...
    if(fd < 0) // cannot open at all
    {
        ...
        else
        {
            ...
            if(perm == UV_ENOENT) // check if the file exists
            {
                ...
                apply_autocmds_exarg(EVENT_BUFNEWFILE,
                                     sfname,
                                     sfname,
                                     FALSE,
                                     curbuf,
                ...
                save_file_ff(curbuf); // remember the current fileformat
            ...
            else
            {
                filemess(curbuf,
                         ...
                         // libuv only returns
                         // errno in Unix and in Windows open()
                         // does not set EOVERFLOW
    ...
    // Only set the 'ro' flag for readonly files the first time
    // they are loaded. Help files always get readonly mode
    ...
    // Create a swap file now, so that other Vims are warned
    // that we are editing this file. Don't do this for a "nofile"
    // or "nowrite" buffer type.
    if(!bt_dontwrite(curbuf))
    {
        check_need_swap(newfile);
    ...
    if(!read_buffer)
    {
        ...
        // The file must be closed again, the autocommands
        // may want to change the file before reading it.
        if(!read_stdin)
        {
        ...
        if(filtering)
        {
            apply_autocmds_exarg(EVENT_FILTERREADPRE, NULL,
                                 sfname, FALSE, curbuf, eap);
        }
        else if(read_stdin)
        {
            apply_autocmds_exarg(EVENT_STDINREADPRE, NULL,
                                 sfname, FALSE, curbuf, eap);
        }
        else if(newfile)
        {
            apply_autocmds_exarg(EVENT_BUFREADPRE, NULL,
                                 sfname, FALSE, curbuf, eap);
        }
        else
        {
            apply_autocmds_exarg(EVENT_FILEREADPRE, sfname,
                                 sfname, FALSE, NULL, eap);
        }
...
retry:
    ...
    // When retrying with another "fenc" and
    // the first time "fileformat" will be reset.
    if(keep_fileformat)
    {
    ...
    if(advance_fenc)
    {
        ...
        if(eap != NULL && eap->force_enc != 0)
        {
            // Conversion given with "++cc=" wasn't possible,
    ...
    if(converted)
    {
        // "ucs-bom" means we need to check
        // the first bytes of the file for a BOM.
        if(ustrcmp(fenc, ENC_UCSBOM) == 0)
        {
          ...
          )
        {
            ...
            // Skip conversion when it's already
            // done (retry for wrong "fileformat").
            if(tmpname == NULL)
            {
    ...
    while(!error && !got_int)
    {
        ...
        // up to max_unsigned characters (and other things).
        {
            if(!skip_read)
            {
                ...
                for(; size >= 10; size /= 2)
                {
                    new_buffer = xmalloc((size_t)size + (size_t)linerest + 1);
                    ...
                    else
                    {
                        ...
                        for(;;)
                        {
                            ...
                            else
                            {
                                ...
                                // Change NL to NUL to reverse the
                                // effect done below.
                                for(ni = 0; ni < n; ++ni)
                                {
                                    if(p[ni] == NL)
            ...
            // Also check for a BOM for other Unicode encodings,
            // but not after converting with 'charconvert' or when
                       ...
                       && (*fenc == 'u' || (*fenc == NUL && enc_utf8)))))
            {
                ...
                {
                    ccname = check_for_bom(ptr,
                                           size,
                                           &blen,
                                           fio_flags == FIO_UCSBOM
                                           ? FIO_ALL : get_fio_flags(fenc));
                }
            ...
            if(iconv_fd != (iconv_t)-1)
            {
                // Attempt conversion of the read
                // bytes to 'encoding' usin iconv().
                const char *fromp;
                char *top;
                size_t from_size;
                size_t to_size;
                ...
                while((iconv(iconv_fd, (void *)&fromp, &from_size, &top, &to_size)
            ...
            if(fio_flags != 0)
            {
                ...
                // If there is a trailing incomplete
                // sequence move it to conv_rest[].
                if(tail != NULL)
                {
...
rewind_retry:
            ...
            if(fileformat == EOL_UNKNOWN)
            {
                ...
                if(try_dos || try_unix)
                {
                    ...
                    {
                        // Looking for CR but found no end-of-line markers
                        // at all: use the default format.
                        ...
                        *ptr = NUL; // end of line
                        ...
                        if(fileformat == EOL_DOS)
                        {
                            ...
                            else if(ff_error != EOL_DOS)
                            {
                                ...
                                // When 'fileformats' includes "unix",
                                // delete all the lines read so far and start
                                // all over again. Otherwise give an error
                                // message later.
                                if(try_unix
                                   && !read_stdin
                                   && (read_buffer
...
failed:
    ...
    if(!recoverymode)
    {
        ...
        if(newfile || read_buffer)
        {
            ...
            // After reading the text into the
            // buffer the diff info needs to be updated.
            diff_invalidate(curbuf);

            // All folds in the window are invalid now.
            // Mark them for update before triggering autocommands.
        ...
        {
            add_quoted_fname((char *)IObuff, IOSIZE,
                             curbuf, (const char *)sfname);
            ...
            {
                ustrcat(IObuff, _("[character special]"));
                c = TRUE;
            }
        #endif
    #endif
            ...
            if(curbuf->b_p_ro)
            {
                ustrcat(IObuff, shortmess(SHM_RO) ? _("[RO]") : _("[readonly]"));
                c = TRUE;
            }
            ...
            if(ff_error == EOL_DOS)
            {
                ustrcat(IObuff, _("[CR missing]"));
                c = TRUE;
            }

            if(split)
            {
                ustrcat(IObuff, _("[long lines split]"));
                c = TRUE;
            }

            if(notconverted)
            {
                ustrcat(IObuff, _("[NOT converted]"));
                c = TRUE;
            }
            else if(converted)
            {
                ustrcat(IObuff, _("[converted]"));
                c = TRUE;
            }
            ...
            if(conv_error != 0)
            {
                sprintf((char *)IObuff + ustrlen(IObuff),
                        _("[CONVERSION ERROR in line %" PRId64 "]"),
                        (int64_t)conv_error);
    ...
    // When reloading a buffer put the
    // cursor at the first line that is different.
    ...
    if(!read_stdin && !read_buffer)
    {
        ...
        if(filtering)
        {
            apply_autocmds_exarg(EVENT_FILTERREADPOST, NULL,
                                 sfname, false, curbuf, eap);
        }
        else if(newfile)
        {
            apply_autocmds_exarg(EVENT_BUFREADPOST, NULL,
                                 sfname, false, curbuf, eap);
        ...
        {
            apply_autocmds_exarg(EVENT_FILEREADPOST, sfname,
                                 sfname, false, NULL, eap);
        }
...
void prep_exarg(exargs_st *eap, filebuf_st *buf)
{
    eap->cmd = xmalloc(ustrlen(buf->b_p_ff) + ustrlen(buf->b_p_fenc) + 15);
    sprintf((char *)eap->cmd, "e ++ff=%s ++enc=%s", buf->b_p_ff, buf->b_p_fenc);
    eap->force_enc = 14 + (int)ustrlen(buf->b_p_ff);
...
static uchar_kt *readfile_charconvert(uchar_kt *fname, uchar_kt *fenc, int *fdp)
{
    ...
    // If the input file is closed,
    // open it (caller should check for error).
    if(*fdp < 0)
    {
...
/// write to file "fname" lines "start" through "end"
///
...
#define SET_ERRMSG(msg)              errnum = NULL, errmsg = msg, errmsgarg = 0
    ...
    {
        // This can happen during startup when
        // there is a stray "w" in the vimrc file.
        EMSG(_(e_emptybuf));
        return FAIL;
    }

    // Disallow writing from .exrc and .vimrc in
    ...
    // After writing a file changedtick changes
    // but we don't want to display the line.
    ex_no_reprint = TRUE;
    ...
    if(buf->b_ffname == NULL
       ...
       && !bt_nofile(buf)
       && !filtering
       && (!append || ustrchr(p_cpo, CPO_FNAMEAPP) != NULL)
       && ustrchr(p_cpo, CPO_FNAMEW) != NULL)
    {
        if(set_rw_fname(fname, sfname) == FAIL)
        {
            return FAIL;
    ...
    buf->b_op_end.col = 0;
    {
        ...
        if(append)
        {
            if(!(did_cmd = apply_autocmds_exarg(EVENT_FILEAPPENDCMD, sfname,
                                                sfname, FALSE, curbuf, eap)))
            {
                ...
                {
                    apply_autocmds_exarg(EVENT_FILEAPPENDPRE, sfname,
                                         sfname, FALSE, curbuf, eap);
                }
            }
        }
        ...
        {
            apply_autocmds_exarg(EVENT_FILTERWRITEPRE, NULL,
                                 sfname, FALSE, curbuf, eap);
        }
        else if(reset_changed && whole)
        {
            int was_changed = curbufIsChanged();
            did_cmd = apply_autocmds_exarg(EVENT_BUFWRITECMD, sfname,
                                           sfname, FALSE, curbuf, eap);
            ...
            else
            {
                ...
                {
                    apply_autocmds_exarg(EVENT_BUFWRITEPRE, sfname,
                                         sfname, FALSE, curbuf, eap);
                }
            }
        }
        ...
        {
            if(!(did_cmd = apply_autocmds_exarg(EVENT_FILEWRITECMD, sfname,
                                                sfname, FALSE, curbuf, eap)))
            {
                if(overwriting && bt_nofile(curbuf))
                {
                ...
                {
                    apply_autocmds_exarg(EVENT_FILEWRITEPRE, sfname,
                                         sfname, FALSE, curbuf, eap);
                }
            }
        }
           ...
           || aborting())
        {
            ...
            if(did_cmd)
            {
                ...
                // We assume it was written (can't retry anyway).
                if(buf == NULL)
                {
                    return OK;
                }
                ...
                if(reset_changed && buf->b_changed && !append
                   && (overwriting || ustrchr(p_cpo, CPO_PLUS) != NULL))
                {
                    // Buffer still changed, the
                    // autocommands didn't work properly.
                    return FAIL;
                }
        ...
        if(buf->b_ml.ml_line_count != old_line_count)
        {
            ...
            else // less lines
            {
                ...
                if(end < start)
                {
                    ...
                    EMSG(_("E204: Autocommand changed number "
                           "of lines in unexpected way"));
        ...
        // The autocommands may have changed the name of
        // the buffer, which may be kept in fname, ffname and sfname.
        if(buf_ffname)
        {
            ffname = buf->b_ffname;
        }
    ...
    // can't allocate big buffer, use small one
    // (to be able to write when out of memory)
    if(buffer == NULL)
    {
        buffer = smallbuf;
        bufsize = SMBUFSIZE;
    ...
    if(!device && !newfile)
    {
        ...
        {
            if(ustrchr(p_cpo, CPO_FWRITE) != NULL)
            {
    ...
    // interruption cancel writing, only hitting CTRL-C while writing
    // should abort it.
        ...
        if(backup_copy && (fd = os_open((char *)fname, O_RDONLY, 0)) >= 0)
        {
            ...
            while(*dirp)
            {
                ...
                fileinfo_st file_info_new;
                {
                    ...
                    if(os_fileinfo((char *)backup, &file_info_new))
                    {
                        if(os_fileinfo_id_equal(&file_info_new, &file_info_old))
                        {
                            ...
                            // May happen when modname() gave the same file
                            // back (e.g. silly link). If we don't check here,
                            // we either ruin the file when copying or erase
                            // it after writing.
                            xfree(backup);
                        ...
                        {
                            // We are not going to keep the backup file,
                            // so don't delete an existing one, and try to
                            // use another name instead. Change one character,
                            // just before the extension.
                            wp = backup + ustrlen(backup) - 1 - ustrlen(backup_ext);
                ...
                if(backup != NULL)
                {
                    ...
                    {
                        // set file protection same as
                        // original file, but strip s-bit.
                        (void)os_setperm((const char *)backup, perm & 0777);

                    #ifdef UNIX
                        // Try to set the group of the backup same as the
                        // original file. If this fails, set the protection
                        // bits for the group same as the protection bits
                        // for others.
                        ...
                        while((write_info.bw_len = read_eintr(fd,
                                                              copybuf,
                                                              BUFSIZE)) > 0)
                        {
                        ...
                        if((error = os_close(bfd)) != 0 && errmsg == NULL)
                        {
                            SET_ERRMSG_ARG(_("E507: Close error for backup file "
                                             "(add ! to override): %s"), error);
                        }

                        if(write_info.bw_len < 0)
                        {
                            SET_ERRMSG(_("E508: Can't read file for backup "
                                         "(add ! to override)"));
                        }
...
nobackup:
        ...
        else
        {
            ...
            if(file_readonly && ustrchr(p_cpo, CPO_FWRITE) != NULL)
            {
                SET_ERRMSG_NUM("E504", _(err_readonly));
                goto fail;
            }
            ...
            while(*dirp)
            {
                ...
                if(backup != NULL)
                {
                    ...
                    {
                        p = backup + ustrlen(backup) - 1 - ustrlen(backup_ext);
    ...
    if(forceit && overwriting && ustrchr(p_cpo, CPO_KEEPRO) == NULL)
    {
        buf->b_p_ro = FALSE;
    ...
    if(converted && (enc_utf8 || ustrcmp(p_enc, "latin1") == 0))
    {
        ...
        if(wb_flags & (FIO_UCS2 | FIO_UCS4 | FIO_UTF16 | FIO_UTF8))
        {
            ...
            write_info.bw_conv_buf = xmalloc(write_info.bw_conv_buflen);
                        ...
                        perm < 0 ? 0666 : (perm & 0777))) < 0)
    {
        ...
        if(errmsg == NULL)
        {
            ...
            else
            {
                ...
                if(forceit && ustrchr(p_cpo, CPO_FWRITE) == NULL && perm >= 0)
                {
                #ifdef UNIX
                    // we write to the file, thus it
                    // should be marked writable after all
                    if(!(perm & 0200))
                    {
                        made_writable = TRUE;
                    }
...
restore_backup:
    ...
    {
        // The next while loop is done once for each
        // character written. Keep it fast!
        ptr = ml_get_buf(buf, lnum, FALSE) - 1;

        if(write_undo_file)
        {
            sha256_update(&sha_ctx, ptr + 1, (uint32_t)(ustrlen(ptr + 1) + 1));
        }
        ...
        if(errmsg == NULL)
        {
            if(write_info.bw_conv_error)
            {
                ...
                else
                {
                    ...
                    xsnprintf(errmsg, 300,
                              _("E513: write error, conversion failed in "
                                "line %" LineNumKtPrtFmt
                                " (make 'fenc' empty to override)"),
    ...
    if(!filtering)
    {
        ...
        if(write_info.bw_conv_error)
        {
            ustrcat(IObuff, _(" CONVERSION ERROR"));
            c = TRUE;

            if(write_info.bw_conv_error_lnum != 0)

                xvsnprintf_add((char *)IObuff,
                               IOSIZE,
                               _(" in line %" PRId64 ";"),
        ...
        if(device)
        {
            ustrcat(IObuff, _("[Device]"));
            c = TRUE;
        }
        else if(newfile)
        {
            ustrcat(IObuff, shortmess(SHM_NEW) ? _("[New]") : _("[New File]"));
            c = TRUE;
        }
        ...
        if(!shortmess(SHM_WRITE))
        {
            if(append)
            {
                ustrcat(IObuff, shortmess(SHM_WRI) ? _(" [a]") : _(" appended"));
            }
            else
            {
                ustrcat(IObuff, shortmess(SHM_WRI) ? _(" [w]") : _(" written"));
            }
        }
    ...
    if(*p_pm && dobackup)
    {
        ...
        // remember that a (new) file was created.
        else
        {
            int empty_fd;
...
nofail:
    ...
    if(errmsg != NULL)
    {
        ...
        if(errnum != NULL)
        {
            if(errmsgarg != 0)
            {
                emsgf("%s: %s%s: %s", errnum,
                      IObuff, errmsg, os_strerror(errmsgarg));
            }
            else
            {
    ...
    // When writing the whole file and 'undofile' is set,
    ...
    if(!should_abort(retval))
    {
        ...
        if(append)
        {
            apply_autocmds_exarg(EVENT_FILEAPPENDPOST, fname,
                                 fname, FALSE, curbuf, eap);
        }
        else if(filtering)
        {
            apply_autocmds_exarg(EVENT_FILTERWRITEPOST, NULL,
                                 fname, FALSE, curbuf, eap);
        }
        else if(reset_changed && whole)
        {
            apply_autocmds_exarg(EVENT_BUFWRITEPOST, fname,
                                 fname, FALSE, curbuf, eap);
        }
        else
        {
            apply_autocmds_exarg(EVENT_FILEWRITEPOST, fname,
                                 fname, FALSE, curbuf, eap);
        }
...
/// @param[in]  buf  filebuf_st file name is coming from.
...
static bool msg_add_fileformat(int eol_type)
{
    ...
    if(eol_type == EOL_DOS)
    {
        ustrcat(IObuff, shortmess(SHM_TEXT) ? _("[dos]") : _("[dos format]"));
        return true;
    }
#endif
    ...
    if(eol_type == EOL_MAC)
    {
        ustrcat(IObuff, shortmess(SHM_TEXT) ? _("[mac]") : _("[mac format]"));
        return true;
    }
    ...
    {
        ustrcat(IObuff, shortmess(SHM_TEXT) ? _("[unix]") : _("[unix format]"));
        return true;
    }
#endif
...
static int check_mtime(filebuf_st *buf, fileinfo_st *file_info)
{
...
static int buf_write_bytes(bwinfo_st *ip)
{
    ...
    if(!(flags & FIO_NOCONVERT))
    {
        ...
        if(ip->bw_iconv_fd != (iconv_t)-1)
        {
            ...
            if((iconv(ip->bw_iconv_fd,
                      (void *)&from,
                      &fromlen,
                      &to,
                      &tolen) == (size_t)-1 && ICONV_ERRNO != ICONV_EINVAL)
...
/// Return true if file encoding "fenc" requires
/// conversion from or to 'encoding'.
///
...
FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    if(*fenc == NUL || ustrcmp(p_enc, fenc) == 0)
    {
        same_encoding = TRUE;
        fenc_flags = 0;
    }
    ...
    {
        // Ignore difference between "ansi" and
        // "latin1", "ucs-4" and "ucs-4be", etc.
...
/// 'name' was unicode or latin1, otherwise 0. If "name" is an
...
/// When "force" is FALSE: Only try to shorten absolute file names. For buffers
...
void shorten_fnames(int force)
{
    ...
    FOR_ALL_BUFFERS(buf)
    {
        ...
        // Always make the swap file name a full path,
        // a "nofile" buffer may also have a swap file.
...
/// If NULL, use current directory name and ext
/// to compute new filename.
///
/// @param ext
/// The extension to add to the filename. 4 chars
/// max if prefixed with a dot, 3 otherwise.
///
/// @param prepend_dot
/// If true, prefix ext with a dot. Does nothing if
/// ext already starts with a dot, or if fname is NULL.
///
/// @return [allocated]
/// - A new filename, made up from:
...
FUNC_ATTR_NONNULL_ARG(2)
{
    ...
    // If there is no file name we must get the name of the
    ...
    // Check that, after appending the extension,
    // the file name is really different.
...
/// Like fgets(), but if the file line is too long,
/// it is truncated and the rest of the line is thrown away.
///
...
int vim_rename(const uchar_kt *from, const uchar_kt *to)
{
    ...
    {
        if(p_fic && (ustrcmp(path_tail((uchar_kt *)from), path_tail((uchar_kt *)to))
                     != 0))
        {
            use_tmp_file = true;
        }
    ...
    if(use_tmp_file)
    {
        ...
        if(ustrlen(from) >= MAXPATHL - 5)
        {
            return -1;
        }

        ustrcpy(tempname, from);
        ...
        for(n = 123; n < 99999; n++)
        {
            ...
            snprintf(tail,
                     (MAXPATHL + 1) - (tail - (char *)tempname - 1),
                     "%d",
                     n);
    ...
    // Avoid xmalloc() here as vim_rename() is
    // called by buf_write() when nvim is preserve_exit().
    buffer = xmalloc(BUFSIZE);
...
#ifndef UNIX
    // For Unix os_open() already set the permission.
...
/// a global command is being executed, a mapping is being executed
/// or an autocommand is busy.
...
int check_timestamps(int focus)
{
    ...
    // Don't check timestamps while system() or another
    // low-level function may cause us to lose and gain focus.
    ...
    else
    {
        ...
        FOR_ALL_BUFFERS(buf)
        {
            ...
            if(buf->b_nwindows > 0)
            {
                ...
                {
                    // Autocommands have removed the buffer,
...
static int move_lines(filebuf_st *frombuf, filebuf_st *tobuf)
{
    filebuf_st *tbuf = curbuf;
    int retval = OK;
    linenum_kt lnum;
    uchar_kt *p;
    ...
    {
        p = ustrdup(ml_get_buf(frombuf, lnum, FALSE));
    ...
    if(retval != FAIL)
    {
        ...
        for(lnum = curbuf->b_ml.ml_line_count; lnum > 0; --lnum)
        {
            ...
            {
                // Oops!  We could try putting back
                // the saved lines, but that might fail again...
                retval = FAIL;
                break;
            }
        }
...
int buf_check_timestamp(filebuf_st *buf, int FUNC_ARGS_UNUSED_REALY(focus))
{
           ...
           || (int)file_info.stat.st_mode != buf->b_orig_mode))
    {
        ...
        // Might contain the file explorer.
        ...
        else
        {
            ...
            // autocommands. Avoid being called recursively by setting "busy".
            busy = true;
            ...
            if(n)
            {
                ...
                if(ustrcmp(s, "reload") == 0 && *reason != 'd')
                {
                    reload = TRUE;
                }
                else if(ustrcmp(s, "ask") == 0)
                {
                    n = FALSE;
                }
                else
            ...
            if(!n)
            {
                ...
                else
                {
                    ...
                    // giving a warning when only the timestamp was set
                    // (e.g., checked out of CVS).
                    // Always warn when the buffer was changed.
                    if(reason[2] == 'n')
                    {
                        mesg = _("W12: Warning: File \"%s\" has changed and "
                                 "the buffer was changed in Vim as well");
                    ...
                    {
                        mesg = _("W16: Warning: Mode of file \"%s\" has "
                                 "changed since editing started");
                        mesg2 = _("See \":help W16\" for more info.");
                    }
                    else
                    {
                        // Only timestamp changed, store it to
    ...
    if(mesg != NULL)
    {
        ...
        const size_t tbuf_len = ustrlen(path) + ustrlen(mesg) + ustrlen(mesg2) + 2;
        char *const tbuf = xmalloc(tbuf_len);
        snprintf(tbuf, tbuf_len, mesg, path);

        // Set warningmsg here, before the unimportant
        // and output-specific mesg2 has been appended.
        ...
        if(can_reload)
        {
            ...
            {
                xstrncat(tbuf, "\n", tbuf_len - 1);
                xstrncat(tbuf, mesg2, tbuf_len - 1);
            }
...
void buf_reload(filebuf_st *buf, int orig_mode)
{
    exargs_st ea;
    apos_st old_cursor;
    ...
    // Restore the topline and cursor position
    // and check it (lines may have been removed).
...
void buf_store_file_info(filebuf_st *buf, fileinfo_st *file_info)
FUNC_ATTR_NONNULL_ALL
{
...
static void vim_maketempdir(void)
{
    ...
    for(size_t i = 0; i < ARRAY_SIZE(temp_dirs); i++)
    {
        ...
        if(!os_isdir(template_buf)) // directory doesn't exist
        {
            continue;
        }

        add_pathsep((char *)template_buf);

        // Concatenate with temporary directory name pattern
        ustrcat(template_buf, "nvimXXXXXX");

        if(os_mkdtemp((const char *)template_buf, (char *)path) != 0)
        {
            continue;
        }
        ...
        {
            // Successfully created and set
            // temporary directory so stop trying.
            break;
        }
        else
        {
            // Couldn't set `vim_tempdir` to
            // `path` so remove created directory.
            os_rmdir((char *)path);
        }
    }
...
void do_augroup(uchar_kt *arg, int del_group)
{
    ...
    else if(ustricmp(arg, "end") == 0) // ":aug end": back to group 0
    {
...
static auto_event_et event_name2nr(const uchar_kt *start, uchar_kt **end)
{
    ...
    for(i = 0; event_names[i].name != NULL; i++)
    {
        ...
        if(len == p - start && ustrnicmp(event_names[i].name, start, len) == 0)
        {
            break;
        }
    }
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    while(*p != NUL)
    {
        if(ustrnicmp(p, "all", 3) == 0 && (p[3] == NUL || p[3] == ','))
        {
            return true;
        }
...
uchar_kt *au_event_disable(char *what)
{
    uchar_kt *new_ei;
    uchar_kt *save_ei;
    save_ei = ustrdup(p_ei);
    new_ei = ustrndup(p_ei, (int)(ustrlen(p_ei) + ustrlen(what)));

    if(*what == ',' && *p_ei == NUL)
    {
        ustrcpy(new_ei, what + 1);
    }
    else
    {
        ustrcat(new_ei, what);
...
/// implements the :autocmd command. Can be used in the following ways:
///
/// - :autocmd <event> <pat> <cmd>
/// Add <cmd> to the list of commands that will be automatically executed
/// for <event> when editing a file matching <pat>, in the current group.
///
/// - :autocmd <event> <pat>
/// Show the auto-commands associated with <event> and <pat>.
///
/// - :autocmd <event>
/// Show the auto-commands associated with <event>.
///
/// - :autocmd
...
/// and add the command <cmd>, for the current group.
///
/// - :autocmd! <event> <pat>
/// Remove all auto-commands associated with <event> and <pat>
...
///
/// - :autocmd! <event>
/// Remove all auto-commands associated with <event> for the current group.
/// - :autocmd!
/// Remove ALL auto-commands for the current group.
///
/// Multiple events and patterns may be given separated by commas.
/// Here are some examples:
/// - :autocmd bufread,bufenter *.c,*.h
/// set tw=0 smartindent noic
///
/// - :autocmd bufleave *
/// set tw=79 nosmartindent ic infercase
///
/// - :autocmd * *.c
...
void do_autocmd(uchar_kt *arg_in, int forceit)
{
    ...
    else
    {
        ...
        // Set 'shellslash', we want forward slashes here.
        if(ustrchr(pat, '$') != NULL || ustrchr(pat, '~') != NULL)
        {
        ...
        if(*cmd != NUL && ustrncmp(cmd, "nested", 6) == 0
           && ascii_iswhite(cmd[6]))
        {
            nested = true;
            cmd = skipwhite(cmd + 6);
...
static int au_get_grouparg(uchar_kt **argp)
{
    ...
    if(p > arg)
    {
        group_name = ustrndup(arg, (int)(p - arg));
                            ...
                            int group)
{
    ...
    for(; *pat; pat = (*endpat == ',' ? endpat + 1 : endpat))
    {
        ...
        for(/* nothing */;
            *endpat && (*endpat != ',' || brace_level || endpat[-1] == '\\');
            ++endpat)
        {
            if(*endpat == '{')
            {
        ...
        if(patlen >= 8 && ustrncmp(pat, "<buffer", 7) == 0
           && pat[patlen - 1] == '>')
        {
            ...
            {
                if(patlen == 13 && ustrnicmp(pat, "<buffer=abuf>", 13) == 0)
                {
                    // "<buffer=abuf>"
        ...
        if(is_buflocal)
        {
            ...
            patlen = (int)ustrlen(buflocal_pat); // but not endpat
        }
        ...
        if(*cmd != NUL)
        {
            ...
            if(ap == NULL)
            {
                ...
                else
                {
                    ...
                    if(reg_pat != NULL)
                    {
                        ap->reg_prog = regexp_compile(reg_pat, RE_MAGIC);
                    }
...
void aucmd_prepbuf(save_autocmd_st *aco, filebuf_st *buf)
{
    ...
    {
        // Strange recursive autocommand, fall back to
        // using the current window. Expect a few side effects...
        win = curwin;
    }
...
void aucmd_restbuf(save_autocmd_st *aco)
{
    ...
    if(aco->use_aucmd_win)
    {
        ...
        // Find "aucmd_win", it can't be closed, but it
        // may be in another tab page. Do not trigger autocommands here.
        block_autocmds();
...
win_found:
        ...
        {
            // Hmm, original window disappeared.
        ...
        check_cursor(); // the buffer contents may have changed
    ...
    else
    {
        ...
        if(win_valid(aco->save_curwin))
        {
            // Restore the buffer which was previously edited by curwin,
            // if it was changed, we are still the same window and the
            // buffer is valid.
            ...
            // In case the autocommand moves the cursor to
            // a position that does not exist in curbuf
            check_cursor();
        }
    }
}
...
/// This takes care of setting v:filearg.
///
/// @param event event that occured
                           ...
                           int *retval)
{
    ...
    bool did_cmd = apply_autocmds_group(event,
                                        fname,
                                        fname_io,
                                        force,
                                        AUGROUP_ALL,
...
/// Return true when there is a CursorHold/CursorHoldI
/// autocommand defined for the current mode.
...
///
/// @param fname
/// filename, NULL or empty means use actual file name
///
/// @param fname_io
/// filename to use for <afile> on cmdline, NULL means use `fname`.
///
/// @param force
/// When true, ignore autocmd_busy
///
/// @param group
/// autocmd group ID or AUGROUP_ALL
///
/// @param buf
/// Buffer for <abuf>
///
                                 ...
                                 exargs_st *eap)
{
    ...
    // for this event or autocommands are blocked.
    if(first_autopat[(int)event] == NULL || autocmd_blocked > 0)
    {
        goto BYPASS_AU;
    }

    // When autocommands are busy, new autocommands are only
    // executed when explicitly enabled with the "nested" flag.
    if(autocmd_busy && !(force || autocmd_nested))
    {
        goto BYPASS_AU;
    }

    // Quickly return when immediately aborting on error, or when
    // an interrupt occurred or an exception was thrown but not caught.
    if(aborting())
    {
        goto BYPASS_AU;
    }
    ...
    // Allow nesting of autocommands, but restrict the depth,
    // because it's possible to create an endless loop.
    if(nesting == 10)
    {
    ...
    // Make a copy to avoid that changing a buffer
    // name or directory makes it invalid.
    if(fname_io == NULL)
    {
    ...
    {
        autocmd_fname = ustrdup(autocmd_fname);
    }
    ...
    if(fname == NULL || *fname == NUL)
    {
        ...
        fname = ustrdup(fname); // make a copy, so we can change it
    }
    else
    {
        sfname = ustrdup(fname);
    ...
    // Note that we are applying autocmds.
    // Some commands need to know.
    autocmd_busy = TRUE;
    filechangeshell_busy = (event == EVENT_FILECHANGEDSHELL);
    ++nesting; // see matching decrement below

    // Remember that FileType was triggered.
    // Used for did_filetype().
    ...
    if(patcmd.curpat != NULL)
    {
        ...
        // mark the last pattern, to avoid an endless loop when
        // more patterns are added when executing autocommands
    ...
    if(!autocmd_busy)
    {
        ...
        {
            filebuf_st *b = au_pending_free_buf->b_next;
...
BYPASS_AU:
    ...
    // When wiping out a buffer make sure all its
    // buffer-local autocommands are deleted.
           ...
           && (apc->group == AUGROUP_ALL || apc->group == ap->group))
        {
               ...
               : ap->buflocal_nr == apc->arg_bufnr)
            {
                ...
                const size_t sourcing_name_len =
                    (ustrlen(s) + strlen(name) + ap->patlen + 1);

                sourcing_name = xmalloc(sourcing_name_len);

                snprintf((char *)sourcing_name,
                         sourcing_name_len,
                         s,
                         name,
                         (char *)ap->pat);
...
/// Return true if there is a matching autocommand for @b fname.
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    // This makes the autocommand patterns portable between Unix and Windows.
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    if(group == AUGROUP_ERROR)
    {
        // Didn't match a group name, assume
    ...
    if(pattern != NULL && ustricmp(pattern, "<buffer>") == 0)
    {
        buflocal_buf = curbuf;
    }
                           ...
                           int allow_dirs)
{
    ...
    regmatch.rm_ic = p_fic; // ignore case if 'fileignorecase' is set
    {
        ...
        {
            regmatch.regprog = regexp_compile(pattern, RE_MAGIC);
        }
    }
...
FUNC_ATTR_NONNULL_ARG(1, 3)
{
    ...
    while(*p)
    {
        ...
        match = match_file_pat(regpat, NULL, ffname,
                               sfname, tail, (int)allow_dirs);
...
/// allow_dirs, otherwise FALSE is put there.
        ...
        switch(*p)
        {
            ...
            case '\\':
                ...
                if(!no_bslash)
                {
                    ...
                    // "\?" to "\\."  e.g., "dir\??.c"
                    // "\+" to "\+"   e.g., "fileX\+.c"
                    if((is_file_name_char(p[1]) || p[1] == '*' || p[1] == '?')
                       && p[1] != '+')
                    {
                        reg_pat[i++] = '[';
                        reg_pat[i++] = '\\';
                ...
                // foo\,bar -> foo,bar
                // foo\ bar -> foo bar
                // Don't unescape \, * and others that are also special
                // in a regexp. An escaped { must be unescaped since we
                // use magic not verymagic. Use "\\\{n,m\}"" to get "\{n,m}".
            #ifdef BACKSLASH_IN_FILENAME
                if(*++p == '?' && no_bslash)
            #else
                if(*++p == '?')
...
#if defined(EINTR)
/// Version of read() that retries when
/// interrupted by EINTR (possibly by a SIGWINCH).
...
/// Version of write() that retries when interrupted
/// by EINTR (possibly by a SIGWINCH).
long write_eintr(int fd, void *buf, size_t bufsize)
{
    long ret = 0;
    long wlen;

    // Repeat the write() so long it didn't fail,
    // other than being interrupted by a signal.
