/// @file nvim/ex_getln.c
///
/// Functions for entering and editing an Ex command line.
...
/// These need to be saved when using CTRL-R |, that's why they are
/// in a structure.
struct cmdline_info
{
    uchar_kt *cmdbuff;    ///< pointer to command line buffer
    ...
    uchar_kt *cmdprompt;  ///< message in front of cmdline
...
typedef struct command_line_state
{
    ...
    uchar_kt *lookfor;   ///< string to match
    ...
    if(ccline.cmdbuff != NULL)
    {
           ...
           && (s->some_key_typed || s->histype == kHistSearch))
        {
            ...
            if(s->firstc == ':')
            {
                ...
                new_last_cmdline = ustrdup(ccline.cmdbuff);
            }
        }
...
static int cmdline_execute(nvim_state_st *state, int key)
{
        ...
        || s->xpc.xp_context == EXPAND_SHELLCMD) && p_wmnu)
    {
        ...
        else if(ustrncmp(s->xpc.xp_pattern, upseg + 1, 3) == 0
                && s->c == K_DOWN)
        {
        ...
        else if(s->c == K_UP)
        {
            ...
            else if(ustrncmp(ccline.cmdbuff + s->j, upseg, 4) == 0)
            {
                s->j += 4;
            }
            else if(ustrncmp(ccline.cmdbuff + s->j, upseg + 1, 3) == 0
                    && s->j == s->i)
            {
                s->j += 3;
            }
    ...
    if(s->c == Ctrl_BSL)
    {
        ...
        if(s->c != Ctrl_N
           && s->c != Ctrl_G
           && (s->c != 'e' || (ccline.cmdfirstc == '=' && KeyTyped)))
        {
            vungetc(s->c);
            s->c = Ctrl_BSL;
        }
    ...
    if(s->c == '\n'
       ...
       || s->c == K_KENTER
       || (s->c == ESC
           && (!KeyTyped || ustrchr(p_cpo, CPO_ESC) != NULL)))
    {
...
static int command_line_handle_key(CommandLineState *s)
{
    ...
    switch(s->c)
    {
        ...
        case Ctrl_P: // previous match
            ...
            {
                if(nextwild(&s->xpc,
                            (s->c == Ctrl_P) ? WILD_PREV : WILD_NEXT,
                            0,
                            s->firstc != '@') == FAIL)
                {
                    break;
        ...
        case K_KPAGEDOWN:
            ...
            if(s->lookfor == NULL)
            {
                s->lookfor = ustrdup(ccline.cmdbuff);
                s->lookfor[ccline.cmdpos] = NUL;
            }

            s->j = (int)ustrlen(s->lookfor);
            ...
            for(;;)
            {
                ...
                if(s->hiscnt < 0
                   || history[s->histype][s->hiscnt].hisstr == NULL)
                {
                    s->hiscnt = s->i;
                    break;
                }
                ...
                if((s->c != K_UP && s->c != K_DOWN)
                   || s->hiscnt == s->i
                   || ustrncmp(history[s->histype][s->hiscnt].hisstr,
            ...
            if(s->hiscnt != s->i)
            {
                ...
                if(s->histype == kHistSearch
                   && p != s->lookfor
                   && (old_firstc = p[ustrlen(p) + 1]) != s->firstc)
                {
                ...
                {
                    alloc_cmdbuff((int)ustrlen(p));
                    ustrcpy(ccline.cmdbuff, p);
                }

                ccline.cmdpos = ccline.cmdlen = (int)ustrlen(ccline.cmdbuff);
                redrawcmd();
                return command_line_changed(s);
            }
...
/// @param attr        attributes for prompt
/// @param xp_context  type of expansion
/// @param xp_arg      user-defined expansion argument
///
/// @return the command line in allocated memory, or NULL.
///
uchar_kt *getcmdline_prompt(int firstc,
                          uchar_kt *prompt,
                          int attr,
                          int xp_context,
                          uchar_kt *xp_arg)
...
uchar_kt *get_text_locked_msg(void)
{
    if(cmdwin_type != 0)
    {
        return e_cmdwin;
...
/// Check if "allbuf_lock" is set and return TRUE when
/// it is and give an error message.
...
/// Compute the offset of the cursor on the command
/// line for the prompt and indent.
                  ...
                  int indent)
{
    ...
    // remove ':' that's in front of each line.
...
///
/// @param promptc       normally ':', NUL for ":append" and '?' for :s prompt
/// @param UNUSED_cookie
/// @param indent        indent for inside conditionals
///
...
redraw:
                ...
                while(p < (uchar_kt *)line_ga.ga_data + line_ga.ga_len)
                {
                    if(*p == TAB)
                    {
                        do
        ...
        {
            msg_outtrans_len(((uchar_kt *)line_ga.ga_data) + line_ga.ga_len, len);
            vcol += char2cells(c1);
        }
        ...
        escaped = FALSE;
        ui_cursor_goto(msg_row, msg_col);
        pend = (uchar_kt *)(line_ga.ga_data) + line_ga.ga_len;
    ...
    return (uchar_kt *)line_ga.ga_data;
}
...
{
    // give some extra space to avoid
    // having to allocate all the time
    if(len < 80)
    {
        len = 100;
    }
...
static void realloc_cmdbuff(int len)
{
       ...
       && ccline.xpc->xp_context != EXPAND_UNSUCCESSFUL)
    {
        ...
        // If xp_pattern points inside the old cmdbuff it needs to
        // be adjusted to point into the newly allocated memory.
...
static uchar_kt   *arshape_buf = NULL;
...
static void draw_cmdline(int start, int len)
{
    ...
    else if(p_arshape && !p_tbidi && len > 0)
    {
        ...
        // Do arabic shaping into a temporary buffer.
        // This is very inefficient!
        if(len * 2 + 2 > buflen)
        {
            // Re-allocate the buffer.
            // We keep it around to avoid a lot of alloc()/free() calls.
            xfree(arshape_buf);
            buflen = len * 2 + 2;
...
/// If len is -1, then ustrlen() is used to calculate the length.
/// If 'redraw' is TRUE then the new part of the command line, and the remaining
/// part will be redrawn, otherwise it will not.  If this function is called
/// twice in a row, then 'redraw' should be FALSE and redrawcmd() should be
/// called afterwards.
void put_on_cmdline(uchar_kt *str, int len, int redraw)
{
    int i;
    int m;
    int c;
...
uchar_kt *save_cmdline_alloc(void)
{
...
void restore_cmdline_alloc(uchar_kt *p)
{
...
static bool cmdline_paste(int regname, bool literally, bool remcr)
{
    ...
    // check for valid regname; also accept special
    // characters for CTRL-R in the command line
    if(regname != Ctrl_F
       && regname != Ctrl_P
       && regname != Ctrl_W
       && regname != Ctrl_A
    ...
    if(i)
    {
        ...
        // When 'incsearch' is set and CTRL-R CTRL-W used:
        // skip the duplicate part of the word.
        p = arg;
        ...
        if(p_is && regname == Ctrl_W)
        {
            ...
            if(p_ic ? ustrnicmp(w, arg, len) == 0 : ustrncmp(w, arg, len) == 0)
            {
                p += len;
            }
        }
...
/// - When "literally" is FALSE, insert as typed,
///   but don't leave the command line.
void cmdline_paste_str(uchar_kt *s, int literally)
{
    int c, cv;
...
void redrawcmd(void)
{
    ...
    // when 'incsearch' is set there may
    // be no command line while redrawing
    if(ccline.cmdbuff == NULL)
    {
...
{
    uchar_kt *p1 = *(uchar_kt **)s1;
    uchar_kt *p2 = *(uchar_kt **)s2;
...
static int nextwild(expand_st *xp, int type, int options, int escape)
{
    ...
    else
    {
        ...
        else
        {
            ...
            p2 = ExpandOne(xp,
                           p1,
                           ustrndup(&ccline.cmdbuff[i], xp->xp_pattern_len),
                           use_options,
                           type);
            xfree(p1);
    ...
    if(p2 != NULL && !got_int)
    {
        difflen = (int)ustrlen(p2) - xp->xp_pattern_len;
        ...
        memmove(&ccline.cmdbuff[i], p2, ustrlen(p2));
                  ...
                  int mode)
{
    uchar_kt *ss = NULL;
    static int findex;
    static uchar_kt *orig_save = NULL; // kept value of orig
    ...
    if(mode == WILD_NEXT || mode == WILD_PREV)
    {
        if(xp->xp_numfiles > 0)
        {
            ...
            // When wrapping around, return the original
            // string, set findex to -1.
            if(findex < 0)
            {
                if(orig_save == NULL)
                {
            ...
            if(findex == -1)
            {
                return ustrdup(orig_save);
            }

            return ustrdup(xp->xp_files[findex]);
        }
        else
        {
            return NULL;
    ...
    if(mode == WILD_LONGEST && xp->xp_numfiles > 0)
    {
        ...
        ss = (uchar_kt *)xstrndup((char *)xp->xp_files[0], len);
    ...
    if(mode == WILD_ALL && xp->xp_numfiles > 0)
    {
        ...
        *ss = NUL;
        ...
        for(i = 0; i < xp->xp_numfiles; ++i)
        {
            ustrcat(ss, xp->xp_files[i]);

            if(i != xp->xp_numfiles - 1)
            {
                ustrcat(ss, (options & WILD_USE_NL) ? "\n" : " ");
            }
        }
    }
    ...
    if(options & WILD_ESCAPE)
    {
           ...
           || xp->xp_context == EXPAND_DIRECTORIES)
        {
            ...
            for(i = 0; i < numfiles; ++i)
            {
                ...
                {
                    p = ustrdup_escape(files[i], (uchar_kt *)" ");
                    xfree(files[i]);
                    files[i] = p;
                ...
                #ifdef BACKSLASH_IN_FILENAME
                p = (uchar_kt *)vim_strsave_fnameescape((const char *)files[i], false);
                #else
                p = (uchar_kt *)vim_strsave_fnameescape((const char *)files[i], xp->xp_shell);
                #endif
                ...
                // If 'str' starts with "\~",
                // replace "~" at start of files[i] with "\~".
...
///
/// @param[in] shell
/// What to escape for: if false, escapes for VimL command,
...
FUNC_ATTR_NONNULL_RETURN
{
    ...
    uchar_kt buf[sizeof(PATH_ESC_CHARS)];
    int j = 0;

    // Don't escape '[', '{' and '!' if they are in 'isfname'.
    for(const char *s = PATH_ESC_CHARS; *s != NUL; s++)
    {
        if((*s != '[' && *s != '{' && *s != '!') || !is_file_name_char(*s))
        {
            buf[j++] = *s;
        }
    }
    ...
    buf[j] = NUL;
    char *p = (char *)ustrdup_escape((const uchar_kt *)fname,
                                      (const uchar_kt *)buf);
#else
    #define PATH_ESC_CHARS   ((uchar_kt *)" \t\n*?[{`$\\%#'\"|!<")
    #define SHELL_ESC_CHARS  ((uchar_kt *)" \t\n*?[{`$\\%#'\"|!<>();&")

    char *p = (char *)ustrdup_escape((const uchar_kt *)fname,
                                      (shell ? SHELL_ESC_CHARS : PATH_ESC_CHARS));

    if(shell && csh_like_shell())
    {
        // For csh and similar shells need to put two backslashes before '!'.
        // One is taken by Vim, one by the shell.
        char *s = (char *)ustrdup_escape((const uchar_kt *)p,
                                              (const uchar_kt *)"!");
        xfree(p);
        p = s;
    }
    ...
    // '>' and '+' are special at the start of some commands,
    // e.g. ":edit" and ":write".  "cd -" has a special meaning.
    if(*p == '>' || *p == '+' || (*p == '-' && p[1] == NUL))
    {
        escape_fname((uchar_kt **)&p);
    }
...
static void escape_fname(uchar_kt **pp)
{
    uchar_kt *p = xmalloc(ustrlen(*pp) + 2);
    p[0] = '\\';
    ustrcpy(p + 1, *pp);
    xfree(*pp);
    *pp = p;
...
void tilde_replace(uchar_kt *orig_pat, int num_files, uchar_kt **files)
{
    int i;
    uchar_kt  *p;
    ...
    else
    {
        ...
        for(i = 0; i < num_files; ++i)
        {
                             ...
                             || xp->xp_context == EXPAND_BUFFERS))
            {
                ...
                j = ustr_scrsize(NameBuff);
            }
            else
            {
                j = ustr_scrsize(L_SHOWFILE(i));
            }
        ...
        for(i = 0; i < lines; ++i)
        {
            ...
            for(k = i; k < num_files; k += lines)
            {
                if(xp->xp_context == EXPAND_TAGS_LISTFILES)
                {
                    ...
                    p = files_found[k] + ustrlen(files_found[k]) + 1;
                   ...
                   || xp->xp_context == EXPAND_BUFFERS)
                {
                    ...
                    if(xp->xp_numfiles != -1)
                    {
                        ...
                        halved_slash = backslash_halve_save(exp_path != NULL
                                                            ? exp_path
                                                            : files_found[k]);
...
uchar_kt *sm_gettail(uchar_kt *s)
{
    uchar_kt *p;
    uchar_kt *t = s;
    int had_sep = FALSE;
    ...
    {
    #ifdef BACKSLASH_IN_FILENAME
        if(vim_ispathsep(*p) && !rem_backslash(p))
    #else
        if(vim_ispathsep(*p))
    #endif
        {
...
static int expand_showtail(expand_st *xp)
{
    ...
    // When not completing file names a
    ...
    for(s = xp->xp_pattern; s < end; s++)
    {
        ...
        else if(ustrchr((uchar_kt *)"*?[", *s) != NULL)
        {
            return FALSE;
        }
    }
...
FUNC_ATTR_NONNULL_RETURN
{
       ...
       && context != EXPAND_DIRECTORIES)
    {
        ...
        // use with regexp_compile(). First work out how long it will be:
        ...
        else
        {
            ...
            retval = xmalloc(new_len);
            {
                ...
                {
                    // Skip backslash.
                    // But why?  At least keep it for custom expansion.
                    if(context != EXPAND_USER_DEFINED
...
/// - xp->xp_pattern
///   - The start of the pattern to be expanded within the
///     command line (ends at the cursor).
/// - xp->xp_context
///   - The type of thing to expand.  Will be one of:
///   - EXPAND_UNSUCCESSFUL
///     - Used sometimes when there is something illegal on
///       the command line, like an unknown command. Caller should beep.
///   - EXPAND_NOTHING
///     - Unrecognised context for completion, use char like a normal char,
///       rather than for completion.  eg :s/^I/
///   - EXPAND_COMMANDS
///     - Cursor is still touching the command, so complete it.
///   - EXPAND_BUFFERS
...
///       directories are of interest.  eg :set dir=^I
///   - EXPAND_SHELLCMD
///     - After ":!cmd", ":r !cmd"  or ":w !cmd".
///   - EXPAND_SETTINGS
///     - Complete variable names.  eg :set d^I
///   - EXPAND_BOOL_SETTINGS
///     - Complete boolean variables only,  eg :set no^I
///   - EXPAND_TAGS
///     - Complete tags from the files in p_tags.  eg :ta a^I
///   - EXPAND_TAGS_LISTFILES
///     - As above, but list filenames on ^D, after :tselect
///   - EXPAND_HELP
///     - Complete tags from the file 'helpfile'/tags
///   - EXPAND_EVENTS
///     - Complete event names
///   - EXPAND_SYNTAX
///     - Complete :syntax command arguments
///   - EXPAND_HIGHLIGHT
///     - Complete highlight (syntax) group names
///   - EXPAND_AUGROUP
///     - Complete autocommand group names
///   - EXPAND_USER_VARS
///     - Complete user defined variable names, eg :unlet a^I
///   - EXPAND_MAPPINGS
///     - Complete mapping and abbreviation names, eg :unmap a^I , :cunab x^I
///   - EXPAND_FUNCTIONS
///     - Complete internal or user defined function names, eg :call sub^I
///   - EXPAND_USER_FUNC
///     - Complete user defined function names, eg :delf F^I
///   - EXPAND_EXPRESSION
///     - Complete internal or user defined function/variable names
///       in expressions, eg :while s^I
///   - EXPAND_ENV_VARS
                     ...
                     int use_ccline)
{
    ...
    // Avoid a UMR warning from Purify, only save the
    // character if it has been written before.
    if(col < len)
    {
        old_char = str[col];
    }
    ...
    // Store the string here so that
    // call_user_expand_func() can get to them easily.
    xp->xp_line = str;
    xp->xp_col = col;
    str[col] = old_char;
}
...
/// xp->xp_pattern points into "str", to where the text
/// that is to be expanded starts.
///
/// @param xp
/// @param str         start of command line
/// @param col         position of cursor
...
static void cleanup_help_tags(int num_file, uchar_kt **file)
{
    uchar_kt buf[4];
    uchar_kt *p = buf;
    ...
    *p = NUL;
    ...
    for(int i = 0; i < num_file; i++)
    {
        ...
        if(ustrcmp(file[i] + len, "@en") == 0)
        {
            ...
            {
                if(j != i
                   && (int)ustrlen(file[j]) == len + 3
                   && ustrncmp(file[i], file[j], len + 1) == 0)
                {
                    break;
                }
            }
                             ...
                             int options)
{
       ...
       || xp->xp_context == EXPAND_FILES_IN_PATH)
    {
        ...
        // for ":set path=" and ":set tags="
        // halve backslashes for escaped space
            ...
            for(i = 0; pat[i]; ++i)
            {
                if(pat[i] == '\\')
                {
                    if(xp->xp_backslash == XP_BS_THREE
                       ...
                       && pat[i + 3] == ' ')
                    {
                        xstrmove(pat + i, pat + i + 3);
                    }

                    if(xp->xp_backslash == XP_BS_ONE && pat[i + 1] == ' ')
                    {
                        xstrmove(pat + i, pat + i + 1);
                    }
                }
            }
        }
    ...
    *num_file = 0;
    ...
    if(xp->xp_context == EXPAND_HELP)
    {
        ...
        if(find_help_tags(*pat == NUL ? (uchar_kt *)"help" : pat,
                          num_file,
                          file,
                          FALSE) == OK)
        {
                   ...
                   int escaped)
{
    ...
    *num_file = count;
    *file = (uchar_kt **)xmalloc(count * sizeof(uchar_kt *));
    ...
    for(i = 0;; ++i)
    {
        ...
        if(vim_regexec(regmatch, str, (columnum_kt)0))
        {
            if(escaped)
            {
                str = ustrdup_escape(str, (uchar_kt *)" \t\\.");
            }
            else
            {
                str = ustrdup(str);
...
///
/// @param filepat
/// is a pattern to match with command names.
///
/// @param[out] num_file
/// is pointer to number of matches.
///
/// @param[out] file
/// is pointer to array of pointers to matches.
/// *file will either be set to NULL or point to allocated memory.
///
/// @param flagsarg
/// is a combination of EW_* flags.
static void expand_shellcmd(uchar_kt *filepat,
                            int *num_file,
                            uchar_kt ***file,
                            int flagsarg)
FUNC_ATTR_NONNULL_ALL
{
    ...
    // for ":set path=" and ":set tags=" halve
    // backslashes for escaped space
    pat = ustrdup(filepat);
    ...
    // Expand matches in that directory and collect them in "ga".
    // When "." is not in $PATH also expaned for the current directory,
    // to find "subdir/cmd".
    ...
    for(s = path; ; s = e)
    {
        if(*s == NUL)
        {
            ...
            // Find directories in the
            // current directory, path is empty.
            did_curdir = true;
        }
        else if(*s == '.')
        {
            ...
            ga_grow(&ga, *num_file);
            {
                for(i = 0; i < *num_file; ++i)
                {
                    ...
                    if(ustrlen(s) > l)
                    {
                        ...
                        xstrmove(s, s + l);
                        ((uchar_kt **)ga.ga_data)[ga.ga_len++] = s;
                    }
                    else
                    {
                        xfree(s);
                                   ...
                                   uchar_kt ***file)
{
    ...
    *file = NULL;
    ...
    args[0] = ustrndup(xp->xp_pattern, xp->xp_pattern_len);
    args[1] = xp->xp_line;
                             ...
                             uchar_kt ***file)
{
    ...
    for(s = retstr; *s != NUL; s = e)
    {
        ...
        *e = 0;
        ...
        GA_APPEND(uchar_kt *, &ga, ustrndup(s, (int)(e - s)));
        *e = keep;
...
static int ExpandUserList(expand_st *xp, int *num_file, uchar_kt ***file)
{
    ...
    for(li = retlist->lv_first; li != NULL; li = li->li_next)
    {
        ...
        GA_APPEND(uchar_kt *, &ga, ustrdup(li->li_tv.vval.v_string));
    }
...
static int ExpandRTDir(uchar_kt *pat,
                       int flags,
                       int *num_file,
                       uchar_kt ***file,
                       char *dirnames[])
{
    *num_file = 0;
    *file = NULL;
    size_t pat_len = ustrlen(pat);
    garray_st ga;
    ga_init(&ga, (int)sizeof(char *), 10);

    for(int i = 0; dirnames[i] != NULL; i++)
    {
        size_t size = ustrlen(dirnames[i]) + pat_len + 7;
        uchar_kt *s = xmalloc(size);
    ...
    if(flags & DIP_START)
    {
        for(int i = 0; dirnames[i] != NULL; i++)
        {
            size_t size = ustrlen(dirnames[i]) + pat_len + 22;
            uchar_kt *s = xmalloc(size);
    ...
    if(flags & DIP_OPT)
    {
        for(int i = 0; dirnames[i] != NULL; i++)
        {
            size_t size = ustrlen(dirnames[i]) + pat_len + 20;
            uchar_kt *s = xmalloc(size);
    ...
    {
        uchar_kt *match = ((uchar_kt **)ga.ga_data)[i];
        uchar_kt *s = match;
        uchar_kt *e = s + ustrlen(s);

        if(e - s > 4 && ustrnicmp(e - 4, ".vim", 4) == 0)
        {
            e -= 4;
    ...
    // Sort and remove duplicates which can happen
    // when specifying multiple directories in dirnames.
...
void globpath(uchar_kt *path, uchar_kt *file, garray_st *ga, int expand_options)
{
    ...
    while(*path != NUL)
    {
        ...
        if(ustrlen(buf) + ustrlen(file) + 2 < MAXPATHL)
        {
            add_pathsep((char *)buf);
            ustrcat(buf, file); // NOLINT
            uchar_kt **p;
            int num_p = 0;
            (void)ExpandFromContext(&xpc,
                                    buf,
                                    &num_p,
                                    &p,
            ...
            if(num_p > 0)
            {
                ...
                {
                    ((uchar_kt **)ga->ga_data)[ga->ga_len] = ustrdup(p[i]);
                    ga->ga_len++;
                }
...
static uchar_kt *get_history_arg(expand_st *FUNC_ARGS_UNUSED_MATCH(xp), int idx)
{
    ...
    char *short_names = ":=@>?/";
    int short_names_count = (int)ustrlen(short_names);
    int history_name_count = ARRAY_SIZE(history_names) - 1;

    if(idx < short_names_count)
    {
        compl[0] = (uchar_kt)short_names[idx];
        return compl;
    }

    if(idx < short_names_count + history_name_count)
    {
        return (uchar_kt *)history_names[idx - short_names_count];
    }
    ...
    if(newlen != oldlen)
    {
        for(int type = 0; type < HIST_COUNT; type++)
        {
            ...
            if(j >= 0)
            {
                ...
                // copy as much entries as they
                // fit to new table, reordering them
                if(newlen)
                {
                    // copy oldest entries
                    memcpy(&temp[0],
                           &history[type][i2],
                           (size_t)l2 * sizeof(*temp));

                    // copy newest entries
                    memcpy(&temp[l2],
                           &history[type][i1],
                           (size_t)l1 * sizeof(*temp));
                }
...
/// @param sep
static int in_history(int type, uchar_kt *str, int move_to_front, int sep)
{
    int i;
    int last_i = -1;
    uchar_kt *p;
    ...
    do
    {
        ...
        // For search history, check that the separator
        // character matches as well.
        p = history[type][i].hisstr;

        if(ustrcmp(str, p) == 0
           && (type != kHistSearch || sep == p[ustrlen(p) + 1]))
        {
            if(!move_to_front)
            {
                return TRUE;
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    {
        if(ustrnicmp(name, history_names[i], len) == 0)
        {
            return i;
        }
    }

    if(ustrchr((uchar_kt *)":=@>?/", name[0]) != NULL && len == 1)
    {
...
/// If the string is already in the history then it is moved to the front.
/// "histype" may be one of he HIST_ values.
///
/// @param histype
/// @param new_entry
...
void add_to_history(int histype, uchar_kt *new_entry, int in_map, int sep)
{
    ...
    if(!in_history(histype, new_entry, true, sep))
    {
        ...
        len = (int)ustrlen(new_entry);
        hisptr->hisstr = ustrndup(new_entry, len + 2);
...
uchar_kt *get_cmdline_str(void)
{
    ...
    return ustrndup(p->cmdbuff, p->cmdlen);
}
...
int set_cmdline_pos(int pos)
{
    ...
    // The position is not set directly but after
    // CTRL-\ e or CTRL-R = has changed the command line.
    if(pos < 0)
    {
        new_cmdpos = 0;
    }
...
/// "histype" may be one of the HIST_ values.
uchar_kt *get_history_entry(int histype, int idx)
{
...
///
/// @return OK if there was something to clean
///         and histype was one of HIST_ values, FAIL otherwise.
...
int del_history_entry(int histype, uchar_kt *str)
{
    ...
    if(hislen != 0
       ...
       && (regmatch.regprog = regexp_compile(str, RE_MAGIC + RE_STRING))
       != NULL)
    {
        i = last = idx;
...
int get_list_range(uchar_kt **str, int *num1, int *num2)
{
    int len;
    int first = false;
    long num;
    *str = skipwhite(*str);
    ...
    {
        str_to_num(*str, NULL, &len, 0, &num, NULL, 0);
        *str += len;
        *num1 = (int)num;
        first = true;
    }
...
void ex_history(exargs_st *eap)
{
    ...
    if(!(ascii_isdigit(*arg) || *arg == '-' || *arg == ','))
    {
        ...
        {
            if(ustrnicmp(arg, "all", end - arg) == 0)
            {
                histype1 = 0;
    ...
    {
        ustrcpy(IObuff, "\n      #  ");
        ustrcat(ustrcat(IObuff, history_names[histype1]), " history");
        ...
        if(idx >= 0 && j <= k)
            for(i = idx + 1; !got_int; ++i)
            {
                   ...
                   && hist[i].hisnum >= j && hist[i].hisnum <= k)
                {
                    ...
                    if(ustr_scrsize(hist[i].hisstr) > (int)Columns - 10)
                    {
                        trunc_string(hist[i].hisstr,
                                     IObuff + ustrlen(IObuff),
                                     (int)Columns - 10,
                                     IOSIZE - (int)ustrlen(IObuff));
                    }
                    else
                    {
                        ustrcat(IObuff, hist[i].hisstr);
                    }
...
{
    if(ccline.cmdpos + offset >= ccline.cmdlen
       || ccline.cmdpos + offset < 0)
    {
        EMSG(_("E198: cmd_pchar beyond the command length"));
        return;
    }

    ccline.cmdbuff[ccline.cmdpos + offset] = (uchar_kt)c;
    ccline.cmdbuff[ccline.cmdlen] = NUL;
}
...
/// Allow editing in the window. Returns when the window is closed.
///
/// @returns:
...
static int ex_window(void)
{
    ...
    // Can't do it when typing a password.
    ...
    if(histtype == kHistCmd || histtype == kHistDebug)
    {
        if(p_wc == TAB)
        {
            add_map((uchar_kt *)"<buffer> <Tab> <C-X><C-V>", kInsertMode);
    ...
    // Replace the empty last line with the current
    // command-line and put the cursor there.
    ...
    // Safety check:
    // The old window or buffer was deleted: It's a bug when this happens!
    ...
    else
    {
        ...
        {
            ccline.cmdbuff = ustrdup((uchar_kt *)"qa");
            cmdwin_result = CAR;
        }
        ...
        {
            ccline.cmdbuff = ustrdup(get_cursor_line_ptr());
        }
        ...
        {
            ccline.cmdlen = (int)ustrlen(ccline.cmdbuff);
...
///
/// @param[out]  lenp
/// Location where length of resulting string is saved.
/// Will be set to zero when skipping.
///
/// @return [allocated]
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    const char *const end_pattern =
        (cmd[2] != NUL
         ? (const char *)skipwhite((const uchar_kt *)cmd + 2) : ".");

    for(;;)
    {
        char *const theline =
            (char *)eap->getline(eap->cstack->cs_looplevel > 0 ? -1 : NUL,
                                 eap->cookie,
                                 0);
        ...
        if(!eap->skip)
        {
            ga_concat(&ga, (const uchar_kt *)theline);
            ga_append(&ga, '\n');
        }
...
///
/// @warning No history-editing functions must be
/// run while iteration is in progress.
///
/// @param[in] iter
/// Pointer to the last history entry.
///
/// @param[in] history_type
/// Type of the history (HIST_*). Ignored if iter parameter is not NULL.
///
/// @param[in] zero
/// If true then zero (but not free) returned items.
///
/// @warning When using this parameter user is responsible
/// for calling clr_history() itself after iteration is over.
/// If clr_history() is not called behaviour is undefined.
/// No functions that work with history must be called during
...
///
/// @return
/// Pointer used in next iteration or NULL to indicate that
...
///
/// @param[in] history_type
/// Type of the history to get array for.
///
/// @param[out] new_hisidx
/// Location where last index in the new array should be saved.
///
/// @param[out] new_hisnum
/// Location where last history number in the new history should be saved.
///
/// @return
