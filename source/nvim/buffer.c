...
/// - never loaded: kWBF_NeverLoaded is set, only the file name is valid
...
/// Open current buffer, that is:
/// open the memfile and read the file into memory.
///
...
int open_buffer(int read_stdin, exargs_st *eap, int flags)
{
    ...
    // The 'readonly' flag is only set when kWBF_NeverLoaded is being reset.
    // When re-entering the same buffer, it should not change, because the
    // user may have reset the flag by hand.
    if(readonlymode && curbuf->b_ffname != NULL
       && (curbuf->b_flags & kWBF_NeverLoaded))
    {
        curbuf->b_p_ro = TRUE;
    }
    ...
    if(ml_open(curbuf) == FAIL)
    {
        ...
        if(curbuf == NULL)
        {
            ...
            exit_nvim_properly(kNEStatusBufAllocateFailed);
        }
    ...
    // The autocommands in readfile() may change the
    // buffer, but only AFTER reading the file.
    ...
    else if(read_stdin)
    {
        ...
        if(retval == OK)
        {
            ...
            if(retval == OK)
            {
                apply_autocmds_retval(EVENT_STDINREADPOST, NULL,
                                      NULL, false, curbuf, &retval);
            }
        }
    }

    // if first time loading this buffer, init b_chartab[]
    if(curbuf->b_flags & kWBF_NeverLoaded)
    {
    ...
    if((got_int && ustrchr(p_cpo, CPO_INTMOD) != NULL)
       || modified_was_set // ":set modified" used in autocmd
       || (aborting() && ustrchr(p_cpo, CPO_INTMOD) != NULL))
    {
        changed();
    }
    ...
    if(!(curwin->w_valid & kWVF_TopLine))
    {
        curwin->w_topline = 1;
        curwin->w_topfill = 0;
    }
    ...
    if(bufref_valid(&old_curbuf) && old_curbuf.br_buf->b_ml.ml_mfp != NULL)
    {
        ...
        do_modelines(0);
        curbuf->b_flags &= ~(kWBF_CheckReadOnly | kWBF_NeverLoaded);

        apply_autocmds_retval(EVENT_BUFWINENTER, NULL,
                              NULL, FALSE, curbuf, &retval);

        aucmd_restbuf(&aco); // restore curwin/curbuf and a few other things
    }
...
void set_bufref(bufref_st *bufref, filebuf_st *buf)
{
    bufref->br_buf = buf;
...
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    for(filebuf_st *bp = lastbuf; bp != NULL; bp = bp->b_prev)
    {
        if(bp == buf)
        {
            return true;
...
void close_buffer(win_st *win, filebuf_st *buf, int action, int abort_if_last)
{
    ...
    // deleting/freeing when 'bufhidden' is "hide", otherwise
    // we could never free or delete a buffer.
    if(!buf->terminal)
    {
        if(buf->b_p_bh[0] == 'd')
        {
    ...
    // Disallow deleting the buffer when it is locked (already being
    // closed or halfway a command that relies on it). Unloading is allowed.
    ...
    if(buf->b_nwindows == 1)
    {
        ...
        if(apply_autocmds(EVENT_BUFWINLEAVE, buf->b_fname,
        ...
        // When the buffer becomes hidden, but
        // is not unloaded, trigger BufHidden
        if(!unload_buf)
        {
            buf->b_locked++;

            if(apply_autocmds(EVENT_BUFHIDDEN, buf->b_fname,
                              buf->b_fname, false, buf)
               && !bufref_valid(&bufref))
            {
                // Autocommands deleted the buffer.
...
#if defined(EXITFREE)
    if(is_curbuf && VIsual_active && !entered_free_all_mem)
#else
    ...
    // This might cause the previous curbuf to be deleted unexpectedly. But in
    // some cases it's OK to delete the curbuf, because a new one is obtained
    // anyway. Therefore only return if curbuf changed to the deleted buffer.
    ...
    {
        // make sure we don't use the buffer now
        win->w_buffer = NULL;
    }
    ...
    else
    {
        if(del_buf)
        {
            ...
            buf->b_flags = kWBF_CheckReadOnly | kWBF_NeverLoaded;
...
void buf_clear_file(filebuf_st *buf)
{
...
/// Careful: get here with "curwin" NULL when exiting.
///
/// @param flags BFA_DEL buffer is going to be deleted
///              BFA_WIPE buffer is going to be wiped out
///              BFA_KEEP_UNDO  do not free undo information
void buf_freeall(filebuf_st *buf, int flags)
{
    bool is_curbuf = (buf == curbuf);
    ...
    buf->b_flags &= ~kWBF_ReadError; // a read error is no longer relevant
}

/// Free a buffer structure and the things it contains related to the buffer
/// itself (not the file, that must have been done already).
static void free_buffer(filebuf_st *buf)
{
...
///
/// @param buf           buffer
/// @param free_options  free options as well
static void free_buffer_stuff(filebuf_st *buf, int free_options)
{
    if(free_options)
    {
...
static void clear_wininfo(filebuf_st *buf)
{
    wininfo_st *wip;
...
///
/// addr_count == 1: ":N bdel" or ":bdel N [N ..]"
/// - first delete buffer "end_bnr", then any other arguments
///
/// addr_count == 2: ":N,N bdel"
/// - delete buffers in range
///
/// command can be DOBUF_UNLOAD (":bunload"),
/// DOBUF_WIPE (":bwipeout") or DOBUF_DEL (":bdel")
///
/// @param command
                    ...
                    int forceit)
{
    ...
    uchar_kt *errormsg = NULL; // return value
    ...
    else
    {
        ...
        for(; !got_int; os_breakcheck())
        {
            ...
            else if(do_buffer(command, DOBUF_FIRST,
                              FORWARD, bnr, forceit) == OK)
            {
                ++deleted;
            }
            ...
            else
            {
                ...
                if(!ascii_isdigit(*arg))
                {
                    ...
                    bnr = buflist_findpat(arg, p,
                                          command == DOBUF_WIPE, FALSE, FALSE);
        ...
        if(deleted == 0)
        {
            ...
            {
                ustrcpy(IObuff, _("E515: No buffers were unloaded"));
            }
            else if(command == DOBUF_DEL)
            {
                ustrcpy(IObuff, _("E516: No buffers were deleted"));
            }
            else
            {
                ustrcpy(IObuff, _("E517: No buffers were wiped out"));
            }
...
static int empty_curbuf(int close_others, int forceit, int action)
{
    ...
    setpcmark();
    retval = do_ecmd(0, NULL, NULL, NULL,
                     ECMD_ONE, forceit ? ECMD_FORCEIT : 0, curwin);
...
int do_buffer(int action, int start, int dir, int count, int forceit)
{
    ...
    int unload =
        (action == DOBUF_UNLOAD || action == DOBUF_DEL || action == DOBUF_WIPE);
    ...
    if(start == DOBUF_MOD)
    {
        while(count-- > 0) // find next modified buffer
        {
            do
            {
                buf = buf->b_next;
    ...
    else
    {
        ...
        {
            // remember the buffer where we start, we come back
            // there when all buffers are unlisted.
            if(bp == NULL)
            {
                bp = buf;
            }
            ...
            if(unload || buf->b_p_bl) // don't count unlisted buffers
            {
                --count;
    ...
    if(buf == NULL)
    {
        if(start == DOBUF_FIRST) // could not find it
        {
            if(!unload) // don't warn when deleting
            {
    ...
    if(unload)
    {
        ...
        if(!forceit && (buf->terminal || bufIsChanged(buf)))
        {
            if((p_confirm || cmdmod.confirm) && p_write && !buf->terminal)
            {
                ...
                if(!bufref_valid(&bufref))
                {
                    // Autocommand deleted buffer,
                    // oops! It's not changed now.
                    return FAIL;
                }

                // If it's still changed fail silently,
                // the dialog already mentioned why it fails.
                if(bufIsChanged(buf))
                {
                    return FAIL;
                }
            ...
            else
            {
                ...
                {
                    EMSGN(_("E89: No write since last change for buffer %"
                            PRId64 " (add ! to override)"), buf->b_id);
                }
        ...
        else if(curwin->w_jumplistlen > 0)
        {
            ...
            while(jumpidx != curwin->w_jumplistidx)
            {
                ...
                if(buf != NULL)
                {
                    break; // found a valid buffer: stop searching
                }
        ...
        if(buf == NULL) // No previous buffer, Try 2'nd approach
        {
            ...
            for(;;)
            {
                ...
                if(buf->b_help == curbuf->b_help && buf->b_p_bl)
                {
                    if(buf->b_ml.ml_mfp != NULL)
                    {
                        break; // found loaded buffer
                    }

                    if(bp == NULL)
                    {
                        bp = buf; // remember unloaded buf for later
                    }
                }
        ...
        if(buf == NULL)
        {
            buf = bp; // No loaded buffer, use unloaded one
        }

        if(buf == NULL)
        {
            FOR_ALL_BUFFERS(buf2) // No loaded buffer, find listed one
            {
    ...
    // make buf current buffer. split window first
    if(action == DOBUF_SPLIT)
    {
        // If 'switchbuf' contains "useopen": jump to
        // first window containing"buf" if one exists
    ...
    if(action == DOBUF_GOTO && !can_abandon(curbuf, forceit))
    {
        if((p_confirm || cmdmod.confirm) && p_write)
        {
            ...
            if(!bufref_valid(&bufref))
            {
                return FAIL; // Autocommand deleted buffer, oops!
            }
        }
    ...
    if(action == DOBUF_SPLIT)
    {
        // reset 'scrollbind' and 'cursorbind'
...
/// Executes autocommands and closes current buffer.
///
...
void set_curbuf(filebuf_st *buf, int action)
{
    filebuf_st *prevbuf;
    ...
    if(!cmdmod.keepalt)
    {
        curwin->w_alt_fnum = curbuf->b_id; // remember alternate file
    }
            ...
            close_buffer(prevbuf == curwin->w_buffer ? curwin : NULL,
                         ...
                         : (action == DOBUF_GOTO
                            ...
                            && !bufIsChanged(prevbuf))
                               ? DOBUF_UNLOAD : 0, FALSE);

            if(curwin != previouswin && win_valid(previouswin))
            {
                curwin = previouswin; // autocommands changed curwin, Grr!
            }
        }
    }
...
void enter_buffer(filebuf_st *buf)
{
    ...
    curwin->w_buffer = buf; // Get the buffer in the current window
    curbuf = buf;
    ++curbuf->b_nwindows;
    ...
    curwin->w_s = &(buf->b_s);
    curwin->w_cursor.lnum = 1; // Cursor on first line by default
    curwin->w_cursor.col = 0;
    curwin->w_cursor.coladd = 0;
    curwin->w_set_curswant = TRUE;
    curwin->w_topline_was_set = FALSE;
    curwin->w_valid = 0; // mark cursor position as being invalid
    ...
    // Make sure the buffer is loaded, need to load the file
    ...
    do_autochdir(); // Change directories when the 'acd' option is set
    ...
    // Can only do this after the bufferhas been properly setup. */
    if(!curbuf->b_help
...
void do_autochdir(void)
{
    if(p_acd)
    {
        if(starting == 0
           ...
           && change_to_filedir(curbuf->b_ffname) == OK)
        {
            shorten_fnames(true);
        }
    }
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    buf->changedtick_di = (changedtick_st) {
                        ...
                        int flags)
{
                                          ...
                                          file_id_valid)) != NULL)
    {
        ...
        {
            // Copy the options now, if 'cpo'
            // doesn't have 's' and not done already.
    ...
    // (A spell file buffer is allocated in spell.c,
    // but that's not a normal buffer.)
    buf = NULL;
        ...
        && (curbuf->b_ml.ml_mfp == NULL || bufempty()))
    {
        ...
        if(aborting())
        {
            return NULL; // autocmds may abort script processing
        }
    ...
    {
        buf = xcalloc(1, sizeof(filebuf_st));

        buf->b_vars = tv_dict_alloc(); // init b: variables
    ...
    if(buf == curbuf)
    {
        ...
        if(buf != curbuf)
        {
            return NULL; // autocommands deleted the buffer!
        }
        ...
        buf->b_p_initialized = false; // Init the options.
    ...
    if(!(flags & BLN_DUMMY))
    {
        ...
        if(apply_autocmds(EVENT_BUFNEW, NULL,
                          NULL, false, buf) && !bufref_valid(&bufref))
        {
            return NULL;
        }
...
///
/// If `free_p_ff` is TRUE also free 'fileformat',
/// 'buftype' and 'fileencoding'.
void free_buf_options(filebuf_st *buf, int free_p_ff)
{
    if(free_p_ff)
    {
...
int buflist_getfile(int n, linenum_kt lnum, int options, int forceit)
{
    ...
    if(options & GETF_SWITCH)
    {
        ...
        if(wp == NULL
           && (swb_flags & (SWB_VSPLIT | SWB_SPLIT | SWB_NEWTAB))
           && !bufempty())
        {
            ...
            else if(win_split(0, (swb_flags & SWB_VSPLIT)
                                 ? WSP_VERT : 0) == FAIL)
            {
                return FAIL;
            }
    ...
    if(getfile(buf->b_id, NULL, NULL,
               (options & GETF_SETMARK), lnum, forceit) <= 0)
    {
        --RedrawingDisabled;

        // cursor is at to BOL and
        // w_cursor.lnum is checked due to getfile()
        if(!p_sol && col != 0)
        {
...
filebuf_st *buflist_findname_exp(uchar_kt *fname)
{
    ...
    uchar_kt *ffname = (uchar_kt *)FullName_save((char *)fname, TRUE);
#else
    // do not get rid of symbolic links
    uchar_kt *ffname = (uchar_kt *)FullName_save((char *)fname, FALSE);
#endif
...
/// Skips dummy buffers.
/// Returns NULL if not found.
filebuf_st *buflist_findname(uchar_kt *ffname)
{
    fileid_st file_id;
...
/// Returns NULL if not found.
static filebuf_st *buflist_findname_file_id(uchar_kt *ffname,
                                       fileid_st *file_id,
                                       bool file_id_valid)
{
    ...
    {
        if((buf->b_flags & kWBF_DummyBuf) == 0
...
/// - Return < 0 for error.
int buflist_findpat(const uchar_kt *pattern,
                    const uchar_kt *pattern_end,
                    int unlisted,
                    int diffmode,
                    int curtab_only)
{
    ...
    if(pattern_end == pattern + 1 && (*pattern == '%' || *pattern == '#'))
    {
        ...
        filebuf_st *found_buf = buflist_findnr(match);
    ...
    else
    {
        ...
        for(;;)
        {
            for(attempt = 0; attempt <= 3; ++attempt)
            {
                ...
                FOR_ALL_BUFFERS_BACKWARDS(buf)
                {
                       ...
                       && buflist_match(&regmatch, buf, false) != NULL)
                    {
                        ...
                        match = buf->b_id; // remember first match
                    }
                }
            ...
            // Only search for unlisted buffers if there
            // was no match with a listed buffer.
...
int ExpandBufnames(uchar_kt *pat, int *num_file, uchar_kt ***file, int options)
{
    int count = 0;
    int round;
    uchar_kt *p;
    ...
    *file = NULL;
    ...
    if(*pat == '^')
    {
        patc = xmalloc(ustrlen(pat) + 11);
        ustrcpy(patc, "\\(^\\|[\\/]\\)");
        ustrcpy(patc + 11, pat + 1);
    }
    else
    {
        patc = pat;
    ...
    for(attempt = 0; attempt <= 1; ++attempt)
    {
        ...
        regmatch_st regmatch;
        regmatch.regprog = regexp_compile(patc + attempt * 11, RE_MAGIC);
        ...
        for(round = 1; round <= 2; ++round)
        {
            ...
            FOR_ALL_BUFFERS(buf)
            {
                if(!buf->b_p_bl)
                {
                    continue; // skip unlisted buffers
                }
        ...
        if(count)
        {
            break; // match(es) found, break here
        }
    }
...
static uchar_kt *buflist_match(regmatch_st *rmp, filebuf_st *buf, bool ignore_case)
{
...
filebuf_st *buflist_findnr(int nr)
{
    if(nr == 0)
    {
...
{
    filebuf_st *buf = buflist_findnr(n);
...
///
/// @param[in,out] buf
/// Buffer for which line and column are set.
///
/// @param[in,out] win
/// Window for which line and column are set.
///
/// @param[in] lnum
/// Line number to be set. If it is zero then only options are touched.
///
/// @param[in] col
...
void buflist_setfpos(filebuf_st *const buf,
                     win_st *const win,
                     linenum_kt lnum,
                     columnum_kt col,
                     bool copy_options)
FUNC_ATTR_NONNULL_ALL
{
    ...
    if(wip == NULL)
    {
        ...
        if(lnum == 0)
        {
            lnum = 1; // set lnum even when it's 0
        }
    }
    else
    {
    ...
    if(copy_options)
    {
        ...
        copy_winopt(&win->w_o_curbuf, &wip->wi_opt);
...
FUNC_ATTR_NONNULL_ALL
{
    if(wip->wi_opt.wo_diff)
    {
        FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
        {
            // return false when it's a window in the current
            // tab page, thus the buffer was in diff mode here
            if(wip->wi_win == wp)
            {
                return false;
            }
...
static wininfo_st *find_wininfo(filebuf_st *buf, int skip_diff_buffer)
{
    wininfo_st *wip;
...
void get_winopts(filebuf_st *buf)
{
    wininfo_st *wip;
    clear_winopt(&curwin->w_o_curbuf);
    clearFolding(curwin);
    wip = find_wininfo(buf, TRUE);

    if(wip != NULL && wip->wi_optset)
    {
        copy_winopt(&wip->wi_opt, &curwin->w_o_curbuf);
    ...
    {
        copy_winopt(&curwin->w_o_allbuf, &curwin->w_o_curbuf);
    }
...
/// Find the position (lnum and col) for the buffer 'buf' for the
/// current window.
///
/// @returns a pointer to no_position if no position is found.
apos_st *buflist_findfpos(filebuf_st *buf)
{
...
linenum_kt buflist_findlnum(filebuf_st *buf)
{
...
void buflist_list(exargs_st *eap)
{
    ...
    for(buf = firstbuf; buf != NULL && !got_int; buf = buf->b_next)
    {
        ...
        if((!buf->b_p_bl && !eap->forceit && !strchr((char *)eap->arg, 'u'))
           ...
           || (strchr((char *)eap->arg, '+')
               && ((buf->b_flags & kWBF_ReadError) || !bufIsChanged(buf)))
           ...
           || (strchr((char *)eap->arg, 'x') && !(buf->b_flags & kWBF_ReadError))
           || (strchr((char *)eap->arg, '%') && buf != curbuf)
           || (strchr((char *)eap->arg, '#')
               && (buf == curbuf || curwin->w_alt_fnum != buf->b_id)))
        {
            continue;
        }
...
/// OK otherwise.
int setfname(filebuf_st *buf, uchar_kt *ffname, uchar_kt *sfname, int message)
{
    filebuf_st *obuf = NULL;
    fileid_st file_id;
    ...
    else
    {
        ...
        sfname = ustrdup(sfname);

        #ifdef USE_FNAME_CASE
        // set correct case for short file name
...
void buf_set_name(int fnum, uchar_kt *name)
{
    filebuf_st *buf;
    buf = buflist_findnr(fnum);
    ...
    if(buf != NULL)
    {
        xfree(buf->b_sfname);
        xfree(buf->b_ffname);
        buf->b_ffname = ustrdup(name);
        buf->b_sfname = NULL;

        // Allocate ffname and expand into full path.
        // Also resolves .lnk files on Win32.
        fname_expand(buf, &buf->b_ffname, &buf->b_sfname);
        buf->b_fname = buf->b_sfname;
    }
}

/// Take care of what needs to be done when
/// the name of buffer "buf" has changed.
void buf_name_changed(filebuf_st *buf)
{
    // If the file name changed,
    // also change the name of the swapfile
    if(buf->b_ml.ml_mfp != NULL)
    {
        ml_setname(buf);
    }
...
///
/// @return the buffer.
filebuf_st *setaltfname(uchar_kt *ffname, uchar_kt *sfname, linenum_kt lnum)
{
    filebuf_st *buf;
...
/// Adjust slashes in file names.
/// Called after 'shellslash' was set.
...
static bool otherfile_buf(filebuf_st *buf,
                          uchar_kt *ffname,
                          fileid_st *file_id_p,
                          bool file_id_valid)
FUNC_ATTR_PURE
FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    if(!file_id_valid)
    {
        return true; // file_id not valid, assume files are different.
    }
...
void buf_set_file_id(filebuf_st *buf)
{
    fileid_st file_id;
...
void fileinfo(int fullname, int shorthelp, int dont_truncate)
{
    ...
    {
        xsnprintf((char *)buffer, IOSIZE, "buf %d: ", curbuf->b_id);
        p = buffer + ustrlen(buffer);
    }
    else
    {
        p = buffer;
    ...
    *p++ = '"';
    ...
    {
        ustrlcpy(p, buf_spname(curbuf), IOSIZE - (p - buffer));
    }
    else
    {
    ...
    xvsnprintf_add((char *)buffer,
                   IOSIZE,
                   "\"%s%s%s%s%s%s",
                   curbufIsChanged()
                   ? (shortmess(SHM_MOD) ? " [+]" : _(" [Modified]")) : " ",
                   (curbuf->b_flags & kWBF_NotEdited) && !bt_dontwrite(curbuf)
                   ? _("[Not edited]") : "",
                   (curbuf->b_flags & kWBF_NewFile) && !bt_dontwrite(curbuf)
                   ? _("[New file]") : "",
                   (curbuf->b_flags & kWBF_ReadError)
                   ? _("[Read errors]") : "",
                   curbuf->b_p_ro
                   ...
                   (curbufIsChanged()
                    || (curbuf->b_flags & kWBF_WriteBufMask)
                    || curbuf->b_p_ro) ? " " : "");
    ...
    {
        xvsnprintf_add((char *)buffer, IOSIZE, "%s", _(no_lines_msg));
    }
    else if(p_ru)
    {
        ...
        {
            xvsnprintf_add((char *)buffer, IOSIZE, _("1 line --%d%%--"), n);
        }
        else
        {
            xvsnprintf_add((char *)buffer,
    ...
    else
    {
        ...
        if(restart_edit != 0 || (msg_scrolled && !need_wait_return))
        {
            ...
            // - When restart_edit is set (otherwise there will be
            //   a delay before redrawing).
            // - When the screen was scrolled but there is no
            //   wait-return prompt.
            set_keep_msg(p, 0);
        }
    }
...
void col_print(uchar_kt *buf, size_t buflen, int col, int vcol)
{
    if(col == vcol)
    {
        xsnprintf((char *)buf, buflen, "%d", col);
    }
    else
    {
        xsnprintf((char *)buf, buflen, "%d-%d", col, vcol);
    }
}

// put file name in title bar of window and in icon title
static uchar_kt *lasttitle = NULL;
static uchar_kt *lasticon = NULL;
...
void maketitle(void)
{
    ...
    if(!redrawing()) // Postpone updating the title when 'lazyredraw' is set.
    {
        need_maketitle = TRUE;
        return;
    }
    ...
    if(p_title)
    {
        ...
        if(*p_titlestring != NUL)
        {
            if(stl_syntax & STL_IN_TITLE)
            {
                ...
                use_sandbox = was_set_insecurely((uchar_kt *)"titlestring", 0);
                called_emsg = FALSE;
        ...
        else // format: "fname + (path) (1 of 2) - VIM"
        {
...
#define SPACE_FOR_ARGNR  (IOSIZE - 10)  // at least room for " - VIM"
            ...
            {
                ustrlcpy(buf, _("[No Name]"), SPACE_FOR_FNAME + 1);
            }
            else
            {
                p = transstr(path_tail(curbuf->b_fname));
                ustrlcpy(buf, p, SPACE_FOR_FNAME + 1);
                xfree(p);
            }
            ...
            if(curbuf->b_fname != NULL)
            {
                ...
                if(p == buf + off) // must be a help buffer
                {
                    ustrlcpy(buf + off, _("help"), SPACE_FOR_DIR - off);
                }
                else
                {
                    *p = NUL;
                ...
                {
                    p = transstr(buf + off);
                    ustrlcpy(buf + off, p, SPACE_FOR_DIR - off + 1);
                    xfree(p);
                }
                else
                {
                    ustrlcpy(buf + off, "...", SPACE_FOR_ARGNR - off + 1);
                }
            ...
            if(maxlen > 0) // make it shorter by removing a bit in the middle
            {
                if(ustr_scrsize(buf) > maxlen)
                {
    ...
    if(p_icon)
    {
        ...
        if(*p_iconstring != NUL)
        {
            if(stl_syntax & STL_IN_ICON)
            {
                int use_sandbox = FALSE;
                int save_called_emsg = called_emsg;
                use_sandbox = was_set_insecurely((uchar_kt *)"iconstring", 0);
                called_emsg = FALSE;
...
/// Used for title and icon:
/// Check if "str" differs from "*last".
/// Set "*last" from "str" if it does by freeing
/// the old value of "*last" and duplicating "str".
///
/// @param          str   desired title string
/// @param[in,out]  last  current title string
//
/// @return true when "*last" changed.
static bool ti_change(uchar_kt *str, uchar_kt **last)
FUNC_ATTR_WARN_UNUSED_RESULT
{
    if((str == NULL) != (*last == NULL)
       || (str != NULL && *last != NULL && ustrcmp(str, *last) != 0))
    {
        xfree(*last);
...
///
/// Normally works for window "wp", except when working
/// for 'tabline' then it is "curwin".
///
                     ...
                     stl_clickrec_st *tabtab)
{
    ...
    {
        fillchar = '-'; // Can't handle a multi-byte fill character yet.
    }

    // Get line & check if empty (cursorpos will show "0-1").
    uchar_kt *line_ptr = ml_get_buf(wp->w_buffer, wp->w_cursor.lnum, false);
    bool empty_line = (*line_ptr == NUL);
    // Get the byte value now, in case we need it below. This is more
    // efficient than making a copy of the line.
    int byteval;

    if(wp->w_cursor.col > (columnum_kt)ustrlen(line_ptr))
    {
        byteval = 0;
    }
    else
    ...
    //
    // Note:
    // The null termination character must occur here or earlier,
    // so any user-visible characters must occur before here.
    uchar_kt *out_end_p = (out + outlen) - 1;

    // Proceed character by character through the statusline
    // format string fmt_p is the current positon in the input buffer
    for(uchar_kt *fmt_p = usefmt; *fmt_p; )
    {
        if(curitem == STL_MAX_ITEM)
        {
            // There are too many items.
            // Add the error code to the statusline
        ...
        //
        // Note:
        // We increment here to skip over the `%` character we are
        // currently on so we can process the item's contents.
        fmt_p++;
        ...
        // STL_SEPARATE: Separation place between
        ...
        // STL_TRUNCMARK: Where to begin
        // truncating if the statusline is too long.
        ...
        if(*fmt_p == ')')
        {
            ...
            //       so `ustr_scrsize` will work.
            uchar_kt *t = items[groupitems[groupdepth]].start;
            *out_p = NUL;
            long group_len = ustr_scrsize(t);
            ...
            //
            // Note:
            // This erases any non-item characters that were in the group.
            ...
            {
                // If the group is shorter than the
                // minimum width, add padding characters.
                ...
                {
                    // If the group is right-aligned, shift everything
                    // to the right and prepend with filler characters.
                    // Move the group to the right
                    memmove(t + min_group_width - group_len,
                            t, (size_t)(out_p - t));
        ...
        // User highlight groups override the min
        // width field to denote the styling to use.
        ...
        if(*fmt_p == STL_TABPAGENR || *fmt_p == STL_TABCLOSENR)
        {
            if(*fmt_p == STL_TABCLOSENR)
            {
                if(minwid == 0)
                {
                    // %X ends the close label, go
                    // back to the previous tab label nr.
        ...
        if(ustrchr(STL_ALL, *fmt_p) == NULL)
        {
            fmt_p++;
            continue;
        }

        uchar_kt opt = *fmt_p++; // The status line item type
        ...
        switch(opt)
        {
            ...
            case STL_FILENAME:
            {
                ...
                {
                    ustrlcpy(NameBuff, buf_spname(wp->w_buffer), MAXPATHL);
                }
                else
                {
            ...
            case STL_VIM_EXPR:  // '{'
            {
                ...
                *out_p = 0;
                ...
                // Move our position in the output
                // buffer to the beginning of the expression
                out_p = t;

                // Evaluate the expression
                // Store the current buffer number as a string variable
                xsnprintf((char *)tmp, sizeof(tmp), "%d", curbuf->b_id);
                set_internal_string_var((uchar_kt *)"actual_curbuf", tmp);
                filebuf_st *o_curbuf = curbuf;
            ...
            case STL_VIRTCOL_ALT:
            {
                ...
                {
                    wp->w_o_curbuf.wo_list = FALSE;
                    getvcol(wp, &wp->w_cursor, NULL, &virtcol, NULL);
                    wp->w_o_curbuf.wo_list = TRUE;
                }
            ...
            case STL_ALTPERCENT:
                ...
                // Note:
                // We cannot store the value in `num` because
            ...
            case STL_ARGLISTSTAT:
                fillable = false;
                // Note:
                // This is important because `append_arg_number` starts
                // appending at the end of the null-terminated string.
                // Setting the first byte to null means it will place the
                // argument number string at the beginning of the buffer.
                tmp[0] = 0;

                // Note:
                // The call will only return true if it actually appended
                // data to the `tmp` buffer.
            ...
            case STL_KEYMAP:
                ...
                if(get_keymap_str(wp, (uchar_kt *)"<%s>", tmp, TMPLEN))
                {
                    str = tmp;
                }
            ...
            case STL_FILETYPE:
                ...
                // Copy the filetype if it is not null and the formatted
                // string will fit in the temporary buffer
                // (including the brackets and null terminating character)
                if(*wp->w_buffer->b_p_ft != NUL
                   && ustrlen(wp->w_buffer->b_p_ft) < TMPLEN - 3)
                {
                    xsnprintf((char *)tmp, sizeof(tmp),
                              "[%s]", wp->w_buffer->b_p_ft);
                    str = tmp;
                }
            ...
            case STL_FILETYPE_ALT:
            {
                ...
                // Copy the filetype if it is not null and the
                // formatted string will fit in the temporary buffer
                // (including the comma and null terminating character)
                if(*wp->w_buffer->b_p_ft != NUL
                   && ustrlen(wp->w_buffer->b_p_ft) < TMPLEN - 2)
                {
                    xsnprintf((char *)tmp, sizeof(tmp),
                              ",%s", wp->w_buffer->b_p_ft);

                    // Uppercase the file extension
                    for(uchar_kt *t = tmp; *t != 0; t++)
                    {
                        *t = (uchar_kt)TOUPPER_LOC(*t);
                    }
        ...
        {
            // Skip the leading `,` or ` `
            // if the item is a flag and the proper conditions are met
            uchar_kt *t = str;
            ...
            // If the item is right aligned and
            // not wide enough, pad with fill characters.
            if(minwid > 0)
            {
            ...
            {
                // Note:
                // The negative value denotes a left aligned item.
            ...
            while(*t && out_p < out_end_p)
            {
                *out_p++ = *t++;
                ...
                // Change a space by fillchar,
                // unless fillchar is '-' and a digit follows.
                if(fillable
                   && out_p[-1] == ' '
            ...
            // For left-aligned items, fill
            // any remaining space with the fillchar
            for(; l < minwid && out_p < out_end_p; l++)
            {
                *out_p++ = fillchar;
            }
        }
        // Otherwise if the item is a number,
        // copy that to the output buffer.
        else if(num >= 0)
        {
            ...
            *t = 0;
            ...
            //
            // Note:
            // We have to cast the base because the compiler uses
            ...
            // VIRTCOL_ALT takes up an extra character
            // because of the `-` we added above.
            if(opt == STL_VIRTCOL_ALT)
            {
                num_chars++;
            }
            ...
            // Figure out the approximate number in "scientific"
            // type notation. Ex: 14532 with maxwid of 4 -> '14>3'
            if(num_chars > maxwid)
            {
                // Add two to the width because the
                // power piece will take two extra characters
                num_chars += 2;
            ...
            // to the end of the number we just printed
            out_p += ustrlen(out_p);
        }
        // Otherwise, there was nothing
        // to print so mark the item as empty
        else
        {
        ...
        curitem++; // Item processed, move to the next
    }
    ...
    *out_p = NUL;
    ...
    if(maxwidth > 0 && width > maxwidth)
    {
        ...
        else // Otherwise, look for the truncation item
        {
            ...
            {
                if(items[i].type == Trunc) // Truncate at %< items.
                {
        ...
        if(width - ustr_scrsize(trunc_p) >= maxwidth)
        {
        ...
        else // Truncate at the truncation point we found
        {
            ...
            // Truncate the string
            uchar_kt *trunc_end_p = trunc_p + trunc_len;

            xstrmove(trunc_p + 1, trunc_end_p);
            *trunc_p = '<'; // Put a `<` to mark where we truncated at

            if(width + 1 < maxwidth)
            {
                // Advance the pointer to the end of the string
                trunc_p = trunc_p + ustrlen(trunc_p);
            }
            ...
            {
                // Items starting at or after the end of the truncated
                // section need to be moved backwards.
        ...
        width = maxwidth;
            ...
            && ustrlen(out) + (size_t)(maxwidth - width) + 1 < outlen)
    {
        ...
        if(num_separators)
        {
            ...
            for(int i = 0; i < num_separators; i++)
            {
                ...
                uchar_kt *seploc =
                    items[separator_locations[i]].start + dislocation;

                xstrmove(seploc, items[separator_locations[i]].start);

                for(uchar_kt *s = items[separator_locations[i]].start;
                    s < seploc;
                    s++)
                {
                    *s = fillchar;
...
void get_rel_pos(win_st *wp, uchar_kt *buf, int buflen)
{
    if(buflen < 3)
    {
        return; // Need at least 3 chars for writing.
    }
    ...
    if(below <= 0)
    {
        ustrlcpy(buf, (above == 0 ? _("All") : _("Bot")), buflen);
    }
    else if(above <= 0)
    {
        ustrlcpy(buf, _("Top"), buflen);
    }
    else
    {
        xsnprintf((char *)buf,
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    uchar_kt *p = buf + ustrlen(buf); // go to the end of the buffer
    ...
    *p++ = '(';
    ...
    xsnprintf((char *)p,
              (size_t)(buflen - (p - buf)),
              wp->w_arg_idx_invalid ? "(%d) of %d)" : "%d of %d)",
              wp->w_arg_idx + 1,
              carg_cnt);
...
void fname_expand(filebuf_st *FUNC_ARGS_UNUSED_MAYBE(buf),
                  uchar_kt **ffname,
                  uchar_kt **sfname)
{
    ...
    *ffname =(uchar_kt *)fix_fname((char *)*ffname); // expand to full path
    ...
    if(!buf->b_p_bin)
    {
        ...
        {
            xfree(*ffname);
            *ffname = (uchar_kt *)rfname;
            *sfname = (uchar_kt *)rfname;
        }
    }
#endif
}
...
uchar_kt *alist_name(aentry_st *aep)
{
    ...
    filebuf_st *bp = buflist_findnr(aep->ae_fnum);
...
void do_arg_all(int count, int forceit, int keep_tabs)
{
    ...
    for(;;)
    {
        ...
        for(win_st *wp = firstwin; wp != NULL; wp = wpnext)
        {
            ...
            else
            {
                ...
                {
                    aentry_st *arg_data = (aentry_st *)(alist->al_ga.ga_data);
                    if(i < alist->al_ga.ga_len
                       && (arg_data[i].ae_fnum == buf->b_id
                           || path_full_compare(alist_name(&arg_data[i]),
                                                buf->b_ffname,
                                                TRUE) & kEqualFiles))
                    {
                        int weight = 1;
                        ...
                        if(wp->w_alist != alist)
                        {
                            // Use the current argument list for all
                            //windows containing a file from it.
            ...
            if(i == opened_len && !keep_tabs) // close this window
            {
                if(P_HID(buf)
                   ...
                   || !bufIsChanged(buf))
                {
                    // If the buffer was changed, and we
                    // would like to hide it, try autowriting.
                    if(!P_HID(buf)
    ...
    // carg_cnt may change while doing this, because of autocommands.
...
void do_modelines(int flags)
{
    ...
    for(lnum = curbuf->b_ml.ml_line_count;
        lnum > 0
        && lnum > nmlines
        && lnum > curbuf->b_ml.ml_line_count - nmlines;
        --lnum)
    {
        if(chk_modeline(lnum, flags) == FAIL)
        {
...
static int chk_modeline(linenum_kt lnum, int flags)
{
    uchar_kt *s;
    uchar_kt *e;
    uchar_kt *linecopy; // local copy of any modeline found
    int prev;
    intmax_t vers;
    int end;
    int retval = OK;
    ...
    for(s = ml_get(lnum); *s != NUL; ++s)
    {
        ...
        {
            if((prev != -1 && ustrncmp(s, "ex:", (size_t)3) == 0)
               || ustrncmp(s, "vi:", (size_t)3) == 0)
            {
                break;
            }
            ...
            if((s[0] == 'v' || s[0] == 'V') && s[1] == 'i' && s[2] == 'm')
            {
                ...
                if(*e == ':'
                   && (s[0] != 'V' || ustrncmp(skipwhite(e + 1), "set", 3) == 0)
                   && (s[3] == ':'
    ...
    s = linecopy = ustrdup(s); // copy the line, it will change
    save_sourcing_lnum = sourcing_lnum;
    save_sourcing_name = sourcing_name;
    sourcing_lnum = lnum; // prepare for emsg()
    sourcing_name = (uchar_kt *)"modelines";
    end = FALSE;
    ...
    while(end == FALSE)
    {
        ...
        if(ustrncmp(s, "set ",
                   (size_t)4) == 0 || ustrncmp(s, "se ", (size_t)3) == 0)
        {
            if(*e != ':') // no terminating ':'?
            {
                break;
...
uchar_kt *buf_spname(filebuf_st *buf)
{
    if(bt_quickfix(buf))
    {
        win_st *win;
        ...
        {
            return (uchar_kt *)_(msg_loclist);
        }
        else
        {
            return (uchar_kt *)_(msg_qflist);
        }
    }

    // There is no _file_ when 'buftype' is "nofile",
    // b_sfname contains the name as specified by the user
    if(bt_nofile(buf))
    {
        ...
        return (uchar_kt *)_("[Scratch]");
    }

    if(buf->b_fname == NULL)
    {
        return (uchar_kt *)_("[No Name]");
    }
...
///
/// If found true is returned and "wp" and "tp" are set
/// to the window and tabpage. If not found, false is returned.
///
...
bool find_win_for_buf(filebuf_st *buf, win_st **wp, tabpage_st **tp)
{
    *wp = NULL;
    *tp = NULL;
                        ...
                        int typenr)
{
    ...
    if(prev == NULL)
    {
        // When adding first sign need to redraw
        // the windows to create the column for signs.
        ...
        buf->b_signlist = newsign; // first sign in signlist
    }
    else
    {
        prev->next = newsign;
    ...
    for(sign = buf->b_signlist; sign != NULL; sign = sign->next)
    {
        if(lnum == sign->lnum && id == sign->id)
        {
            ...
            return;
                ...
                || (id < 0 && lnum < sign->lnum))
        {
            // attempt to keep signs sorted by lnum
...
int buf_getsigntype(filebuf_st *buf, linenum_kt lnum, int type)
{
    ...
    for(sign = buf->b_signlist; sign != NULL; sign = sign->next)
    {
        if(sign->lnum == lnum
           && (type == SIGN_ANY
               || (type == SIGN_TEXT && sign_get_text(sign->typenr) != NULL)
               || (type == SIGN_LINEHL && sign_get_attr(sign->typenr, TRUE) != 0)))
        {
            return sign->typenr;
        }
    }
...
linenum_kt buf_delsign(filebuf_st *buf, int id)
{
    ...
    // When deleted the last sign needs to redraw
    // the windows to remove the sign column.
    if(buf->b_signlist == NULL)
    {
        redraw_buf_later(buf, NOT_VALID);
        changed_cline_bef_curs();
...
/// Find the line number of the sign with the requested id.
/// If the sign does not exist, return 0 as the line number.
/// This will still let the correct file get loaded.
///
/// @param buf  buffer sign is stored in
/// @param id   sign id
///
...
/// List placed signs for "rbuf".
/// If "rbuf" is NULL do it for all buffers.
void sign_list_placed(filebuf_st *rbuf)
{
    ...
    while(buf != NULL && !got_int)
    {
        if(buf->b_signlist != NULL)
        {
            xsnprintf(lbuf, BUFSIZ, _("Signs for %s:"), buf->b_fname);
...
///
/// @param buf
/// The buffer to add highlights to
///
/// @param src_id
/// src_id to use or 0 to use a new src_id group, or -1 for ungrouped highlight.
///
/// @param hl_id
/// Id of the highlight group to use
///
/// @param lnum
/// The line to highlight
///
/// @param col_start
/// First column to highlight
///
/// @param col_end
/// The last column to highlight, or -1 to highlight to end of line
///
/// @return The src_id that was used
int bufhl_add_hl(filebuf_st *buf,
                 int src_id,
                 int hl_id,
                 linenum_kt lnum,
                 ...
                 columnum_kt col_end)
{
    ...
    if(hl_id <= 0)
    {
        // no highlight group or
        // invalid line, just return src_id
        return src_id;
    }
...
///
/// @param buf
/// The buffer to remove highlights from
///
/// @param src_id
/// highlight source group to clear, or -1 to clear all groups.
///
/// @param line_start
/// first line to clear
///
...
void bufhl_clear_line_range(filebuf_st *buf,
                            int src_id,
...
///
/// @param bufhl_info
/// The highlight info for the buffer
///
/// @param src_id
/// Highlight source group to clear, or -1 to clear all groups.
///
...
void bufhl_clear_all(filebuf_st *buf)
{
    if(!buf->b_bufhl_info)
    {
        return;
...
void bufhl_mark_adjust(filebuf_st *buf,
                       linenum_kt line1,
                       linenum_kt line2,
                       long amount,
                       long amount_after)
...
bool bufhl_start_line(filebuf_st *buf, linenum_kt lnum, bufhl_lineinfo_st *info)
{
    if(!buf->b_bufhl_info)
    {
        return false;
...
///
/// It is is assumed this will be called with non-decreasing
/// column nrs, so that it is possible to only recalculate
/// highlights at endpoints.
///
...
bool buf_contents_changed(filebuf_st *buf) FUNC_ATTR_NONNULL_ALL
{
    ...
    filebuf_st *newbuf = buflist_new(NULL, NULL, (linenum_kt)1, BLN_DUMMY);
                   ...
                   READ_NEW | READ_DUMMY) == OK)
    {
        ...
        if(buf->b_ml.ml_line_count == curbuf->b_ml.ml_line_count)
        {
            ...
            {
                if(ustrcmp(ml_get_buf(buf, lnum, false), ml_get(lnum)) != 0)
                {
                    differ = true;
                    break;
                }
...
/// Wipe out a buffer and decrement the last buffer number if it was used
/// for this buffer. Call this to wipe out a temp buffer that does not
/// contain any marks.
///
/// @param buf
/// @param aucmd    When TRUE trigger autocommands.
void wipe_buffer(filebuf_st *buf, int aucmd)
{
    if(!aucmd)
    {
        block_autocmds(); // Don't trigger BufDelete autocommands here.
    }
