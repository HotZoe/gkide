...
/// op_shift, op_delete, op_tilde, op_change, op_yank, do_put, do_join
...
static void shift_block(oparg_st *oap, int amount)
{
    ...
    {
        // column to which text in block will be shifted
        columnum_kt destination_col;
        // end of the part of the line which is copied verbatim
        uchar_kt *verbatim_copy_end;
        // the (displayed) width of this part of line
        columnum_kt verbatim_copy_width;
        // nr of spaces that replace a TAB
        size_t fill;
        // the length of the line after the block shift
        size_t new_line_len;
        columnum_kt block_space_width;
        columnum_kt shift_amount;
        uchar_kt *non_white = bd.textstart;
        columnum_kt non_white_col;
        ...
        {
            incr = lbr_chartabsize_adv(bd.textstart,
                                       &non_white,
                                       non_white_col);
        ...
        // Now let's find out how much of the beginning of the
        // line we can reuse without modification.
        ...
        {
            uchar_kt *line = verbatim_copy_end;
    ...
    changed_bytes(curwin->w_cursor.lnum, (columnum_kt)bd.textcol);
    curmod = oldstate;
...
void op_reindent(oparg_st *oap, Indenter how)
{
    ...
    uchar_kt *l;
    int amount;
    linenum_kt first_changed = 0;
    linenum_kt last_changed = 0;
...
static uchar_kt *expr_line = NULL;
...
void set_expr_line(uchar_kt *new_line)
{
    xfree(expr_line);
    expr_line = new_line;
}
...
uchar_kt *get_expr_line_src(void)
{
    if(expr_line == NULL)
    {
        return NULL;
...
///
/// - YREG_PASTE:
///   Prepare for pasting the register @b regname. With no regname specified,
///   read from last written register, or from unnamed clipboard (depending on
///   the `clipboard=unnamed` option).
///   Queries the clipboard provider if necessary.
///
/// - YREG_YANK:
///   Preparare for yanking into @b regname. With no regname specified,
...
yankreg_st *get_yank_register(int regname, int mode)
{
    ...
    if(mode == YREG_PASTE && get_clipboard(regname, &reg, false))
    {
        ...
        return reg;
            ...
            && y_previous != NULL)
    {
        // in case clipboard not available,
...
FUNC_ATTR_NONNULL_RET
{
    ...
    *copy = *reg;
    ...
    {
        copy->y_array = xcalloc(copy->y_size, sizeof(uchar_kt *));
...
///
/// @return FAIL for failure, OK otherwise
static int stuff_yank(int regname, uchar_kt *p)
{
    ...
    else
    {
        ...
        reg->y_array = (uchar_kt **)xmalloc(sizeof(uchar_kt *));
        reg->y_array[0] = p;
        reg->y_size = 1;
...
int do_execreg(int regname, int colon, int addcr, int silent)
{
    ...
    else
    {
        ...
        {
            // insert NL between lines and after
            // last line if type is kMTLineWise
            if(reg->y_type == kMTLineWise
               || i < reg->y_size - 1 || addcr)
            {
                if(ins_typebuf((uchar_kt *)"\n",
                               remap,
                               0,
                               true,
                               silent) == FAIL)
            ...
            if(colon && ins_typebuf((uchar_kt *)":", remap, 0, TRUE, silent)
               == FAIL)
            {
                return FAIL;
            }
...
static void put_reedit_in_typebuf(int silent)
{
    ...
    if(restart_edit != NUL)
    {
        ...
        {
            buf[0] = (uchar_kt)(restart_edit == 'I' ? 'i' : restart_edit);
            buf[1] = NUL;
        }
...
/// Insert register contents "s" into the typeahead buffer,
/// so that it will be executed again.
///
/// When "esc" is TRUE it is to be taken literally:
/// Escape CSI characters and no remapping.
///
/// @param s
/// @param esc
...
///
/// @return
static int put_in_typebuf(uchar_kt *s, int esc, int colon, int silent)
{
    int retval = OK;
    put_reedit_in_typebuf(silent);
    ...
    if(retval == OK)
    {
        ...
        {
            retval = ins_typebuf(p,
                                 esc ? REMAP_NONE : REMAP_YES,
                                 0,
                                 TRUE,
                                 silent);
        }
    ...
    if(colon && retval == OK)
    {
        retval = ins_typebuf((uchar_kt *)":", REMAP_NONE, 0, TRUE, silent);
    }
...
///
/// @param regname
/// @param literally  insert literally, not as if typed
///
/// @return FAIL for failure, OK otherwise
...
static void stuffescaped(const char *arg, int literally)
{
    while(*arg != NUL)
    {
        ...
        while((*arg >= ' ' && *arg < DEL)
              || ((uint8_t)(*arg) == K_SPECIAL && !literally))
        {
            arg++;
        }
...
/// If @b regname is a special register, return TRUE
/// and store a pointer to its value in @b argp.
///
/// @param regname
/// @param argp
...
///
/// @return
int get_spec_reg(int regname, uchar_kt **argp, int *allocated, int errmsg)
{
    size_t cnt;
    *argp = NULL;
    *allocated = FALSE;
    ...
    switch(regname)
    {
        ...
        case Ctrl_P: // Path under cursor, expand via "path"
            ...
            *argp = file_name_at_cursor(FNAME_MESS
                                        | FNAME_HYP
                                        | (regname == Ctrl_P ? FNAME_EXP : 0),
                                        1L,
                                        NULL);
            *allocated = TRUE;
            return TRUE;
...
bool cmdline_paste_reg(int regname, bool literally, bool remcr)
{
    ...
    for(size_t i = 0; i < reg->y_size; i++)
    {
        ...
        {
            cmdline_paste_str((uchar_kt *)"\r", literally);
        }

        // Check for CTRL-C, in case someone tries
        // to paste a few thousand lines and gets bored.
        os_breakcheck();
...
int op_delete(oparg_st *oap)
{
    ...
    // Nothing to delete, return here.
    // Do prepare undo, for op_change().
    if(oap->empty)
    {
    ...
    // and the result is a blank line, make the delete linewise.
    ...
    if(oap->motion_type == kMTBlockWise)
    {
        ...
        changed_lines(curwin->w_cursor.lnum,
                      curwin->w_cursor.col,
                      oap->end.lnum + 1, 0L);
    ...
    else
    {
        if(virtual_op)
        {
               ...
               && oap->inclusive)
            {
                ...
                if(u_save((linenum_kt)(oap->end.lnum - 1),
                          (linenum_kt)(oap->end.lnum + 1)) == FAIL)
                {
                    return FAIL;
                }
        ...
        if(oap->line_count == 1) // delete characters within one line
        {
            ...
            (void)del_bytes((columnum_kt)n, !virtual_op,
        ...
        else
        {
            ...
            if(u_save((linenum_kt)(curwin->w_cursor.lnum - 1),
                      (linenum_kt)(curwin->w_cursor.lnum
                                 + oap->line_count)) == FAIL)
            {
                return FAIL;
            }
...
int op_replace(oparg_st *oap, int c)
{
    ...
    if(oap->motion_type == kMTBlockWise)
    {
            ...
            ++curwin->w_cursor.lnum)
        {
            ...
            // A double-wide character can be
            // replaced only up to half the times.
            ...
            else
            {
                ...
                after_p = (uchar_kt *)xmalloc(after_p_len);
    ...
    else
    {
        ...
        while(ltoreq(curwin->w_cursor, oap->end))
        {
            ...
            if(n != NUL)
            {
                ...
                else
                {
                    if(n == TAB)
                    {
                        ...
                        if(curwin->w_cursor.lnum == oap->end.lnum)
                        {
                            // oap->end has to be recalculated
                            // when the tab breaks
            ...
            else if(virtual_op && curwin->w_cursor.lnum == oap->end.lnum)
            {
                ...
                coladvance_force(getviscol2(oap->end.col,
                                            oap->end.coladd) + 1);
...
void op_tilde(oparg_st *oap)
{
    ...
    else // not block mode
    {
        ...
        {
            did_change = swapchars(oap->op_type,
                                   &pos,
...
/// Invoke swapchar() on @b length bytes at position @b pos.
///
/// @b pos is advanced to just after the changed characters.
/// @b length is rounded up to include the whole last multi-byte character.
...
void op_insert(oparg_st *oap, long count1)
{
    ...
    if(oap->op_type == OP_APPEND)
    {
           ...
           && curwin->w_cursor.coladd == 0)
        {
            ...
            {
                // First line was too short, make it longer
                // and adjust the values in "bd".
                if(u_save_cursor() == FAIL)
                {
                    return;
                }
    ...
    t1 = oap->start;
    edit(NUL, false, (linenum_kt)count1);
...
int op_change(oparg_st *oap)
{
    ...
    retval = edit(NUL, FALSE, (linenum_kt)1);

    // In Visual block mode, handle copying the new text to all lines of
    // the block. Don't repeat the insert when Insert mode ended with CTRL-C.
    if(oap->motion_type == kMTBlockWise
       && oap->start.lnum != oap->end.lnum && !got_int)
    {
        ...
        if(bd.textcol > (columnum_kt)pre_indent)
        {
                        ...
                        bool append)
{
    ...
    reg->y_width = 0;
    reg->y_array = xcalloc(yanklines, sizeof(uchar_kt *));
    ...
    for(; lnum <= yankendlnum; lnum++, y_idx++)
    {
        switch(reg->y_type)
        {
            ...
            case kMTCharWise:
            {
                columnum_kt startcol = 0, endcol = MAXCOL;
    ...
    {
        new_ptr = xmalloc(sizeof(uchar_kt *) * (curr->y_size + reg->y_size));
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    // name of requested register or the
...
/// Put contents of register @b regname into the text.
/// Caller must check @b regname to be valid!
///
/// @param flags
/// - PUT_FIXINDENT   make indent look nice
...
void do_put(int regname, yankreg_st *reg, int dir, long count, int flags)
{
    ...
    // because the register includes special characters (newlines, etc.).
    if(regname == '.')
    {
        ...
        // To avoid 'autoindent' on linewise puts,
        // create a new line with `:put _`.
        if(flags & PUT_LINE)
        {
            do_put('_', NULL, dir, 1, PUT_LINE);
        }

        // If given a count when putting linewise,
        // we stuff the readbuf with the dot register
        ...
        if(flags & PUT_LINE)
        {
            ...
            for(; count > 0; count--)
            {
                ...
                if(count != 1)
                {
                    // To avoid 'autoindent' affecting the text,
                    // use Ctrl_U to remove any whitespace.
                    // Can't just insert Ctrl_U into readbuf1,
                    // this would go back to the previous line in
                    // the case of 'noautoindent' and 'backspace'
                    // includes "eol". So we insert a dummy space
                    // for Ctrl_U to consume.
                    stuffReadbuff("\n ");
        ...
        if(flags & PUT_CURSEND)
        {
            ...
            {
                // Avoid ringing the bell from attempting to move
                // into the space after the current line. We can stuff
                ...
                // curwin->w_cursor.col.
                uchar_kt *cursor_pos = get_cursor_pos_ptr();
        ...
        // So the 'u' command restores cursor position after ".p,
        // save the cursor position now (though not saving any text).
    ...
    if(!curbuf->terminal)
    {
        ...
        {
            ERROR_LOG(_("Failed to save undo information"));
            return;
        }
    }
    ...
    if(insert_string != NULL)
    {
        ...
        if(regname == '=')
        {
            ...
            for(;;)
            {
                ...
                y_array = (uchar_kt **)xmalloc(y_size * sizeof(uchar_kt *));
            }
        }
        else
        {
    ...
    if(y_type == kMTLineWise)
    {
        ...
        {
            // "p" or "P" in Visual mode: split the
            // lines to put the text in between.
            if(u_save_cursor() == FAIL)
            {
                goto end;
            }
    ...
    if(y_size == 0 || y_array == NULL)
    {
        EMSG2(_("E353: Nothing in register %s"),
              regname == 0 ? (uchar_kt *)"\"" : transchar(regname));
    ...
    {
        lnum = curwin->w_cursor.lnum + (linenum_kt)y_size + 1;
    ...
    else if(y_type == kMTLineWise)
    {
        ...
        // Don't move the cursor yet, u_save() uses it.
        if(dir == BACKWARD)
        {
        ...
        // In an empty buffer the empty line is going
        // to be replaced, include it in the saved lines.
    ...
    if(ve_flags == VE_ALL && y_type == kMTCharWise)
    {
        if(gchar_cursor() == TAB)
        {
            // Don't need to insert spaces when "p" on the last
            // position of a tab or "P" on the first position.
            if(dir == FORWARD
    ...
    if(y_type == kMTBlockWise)
    {
        ...
        for(i = 0; i < y_size; i++)
        {
            ...
            {
                if(ml_append(curbuf->b_ml.ml_line_count, (uchar_kt *)"",
                             (columnum_kt)1, FALSE) == FAIL)
                {
                    break;
                }
            ...
            for(ptr = oldp; vcol < col && *ptr;)
            {
                ...
                incr = lbr_chartabsize_adv(oldp, &ptr, (columnum_kt)vcol);
                vcol += incr;
            }

            bd.textcol = (columnum_kt)(ptr - oldp);
            ...
            newp = (uchar_kt *) xmalloc(totlen + oldlen + 1);
        ...
        curbuf->b_op_end.col = bd.textcol + (columnum_kt)totlen - 1;
    ...
    else
    {
        ...
        {
            // if type is kMTCharWise, FORWARD is the same
            // as BACKWARD on the next char
            ...
            do
            {
                ...
                if(totlen > 0)
                {
                    ...
                    if(lnum == curwin->w_cursor.lnum)
                    {
                        ...
                        curwin->w_cursor.col += (columnum_kt)(totlen - 1);
                    }
                }
        ...
        {
            // Insert at least one line.
            // When y_type is kMTCharWise, break the first line in two.
            for(cnt = 1; cnt <= count; cnt++)
            {
                ...
                if(y_type == kMTCharWise)
                {
                    ...
                    xfree(newp);
                    oldp = ml_get(lnum);
                    newp = (uchar_kt *) xmalloc((size_t)(col + yanklen + 1));
                ...
                for(; i < y_size; i++)
                {
                    if((y_type != kMTCharWise || i < y_size - 1)
                       && ml_append(lnum, y_array[i],
                                    (columnum_kt)0, false) == FAIL)
                    {
                        goto error;
                    }
        ...
        error:
            ...
            // Skip mark_adjust when adding lines after
            // the last one, there can't be marks there.
            if(curbuf->b_op_start.lnum
               + (y_type == kMTCharWise)
               - 1
               + nr_lines
...
/// should not be there move it left.
...
static void dis_msg(uchar_kt *p, int skip_esc)
{
    int n;
    int l;
    n = (int)Columns - 6;
...
/// If @b process is true and the line begins with a comment leader
/// (possibly after some white space), return a pointer to the text after it.
...
uchar_kt *skip_comment(uchar_kt *line,
                     bool process,
                     bool include_space,
                     bool *is_comment)
{
    uchar_kt *comment_flags = NULL;
    int lead_len;
    ...
    *is_comment = false;
    ...
    if(leader_offset != -1)
    {
        // Let's check whether the line ends with
        // an unclosed comment. If the last comment
        // leader has COM_END in flags, there's no comment.
        while(*comment_flags)
        {
...
/// Join @b count lines (minimal 2) at cursor position.
/// When @b save_undo is TRUE save lines for undo first.
/// Set @b use_formatoptions to FALSE when e.g.
/// processing backspace and comment leaders should not be removed.
...
{
    uchar_kt *curr = NULL;
    uchar_kt *curr_start = NULL;
    uchar_kt *cend;
    uchar_kt *newp;
    uchar_kt *spaces; // number of spaces inserted before a line
    int endcurr1 = NUL;
    int endcurr2 = NUL;
    ...
    if(save_undo && u_save(curwin->w_cursor.lnum - 1,
                           curwin->w_cursor.lnum + (linenum_kt)count) == FAIL)
    {
        return FAIL;
    }
    ...
    for(t = 0; t < (linenum_kt)count; t++)
    {
        curr = curr_start = ml_get((linenum_kt)(curwin->w_cursor.lnum + t));
        ...
        if(remove_comments)
        {
            ...
            {
                uchar_kt *new_curr = skip_comment(curr, TRUE,
                                                insert_space,
                                                &prev_was_comment);
        ...
        if(insert_space && t > 0)
        {
              ...
              )
            {
                ...
                // Extra space when 'joinspaces' set
                // and line ends in '.', '?', or '!'.
                if(p_js && (endcurr1 == '.'
                            || endcurr1 == '?'
                            || endcurr1 == '!'))
    ...
    newp = (uchar_kt *) xmalloc((size_t)(sumsize + 1));
    cend = newp + sumsize;
    *cend = 0;

    // Move affected lines to the new long one.
    //
    // Move marks from each deleted line to the joined line,
    // adjusting the column. This is not Vi compatible, but
    // Vi deletes the marks, thus that should not really be a problem.
    for(t = (linenum_kt)count - 1;; t--)
    {
        cend -= currsize;
        ...
        mark_col_adjust(curwin->w_cursor.lnum + t,
                        (columnum_kt)0,
                        (linenum_kt)-t,
                        (long)(cend
                               - newp
        ...
        curr = curr_start = ml_get((linenum_kt)(curwin->w_cursor.lnum
                                              + t
                                              - 1));
...
/// 'leader1' must match 'leader2_len' characters from 'leader2'
static int same_leader(linenum_kt lnum,
                       int leader1_len,
                       uchar_kt *leader1_flags,
                       int leader2_len,
                       uchar_kt *leader2_flags)
{
    int idx1 = 0, idx2 = 0;
    uchar_kt *p;
    uchar_kt *line1;
    ...
    // If fist leader has 's' flag, the lines can only be joined if
    // there is some text after it and the second line has the 'm' flag.
...
void op_format(oparg_st *oap, int keep_cursor)
{
    ...
    // Place the cursor where the "gq" or "gw" command
    // was given, so that "u" can put it back there.
    ...
    if(oap->is_VIsual)
    {
        // When there is no change: need
        // to remove the Visual selection
        redraw_curbuf_later(INVERTED);
    }
    ...
    // For "gw" remember the cursor position and put
    // it back below (adjusted for joined and split lines).
    if(keep_cursor)
    {
...
void op_formatexpr(oparg_st *oap)
{
    ...
    {
        // As documented: when 'formatexpr' returns
        // non-zero fall back to internal formatting.
        op_format(oap, FALSE);
    }
}
...
///
/// @return
int fex_format(linenum_kt lnum, long count, int c)
{
    int r;
    uchar_kt *fex;
...
/// Format @b line_count lines, starting at the cursor position.
/// When @b line_count is negative, format until the end of the paragraph.
/// Lines after the cursor line are saved for undo, caller must have saved the
/// first line.
///
/// @param line_count
/// @param avoid_fex   don't use 'formatexpr'
void format_lines(linenum_kt line_count, int avoid_fex)
{
    int max_len;
    ...
    uchar_kt *leader_flags = NULL; // flags for leader of current line
    uchar_kt *next_leader_flags; // flags for leader of next line
    ...
    for(count = line_count; count != 0 && !got_int; --count)
    {
        ...
        {
            // For the first line of a paragraph, check indent of
            // second line. Don't do this for comments and empty lines.
            if(first_par_line
            ...
            // If we have got to the end of a paragraph,
            // or the line is getting long, format it.
            ...
            if(!is_end_par)
            {
                ...
                {
                    uchar_kt *p = get_cursor_line_ptr();
...
static int ends_in_white(linenum_kt lnum)
{
    size_t l;
    uchar_kt *s = ml_get(lnum);
...
static int fmt_check_par(linenum_kt lnum,
                         int *leader_len,
                         uchar_kt **leader_flags,
                         int do_comments)
{
    uchar_kt *flags = NULL; // init for GCC
    uchar_kt *ptr;
    ptr = ml_get(lnum);
...
int paragraph_start(linenum_kt lnum)
{
    uchar_kt *p;
    int leader_len = 0; // leader len of current line
    uchar_kt *leader_flags = NULL; // flags for leader of current line
    int next_leader_len = 0; // leader len of next line
    uchar_kt *next_leader_flags = NULL; // flags for leader of next line
    int do_comments; // format comments
...
///   first/last deleted char minus the number of columns that have
///   to be deleted.
/// for yank and tilde:
                       ...
                       int is_del)
{
    ...
    {
        // Count a tab for what it's worth (if list mode not on)
        incr = lbr_chartabsize(line, pstart, (columnum_kt)bdp->start_vcol);
        bdp->start_vcol += incr;
    ...
    {
        // notice: this converts partly selected Multibyte
        // characters to spaces, too.
        ...
        else
        {
            ...
            while(bdp->end_vcol <= oap->end_vcol && *pend != NUL)
            {
                ...
                incr = lbr_chartabsize_adv(line, &pend,
                                           (columnum_kt)bdp->end_vcol);
    ...
    bdp->textcol = (columnum_kt)(pstart - line);
...
void op_addsub(oparg_st *oap, linenum_kt Prenum1, bool g_cmd)
{
    ...
    else
    {
        ...
        if(change_cnt > p_report)
        {
            ...
            {
                smsg((char *)_("%" PRId64 " lines changed"),
                     (int64_t)change_cnt);
            }
        }
    }
...
int do_addsub(int op_type, apos_st *pos, int length, linenum_kt Prenum1)
{
    ...
    // If a number was found, and saving
    // for undo works, replace the number.
    firstdigit = ptr[col];
    ...
    else
    {
        ...
        // Don't include the '-' in the length, only the
        // length of the part after it is kept the same.
        if(c == '-')
        {
            length--;
        }
...
/// @param reg_width The width, only used if @b reg_type is kMTBlockWise.
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    switch(reg_type)
    {
        ...
        case kMTBlockWise:
            snprintf(buf, buf_len, CTRL_V_STR "%" ColumNumKtPrtFmt, reg_width + 1);
            break;
...
/// When @b flags has @ref kGRegList return a list with text @b s.
/// Otherwise just return @b s.
///
/// Returns a void * for use in get_reg_contents().
static void *get_reg_wrap_one_line(uchar_kt *s, int flags)
{
    if(!(flags & kGRegList))
    {
        return s;
...
{
    // Don't allow using an expression
    // register inside an expression.
    if(regname == '=')
    {
...
/// store @b str in register @b name
///
                            ...
                            columnum_kt block_len)
{
    if(name == '/' || name == '=')
    {
        ...
        {
            EMSG(_("E883: search pattern and expression register "
                   "may not contain two or more lines"));
            return;
        }
                           ...
                           columnum_kt block_len)
{
    ...
    if(name == '=')
    {
        ...
        // modify the global expr_line, extend/shrink it
        // if necessary (realloc). Copy the input string
        // into the adjusted memory at the specified offset.
...
FUNC_ATTR_NONNULL_ALL
{
    ...
    if(str_list)
    {
        for(uchar_kt **ss = (uchar_kt **) str; *ss != NULL; ++ss)
        {
            newlines++;
        }
    }
    ...
    uchar_kt **pp = xrealloc(y_ptr->y_array,
                           (y_ptr->y_size + newlines) * sizeof(uchar_kt *));
    y_ptr->y_array = pp;
    ...
    else
    {
        ...
        for(const uchar_kt *start = str, *end = str + len;
            start < end + extraline;
            start += line_len + 1, lnum++)
        {
            ...
            line_len =
                (size_t)((uchar_kt *)xmemscan(start, '\n',
    ...
    if(yank_type == kMTBlockWise)
    {
        y_ptr->y_width = (blocklen == -1 ? (columnum_kt) maxlen - 1 : blocklen);
    }
    else
    {
        y_ptr->y_width = 0;
...
static long line_count_info(uchar_kt *line,
                            long *wc,
                            long *cc,
                            long limit,
                            int eol_size)
        ...
        for(lnum = 1; lnum <= curbuf->b_ml.ml_line_count; ++lnum)
        {
               ...
               && lnum <= max_pos.lnum)
            {
                ...
                switch(l_VIsual_mode)
                {
                    ...
                    case 'v':
                    {
                        columnum_kt start_col = (lnum == min_pos.lnum)
                                            ? min_pos.col : 0;

                        columnum_kt end_col = (lnum == max_pos.lnum)
                                          ? max_pos.col - start_col + 1
                                          : MAXCOL;
                ...
                if(s != NULL)
                {
                    byte_count_cursor += line_count_info(s,
                                                         &word_count_cursor,
                                                         &char_count_cursor,
                                                         len, eol_size);
                ...
                if(lnum == curwin->w_cursor.lnum)
                {
                    ...
                    byte_count_cursor =
                        ...
                        + line_count_info(ml_get(lnum),
                                          &word_count_cursor,
                                          &char_count_cursor,
                                          (long)(curwin->w_cursor.col + 1),
                                          eol_size);
                }
            }

            // Add to the running totals
            byte_count += line_count_info(ml_get(lnum),
                                          &word_count,
                                          &char_count,
                                          (long)MAXCOL,
                                          eol_size);
...
static yankreg_st *adjust_clipboard_name(int *name, bool quiet, bool writing)
{
    ...
    else if((*name == NUL) && (cb_flags & CB_UNNAMEDMASK))
    {
        if(!eval_has_provider("clipboard"))
        {
            ...
            {
                msg((uchar_kt *)"clipboard: No provider. "
                              "Try \":CheckHealth\" or \":h clipboard\".");
...
static bool get_clipboard(int name, yankreg_st **target, bool quiet)
{
    ...
    if(res->lv_len == 2 && res->lv_first->li_tv.v_type == kNvarList)
    {
        ...
        uchar_kt *regtype = res->lv_last->li_tv.vval.v_string;
    ...
    for(listitem_st *li = lines->lv_first; li != NULL; li = li->li_next)
    {
        ...
        reg->y_array[i++] =
            (uchar_kt *)xstrdupnul((char *)li->li_tv.vval.v_string);
    }
...
///
/// @return
/// Pointer that needs to be passed to next op_register_iter()
