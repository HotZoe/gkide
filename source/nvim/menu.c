...
static uchar_kt menu_mode_chars[] = {'n', 'v', 's', 'o', 'i', 'c', 't'};

static uchar_kt e_notsubmenu[] = N_("E327: Part of menu-item path is not sub-menu");
static uchar_kt e_othermode[] = N_("E328: Menu only exists in another mode");
static uchar_kt e_nomenu[] = N_("E329: No menu \"%s\"");
...
void ex_menu(exargs_st *eap)
{
    ...
    if(ustrncmp(arg, "enable", 6) == 0 && ascii_iswhite(arg[6]))
    {
        enable = TRUE;
        arg = skipwhite(arg + 6);
    }
    else if(ustrncmp(arg, "disable", 7) == 0 && ascii_iswhite(arg[7]))
    {
        enable = FALSE;
        arg = skipwhite(arg + 7);
    }
    ...
    if(enable != MAYBE)
    {
        ...
        if(ustrcmp(menu_path, "*") == 0) // meaning: do all menus
        {
            menu_path = (uchar_kt *)"";
        }
    ...
    {
        // Delete menu(s).
        if(ustrcmp(menu_path, "*") == 0) // meaning: remove all menus
        {
            menu_path = (uchar_kt *)"";
        }
    ...
    else
    {
        ...
        if(ustricmp(map_to, "<nop>") == 0) // "<Nop>" means nothing
        {
            map_to = (uchar_kt *)"";
            map_buf = NULL;
        }
                         ...
                         uchar_kt *call_data)
{
    ...
    while(*name)
    {
        ...
        map_to = menutrans_lookup(name, (int)ustrlen(name));
    ...
    if(menu != NULL && modes)
    {
        p = (call_data == NULL) ? NULL : ustrdup(call_data);
        ...
        for(i = 0; i < MENU_MODES; ++i)
        {
            if(modes & (1 << i))
            {
                ...
                if(c != 0)
                {
                    menu->strings[i] = xmalloc(ustrlen(call_data) + 5);
                    menu->strings[i][0] = c;

                    if(d == 0)
                    {
                        ustrcpy(menu->strings[i] + 1, call_data);
                    }
                    else
                    {
                        menu->strings[i][1] = d;
                        ustrcpy(menu->strings[i] + 2, call_data);
                    }

                    if(c == Ctrl_C)
                    {
                        int len = (int)ustrlen(menu->strings[i]);
...
static int show_menus(uchar_kt *path_name, int modes)
{
    uchar_kt *p;
    uchar_kt *name;
    vimmenu_st *menu;
    ...
    menu = root_menu;
    name = path_name = ustrdup(path_name);
                                ...
                                int forceit)
{
    ...
    if(!ascii_iswhite(*p))
    {
        if(ustrncmp(arg, "enable", 6) == 0
           && (arg[6] == NUL ||  ascii_iswhite(arg[6])))
        {
            p = arg + 6;
        }
        else if(ustrncmp(arg, "disable", 7) == 0
    ...
    if(*p == NUL) // Complete the menu name
    {
        ...
        if(after_dot > arg)
        {
            ...
            ustrlcpy(path_name, arg, path_len);
        }
...
#define TBUFFER_LEN 256
    ...
    while(menu != NULL
          && (menu_is_hidden(menu->dname)
              ...
              || menu->dname[ustrlen(menu->dname) - 1] == '.'))
    {
        menu = menu->next;
    }
    ...
    if(menu->modes & expand_modes)
    {
        if(menu->children != NULL)
        {
            if(should_advance)
            {
                ustrlcpy(tbuffer, menu->en_dname, TBUFFER_LEN - 1);
            }
            else
            {
                ustrlcpy(tbuffer, menu->dname,  TBUFFER_LEN - 1);
...
uchar_kt *menu_name_skip(uchar_kt *name)
{
    uchar_kt  *p;
...
static int menu_namecmp(uchar_kt *name, uchar_kt *mname)
{
    int i;
...
static int get_menu_cmd_modes(uchar_kt *cmd,
                              int forceit,
                              int *noremap,
                              int *unmenu)
{
...
static uchar_kt *popup_mode_name(uchar_kt *name, int idx)
{
    size_t len = ustrlen(name);
    assert(len >= 4);

    uchar_kt *p = ustrndup(name, len + 1);
...
FUNC_ATTR_NONNULL_ARG(1)
{
    ...
    if(p != NULL)
    {
        ...
        assert(p >= str);
        text = ustrndup(str, (size_t)(p - str));
    }
    else
    {
        text = ustrdup(str);
...
int menu_is_menubar(uchar_kt *name)
{
...
int menu_is_popup(uchar_kt *name)
{
    return ustrncmp(name, "PopUp", 5) == 0;
}


/// Return TRUE if "name" is a toolbar menu name.
int menu_is_toolbar(uchar_kt *name)
{
    return ustrncmp(name, "ToolBar", 7) == 0;
}

/// Return TRUE if the name is a menu
/// separator identifier: Starts and ends with '-'
int menu_is_separator(uchar_kt *name)
{
    return name[0] == '-' && name[ustrlen(name) - 1] == '-';
}

/// Return TRUE if the menu is hidden:  Starts with ']'
static int menu_is_hidden(uchar_kt *name)
{
...
void ex_emenu(exargs_st *eap)
{
    ...
    {
        mode = (uchar_kt *)"Insert";
        idx = MENU_INDEX_INSERT;
    }
    else if(curmod & kCmdLineMode)
    {
        mode = (uchar_kt *)"Command";
...
void ex_menutranslate(exargs_st *eap)
{
    ...
    if(ustrncmp(arg, "clear", 5) == 0 && ends_excmd(*skipwhite(arg + 5)))
    {
...
static uchar_kt *menu_skip_part(uchar_kt *p)
{
...
static uchar_kt *menutrans_lookup(uchar_kt *name, int len)
{
    ...
    {
        if(ustrncmp(name, tp[i].from, len) == 0 && tp[i].from[len] == NUL)
        {
            return tp[i].to;
        }
    }
    ...
    for(int i = 0; i < menutrans_ga.ga_len; i++)
    {
        if(ustrcmp(dname, tp[i].from_noamp) == 0)
        {
            xfree(dname);
            return tp[i].to;
        }
...
static void menu_unescape_name(uchar_kt *name)
{
    uchar_kt *p;
...
static uchar_kt *menu_translate_tab_and_shift(uchar_kt *arg_start)
{
    ...
    while(*arg && !ascii_iswhite(*arg))
    {
        ...
        else if(ustrnicmp(arg, "<TAB>", 5) == 0)
        {
            *arg = TAB;
            xstrmove(arg + 1, arg + 5);
        }
