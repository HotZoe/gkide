/// @file nvim/screen.c
///
/// code for displaying on the screen
///
/// Output to the screen (console, terminal emulator or GUI window) is
/// minimized by remembering what is already on the screen, and only updating
/// the parts that changed.
///
/// - ScreenLines[off]
///   Contains a copy of the whole screen, as it is currently
///   displayed (excluding text written by external commands).
/// - ScreenAttrs[off]
///   Contains the associated attributes.
/// - LineOffset[row]
///   Contains the offset into ScreenLines*[] and ScreenAttrs[]
///   for each line.
/// - LineWraps[row]
...
///
/// The screen_*() functions write to the screen and handle
/// updating ScreenLines[].
///
...
/// check_cursor() to move the cursor into the visible part of the window,
/// and call redraw_later(VALID) to have the window displayed by
/// update_screen() later.
///
...
/// character used when a double-width character doesn't fit.
...
/// Mark all windows that are editing the current
/// buffer to be updated later.
void redraw_curbuf_later(int type)
{
    redraw_buf_later(curbuf, type);
}

void redraw_buf_later(filebuf_st *buf, int type)
{
    FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
    {
        if(wp->w_buffer == buf)
...
void update_screen(int type)
{
    ...
    // Don't do anything if the screen
    // structures are (not yet) valid.
    ...
    // Postpone the redrawing when it's not needed
    // and when being called recursively.
    ...
    // let syntax code know we're in a
    // next round of display updating
    ++display_tick;

    // if the screen was scrolled up
    // when displaying a message, scroll it down
    if(msg_scrolled)
    {
        clear_cmdline = TRUE;
        ...
        else if(type != CLEAR)
        {
            ...
            if(screen_ins_lines(0, 0, msg_scrolled,
                                (int)Rows, NULL) == FAIL)
            {
                type = CLEAR;
            }
    ...
    // Force redraw when width of 'number' or
    // 'relativenumber' column changes.
    if(curwin->w_redr_type < NOT_VALID
       && curwin->w_nrwidth
          != ((curwin->w_o_curbuf.wo_nu || curwin->w_o_curbuf.wo_rnu)
              ? number_width(curwin) : 0))
    {
        curwin->w_redr_type = NOT_VALID;
    }

    // Only start redrawing if
    ...
    // Correct stored syntax highlighting info for changes
    // in each displayed buffer. Each buffer must only be done once.
    FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
    {
    ...
    // Go from top to bottom through the windows,
    // redrawing the ones that need it.
    did_one = FALSE;
    ...
    FOR_ALL_WINDOWS_IN_TAB(wp, curtab)
    {
        ...
        // redraw status line after the
        // window to minimize cursor movement
        if(wp->w_redr_status)
        {
            win_redr_status(wp);
        }
...
{
    if(curwin->w_o_curbuf.wo_cole > 0 && conceal_cursor_line(curwin))
    {
        need_cursor_line_redraw = TRUE;

        // Need to recompute cursor column, e.g.,
        // when starting Visual mode without concealing.
        curs_columns(TRUE);
    }
}
...
void update_debug_sign(filebuf_st *buf, linenum_kt lnum)
{
    int  doit = FALSE;
...
static void win_update(win_st *wp)
{
    ...
    // first row of the mid area that needs
    // updating. 999 when no mid area updating.
    int mid_start = 999;

    // Below last row of the mid area that needs
    // updating. 0 when no mid area updating.
    int mid_end = 0;

    // first row of the bot area that needs
...
#define DID_FOLD  3 // updated a folded line
    ...
    // Only need to draw the separator.
    if(wp->w_width == 0)
    {
    ...
    // Force redraw when width of 'number' or
    // 'relativenumber' column changes.
    i = (wp->w_o_curbuf.wo_nu || wp->w_o_curbuf.wo_rnu)
        ? number_width(wp) : 0;
    ...
    else
    {
        ...
        if(buf->b_mod_set)
        {
            if(mod_top == 0 || mod_top > buf->b_mod_top)
            {
                ...
                // Need to redraw lines above the change that
                // may be included in a pattern match.
        ...
        if(mod_top != 0 && buf->b_mod_xlines != 0 && wp->w_o_curbuf.wo_nu)
        {
            mod_bot = MAXLNUM;
        }
    }
       ...
       && !wp->w_botfill && !wp->w_old_botfill)
    {
        ...
        {
            // w_topline is the first changed line,
            // the scrolling will be done further down.
        }
                        ...
                        && wp->w_topfill > wp->w_old_topfill)))
        {
            ...
            {
                i = plines_m_win(wp, wp->w_topline,
                                 wp->w_lines[0].wl_lnum - 1);
        ...
        else
        {
            ...
            if(j == -1)
            {
                // if wp->w_topline is not in wp->w_lines[].wl_lnum
                // redraw all lines
                mid_start = 0;
            }
            else
            {
                ...
                if((row == 0 || bot_start < 999) && wp->w_lines_valid != 0)
                {
                    ...
                    // valid and don't need redrawing. Copy their info upwards,
                    // to compensate for the deleted lines. Set bot_start to
                    // the first row that needs redrawing.
                    bot_start = 0;
                    idx = 0;
                    ...
                    // Correct the first entry for filler lines
                    // at the top when it won't get updated below.
                    if(wp->w_o_curbuf.wo_diff && bot_start > 0)
                    {
        ...
        // When starting redraw in the first line,
        // redraw all lines. When there is only one
        // window it's probably faster to clear the screen first.
        if(mid_start == 0)
        {
            mid_end = wp->w_height;
       ...
       || (wp->w_old_cursor_lnum != 0 && type != NOT_VALID))
    {
        ...
        if(VIsual_active)
        {
            ...
            // If in block mode and changed column or
            // curwin->w_curswant: update all lines.
        ...
        // If we know the value of w_botline, use it to restrict
        // the update to the lines that are visible in the window.
        if(wp->w_valid & kWVF_BotLine)
        {
    ...
    for(;;)
    {
        // stop updating when reached the end of the window (check
        // for _past_ the end of the window is at the end of the loop)
        if(row == wp->w_height)
        {
            didline = TRUE;
            break;
                               ...
                               && buf->b_mod_xlines != 0))))))
        {
               ...
               && !(dollar_vcol >= 0 && mod_bot == mod_top + 1))
            {
                ...
                for(i = idx; i < wp->w_lines_valid; ++i)
                {
                    ...
                    {
                        // Must have found the last valid entry
                        // above mod_bot. Add following invalid entries.
                        ++i;
                ...
                else
                {
                    ...
                    if(xtra_rows < 0)
                    {
                        ...
                        else
                        {
                            ...
                            if(win_del_lines(wp, row, -xtra_rows,
                                             FALSE, FALSE) == FAIL)
                            {
                                mod_bot = MAXLNUM;
                            }
                    ...
                    else if(xtra_rows > 0)
                    {
                        ...
                        // remaining text of scrolling fails, must redraw
                        // the rest.
                        if(row + xtra_rows >= wp->w_height - 2)
                        {
                            mod_bot = MAXLNUM;
                        ...
                        else
                        {
                            ...
                            if(win_ins_lines(wp, row + old_rows, xtra_rows,
                                             FALSE, FALSE) == FAIL)
                            {
                                mod_bot = MAXLNUM;
                            }
                            ...
                            {
                                // Scrolled the part at the top
                                // that requires updating down.
                                top_end += xtra_rows;
                            }
                        }
                    }

                    // When not updating the rest, may need
                    // to move w_lines[] entries.
            ...
            // Otherwise, display normally (can be several display lines
            // when 'wrap' is on).
            ...
            {
                // This line is not going to fit. Don't draw
                // anything here, will draw "@  " lines below.
                row = wp->w_height + 1;
            }
            else
            {
    ...
    if(!eof && !didline)
    {
        ...
        else if(dy_flags & DY_TRUNCATE) // 'display' has "truncate"
        {
            ...
            screen_fill(scr_row, scr_row + 1, (int)wp->w_wincol + 2,
                        (int)W_ENDCOL(wp), '@', ' ', hl_attr(HLF_AT));
    ...
    // Reset the type of redrawing required,
    ...
    // restore got_int, unless
    // CTRL-C was hit while redrawing
    if(!got_int)
    {
                         ...
                         hlf_T hl)
{
    ...
    {
        // No check for cmdline window:
        // should never be right-left.
        n = fdc;
...
/// Advance @b color_cols and return TRUE when there are columns to draw.
                      ...
                      int row)
{
    ...
    if(fdc > 0)
    {
        ...
        {
            int i;
            copy_text_attr(off + wp->w_width - fdc - col,
                           buf, fdc, hl_attr(HLF_FC));
...
#define RL_MEMSET(p, v, l)                                         \
    ...
    // Set all attributes of the 'number' or 'relativenumber'
    // column and the text
    ...
    //    in columns number-col - window-width.
    {
        ...
        for(p = text; *p != NUL;)
        {
            ...
            if(col + cells > wp->w_width - (wp->w_o_curbuf.wo_rl ? col : 0))
            {
                break;
            }
            ...
            else
            {
                if(p_arshape && !p_tbidi && arabic_char(u8c))
                {
                    ...
                    u8c = arabic_shape(u8c, &firstbyte,
                                       &u8cc[0], pc, pc1, nc);
    ...
    while(col < wp->w_width - (wp->w_o_curbuf.wo_rl ? txtcol : 0))
    {
        if(fill_fold >= 0x80)
        {
    ...
    // Show colorcolumn in the fold line,
    ...
    if(wp->w_o_curbuf.wo_cuc)
    {
        ...
        {
            ScreenAttrs[off + txtcol] =
                hl_combine_attr(ScreenAttrs[off + txtcol], hl_attr(HLF_CUC));
        }
    }

    SCREEN_LINE(row + wp->w_winrow, wp->w_wincol,
                wp->w_width, wp->w_width, FALSE);
       ...
       && lnume >= curwin->w_cursor.lnum)
    {
        ...
        curwin->w_valid |= (kWVF_CLHeight | kWVF_CLRow);
    }
}
...
/// Display line @b lnum of window @b wp on the screen.
/// Start at row @b startrow, stop when @b endrow is reached.
                    ...
                    bool nochange)
{
    ...
    // Visual or incsearch highlighting in this line
    ...
    bufhl_lineinfo_st bufhl_info; // bufhl data for this line
...
#define FIX_FOR_BOGUSCOLS           \
    ...
    extra_check = wp->w_o_curbuf.wo_lbr;
       ...
       && *(char **)(wp->w_s->b_langp.ga_data) != NULL)
    {
        ...
        // Get the start of the next line, so that words that
        // wrap to the next line are found too: "et<line-break>al.".
        ...
        // When a word wrapped from the previous line the start
        // of the current line is valid.
    ...
    if(has_spell)
    {
        ...
        else
        {
            ...
            if(v < SPWORDLEN)
            {
                // Short line, use it completely and append
    ...
    if(v > 0)
    {
        ...
        if(has_spell)
        {
            ...
            {
                // no bad word found at line start, don't check
                // until end of a word
                spell_hlf = HLF_COUNT;
    ...
    while(cur != NULL || !shl_flag)
    {
        ...
        // regexp may have made it invalid.
        line = ml_get_buf(wp->w_buffer, lnum, false);
        ptr = line + v;
    ...
    // Not when Visual mode is active, because it's not clear what is
    // selected then.
    if(wp->w_o_curbuf.wo_cul && lnum == wp->w_cursor.lnum
    ...
    {
        terminal_get_line_attributes(wp->w_buffer->terminal,
                                     wp, lnum, term_attrs);
        extra_check = true;
    }
    ...
    for(;;)
    {
        ...
        if(draw_state != WL_LINE)
        {
            ...
            if(draw_state == WL_SIGN - 1 && n_extra == 0)
            {
                ...
                // Show the sign column when there are any
                // signs in this buffer or when using Netbeans.
                if(signcolumn_on(wp))
                {
                    int text_sign;
                    ...
                    {
                        text_sign = buf_getsigntype(wp->w_buffer,
                                                    lnum, SIGN_TEXT);
            ...
            if(draw_state == WL_NR - 1 && n_extra == 0)
            {
                       ...
                       || ustrchr(p_cpo, CPO_NUMCOL) == NULL))
                {
                    ...
                    if(row == startrow + filler_lines)
                    {
                        ...
                        if(wp->w_o_curbuf.wo_rl) // reverse line numbers
                        {
                            rl_mirror(extra);
                        }
                    ...
                    if((wp->w_o_curbuf.wo_cul || wp->w_o_curbuf.wo_rnu)
            ...
            if(draw_state == WL_BRI - 1 && n_extra == 0)
            {
                   ...
                   && filler_lines == 0)
                {
                    ...
                    if(diff_hlf != (hlf_T)0)
                    {
                        ...
                        {
                            char_attr =
                                hl_combine_attr(char_attr, hl_attr(HLF_CUL));
                        }
                    }
           ...
           && filler_todo <= 0)
        {
            SCREEN_LINE(screen_row, wp->w_wincol,
                        col, -wp->w_width, wp->w_o_curbuf.wo_rl);
        ...
        if(draw_state == WL_LINE && area_highlighting)
        {
            ...
            if(!n_extra)
            {
                ...
                // Do this for 'search_hl' and the match list
                // (ordered by priority).
                v = (long)(ptr - line);
                cur = wp->w_match_head;
                shl_flag = FALSE;
                ...
                while(cur != NULL || shl_flag == FALSE)
                {
                    ...
                    // mark that a position match search is
                          ...
                          || (cur != NULL && pos_inprogress))
                    {
                        ...
                        else if(v == (long)shl->endcol)
                        {
                            ...
                            if(shl->lnum == lnum)
                            {
                                ...
                                if(shl->startcol == shl->endcol)
                                {
                                    // highlight empty match,
                                    // try again after it
                                    shl->endcol +=
                                        (*mb_ptr2len)(line + shl->endcol);
                                }

                                // Loop to check if the match
                ...
                // Use attributes from match with highest
                // priority among 'search_hl' and the match list.
        ...
        if(n_extra > 0)
        {
            ...
            else
            {
                ...
                // If a double-width char doesn't fit
                // display a '>' in the last column.
                if((wp->w_o_curbuf.wo_rl
                    ? (col <= 0) : (col >= wp->w_width - 1))
                   && (*mb_char2cells)(mb_c) == 2)
                {
                    ...
                    if(wp->w_hl_attr != 0)
                    {
                        multi_attr = hl_combine_attr(wp->w_hl_attr,
                                                     multi_attr);
                    }
        ...
        else
        {
            ...
            // If the UTF-8 character is more than
            // one byte: Decode it into "mb_c".
            mb_l = (*mb_ptr2len)(ptr);
            mb_utf8 = FALSE;
            ...
            if(mb_l > 1)
            {
                ...
                // Overlong encoded ASCII or ASCII with composing
                // char is displayed normally, except a NUL.
                if(mb_c < 0x80)
                {
                    c = mb_c;
                }
               ...
               || (mb_l > 1 && (!is_print_char(mb_c))))
            {
                ...
                if(wp->w_o_curbuf.wo_rl) // reverse
                {
                    rl_mirror(extra);
                }
            ...
            {
                bool can_spell = true;
                // Get syntax attribute, unless still at the start
                // of the line (double-wide char that doesn't fit).
                v = (long)(ptr - line);
                ...
                if(has_spell && v >= word_end && v > cur_checked_col)
                {
                    ...
                    if(c != 0 && (!has_syntax || can_spell))
                    {
                        ...
                        // Use nextline[] if possible, it has the
                        // start of the next line concatenated.
                        ...
                        size_t tmplen = spell_check(wp, p, &spell_hlf,
                                                    &cap_col, nochange);
            ...
            if(!is_print_char(c))
            {
                ...
                // into "ScreenLines".
                if(c == TAB && (!wp->w_o_curbuf.wo_list || lcs_tab1))
                {
                    int tab_len = 0;
                    ...
                    if(!wp->w_o_curbuf.wo_lbr || !wp->w_o_curbuf.wo_list)
                    {
                        n_extra = tab_len;
                    }
                    else
                    {
                        ...
                        // to use for a tab, else we need to calculate
                        // the width for a tab
                        ...
                        && lcs_eol_one > 0)
                {
                    ...
                    if(wp->w_o_curbuf.wo_list && lcs_eol > 0)
                    {
                        c = lcs_eol;
                    }
                    else
                ...
                else if(c != NUL)
                {
                    ...
                    if((dy_flags & DY_UHEX) && wp->w_o_curbuf.wo_rl)
                    {
                    ...
                    && ustrchr(wp->w_o_curbuf.wo_cocu, 'v') == NULL))
            {
                ...
                if((prev_syntax_id != syntax_seqnr
                   ...
                   && (syn_get_sub_char() != NUL
                       || match_conc
                       || wp->w_o_curbuf.wo_cole == 1)
                   && wp->w_o_curbuf.wo_cole != 3)
                {
                    // First time at this concealed item:
                    ...
                    if(wp->w_o_curbuf.wo_wrap && n_extra > 0)
                    {
                        if(wp->w_o_curbuf.wo_rl)
                        {
                            col -= n_extra;
        ...
        if(lcs_prec_todo != NUL
           && wp->w_o_curbuf.wo_list
           && (wp->w_o_curbuf.wo_wrap ? wp->w_skipcol > 0 : wp->w_leftcol > 0)
           && filler_todo <= 0
           && draw_state > WL_NR
           && c != NUL)
        {
            ...
            {
                // Double-width character being overwritten by the
                // "precedes" character, need to fill up half the character.
                c_extra = MB_FILLER_CHAR;
                n_extra = 1;
                n_attr = 2;
                extra_attr = hl_attr(HLF_AT);
        ...
        if(c == NUL || did_line_attr == 1)
        {
            ...
            long currcol = wp->w_o_curbuf.wo_wrap
                           ? wp->w_skipcol : wp->w_leftcol;
        ...
        {
            if(eol_hl_off > 0
               && vcol - eol_hl_off == (long)wp->w_virtcol
               && lnum == wp->w_cursor.lnum)
            {
            ...
            // Highlight 'cursorcolumn' & 'colorcolumn'
            ...
            // Get rid of the boguscols now, we want to draw
            // until the right edge for 'cursorcolumn'.
            col -= boguscols;

            // Disabled because value never read after this
            // boguscols = 0;
            ...
            if(((wp->w_o_curbuf.wo_cuc
                 ...
                 && lnum != wp->w_cursor.lnum) || draw_color_col)
               && !wp->w_o_curbuf.wo_rl)
            {
                ...
                while(col < wp->w_width)
                {
                    ...
                    {
                        draw_color_col =
                            advance_color_col(VCOL_HLC, &color_cols);
                    }

                    if(wp->w_o_curbuf.wo_cuc
                       && VCOL_HLC == (long)wp->w_virtcol)
                    {
            ...
            if(wp->w_buffer->terminal)
            {
                // terminal buffers may need to highlight
                // beyond the end of the logical line
                while(col < wp->w_width)
                {
                    ScreenLines[off] = ' ';
                    ScreenLinesUC[off] = 0;
            ...
            // Update w_cline_height and w_cline_folded if the cursor
            // line was updated (saves a call to plines() later).
        ...
        if(lcs_ext
           && !wp->w_o_curbuf.wo_wrap
           && filler_todo <= 0
           && (wp->w_o_curbuf.wo_rl ? col == 0 : col == wp->w_width - 1)
           && (*ptr != NUL
               || (wp->w_o_curbuf.wo_list && lcs_eol_one > 0)
        ...
        if(draw_state < WL_LINE || n_skip <= 0)
        {
            // Store the character.
            if(wp->w_o_curbuf.wo_rl
               && (*mb_char2cells)(mb_c) > 1)
            {
                // A double-wide character is:
                // put first halve in left cell.
                --off;
                --col;
            }
            ...
            if((*mb_char2cells)(mb_c) > 1)
            {
                ...
                // When "tocol" is halfway through a character, set it to
                // the end of the character, otherwise highlighting won't stop.
                if(tocol == vcol)
                {
                    tocol++;
                }
        ...
        else if(wp->w_o_curbuf.wo_cole > 0 && is_concealing)
        {
            --n_skip;
            ++vcol_off;
        ...
        // Only advance the "vcol" when after the
        // 'number' or 'relativenumber' column.
        if(draw_state > WL_NR
           && filler_todo <= 0
          )
        {
        ...
        if((wp->w_o_curbuf.wo_rl ? (col < 0) : (col >= wp->w_width))
           && (*ptr != NUL
               || filler_todo > 0
               || (wp->w_o_curbuf.wo_list
                   && lcs_eol != NUL
        ...
        {
            SCREEN_LINE(screen_row, wp->w_wincol,
                        col - boguscols, wp->w_width, wp->w_o_curbuf.wo_rl);
               ...
               && wp->w_width == Columns)
            {
                                            ...
                                            + screen_Columns) == 2))
                {
                    ...
                    if(MB_BYTE2LEN(ScreenLines[LineOffset[screen_row - 1]
                                   + (Columns - 1)]) > 1)
                    {
                        ui_putc(' ');
                    }
                    ...
                    {
                        ui_putc(ScreenLines[LineOffset[screen_row - 1]
                                + (Columns - 1)]);
                    }
...
/// Return if the composing characters at @b off_from and @b off_to differ.
...
static int char_needs_redraw(int off_from, int off_to, int cols)
{
    ...
    flag = ScreenLines[off_from] != ScreenLines[off_to];
    if(flag)
    {
        return true;
    }

    flag = ScreenAttrs[off_from] != ScreenAttrs[off_to];
    if(flag)
    {
        return true;
    }

    flag = ScreenLinesUC[off_from] != ScreenLinesUC[off_to];
    if(flag)
    {
        return true;
    }

    flag = ScreenLinesUC[off_from] != 0 && comp_char_differs(off_from, off_to);
    if(flag)
    {
        return true;
    }

    flag = (*mb_off2cells)(off_from, off_from + cols) > 1
...
static void screen_line(int row,
                        int coloff,
                        int endcol,
                        int clear_width,
                        int rlflag)
{
    ...
    if(clear_width > 0)
    {
        // For a window that's left of another,
        // draw the separator char.
...
/// Show at least the @b match item.
/// We start at item @b first_match in the list and show all matches that fit.
///
                             ...
                             int showtail)
{
    ...
    else
    {
        ...
        if((long)clen > Columns)
        {
            ...
            // if showing the last match, we
            // can add some on the left
            clen = 2;
...
void win_redr_status(win_st *wp)
{
    ...
    else if(*p_stl != NUL || *wp->w_o_curbuf.wo_stl != NUL)
    {
...
static void win_redr_custom(win_st *wp, int draw_ruler)
{
    ...
    {
        // Use 'tabline'.
        // Always at the first line of the screen.
        stl = p_tal;
        row = 0;
        fillchar = ' ';
    ...
    else
    {
        ...
        if(draw_ruler)
        {
            ...
            // advance past any leading group spec
            // implicit in ru_col
            if(*stl == '%')
            {
                if(*++stl == '-')
        ...
        {
            if(*wp->w_o_curbuf.wo_stl != NUL)
            {
                stl = wp->w_o_curbuf.wo_stl;
            }
            else
            {
                stl = p_stl;
    ...
    // Temporarily reset 'cursorbind', we don't
    // want a side effect from moving the cursor away and back.
    ewp = wp == NULL ? curwin : wp;
    p_crb_save = ewp->w_o_curbuf.wo_crb;
    ewp->w_o_curbuf.wo_crb = FALSE;
    ...
    width = build_stl_str_hl(ewp,
                             ...
                             tabtab);
    xfree(stl);
    ewp->w_o_curbuf.wo_crb = p_crb_save;
    ...
    if(wp == NULL)
    {
        // Fill the tab_page_click_defs
        // array for clicking in the tab pages line.
        col = 0;
        len = 0;
        p = buf;
...
void screen_puts_len(uchar_kt *text, int textlen, int row, int col, int attr)
{
          ...
          && *ptr != NUL)
    {
        ...
        if(p_arshape && !p_tbidi && arabic_char(u8c))
        {
            ...
            {
                nc = utfc_ptr2char_len(ptr + mbyte_blen,
                                       pcc,
                                       (int)((text + len)
                                             - ptr
                                             - mbyte_blen));
                           ...
                           matchitem_st *cur)
{
    ...
    // Repeat searching for a match until one is found
    // that includes "mincol" or none is found in this line.
    called_emsg = FALSE;
    ...
    for(;;)
    {
        ...
        if(shl->rm.regprog != NULL)
        {
            // Remember whether shl->rm is using a
            // copy of the regprog in cur->match.
            bool regprog_is_copy = (shl != &search_hl
                                    && cur != NULL
                                    && shl == &cur->hl
                                    && cur->match.regprog == cur->hl.rm.regprog);

            nmatched = vim_regexec_multi(&shl->rm, win, shl->buf,
                                         lnum, matchcol, &(shl->tm));
                              ...
                              columnum_kt mincol)
{
    ...
    for(i = posmatch->cur; i < MAX_POS_NUM_MATCH; i++)
    {
        ...
        if(pos->lnum == lnum)
        {
            if(found >= 0)
            {
                // if this match comes before the one
...
/// Put character ScreenLines["off"] on the screen at position
/// "row" and "col", using the attributes from ScreenAttrs["off"].
static void screen_char(unsigned off, int row, int col)
{
    int attr;

    // Check for illegal values, just in case
    // (could happen just after resizing).
...
/// Use attributes 'attr'.
void screen_fill(int start_row,
                 int end_row,
                 int start_col,
                 int end_col,
                 int c1,
...
/// Used before clearing or redrawing the
...
retry:
    ...
    new_ScreenAttrs =
        xmalloc((size_t)((Rows + 1) * Columns * sizeof(sattr_T)));
...
void free_screenlines(void)
{
    ...
    clear_tab_page_click_defs(tab_page_click_defs,
...
static void linecopy(int to, int from, win_st *wp)
{
    ...
    memmove(ScreenAttrs + off_to,
            ScreenAttrs + off_from,
                  ...
                  int mayclear)
{
    ...
    // that will end up below the window
    if(!did_delete)
    {
        wp->w_redr_status = TRUE;
        redraw_cmdline = TRUE;
        ...
        screen_fill(nextrow - line_count, lastrow - line_count,
                    wp->w_wincol, W_ENDCOL(wp), ' ', ' ', 0);
    }

    if(screen_ins_lines(0, wp->w_winrow + row,
                  ...
                  int mayclear)
{
    ...
    if(screen_del_lines(0, wp->w_winrow + row,
                        line_count, (int)Rows, NULL) == FAIL)
    {
        return FAIL;
    }
    ...
    {
        // If this is the last window and there is no
        // status line, redraw the command line later.
        redraw_cmdline = TRUE;
    }
                        ...
                        int del)
{
    ...
    // when scrolling, the message on the command line
    // should be cleared, otherwise it will stay there forever.
    clear_cmdline = TRUE;
    int retval;
...
/// @param line_count
/// @param end
/// @param wp          NULL or window to use width from
///
/// @return FAIL for failure, OK for success.
int screen_ins_lines(int off,
                     int row,
                     int line_count,
                     int end,
                     win_st *wp)
...
int showmode(void)
{
        ...
        if(do_mode)
        {
            ...
            else
            {
                ...
                if(VIsual_active)
                {
                    ...
                    // Don't concatenate separate words
    ...
    // In Visual mode the size of
    // the selected area must be redrawn.
    if(VIsual_active)
    {
        clear_showcmd();
    }
...
static void draw_tabline(void)
{
    ...
    // Reset the flag here again, in case evaluating
    // 'tabline' causes it to be set.
    redraw_tabline = FALSE;
}
...
void get_trans_bufname(filebuf_st *buf)
{
...
/// Get the character to use in a status line.
...
/// Get the character to use in a separator between
/// vertically split windows. Get its attributes in "*attr".
...
void showruler(int always)
{
    ...
    if((*p_stl != NUL || *curwin->w_o_curbuf.wo_stl != NUL)
...
static void win_redr_ruler(win_st *wp, int always)
{
    ...
    // it might overwrite the (long) mode message.
    ...
    // Don't draw the ruler when the
    // popup menu is visible, it may overlap.
    if(pum_drawn())
    {
        return;
    }
    ...
    // Check if not in Insert mode and
...
int number_width(win_st *wp)
{
    ...
    if(n < wp->w_o_curbuf.wo_nuw - 1)
    {
        n = wp->w_o_curbuf.wo_nuw - 1;
    }
...
/// Check if the new shell size is valid, correct
/// it if it's too small or way too big.
