/// @file plugins/bin/snail/shell.cpp
...
#include "plugins/bin/snail/input.h"
#include "plugins/bin/snail/shell.h"
#include "plugins/bin/snail/msgpackrequest.h"
#include "plugins/bin/snail/konsole_wcwidth.h"
...
void Shell::fontError(const QString &msg)
{
    if(m_attached)
    {
        m_nvimCon->neovimObject()->nvim_errmsg_writeln(m_nvimCon->encode(msg));
    }
}
...
bool Shell::setGuiFont(const QString &fdesc, bool force)
{
    ...
    if(attrs.size() < 1)
    {
        m_nvimCon->neovimObject()->nvim_errmsg_writeln("Invalid font");
        return false;
    }
    ...
    foreach(QString attr, attrs)
    {
        if(attr.size() >= 2 && attr[0] == 'h')
        {
            ...
            if(!ok)
            {
                m_nvimCon->neovimObject()->nvim_errmsg_writeln("Invalid font height");
                return false;
            }
    ...
    {
        resizeNeovim(size());
        m_nvimCon->neovimObject()->nvim_set_var("GuiFont", fontDesc());
    }
...
void Shell::setAttached(bool attached)
{
    ...
    if(attached)
    {
        ...
        m_nvimCon->neovimObject()->nvim_command("runtime plugin/nvim_gui_shim.vim");
        m_nvimCon->neovimObject()->nvim_command("runtime! ginit.vim");
...
/// Get the area filled by the cursor at an arbitrary position
...
void Shell::init(void)
{
    ...
    connect(m_nvimCon->neovimObject(), &Nvim::on_nvim_ui_try_resize,
...
void Shell::nvimError(NvimConnector::NvimError VATTR_UNUSED_MATCH(err))
{
    setAttached(false);
}
...
void Shell::handleResize(uint64_t n_cols, uint64_t n_rows)
{
    ...
    if(isWindow())
    {
        // Never call resize on a maximized window QTBUG-45806
        resizeNeovim(size());
    }
                                     ...
                                     const QVariantList &args)
{
    if(name == "Gui" && args.size() > 0)
    {
        ...
        else if(guiEvName == "Linespace" && args.size() == 2)
        {
            ...
            setLineSpace(val);
            m_nvimCon->neovimObject()->nvim_set_var("GuiLinespace", val);
            resizeNeovim(size());
        }
        else if(guiEvName == "Mousehide" && args.size() == 2)
        {
            ...
            m_nvimCon->neovimObject()->nvim_set_var("GuiMousehide", val);
        }
...
void Shell::keyPressEvent(QKeyEvent *ev)
{
    ...
    m_nvimCon->neovimObject()->nvim_input(m_nvimCon->encode(inp));
...
void Shell::neovimMouseEvent(QMouseEvent *ev)
{
    ...
    {
        inp = Input.convertMouse(ev->button(),
                                 ev->type(),
                                 ev->modifiers(),
                                 pos,
                                 m_mouseclick_count);
    ...
    m_nvimCon->neovimObject()->nvim_input(inp.toLatin1());
}
...
void Shell::updateWindowId(void)
{
       ...
       && m_nvimCon->connectionType() == NvimConnector::SpawnedConnection)
    {
        ...
        m_nvimCon->neovimObject()->nvim_set_var("GuiWindowId",
                                                QVariant(window_id));
    }
}
...
void Shell::resizeNeovim(int n_cols, int n_rows)
{
    ...
    {
        m_nvimCon->neovimObject()->nvim_ui_try_resize(n_cols, n_rows);
        m_resizing = true;
    }
}
...
/// Finished call to nvim_ui_try_resize()
...
void Shell::updateGuiWindowState(Qt::WindowStates state)
{
    ...
    {
        m_nvimCon->neovimObject()->nvim_set_var("GuiWindowMaximized", 1);
    }
    else
    {
        m_nvimCon->neovimObject()->nvim_set_var("GuiWindowMaximized", 0);
    }

    if(state & Qt::WindowFullScreen)
    {
        m_nvimCon->neovimObject()->nvim_set_var("GuiWindowFullScreen", 1);
    }
    else
    {
        m_nvimCon->neovimObject()->nvim_set_var("GuiWindowFullScreen", 0);
    }
}
...
void Shell::closeEvent(QCloseEvent *ev)
{
    if(m_attached
       && m_nvimCon->connectionType() == NvimConnector::SpawnedConnection)
    {
        // If attached to a spawned Nvim process,
        // ignore the event and try to close Nvim as :qa
        ev->ignore();
        m_nvimCon->neovimObject()->nvim_command("qa");
    }
    else
    {
...
{
    m_nvimCon->neovimObject()->nvim_input("<FocusGained>");
    QWidget::focusInEvent(ev);
}

void Shell::focusOutEvent(QFocusEvent *ev)
{
    m_nvimCon->neovimObject()->nvim_input("<FocusLost>");
...
void Shell::inputMethodEvent(QInputMethodEvent *ev)
{
    if(!ev->commitString().isEmpty())
    {
        ...
        m_nvimCon->neovimObject()->nvim_input(s);
        tooltip("");
    }
    else
    {
...
void Shell::openFiles(QList<QUrl> urls)
{
    if(m_nvimCon && m_attached)
    {
        ...
        m_nvimCon->neovimObject()->nvim_call_function("GuiDrop", args);
    }
    else
    {
