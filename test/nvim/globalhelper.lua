...
local config_path = require('config.paths')

local nvim_prog = (os.getenv('NVIM_PROG') or os.getenv('NVIM_PRG')  or
                   os.getenv('NVIM_EXE')  or os.getenv('NVIM_BIN')  or
                   config_path.build_dir .. '/bin/nvim')
...
local ko = function(res)
    return assert.is_false(res)
end
...
local function get_platform_name()
    ...
    local function check_platform_name()
        if platform then
            return platform
        end
    ...
    return check_platform_name
end
local uname = get_platform_name()

local function get_temp_dir()
    local tmp_dir = nil
    
    tmp_dir = os.getenv('TEMPDIR')
    if tmp_dir then
        return tmp_dir
    end
...
local function is_temp_dir_local(dir)
    if not dir then
        return false
    end
    
    if string.find(dir, 'snail/build') then
        return true
    end
...
local function get_temp_file_name()
    ...
    local function make_temp_file_name()
        if is_temp_dir_local(tmpdir) then
    ...
    return make_temp_file_name
end
local tmpname = get_temp_file_name()
...
local function check_cores(app, force)
    ...
    local local_tmpdir = get_temp_dir()

    if not is_temp_dir_local(local_tmpdir) then
        local_tmpdir = nil
    end
    ...
    for _, core in ipairs(cores) do
        ...
        local pipe = io.popen(db_cmd:gsub('%$_NVIM_TEST_APP', app):gsub('%$_NVIM_TEST_CORE', core)
                              .. ' 2>&1', 'r')
        if pipe then
...
local function dedent(str)
    ...
    indent = indent:gsub('%s', '[ \t]')   -- create a pattern for the indent
    str = str:gsub('^'..indent, '')       -- strip it from the first line
