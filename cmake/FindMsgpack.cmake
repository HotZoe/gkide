...
if(NOT MSGPACK_USE_BUNDLED)
    ...
    if(PKG_CONFIG_FOUND)
    pkg_search_module(PC_MSGPACK
                      QUIET msgpackc>=${Msgpack_FIND_VERSION}
...
find_path(MSGPACK_INCLUDE_DIR msgpack/version_master.h
          HINTS ${PC_MSGPACK_INCLUDEDIR} ${PC_MSGPACK_INCLUDE_DIRS} ${LIMIT_SEARCH})

if(MSGPACK_INCLUDE_DIR)
    file(READ ${MSGPACK_INCLUDE_DIR}/msgpack/version_master.h msgpack_version_h)
    string(REGEX REPLACE ".*MSGPACK_VERSION_MAJOR +([0-9]+).*" "\\1"
                         MSGPACK_VERSION_MAJOR "${msgpack_version_h}")

    string(REGEX REPLACE ".*MSGPACK_VERSION_MINOR +([0-9]+).*" "\\1"
                         MSGPACK_VERSION_MINOR "${msgpack_version_h}")

    string(REGEX REPLACE ".*MSGPACK_VERSION_REVISION +([0-9]+).*" "\\1"
                         MSGPACK_VERSION_REVISION "${msgpack_version_h}")

    set(MSGPACK_VERSION_STRING
        "${MSGPACK_VERSION_MAJOR}.${MSGPACK_VERSION_MINOR}.${MSGPACK_VERSION_REVISION}")
else()
    set(MSGPACK_VERSION_STRING)
endif()
...
find_library(MSGPACK_LIBRARY
             NAMES ${MSGPACK_NAMES}
             NAMES_PER_DIR
             HINTS ${PC_MSGPACK_LIBDIR} ${PC_MSGPACK_LIBRARY_DIRS} ${LIMIT_SEARCH})
